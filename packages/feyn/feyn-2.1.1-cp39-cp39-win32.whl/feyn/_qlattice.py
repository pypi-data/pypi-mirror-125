...
class QLattice(QLatticeNotebookMixin if detect_notebook() else object):
    ...
    def _extract_sub_models(self, model: Model) -> List[Model]:
        sub_programs, sub_params = [], []
        output_code, output_params = model._program[0], model.params[0]
        prog_len = len(model._program)

        for sub_len in range(1, prog_len): # Ignore the output
            # re-add output node + subprogram
            sub_prog = [output_code] + model._program[sub_len:prog_len]
            # re-add output param + subprogram params
            sub_param = [output_params] + model.params[sub_len:]

            if sub_prog[1] >= 10000:
                # The immediate following node is an operand, consider only up until the first non-output node.
                sub_param = sub_param[0:2]
                sub_prog[0:2]
            # Otherwise, it's an operator, the rest of the tree should be valid to just append.

            sub_params.append(sub_param.copy())
            sub_programs.append(Program(sub_prog, model._program.qid, autopad=True))

        stypes = dict([(elem.name, 'c') for elem in model if 'in:cat' == elem.fname])
        if model[0].fname == 'out:lr':
            stypes[model[0].name] = 'b'
        models = [self.context.to_model(p, model.output, stypes=stypes) for p in sub_programs]
        for i, m in enumerate(models):
            m.params = sub_params[i][:len(m._program)] # Filter away stray right-leaning inputs
