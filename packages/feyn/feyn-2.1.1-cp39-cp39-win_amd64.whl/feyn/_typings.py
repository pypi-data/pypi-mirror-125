...
# Python 3.6 is difficult - so we just skip type checking entirely.
import feyn
import numpy as np

skip_type_checking = version_info.minor <= 6
...
def check_types(exceptions: Iterable = [], verbose=False):
    ...
    def inner(func):
        ...
        def wrapper(*args, **kwargs):
            if skip_type_checking or feyn._disable_type_checks:
            ...
            for param_name, param_value in params.items():
                if param_name in exceptions:
                    if verbose:
                        print(f"{param_name} in exceptions")
                    continue
                if param_name not in type_hints:
                    if verbose:
                        print(f"{param_name} not in type hints")
                    continue
                expected_type = type_hints[param_name]

                verbose and print(f"Validating def({param_name}: {expected_type}) - passed({param_value}: {type(param_value)})")
...
def _validate_type(expected_type, value, verbose=False) -> bool:
    ...
    verbose and print(f"(expected base type: {expected_base_type})")
...
def _validate_dictionary(expected_type, dictionary, verbose):
    ...
    if dictionary and type_args is not None:
        ...
        for type_arg, dict_values in zip(type_args, [dictionary.keys(), dictionary.values()]):
            if type_arg == Any:
                continue
            ...
            if _is_special_type_arg(type_arg):
                ...
                validated &= _validate_iterable_type_args(type_arg, dict_values, verbose)
                ...
                for maybe_list in dict_values:
                    if not isinstance(maybe_list, list):
                        ...
                        validated &= _validate_iterable_type_args(type_arg, maybe_list, verbose)
                ...
                for maybe_iterable in dict_values:
                    if not _value_is_iterable_like(maybe_iterable):
                        ...
                        # Each element in the list needs to be validated as well
                        validated &= _validate_iterable_type_args(type_arg, maybe_iterable, verbose)
            else:
...
_equivalent_types = {
    int: [np.int_],
    float: [int, np.int_, np.float_],
    bool: [np.bool_],
    str: [np.str_]
}

def _is_special_type_arg(type_arg):
    return hasattr(type_arg, '__origin__') and isinstance(type_arg.__origin__, _SpecialForm)
...
def _is_iterable_type_arg(type_arg):
    return hasattr(type_arg, '__origin__') and (type_arg.__origin__ == collections.abc.Iterable)

def _value_is_iterable_like(value):
    return hasattr(value, "__iter__") and not type(value) == str and not type(value) == feyn.Model

def _raise(param_name, expected_type):
    raise TypeError(f"{param_name} should be of type {expected_type}.".replace('typing.', ''))
