        ...
        **kwargs
        ...
        if stypes:
            self.categorical_features = [
                feature for feature in stypes.keys() if stypes[feature] == "c"
            ]
            data[self.categorical_features] = data[self.categorical_features].astype(str)
...
class SKLearnRegressor(BaseReferenceModel):
    def __init__(
        self, sklearn_regressor: type, data: DataFrame, output_name: str, **kwargs
    ):
        ...
        self._model.fit(X=data[self.inputs].values, y=data[self.output].values)
