...
import feyn
from feyn.qepler import qeplertrainer
...
def count_cats(params):
    num_cats = []
    for element in params:
        try:
            num_cats.append(len(element['categories']))
        except:
            pass
    return np.array(num_cats)
...
def fit_models(
    ...
    for ix, m in enumerate(models):
        ...
        m.num_cats = count_cats(params)
        m.bic = feyn.criteria.bic(m.loss_value, m._paramcount + sum(m.num_cats - 1), data_length, m.kind)
        m.aic = feyn.criteria.aic(m.loss_value, m._paramcount + sum(m.num_cats - 1), data_length, m.kind)
