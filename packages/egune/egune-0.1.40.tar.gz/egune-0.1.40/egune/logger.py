...
def egune_response_log(func: Callable[[ActorMessage, UserMessage], ActorMessage]):
    def wrapper(data: Dict[str, Any], msg: Dict[str, Any]) -> Dict[str, Any]:
        s = time.time()
        try:
            result = func(
                ActorMessage.from_dict(data),
                UserMessage.from_dict(data)
            ).to_dict()
            logger.info("Processed User message", extra={
                "log_event": "user message",
                "user_id": data["user_id"],
