...
class PiecewiseConstantCoalescentGrid(ConstantCoalescent):
    ...
    def log_prob(self, node_heights: torch.Tensor) -> torch.Tensor:
        ...
        if heights.shape[0] == 1:
            permutation = soft_sort(-heights.unsqueeze(-1), 0.0001)
            heights_sorted = permutation @ heights.squeeze()
            node_mask_sorted = permutation @ node_mask.squeeze().to(dtype=torch.float64)
        else:
        ...
        if heights.shape[0] == 1:
            thetas = torch.empty_like(heights)
            thetas[0, 0, ...] = self.theta[0, 0, ...]
            node_mask2 = torch.cat(
                [
                    # sampling event
                    torch.ones(taxa_shape),
                    # coalescent event
                    torch.ones(taxa_shape[:-1] + (taxa_shape[-1] - 1,)),
                    # no event
                    torch.zeros(grid.shape),
                ],
                dim=-1,
            )
            theta2 = torch.cat(
            ...
            node_mask2_sorted = permutation @ node_mask2.squeeze()
            theta2_sorted = permutation @ theta2.squeeze()
            for i in range(1, node_mask2_sorted.shape[-1]):
                thetas[0, i, ...] = (
                    thetas[0, i - 1, ...].clone() * node_mask2_sorted[0, i, ...]
                    + theta2_sorted[0, i, ...]
                )
            log_thetas = thetas.log() * (permutation @ node_mask3.squeeze())
        else:
            thetas_indices = torch.where(
                node_mask_sorted == 0,
                torch.tensor([1], dtype=torch.long),
