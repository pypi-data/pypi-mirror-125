...
def measurement_value(measurement: dict) -> float:
    term_id = measurement.get('term', {}).get('@id')
    lookup = download_lookup('measurement.csv', True)
    reducer = get_table_value(lookup, 'termid', term_id, column_name('arrayTreatment')) or 'mean'
...
def most_relevant_measurement_value(measurements: list, term_id: str, date: str):
    ...
    return measurement_value(measurement) if measurement else 0
