...
def get_lookup_value(term: dict, column: str):
    value = get_table_value(
        download_lookup(f"{term.get('termType')}.csv"), 'termid', term.get('@id'), column_name(column)
    ) if term else None
    debugMissingLookup(f"{term.get('termType')}.csv", 'termid', term.get('@id'), column, value) if term else None
    return value
