...
class ADSData(object):
    def __init__(self, X=None, y=None, name="", dataset_type=None):
        ...
        It's primary purpose is to hold any metadata relavant to these tasks. This can include it's:
        ...
        structure with the method .to_onnx_rt(), which takes and onnx session as input.
    ...
    def build(X=None, y=None, name="", dataset_type=None, **kwargs):
        ...
        if isinstance(X, dd.core.DataFrame):
            X = X.compute()
        if isinstance(y, dd.core.Series):
            y = y.compute()
    ...
    def to_onnxrt(self, sess, idx_range=None, model=None, **kwargs):  # pragma: no cover
        ...
        kwargs:
            ...
            sess_inputs:
                Pass in the output from onnxruntime.InferenceSession("model.onnx").get_inputs()
            input_dtypes: list
                If sess_inputs cannot be passed in, pass in the numpy dtypes of each input
            input_shapes: list
                If sess_inputs cannot be passed in, pass in the shape of each input
            input_names: list
                If sess_inputs cannot be passed in, pass in the name of each input
