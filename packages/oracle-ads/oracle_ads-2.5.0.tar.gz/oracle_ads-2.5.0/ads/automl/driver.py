...
def get_ml_task_type(X, y, classes):
    ...
    Returns
    -------
    ml_task_type: A particular task type like `REGRESSION`, `MULTI_CLASS_CLASSIFICATION`...
    """
...
class AutoML:
    def __init__(
        self, training_data, validation_data=None, provider=None, baseline="dummy"
    ):
        """
        ...
        X = (
            ...
            if isinstance(training_data.X, dd.DataFrame)
            else training_data.X
        )
        y = (
            training_data.y.compute()
            if isinstance(training_data.y, dd.Series)
        ...
        if validation_data is not None:
            X_valid = (
                validation_data.X.compute()
                if isinstance(validation_data.X, dd.DataFrame)
                else validation_data.X
            )
            y_valid = (
                validation_data.y.compute()
                if isinstance(validation_data.y, dd.Series)
                else validation_data.y
            )
        self.target_name = y.name
        self.feature_names = X.columns.values
        self.client = ml_runtime.compute_accelerator()
    ...
    def train(self, **kwargs):
        ...
        avail_n_cores = max(
            utils.get_compute_accelerator_ncores(), utils.get_cpu_count()
        )
