...
from ads.dataset.dask_series import DaskSeries
...
from ads.dataset.helper import deprecate_variable
from IPython.core.display import display, HTML
from ipywidgets import widgets, HTML, HBox, VBox, Box, Layout, Label
...
class ADSDataset(PandasDataset):
    ...
    Pandas/Dask functions like `head()`. There are also a variety of converters, to_h2o_dataframe,
    to_dask_dataframe, to_pandas_dataframe, to_h2o_dataframe, to_xgb_dmatrix, to_csv, to_parquet,
    to_json & to_hdf .
    """
    ...
    def _head(self, n=5):
        ...
        * displays the first 5 rows of the dataset, just as the traditional head() function would *
        """
        df = self.ddf.head(n=n, npartitions=-1)  # convert to pandas dataframe
    ...
    def call(self, func, *args, sample_size=None, **kwargs):
        ...
        **kwargs: mapping, optional
        ...
        if sample_size is None:
            ...
            df = self.ddf.sample(n=sample_size).compute()
        kwargs[data] = df
    ...
    def set_target(self, target, type_discovery=True, target_type=None):
        ...
        target_type: type
            If provided, then the target will be typed with the
                     provided value.
    ...
    def to_dask_dataframe(
        ...
        filter: str, optional
            ...
            ds.to_dask_dataframe(filter="age > 50 and location == 'san francisco")
        ...
        ds_dask = ds.to_dask_dataframe()

        See also:

        http://docs.dask.org/en/latest/dataframe-api.html#dataframe

        https://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html#sklearn.pipeline.Pipeline
        """
        ddf = self.ddf.query(filter) if filter is not None else self.ddf.copy()
        ddf = (
            ddf
            if frac is None
            else ddf.sample(frac=frac, random_state=utils.random_state)
        )
        return (
            (ddf, copy.deepcopy(self.transformer_pipeline))
    ...
    def to_pandas_dataframe(
        ...
        filter: str, optional
            The query string to filter the dataframe, for example
            ds.to_pandas_dataframe(filter="age > 50 and location == 'san francisco")
            See also: https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.query.html
        frac: float, optional
            fraction of original data to return.
        include_transformer_pipeline: bool, default: False
        ...
        ds_as_df = ds.to_pandas_dataframe()
    ...
    def to_h2o_dataframe(
        ...
        filter: str, optional
            The query string to filter the dataframe, for example
            ds.to_h2o_dataframe(filter="age > 50 and location == 'san francisco")
            See also: https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.query.html
        frac: float, optional
            fraction of original data to return.
        include_transformer_pipeline: bool, default: False
        ...
        try:
            ...
            utils._log_missing_module("h2o", "h2o")
            raise e

        res = self.to_pandas_dataframe(
            filter=filter,
            frac=frac,
            include_transformer_pipeline=include_transformer_pipeline,
        )
    ...
    def to_xgb_dmatrix(
        ...
        filter: str, optional
            The query string to filter the dataframe, for example
            ds.to_xgb_dmatrix(filter="age > 50 and location == 'san francisco")
            See also: https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.query.html
        frac: float, optional
            fraction of original data to return.
        include_transformer_pipeline: bool, default: False
        ...
        try:
            ...
            utils._log_missing_module("xgboost", "ads[boosted]")
            raise e
        res = self.to_pandas_dataframe(
            filter=filter,
            frac=frac,
    ...
    def sample(self, frac=None, random_state=utils.random_state):
        ...
        ds_sample = ds.sample()
        """
        ddf = self.ddf.sample(frac=frac, random_state=random_state)
        return self._build_new_dataset(ddf)
    ...
    def assign_column(self, column, arg):
        ...
        Used for adding a new column or substituting each value in a column with another value, that may be derived from
         a function, a :class:`dask.dataframe.core.Series` or a :class:`dask.dataframe.core.DataFrame` .
        ...
        if isinstance(arg, DaskSeries):
            sampled_df = self.sampled_df.copy()
            ddf = self.ddf.copy()
            if len(arg.transformers) == 0:
                # simple assignment of one column to another
                def column_transform(df, column1=None, column2=None):
                    df[column1] = df[column2]
                    return df

                ft = FunctionTransformer(
                    func=column_transform,
                    kw_args={"column1": column, "column2": arg.name},
                    validate=False,
                )
            else:

                def column_transform(df, column_name=None, transformers=[]):
                    for transformer in transformers:
                        df[column_name] = transformer[1].transform(df)
                    return df

                ft = FunctionTransformer(
                    func=column_transform,
                    kw_args={"column_name": column, "transformers": arg.transformers},
                    validate=False,
                )
            ddf = ft.fit_transform(ddf)
            sampled_df = ft.fit_transform(sampled_df)
            return self._build_new_dataset(
                ddf,
                sampled_df,
                target=target_name,
                target_type=self.target.type if target_name != column else None,
                transformers=[("assign " + column, ft)]
                if target_name != column
                else None,
            )

        elif isinstance(arg, Iterable) or isinstance(arg, ADSDataset):
            ddf = self.ddf.copy()
            if type(arg) in [pd.DataFrame, dd.core.DataFrame]:
                col_to_add = arg
            ...
            elif type(arg) in [pd.Series, dd.core.Series]:
                col_to_add = arg.rename("new_col").to_frame()
            else:
                raise ValueError(
                    f"assign_column currently does not support arg of type {type(arg)}. Please reformat "
                    f"as types: Pandas, numpy, list, or dict"
                )
            if column in ddf.columns:
                ddf = ddf.drop(columns=column)
            new_ddf = ddf.merge(col_to_add).rename(columns={"new_col": column})
            return self._build_new_dataset(new_ddf)

        else:
            sampled_df = self.sampled_df.copy()
            ddf = self.ddf.copy()
            sampled_df[column] = sampled_df[column].apply(arg)
            ddf[column] = ddf[column].apply(arg)
            if column == target_name:
                target_type = get_feature_type(target_name, sampled_df[target_name])
                return self._build_new_dataset(
                    ddf, sampled_df, target=target_name, target_type=target_type
                )
            else:
                return self._build_new_dataset(
                    ddf,
    ...
    def snapshot(self, snapshot_dir=None, name="", storage_options=None):
        ...
        return helper.write_parquet(
            parquet_file,
            self.ddf,
            {"metadata": self.feature_types, "transformer": self.transformer_pipeline},
            storage_options=storage_options,
        )

    def to_csv(self, path, npartitions=1, storage_options=None, **kwargs):
        """
        ...
        npartitions: int, default: 1
            Number of file parts to split the data.
        storage_options: dict, optional
             Parameters passed on to the backend filesystem class.
             Defaults to storage_options set using DatasetFactory.set_default_storage().
        kwargs: dict, optional
            See http://docs.dask.org/en/latest/dataframe-api.html#dask.dataframe.to_csv
        ...
        if storage_options is None:
            ...
            storage_options = factory.default_storage_options
            logger.info("Using default storage options")
        return self.ddf.compute().to_csv(
            path, storage_options=storage_options, **kwargs
        )
        # ddf, name_function = self._repartition_for_save(npartitions=npartitions)
        #
        # # Dask requires a glob in file paths for partitioning
        # if '*' not in path:
        #     path = path + '*'
        #
        # return ddf.to_csv(path, name_function=name_function, storage_options=storage_options, **kwargs)
    ...
    def to_parquet(self, path, storage_options=None, **kwargs):
        ...
        storage_options: dict, optional
             ...
             Defaults to storage_options set using DatasetFactory.set_default_storage().
        kwargs: dict, optional
            See http://docs.dask.org/en/latest/dataframe-api.html#dask.dataframe.to_parquet
        ...
        if storage_options is None:
            ...
            storage_options = factory.default_storage_options
            logger.info("Using default storage options")
        return self.ddf.compute().to_parquet(
            path, storage_options=storage_options, **kwargs
        )

    def to_json(self, path, npartitions=None, storage_options=None, **kwargs):
        """
        ...
        path: str
             ...
             Supports protocol specifications such as ``"oci://"`, `"s3://"``
         npartitions: int, default: 1
             Number of file parts to split the data to.
         storage_options: dict, optional
              Parameters passed on to the backend filesystem class.
              Defaults to storage_options set using DatasetFactory.set_default_storage().
         kwargs: dict, optional
             See http://docs.dask.org/en/latest/dataframe-api.html#dask.dataframe.to_json
        ...
        return self.ddf.compute().to_json(
            path, storage_options=storage_options, **kwargs
        )

    def to_hdf(self, path, key, npartitions=None, **kwargs):
        """
        ...
        npartitions: int, default: 1
            Number of file parts to split the data to.
        kwargs: dict, optional
            See http://docs.dask.org/en/latest/dataframe-api.html#dask.dataframe.to_hdf

        Examples
        --------
        ds = DatasetFactory.open("data.csv")

        ds.to_hdf("my/path.h5")
        """
        # ddf, name_function = self._repartition_for_save(npartitions=npartitions)
        return self.ddf.compute().to_hdf(path, key, **kwargs)

    def to_avro(self, path, schema=None, **kwargs):
        """
        ...
        data_row_by_row = []
        for i, row in self.ddf.iterrows():
        ...
        parsed_schema = avro_parse_schema(schema=schema)
        new_path = (
            path.replace("*", "0")
            if path[-5:] == ".avro"
            else path.replace("*", "0") + ".avro"
        )
        with fsspec.open(new_path, "wb", **kwargs) as fo:
            avro_writer(fo, parsed_schema, data_row_by_row)
        return new_path
    ...
    def merge(self, data, **kwargs):
        """
        Merges this dataset with another ADSDataset, dask dataframe or pandas dataframe.

        Parameters
        ----------
        data : Union[ADSDataset, dask.dataframe.core.DataFrame or pandas.DataFrame]
            Data to merge.
        kwargs : dict, optional
            kwargs: additional keyword arguments that would be passed to underlying dask dataframe's merge API.
            See https://docs.dask.org/en/latest/dataframe-api.html#dask.dataframe.DataFrame.merge for the list of kwargs
            supported.
        ...
        assert (
            ...
            or isinstance(data, dd.core.DataFrame)
            or isinstance(data, ADSDataset)
        ), "Can only merge datasets if they are of the types pandas, dask, or ads"
        ddf = self.ddf.merge(
            data.ddf if isinstance(data, ADSDataset) else data, **kwargs
        )
        return self._build_new_dataset(ddf, progress=utils.get_progress_bar(3))

    """
    Internal methods
    """

    def _repartition_for_save(self, npartitions=None):
        ddf = self.ddf
        name_function = None
        if npartitions is not None and ddf.npartitions != npartitions:
            ddf = self.ddf.repartition(npartitions=npartitions)

            if npartitions == 1:
                name_function = lambda i: ""
        return ddf, name_function
    ...
    def __getitem__(self, key):
        if isinstance(key, str) or isinstance(key, (tuple, six.string_types)):
            return DaskSeries(self.ddf[key], key)
        else:
            return self._build_new_dataset(self.ddf[key])
    ...
    def _apply(self, func):
        def df_func(*args, _new_target=None, **kwargs):
            ...
            for i, arg in enumerate(args):
                if (
                    isinstance(arg, ADSDataset)
                    or isinstance(arg, dd.core.DataFrame)
            ...
            if (
                ...
                if not has_dataframe_arg:
                    ...
                    df, transformed = ft._transform(self.sampled_df.copy())
    ...
    def _validate_feature(self, feature_names):
        ...
        for feature in feature_names:
            if feature not in self.ddf.columns:
    ...
    def corr(
        ...
        try:
            ...
            logger.error(f"The distributed correlation calculation {e} has failed.")
            return None
    ...
    def _compute_correlation(
        ...
        corr_ddf = self.ddf if not frac else self.ddf.sample(frac=frac)
        ...
        if is_wide_dataset and include_n_features:
            corr_ddf, feature_types_df = self._reduce_dim_for_wide_dataset(
                corr_ddf, feature_types_df, include_n_features
            )
    ...
    def _calc_pearson(
        self, ddf: dd.core.DataFrame, continuous_columns: list
    ) -> pd.DataFrame:
        self._pearson = (
            ddf[continuous_columns].corr().compute()
    ...
    def _return_correlation(
        ...
        if not force_recompute and hasattr(self, "_" + "_".join(method.split())):
            ...
            if method == "pearson":
                self._calc_pearson(corr_ddf, continuous_columns)
                return self._pearson
    ...
    def show_corr(
        ...
        **kwargs,
        ...
        Set correlation_methods='all' to show all correlation charts.
           ...
           correlation_target: str, default to None.
        ...
        **kwargs,
        ...
        with correlations:
            ...
            if not is_wide_dataset:
                feature_types_df = feature_types_df[
                    self.df_stats["missing"] < len(self.ddf)
                ]
