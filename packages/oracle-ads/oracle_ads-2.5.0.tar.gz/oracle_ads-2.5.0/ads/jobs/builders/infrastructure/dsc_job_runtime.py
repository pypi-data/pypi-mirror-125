...
from ads.jobs.builders.runtimes.artifact import ScriptArtifact, NotebookArtifact
...
class CondaRuntimeHandler(RuntimeHandler):
    ...
    def _translate_env(self, runtime: CondaRuntime) -> dict:
        ...
        if runtime.conda:
            envs[self.CONST_CONDA_TYPE] = runtime.conda.get(PythonRuntime.CONST_CONDA_TYPE)
            if runtime.conda.get(PythonRuntime.CONST_CONDA_TYPE) == PythonRuntime.CONST_CONDA_TYPE_SERVICE:
                envs.update(
                    {
                        self.CONST_CONDA_SLUG: runtime.conda.get(PythonRuntime.CONST_CONDA_SLUG),
                    }
                )
            elif runtime.conda.get(PythonRuntime.CONST_CONDA_TYPE) == PythonRuntime.CONST_CONDA_TYPE_CUSTOM:
                ...
                if not region:
                    raise AttributeError(
                        ...
                        "Please specify the region using with_custom_conda(uri, region)."
                    )
                envs.update(
                    {
...
class PythonRuntimeHandler(ScriptRuntimeHandler):
...
class NotebookRuntimeHandler(CondaRuntimeHandler):
    ...
    def extract(self, dsc_job):
        """Extracting NotebookRuntime is not implemented yet.
        Notebook runtime will be extracted as PythonRuntime
        """
...
class GitPythonRuntimeHandler(CondaRuntimeHandler):
    ...
    CONST_GIT_ENTRYPOINT = "GIT_ENTRYPOINT"
    CONST_GIT_ENTRY_FUNCTION = "ENTRY_FUNCTION"
    ...
    SPEC_MAPPINGS = {
        ...
        GitPythonRuntime.CONST_ENTRYPOINT: CONST_GIT_ENTRYPOINT,
        GitPythonRuntime.CONST_ENTRY_FUNCTION: CONST_GIT_ENTRY_FUNCTION,
    ...
    def _extract_envs(self, dsc_job) -> dict:
        ...
        if self.CONST_GIT_URL not in envs or self.CONST_GIT_ENTRYPOINT not in envs:
            raise IncompatibleRuntime()
...
class ContainerRuntimeHandler(RuntimeHandler):
    ...
    def _translate_env(self, runtime: ContainerRuntime) -> dict:
        ...
        if not runtime.image:
            raise ValueError("Please specify container image for ContainerRuntime.")
        envs = super()._translate_env(runtime)
        spec_mappings = {
