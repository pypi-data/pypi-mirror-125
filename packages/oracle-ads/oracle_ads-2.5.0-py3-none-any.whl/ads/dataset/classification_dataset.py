...
class ClassificationDataset(ADSDatasetWithTarget):
    ...
    def __init__(self, ddf, sampled_df, target, target_type, shape, **kwargs):
        ADSDatasetWithTarget.__init__(
            self, ddf, sampled_df, target, target_type, shape, **kwargs
        )
    ...
    def convert_to_text_classification(self, text_column):
        """
        ...
        transformer = (
            "convert_to_text_classification using feature %s".format(text_column),
            FunctionTransformer(
        ...
        if utils.is_same_class(self, BinaryClassificationDataset):
            new_ds = BinaryTextClassificationDataset(
                self.ddf[[self.target.name, text_column]],
    ...
    def down_sample(self, sampler=None):
        ...
        sampler: An instance of SamplerMixin
            ...
            If None, does random down sampling
        ...
        """
        return self._build_new_dataset(
            helper.down_sample(self.ddf, self.target.name)
            if sampler is None
            else helper.sample(
                sampler,
...
class BinaryClassificationDataset(ClassificationDataset):
    ...
    def __init__(
        self, ddf, sampled_df, target, target_type, shape, positive_class=None, **kwargs
    ):
        if positive_class is not None:
            ...
            ddf = ddf.map_partitions(mapper, target, update_arg)
