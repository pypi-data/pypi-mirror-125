...
import dask
import pandas as pd
import six  # python version compatibility layer
...
from ads.dataset.helper import (
    ...
    _num_partitions_for_dataframe,
)
from ads.dataset.regression_dataset import RegressionDataset
from ads.environment import ml_runtime as runtime
...
class DatasetFactory:
    ...
    def open(
        ...
        **kwargs,
        ...
        Examples
        --------
        >>> ds = DatasetFactory.open("/path/to/data.data", format='csv', delimiter=" ", na_values="n/a", \
                    skipinitialspace=True)

        >>> ds = DatasetFactory.open("/path/to/data.csv", target="col_1", prefix="col_",  skiprows=1, \
                    encoding="ISO-8859-1")

        >>> ds = DatasetFactory.open("oci://bucket@namespace/path/to/data.tsv", column_names=["col1", "col2", "col3"], header=0)

        >>> ds = DatasetFactory.open("oci://bucket@namespace/path/to/data.csv", storage_options={"config": "~/.oci/config",
                    "profile": "USER_2"}, delimiter = ';')

        >>> ds = DatasetFactory.open("/path/to/data.parquet", engine='pyarrow', types={"col1": "ordinal", \
                    "col2": "categorical", "col3" : "continuous", "col4" : "float64"})
        ...
        if isinstance(source, str) or isinstance(source, list):
            ...
            name = path.name
        elif isinstance(source, pd.DataFrame) or isinstance(source, dd.DataFrame):
            progress = utils.get_progress_bar(4)
            progress.update("Partitioning data")
            df = source if isinstance(source, pd.DataFrame) else source.compute()
        ...
        ddf = dd.from_pandas(
            df,
            npartitions=_num_partitions_for_dataframe(df)
    ...
    @staticmethod
    def from_dataframe(df, target: str = None, npartitions: int = None, **kwargs):
        """
        Returns an object of ADSDatasetWithTarget or ADSDataset  given a pandas.DataFrame or dask.dataframe.core.DataFrame
        ...
        """
        return DatasetFactory.open(df, target=target, npartitions=npartitions, **kwargs)
                ...
                with remote_file as f1:
                    ...
                    if os.path.exists(local_filepath) and not overwrite:
                        raise FileExistsError(
                            ...
                            f"overwrite these files, please set force_overwrite to True."
                        )
    ...
    def _build_dataset(
        ...
        **kwargs,
        ...
        if target is None:
            ...
            result = ADSDataset(ddf=ddf, sampled_df=sampled_df, shape=shape, **kwargs)
            if progress:
    ...
    def _get_dataset(
        ...
        **init_kwargs,
        ...
        ) or ddf.index.dtype.name.startswith("datetime"):
        ...
        raise ValueError(
            f"Unable to identify problem type. Please specify the data type of {target} using 'types'. "
...
class CustomFormatReaders:
    ...
    def read_sql(cls, path: str, table: str = None, **kwargs) -> pd.DataFrame:
        ...
        :return: pd.DataFrame
        """
        # ToDo: needs some love from the ADB story
        if table is None:
            raise ValueError(
    ...
    def read_xml(path: str, **kwargs) -> pd.DataFrame:
        ...
        path: str
            Path to XML file
        storage_options: dict, optional
            Storage options passed to dask to read the file
        Returns
        -------
...
def get_format_reader(path: ElaboratedPath, **kwargs) -> Callable:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"as {format_key}, if this is not correct, please call again and set the `format` parameter = "
    ...
    for filename in path.paths:
        ...
        if not isinstance(data, pd.DataFrame):
            ...
            raise ValueError(
                ...
                f"Please refer to the usage of {fn_name} to set the correct arguments."
            )
        dfs.append(data)
    if len(dfs) == 0:
        raise ValueError(
