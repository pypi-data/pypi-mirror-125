...
class ADSDatasetWithTarget(ADSDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.ddf[target].isna().sum().compute():
            #
            ...
            # to the ddf - we must rebuild the sample from the new ddf
            #
    ...
    def select_best_features(self, score_func=None, k=12):
        ...
        return self._build_new_dataset(
            tf[1].transform(self.ddf), tf[1].transform(self.sampled_df), transformers=tf
        )
    ...
    def auto_transform(
        ...
        with utils.get_progress_bar(5) as progress:
            ddf, sampled_df, transformer_pipeline = self._transform(
                progress=progress,
    ...
    def visualize_transforms(self):
        ...
        helper.visualize_transformation(
            ...
            text="- partition(s): {}\\l- rows: {}\\l- columns: {}\\l".format(
                self.ddf.npartitions, *self.shape
            ),
        )
    ...
    def _suggested_code(self, action, recommendation_type, variable):
        ...
        if action == "Up-sample":
            if importlib.util.find_spec("imblearn") is None:
                return ".up_sample(sampler='default') \\n Please `pip install imbalanced-learn` to use default up-sampler."
            else:
                return ".up_sample(sampler='default')"
    ...
    def get_recommendations(
        ...
        display_format: string, defaults to 'widget'. Should be either 'widget' or 'table'. If 'widget',
    ...
    def train_test_split(self, test_size=0.1, random_state=utils.random_state):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            self.ddf.drop(self.target.name, axis=1),
            self.ddf[self.target.name],
            test_size=test_size,
    ...
    def _transform(
        ...
        for step in transformer_pipeline.steps:
            ...
            ddf = step[1].transform(ddf, progress=progress, fit_transform=True)
        return ddf, sampled_df, transformer_pipeline
