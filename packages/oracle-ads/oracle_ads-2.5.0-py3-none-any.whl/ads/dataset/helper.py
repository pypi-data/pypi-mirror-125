    ...
    def _determine_format(self):
        ...
        if self._format in [None, "infer"]:
            ...
            if format_keys.count(format_keys[0]) != len(format_keys):
                raise ValueError(
                    f"Got multiple formats from the source: {self._original_source}. Please run again "
...
def _get_dtype_from_error(e):
    ...
    if "mismatched dtypes" in error_string.lower():
        ...
        if found_dtype_dict:
            logger.warning(
                "Dask type-inference/coercion failed. Retrying with dtype=%s.".format(
                    found_dtype_dict
                ),
                exc_info=True,
            )
...
# convert spark dataframe into dask dataframe
def _spark_to_dask(sdf):
    df_dask = sdf.rdd.mapPartitions(_map_to_pandas).collect()
    if len(df_dask) == 1:
        # edge case handling: when the spark dataframe is not partitioned, meaning npartition = 1
        df_dask = dd.from_pandas(df_dask[0], npartitions=1)
    else:
        df_dask = dd.concat(df_dask)
    df_dask.columns = sdf.columns
    return df_dask
...
def write_parquet(path, data, metadata_dict={}, storage_options=None):
    """
    ...
    path : str
        Path to write to
    data : dask.dataframe.core.DataFrame
    metadata_dict: dict, optional
         custom metadata for parquet file
    open_with: function, optional
         function to open the given path
         Uses local file system open if None
    ...
    pf : ParquetFile
    """
    assert isinstance(data, dask.dataframe.core.DataFrame)

    # Write dataframe
    data.to_parquet(path, compression="GZIP", storage_options=storage_options)

    # # Open file
    # open_with = fsspec.open_files(path, mode='rb', **storage_options)[0].fs.open
    # pf = ParquetFile(path, open_with=open_with)
    #
    # # Add metadata
    # new_fmd = copy.copy(pf.fmd)
    # for key in metadata_dict.keys():
    #     metadata_key = parquet_thrift.KeyValue()
    #     metadata_key.key = key
    #     metadata_key.value = base64.b64encode(cloudpickle.dumps(metadata_dict[key])).decode('utf8')
    #     new_fmd.key_value_metadata.append(metadata_key)
    #
    # # Overwrite file metadata
    # fn = os.path.join(path, '_metadata')
    # write_common_metadata(fn, new_fmd, no_row_groups=False, open_with=open_with)
    #
    # fn = os.path.join(path, '_common_metadata')
    # write_common_metadata(fn, new_fmd, open_with=open_with)
...
def generate_sample(
    ...
    **kwargs,
    ...
    if sample_size and len(ddf) > sample_size:
        frac = min(1.0, sample_size * 1.05 / n)
        df = ddf.sample(frac=frac, random_state=42).compute()
    ...
    if sampler == "default":
        ...
        if not imblearn_found:
            raise ModuleNotFoundError(
                ...
                Please install `imblearn` with `pip install imbalanced-learn`
...
def _get_imblearn_sampler(X, y):
    ...
    if isinstance(y, pd.Series) or isinstance(y, pd.DataFrame):
        ...
        min_sample_size = y.value_counts().min().compute()
...
def down_sample(df, target):
    ...
    Returns
    -------
    downsampled_df : Union[pandas.DataFrame, dask.dataframe.core.DataFrame]
    """
    dfs = []
    if isinstance(df, dd.core.DataFrame) and all(
        [division is None for division in df.divisions]
    ):
        # for dask dataframes, calling set_index will set the divisions. Concatenation with unknown divisions fails.
    ...
    return (
        ...
        else dd.concat(dfs, interleave_partitions=True)
    )
...
def _sample(sampler, X, y):
    ...
    if isinstance(X_resampled, pd.DataFrame):
        X_resampled = dd.from_pandas(
            X_resampled, npartitions=_num_partitions_for_dataframe(X_resampled)
        )
    else:
        X_resampled = dd.from_array(X_resampled, columns=X.columns.values)
    if isinstance(y_resampled, pd.Series):
        y_resampled = dd.from_pandas(
            y_resampled, npartitions=_num_partitions_for_dataframe(y_resampled)
        )
    else:
        y_resampled = dd.from_array(y_resampled, columns=[y.name])[y.name]

    for k in X.dtypes.keys():
        X_resampled[k] = X_resampled[k].astype(X.dtypes[k].name)
    balanced_df = concatenate(X_resampled, y_resampled)
    return balanced_df.compute() if isinstance(X, pd.DataFrame) else balanced_df
