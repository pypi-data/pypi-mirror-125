...
AlreadyWrittenModels = ["torch", "mxnet"]
...
def prepare_generic_model(
    ...
    **kwargs
    ...
    if use_case_type and use_case_type not in UseCaseType:
        raise ValueError(
            f"Invalid usecase type. Please choose from {UseCaseType.values()}"
        )
    ...
    with progress if artifact_type_generic else utils.get_progress_bar(
        ...
        try:
            ...
            logger.warning("Failed to populate the input and output schema.")
...
def serialize_model(
    ...
    try:
        ...
        if model_type == "sklearn":
            ...
            onx = _automl_to_onnx(model=model, X=X, y=y)
...
def _automl_to_onnx(model=None, target_dir=None, X=None, y=None, **kwargs):
    if not hasattr(model, "to_onnx"):
        logger.error(
            "This version of AutoML does not support ONNX serialization. "
            "Upgrade to version >= 0.5.0.",
        )
        raise Exception("Serialization not supported, please upgrade automl>=0.5.0")
    if X is None:
        raise Exception(
            "To serialize an Oracle Labs automl model, please pass in an example of the dataset."
        )
    onx = model.to_onnx(X, y)
    return onx
...
def _mxnet_to_onnx(model=None, target_dir=None, X=None, y=None, **kwargs):
    ...
    assert sym is not None and params is not None, (
        "Please pass `sym` [Path to the json file or Symbol object] and "
...
# # Note to developers: If you make any changes to this class, please copy and paste those changes over to
