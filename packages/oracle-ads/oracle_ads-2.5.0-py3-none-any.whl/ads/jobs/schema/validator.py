...
def load_schema(schema_path):
    if path.exists(path.join(path.dirname(__file__), schema_path)):
        with fsspec.open(path.join(path.dirname(__file__), schema_path), mode='r', encoding='utf8') as f:
            schema = json.load(f)
        return schema
    else:
...
class ValidateJob():
    ...
    def validate(self):
        ...
        if "infrastructure" in self.file["spec"]:
            infrastructure_valid = ValidateInfrastructure(self.file["spec"]["infrastructure"]).validate()
