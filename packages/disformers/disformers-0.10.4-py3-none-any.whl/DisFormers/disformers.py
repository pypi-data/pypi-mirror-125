...
class DisFormersBot:
    ...
    async def __hendle_messages(self, message: Message):
        ...
        if message.content.startswith(self.prefix):
            async with message.channel.typing():
                ...
                chat_ = self.bot.loop.run_until_complete(self.chat(user_input))
                return await message.reply(content=chat_)
    ...
    async def client_message(self,message:Message):
        ...
        if message.content.startswith(self.prefix):
            async with message.channel.typing():
                user_input = message.content[len(self.prefix):]
                chat_ = self.bot.loop.run_in_executor(None,self.chat(user_input))
                return await message.reply(content=chat_)
