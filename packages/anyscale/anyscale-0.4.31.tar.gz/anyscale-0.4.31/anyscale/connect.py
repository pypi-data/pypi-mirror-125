...
class ClientBuilder:
    ...
    def connect(self) -> AnyscaleClientContext:  # noqa
        ...
        if self._project_dir:
            ...
            (
                ...
                project_id,
                project_name,
            ) = self._get_or_create_scratch_project()
        else:
            raise ValueError(
    ...
    def _get_or_create_scratch_project(self) -> Tuple[str, str, str]:
        """
        Get or create a scratch project, including the directory.

        Returns:
        project_dir, project_id, and project_nae
        """
        ...
        print(project_id)
        if not os.path.exists(project_dir) and project_id:
            self._clone_project(project_dir, project_name)
        else:
            project_id, project_name = self._ensure_project_setup_at_dir(
                project_dir, project_name
            )
        return project_dir, project_id, project_name
