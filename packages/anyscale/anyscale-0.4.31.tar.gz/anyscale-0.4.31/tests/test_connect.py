...
def test_autosuspend_init_args() -> None:
    builder = ClientBuilder(
        anyscale_sdk=Mock(), api_client=Mock(), anyscale_api_client=Mock()
    )
    builder._init_args()
...
def test_autosuspend_from_env() -> None:
    unconfigured_builder = ClientBuilder(
        anyscale_sdk=Mock(), api_client=Mock(), anyscale_api_client=Mock()
    )
    unconfigured_builder._fill_unset_configs_from_env()
...
def test_clone_scratch_dir(tmp_path: Path, project_test_data: Project) -> None:
    ...
    builder._find_project_id = lambda _: "12345"
    builder._log_cluster_configs = Mock()
    sdk.create_project.return_value = ProjectResponse(result=project_test_data)
...
def test_log_cluster_configs(tmp_path: Path, project_test_data):
    (tmp_path / "scratch").absolute().as_posix()
    api_client = Mock()
    sess = _make_session(0, "Running")
    sess_resp = Mock()
    ...
    builder._log_cluster_configs.assert_called_with(
        ...
        compute_template_id="mock_compute_template_id",
        url="https://beta.anyscale.com/projects/mock_project_id/clusters/mock_id",
    )
...
def test_forward_argument() -> None:
