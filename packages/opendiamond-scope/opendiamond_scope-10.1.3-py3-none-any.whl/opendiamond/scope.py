...
class ScopeCookie:
    ...
    def __repr__(self):
        return (
            "<ScopeCookie {}, blaster {}, servers {}, urls {}, expiration {}>".format(
                self.serial,
                self.blaster,
                self.servers,
                self.scopeurls,
    ...
    def parse(cls, data):
        ...
        try:
            return cls(serial, expires, blaster, servers, scopeurls, data, signature)
        except NameError:
...
def generate_cookie(
    scopeurls, servers, proxies=None, keyfile=None, expires=None, blaster=None
):
    ...
    for i in range(n):
        scope = [
            "/proxy/%dof%d/%s:5873%s" % (i + 1, n, server, url)
