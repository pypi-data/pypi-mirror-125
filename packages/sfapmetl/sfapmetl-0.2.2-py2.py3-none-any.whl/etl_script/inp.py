...
def form_task_record(data, flag, tags, taskId, bufferData=None):
    metadata = {
...
def form_job_record(data, flag, tags, bufferData=None):
    rec = {
    ...
    for data in started_rec:
        ...
        if data.get("TABLE_NAME","") == "INCR ETL":
            if 'job' not in bufferData[jobId].keys():
                record = form_job_record(data, 'started', tags)
            ...
            if taskId not in task_exclusion_list:
                record = form_task_record(data, 'started', tags, taskId)
    ...
    for data in completed_rec:
        ...
        if data.get("TABLE_NAME","") == "INCR ETL":
            if data['jobId'] in started_records.keys():
                ...
                completed_record['time'] = data['time']
                ...
                record = form_job_record(data, 'completed', tags, bufferData)
            ...
            if taskId not in task_exclusion_list:
                ...
                """
                if taskId in started_records.keys():
                    ...
                    rec = form_task_record(data, 'completed', tags, taskId, bufferData)
                    if rec is not None:
                        del bufferData[data['jobId']][taskId]
