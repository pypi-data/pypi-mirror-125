...
from aug_sfutils import sfdics, sfhread, sfobj, manage_ed, parse_kwargs, str_byt, libddc
...
def getlastshot():
    ...
    if bshot is None:
        ...
        try:
            ...
            logger.error('ERROR sfread.getlastshot: cannot convert string %s to integer' %sshot)
            nshot = None
...
# assuming first entry wins
    ...
    def getparset(self, pset):
        ...
        sfo = self.sfh[pset]
        logger.debug('ParSet %s %d', pset, sfo.length)
        otyp = sfo.obj_type
        if otyp not in (3, 4):
            return None
...
#            meta.phys_unit = sfdics.unit_d[unit]
            ...
            logger.debug('%s %d %d %d', pname, unit, dfmt, n_items)
            ...
            logger.debug('PS %s, PN %s', pset, pname)
            print(data)
    ...
    def getts06(self, obj):
        ...
        """
        ts6 = 0
        for obj in self.getrelnames(obj):
        ...
        logger.debug('getts06 %s %d', obj, ts6)
        if ts6 < 1e15:
