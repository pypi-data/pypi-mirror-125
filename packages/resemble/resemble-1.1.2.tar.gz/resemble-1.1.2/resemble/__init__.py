	...
	class _V2:
		...
		class _VoicesV2:
			...
			def create(self, name: str):
				r = requests.post(Resemble.endpoint('v2', 'voices'), headers=Resemble._headers, json={
					'name': name
				})
				return r.json()

			def update(self, uuid: str, name: str):
				r = requests.put(Resemble.endpoint('v2', f"voices/{uuid}"), headers=Resemble._headers, json={
					'name': name
				})
				return r.json()
