...
class Context:
    ...
    async def callback(self, content: str = '', embeds: list = [], components: list = [], mentions: list = [], ephemeral: bool= False):
        ...
        async with self.__socket.session.post(self.callback_url, json=message, headers=self.__socket.headers) as maybe_send:
            self.message = Message(self.__socket, await (await self.__socket.session.get(f"https://discord.com/api/v8/webhooks/{self.raw['application_id']}/{self.token}/messages/@original", headers=self.__socket.headers)).json())
...
class SelectMenuContext:
    ...
    async def callback(self, content: str = '', embeds: list = [], components: list = [], mentions: list = [], ephemeral: bool= False):
        ...
        async with self.__socket.session.post(self.callback_url, json=message, headers=self.__socket.headers) as maybe_send:
            self.message = Message(self.__socket, await (await self.__socket.session.get(f"https://discord.com/api/v8/webhooks/{self.raw['application_id']}/{self.raw['token']}/messages/@original", headers=self.__socket.headers)).json())
            if maybe_send.status != 204:
                print(await maybe_send.json())
...
class ButtonContext:
    ...
    async def edit(self, content: str = '', embeds: list = [], components: list = [], mentions: list = [], ephemeral: bool= False):
        message = {
            "type": 4,
            "data": {
                "content": content,
        ...
        if ephemeral:
            message['data']['flags'] = 64
        try:
            await self.__socket.session.patch(f"https://discord.com/api/v8/webhooks/{self.raw['application_id']}/{self.raw['token']}/messages/@original", json=message, headers=self.__socket.headers)
            self.message = Message(self.__socket, await (await self.__socket.session.get(f"https://discord.com/api/v8/webhooks/{self.raw['application_id']}/{self.raw['token']}/messages/@original", headers=self.__socket.headers)).json())
        except Exception:
            return
