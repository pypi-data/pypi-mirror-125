...
class Mx(BaseMx):
    ...
    def package_data(self, data: t.Any, callback: str) -> json:
        """
        打包响应数据
        :param data: 视图返回的数据
        :param callback: 回调标识
        ...
        if isinstance(data, tuple):
            data = json.dumps(
                {'status': 'success', 'errmsg': self._(data[0]), 'data': data[1] if len(data) == 2 else data[1:]},
                ensure_ascii=False) if self._ else json.dumps(
                {'status': 'success', 'errmsg': data[0], 'data': data[1] if len(data) == 2 else data[1:]},
                ensure_ascii=False)
        else:
            data = json.dumps({'status': 'failed', 'errmsg': self._(data)},
                              ensure_ascii=False) if self._ else json.dumps({'status': 'failed', 'errmsg': data},
                                                                            ensure_ascii=False)
