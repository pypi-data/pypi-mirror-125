...
class _Authenticator:
    ...
    def initialize(self, project: Project, api_id: Optional[str] = None,
        ...
        if not api_key or not api_id:
            self._api_key, self._api_id = _Authenticator.read_credentials()

        if not self._api_key or not self._api_id:
            raise CredentialsNotFound()

        if not _Authenticator.is_valid_api_id(api_id):
            if _Authenticator.is_valid_api_key(api_id):
                raise KeysMixedUp()
            raise MalformedKeys()

        if not _Authenticator.is_valid_api_key(api_key):
            raise MalformedKeys()
