...
version = '2021.10.12-p[3.8]wr-05.01.007'  # year.month.day-version_type-major.minor.patch
...
class SdDataDecoding:
    ...
    def all_keys_searching(self, cur_str=None):
        ...
        if not cur_str:
            ...
            cur_string = cur_str
        search_pattern = r'(^ *\.* *([A-Za-z0-9_][A-Za-z0-9_.]+)\s*(?=:))|((?<=,) *\.* *([A-Za-z0-9_][A-Za-z0-9_.]+)\s*(?=:))'
    ...
    def searching_of_initial_key(self, cur_str=None):
        ...
        if not cur_str:
            ...
            cur_string = cur_str
        final_result = ''
        search_pattern = r'^( *\.* *([A-Za-z0-9_][A-Za-z0-9_.]*)\s*(?=\:))'
        search_result = re.search(pattern=search_pattern, string=cur_string)
        if search_result:
    ...
    def primary_keys_searching(self):
        ...
        """
        cur_string = self.cur_string
        searching_root_section_names_regex = r'^(\s*|[a-zA-Z0-9_])[a-zA-Z0-9._]*\s*(?==)'
