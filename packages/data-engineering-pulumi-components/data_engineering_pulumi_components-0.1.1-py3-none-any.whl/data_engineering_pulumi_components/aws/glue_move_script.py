...
def date_converter(dataframe: DataFrame):
    ...
    for column in date_columns:
        # check for iso formatting
        if date_columns[column] == "iso":
            dataframe = dataframe.withColumn(
                column, F.to_timestamp(column, "yyyy-MM-dd'T'HH:mm:ss.SSSSSXXX")
            )
            print("converted column " + column)
        else:
            dataframe = dataframe.withColumn(
                column, F.to_timestamp(column, date_columns[column])
            )
            print("converted column " + column)
...
def files_to_dynamic_frame(
    ...
    try:
        ...
        finalised_datasource.printSchema()

        finalised_datasource.show(n=2)
