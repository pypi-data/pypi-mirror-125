...
class App(wx.App):
  ...
  'file_name_play_audio', 'file_name_play_samples', 'file_name_play_cq', 'freedv_sideband', 'freedv_mode'] 
  def __init__(self):
    global application
    QS.AppStatus(1)
    application = self
  ...
  def OnInit(self):
    ...
    self.freedv_sideband = 'Upper sideband'	# restore FreeDV sideband setting
  ...
  def MakeButtons(self, frame, gbs):
    ...
    if conf.add_freedv_button:
      self.OnFreedvMenu(None, self.freedv_sideband)	# Restore sideband
