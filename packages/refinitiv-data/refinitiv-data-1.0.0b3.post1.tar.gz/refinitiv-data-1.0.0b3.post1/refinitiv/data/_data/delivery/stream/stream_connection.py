...
class StreamConnection(threading.Thread, LogReporter):
    ...
    def _on_ws_message(self, ws: websocket.WebSocketApp, s: str) -> None:
        self.debug(f"{self._classname} on_ws_message")
        ...
        DEBUG and self.debug(f"{self._classname} on_ws_message {messages}")
    ...
    def _do_on_messages(self, messages: List[Dict]) -> None:
    ...
    def _on_ws_close(
        ...
        if self.is_disposed:
            self.debug(
                f"{self._classname} cannot process on_ws_close "
                f"(state={self.state}, "
