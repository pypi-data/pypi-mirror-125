...
class StreamCxnCache(object):
    ...
    async def get_cxn_async(
        ...
        if self.has_cxn(session, content_type):
            ...
            from ...delivery.stream._stream_factory import create_stream_cxn_async

            cxn = await create_stream_cxn_async(content_type, protocol_type, session)
            cxn.daemon = True
            cxn.start()
    ...
    async def close_cxns_async(self, session: "Session") -> None:
        tasks = (
            item.cxn.disconnect_async() if item.is_using else item.cxn.dispose_async()
