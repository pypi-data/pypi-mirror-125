...
date_fields = ['year', 'month', 'day', 'hour', 'minute', 'second',
    'microsecond', 'nanosecond', 'quarter', 'dayofyear', 'dayofweek',
    'daysinmonth', 'days_in_month', 'is_leap_year', 'is_month_start',
    'is_month_end', 'is_quarter_start', 'is_quarter_end', 'is_year_start',
    'is_year_end', 'week', 'weekofyear', 'weekday']
...
class PandasTimestampModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oux__xlz = [('year', ts_field_typ), ('month', ts_field_typ), ('day',
        ...
        models.StructModel.__init__(self, dmm, fe_type, oux__xlz)
...
def unbox_pandas_timestamp(typ, val, c):
    tafby__swdjv = c.pyapi.object_getattr_string(val, 'year')
    lmur__xinbo = c.pyapi.object_getattr_string(val, 'month')
    msd__umn = c.pyapi.object_getattr_string(val, 'day')
    pkzy__xyr = c.pyapi.object_getattr_string(val, 'hour')
    zjuu__krn = c.pyapi.object_getattr_string(val, 'minute')
    kcm__yjdf = c.pyapi.object_getattr_string(val, 'second')
    biq__teyco = c.pyapi.object_getattr_string(val, 'microsecond')
    ipxkj__cfx = c.pyapi.object_getattr_string(val, 'nanosecond')
    pttq__ssqj = c.pyapi.object_getattr_string(val, 'value')
    ldshm__qnqo = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    ldshm__qnqo.year = c.pyapi.long_as_longlong(tafby__swdjv)
    ldshm__qnqo.month = c.pyapi.long_as_longlong(lmur__xinbo)
    ldshm__qnqo.day = c.pyapi.long_as_longlong(msd__umn)
    ldshm__qnqo.hour = c.pyapi.long_as_longlong(pkzy__xyr)
    ldshm__qnqo.minute = c.pyapi.long_as_longlong(zjuu__krn)
    ldshm__qnqo.second = c.pyapi.long_as_longlong(kcm__yjdf)
    ldshm__qnqo.microsecond = c.pyapi.long_as_longlong(biq__teyco)
    ldshm__qnqo.nanosecond = c.pyapi.long_as_longlong(ipxkj__cfx)
    ldshm__qnqo.value = c.pyapi.long_as_longlong(pttq__ssqj)
    c.pyapi.decref(tafby__swdjv)
    c.pyapi.decref(lmur__xinbo)
    c.pyapi.decref(msd__umn)
    c.pyapi.decref(pkzy__xyr)
    c.pyapi.decref(zjuu__krn)
    c.pyapi.decref(kcm__yjdf)
    c.pyapi.decref(biq__teyco)
    c.pyapi.decref(ipxkj__cfx)
    c.pyapi.decref(pttq__ssqj)
    epcg__gomr = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(ldshm__qnqo._getvalue(), is_error=epcg__gomr)


@box(PandasTimestampType)
def box_pandas_timestamp(typ, val, c):
    kiril__vvd = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    tafby__swdjv = c.pyapi.long_from_longlong(kiril__vvd.year)
    lmur__xinbo = c.pyapi.long_from_longlong(kiril__vvd.month)
    msd__umn = c.pyapi.long_from_longlong(kiril__vvd.day)
    pkzy__xyr = c.pyapi.long_from_longlong(kiril__vvd.hour)
    zjuu__krn = c.pyapi.long_from_longlong(kiril__vvd.minute)
    kcm__yjdf = c.pyapi.long_from_longlong(kiril__vvd.second)
    ykusu__mqhoi = c.pyapi.long_from_longlong(kiril__vvd.microsecond)
    nvolf__kjy = c.pyapi.long_from_longlong(kiril__vvd.nanosecond)
    umbi__xirn = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timestamp))
    res = c.pyapi.call_function_objargs(umbi__xirn, (tafby__swdjv,
        lmur__xinbo, msd__umn, pkzy__xyr, zjuu__krn, kcm__yjdf,
        ykusu__mqhoi, nvolf__kjy))
    c.pyapi.decref(tafby__swdjv)
    c.pyapi.decref(lmur__xinbo)
    c.pyapi.decref(msd__umn)
    c.pyapi.decref(pkzy__xyr)
    c.pyapi.decref(zjuu__krn)
    c.pyapi.decref(kcm__yjdf)
    c.pyapi.decref(ykusu__mqhoi)
    c.pyapi.decref(nvolf__kjy)
    return res
...
def init_timestamp(typingctx, year, month, day, hour, minute, second,
    ...
    def codegen(context, builder, sig, args):
        (year, month, day, hour, minute, second, inxy__vje, vqu__cekx, value
            ) = args
        ...
        ts.minute = minute
        ts.second = second
        ts.microsecond = inxy__vje
        ts.nanosecond = vqu__cekx
        ts.value = value
        return ts._getvalue()
...
def constant_timestamp(context, builder, ty, pyval):
    ...
    ldshm__qnqo = cgutils.create_struct_proxy(ty)(context, builder)
    ldshm__qnqo.year = year
    ldshm__qnqo.month = month
    ldshm__qnqo.day = day
    ldshm__qnqo.hour = hour
    ldshm__qnqo.minute = minute
    ldshm__qnqo.second = second
    ldshm__qnqo.microsecond = microsecond
    ldshm__qnqo.nanosecond = nanosecond
    ldshm__qnqo.value = value
    return ldshm__qnqo._getvalue()
...
def overload_pd_timestamp(ts_input=_no_input, freq=None, tz=None, unit=None,
    ...
    if isinstance(ts_input, types.Number):
        ...
        if not is_overload_constant_str(unit):
            raise BodoError(
                'pd.Timedelta(): unit argument must be a constant str')
        unit = pd._libs.tslibs.timedeltas.parse_timedelta_unit(
            get_overload_const_str(unit))
        apq__dpi, precision = pd._libs.tslibs.conversion.precision_from_unit(
            unit)
        ...
        def impl_float(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            edtrx__hkoo = np.int64(ts_input)
            fhei__bobwb = ts_input - edtrx__hkoo
            if precision:
                fhei__bobwb = np.round(fhei__bobwb, precision)
            value = edtrx__hkoo * apq__dpi + np.int64(fhei__bobwb * apq__dpi)
            return convert_datetime64_to_timestamp(integer_to_dt64(value))
        return impl_float
...
def overload_week_number(ptt):
    ...
    def pd_week_number(ptt):
        ljmuw__vugdu, axah__zxlp, ljmuw__vugdu = get_isocalendar(ptt.year,
            ptt.month, ptt.day)
        return axah__zxlp
...
def overload_pd_timestamp_isocalendar(ptt):
    ...
    def impl(ptt):
        year, axah__zxlp, oad__jfey = get_isocalendar(ptt.year, ptt.month,
            ptt.day)
        return year, axah__zxlp, oad__jfey
    return impl
...
def overload_pd_timestamp_isoformat(ts, sep=None):
    if is_overload_none(sep):
        ...
        def timestamp_isoformat_impl(ts, sep=None):
            ...
            jhogs__agv = str_2d(ts.hour) + ':' + str_2d(ts.minute
                ) + ':' + str_2d(ts.second)
...
def overload_pd_timestamp_day_name(ptt, locale=None):
    goon__imoh = dict(locale=locale)
    zfu__htr = dict(locale=None)
    check_unsupported_args('Timestamp.day_name', goon__imoh, zfu__htr)

    def impl(ptt, locale=None):
        xjbnt__hghx = ('Monday', 'Tuesday', 'Wednesday', 'Thursday',
            'Friday', 'Saturday', 'Sunday')
        ljmuw__vugdu, ljmuw__vugdu, zqxxi__nsh = ptt.isocalendar()
        return xjbnt__hghx[zqxxi__nsh - 1]
    return impl
...
def overload_pd_timestamp_month_name(ptt, locale=None):
    goon__imoh = dict(locale=locale)
    zfu__htr = dict(locale=None)
    check_unsupported_args('Timestamp.month_name', goon__imoh, zfu__htr)

    def impl(ptt, locale=None):
        tzh__jrcsw = ('January', 'February', 'March', 'April', 'May',
            'June', 'July', 'August', 'September', 'October', 'November',
            'December')
        return tzh__jrcsw[ptt.month - 1]
    return impl
...
def extract_year_days(typingctx, dt64_t=None):
    ...
    def codegen(context, builder, sig, args):
        ossn__lnop = cgutils.alloca_once(builder, lir.IntType(64))
        builder.store(args[0], ossn__lnop)
        year = cgutils.alloca_once(builder, lir.IntType(64))
        aot__kwp = cgutils.alloca_once(builder, lir.IntType(64))
        dce__odf = lir.FunctionType(lir.VoidType(), [lir.IntType(64).
            as_pointer(), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        eiwm__hpkpg = cgutils.get_or_insert_function(builder.module,
            dce__odf, name='extract_year_days')
        builder.call(eiwm__hpkpg, [ossn__lnop, year, aot__kwp])
        return cgutils.pack_array(builder, [builder.load(ossn__lnop),
            builder.load(year), builder.load(aot__kwp)])
...
def get_month_day(typingctx, year_t, days_t=None):
    ...
    def codegen(context, builder, sig, args):
        ...
        dce__odf = lir.FunctionType(lir.VoidType(), [lir.IntType(64), lir.
            IntType(64), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        eiwm__hpkpg = cgutils.get_or_insert_function(builder.module,
            dce__odf, name='get_month_day')
        builder.call(eiwm__hpkpg, [args[0], args[1], month, day])
...
def get_day_of_year(year, month, day):
    zats__biugh = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 
        365, 0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366]
    nsclh__vkzhl = is_leap_year(year)
    biq__avoo = zats__biugh[nsclh__vkzhl * 13 + month - 1]
    jqwc__sundm = biq__avoo + day
    return jqwc__sundm


@register_jitable
def get_day_of_week(y, m, d):
    anul__ipphx = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4]
    y -= m < 3
    day = (y + y // 4 - y // 100 + y // 400 + anul__ipphx[m - 1] + d) % 7
    return (day + 6) % 7
...
def get_days_in_month(year, month):
    ...
    kyrg__dltz = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 29, 
        31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    return kyrg__dltz[12 * is_leap_year + month - 1]
...
def convert_datetime64_to_timestamp(dt64):
    ossn__lnop, year, aot__kwp = extract_year_days(dt64)
    month, day = get_month_day(year, aot__kwp)
    return pd.Timestamp(year, month, day, ossn__lnop // (60 * 60 * 
        1000000000), ossn__lnop // (60 * 1000000000) % 60, ossn__lnop // 
        1000000000 % 60, ossn__lnop // 1000 % 1000000, ossn__lnop % 1000)


@numba.njit(no_cpython_wrapper=True)
def convert_numpy_timedelta64_to_datetime_timedelta(dt64):
    van__uuxjq = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    jds__vsfhn = van__uuxjq // (86400 * 1000000000)
    udh__pga = van__uuxjq - jds__vsfhn * 86400 * 1000000000
    toc__gwhf = udh__pga // 1000000000
    ezvp__axb = udh__pga - toc__gwhf * 1000000000
    ktnhr__zmvu = ezvp__axb // 1000
    return datetime.timedelta(jds__vsfhn, toc__gwhf, ktnhr__zmvu)
...
def convert_numpy_timedelta64_to_pd_timedelta(dt64):
    van__uuxjq = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    return pd.Timedelta(van__uuxjq)
...
def overload_to_datetime(arg_a, errors='raise', dayfirst=False, yearfirst=
    ...
    if isinstance(arg_a, bodo.hiframes.pd_series_ext.SeriesType):
        ...
        def impl_series(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            dvzc__ypgzs = bodo.hiframes.pd_series_ext.get_series_index(arg_a)
            zedlh__tkwj = bodo.hiframes.pd_series_ext.get_series_name(arg_a)
            olx__kxrny = bodo.utils.conversion.coerce_to_ndarray(pd.
            ...
            return bodo.hiframes.pd_series_ext.init_series(olx__kxrny,
                dvzc__ypgzs, zedlh__tkwj)
        return impl_series
    if arg_a == bodo.hiframes.datetime_date_ext.datetime_date_array_type:
        ddux__tjs = np.dtype('datetime64[ns]')
        iNaT = pd._libs.tslibs.iNaT

        def impl_date_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            zszzg__lwkxh = len(arg_a)
            sjj__eazhf = np.empty(zszzg__lwkxh, ddux__tjs)
            for hqivr__xkk in numba.parfors.parfor.internal_prange(zszzg__lwkxh
                ):
                val = iNaT
                if not bodo.libs.array_kernels.isna(arg_a, hqivr__xkk):
                    data = arg_a[hqivr__xkk]
                    val = (bodo.hiframes.pd_timestamp_ext.
                        npy_datetimestruct_to_datetime(data.year, data.
                        month, data.day, 0, 0, 0, 0))
                sjj__eazhf[hqivr__xkk
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(val)
            return bodo.hiframes.pd_index_ext.init_datetime_index(sjj__eazhf,
                None)
    ...
    if isinstance(arg_a, CategoricalArrayType
        ...
        def impl_cat_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            zszzg__lwkxh = len(arg_a)
            sjj__eazhf = np.empty(zszzg__lwkxh, ddux__tjs)
            tmxes__xvx = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(arg_a))
            kcuv__ocdgi = pandas_string_array_to_datetime(arg_a.dtype.
                categories.values, errors, dayfirst, yearfirst, utc, format,
                exact, unit, infer_datetime_format, origin, cache).values
            for hqivr__xkk in numba.parfors.parfor.internal_prange(zszzg__lwkxh
                ):
                c = tmxes__xvx[hqivr__xkk]
                if c == -1:
                    bodo.libs.array_kernels.setna(sjj__eazhf, hqivr__xkk)
                    continue
                sjj__eazhf[hqivr__xkk] = kcuv__ocdgi[c]
            return bodo.hiframes.pd_index_ext.init_datetime_index(sjj__eazhf,
                None)
        return impl_cat_arr
...
def overload_to_timedelta(arg_a, unit='ns', errors='raise'):
    ...
    if isinstance(arg_a, types.Float):
        m, mofu__qam = pd._libs.tslibs.conversion.precision_from_unit(unit)

        def impl_float_scalar(arg_a, unit='ns', errors='raise'):
            val = float_to_timedelta_val(arg_a, mofu__qam, m)
            return pd.Timedelta(val)
        return impl_float_scalar
    if isinstance(arg_a, types.Integer):
        m, ljmuw__vugdu = pd._libs.tslibs.conversion.precision_from_unit(unit)
    ...
    if is_iterable_type(arg_a) and not isinstance(arg_a, types.BaseTuple):
        m, mofu__qam = pd._libs.tslibs.conversion.precision_from_unit(unit)
        yqq__kvjn = np.dtype('timedelta64[ns]')
        if isinstance(arg_a.dtype, types.Float):

            def impl_float(arg_a, unit='ns', errors='raise'):
                zszzg__lwkxh = len(arg_a)
                sjj__eazhf = np.empty(zszzg__lwkxh, yqq__kvjn)
                for hqivr__xkk in numba.parfors.parfor.internal_prange(
                    zszzg__lwkxh):
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, hqivr__xkk):
                        val = float_to_timedelta_val(arg_a[hqivr__xkk],
                            mofu__qam, m)
                    sjj__eazhf[hqivr__xkk
        ...
        if arg_a.dtype == datetime_timedelta_type:
            ...
            def impl_datetime_timedelta(arg_a, unit='ns', errors='raise'):
                ...
                for hqivr__xkk in numba.parfors.parfor.internal_prange(
                    ...
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, hqivr__xkk):
                        kbxw__aeyju = arg_a[hqivr__xkk]
                        val = (kbxw__aeyju.microseconds + 1000 * 1000 * (
                            kbxw__aeyju.seconds + 24 * 60 * 60 *
                            kbxw__aeyju.days)) * 1000
                    sjj__eazhf[hqivr__xkk
                        ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                        val)
                return bodo.hiframes.pd_index_ext.init_timedelta_index(
...
def float_to_timedelta_val(data, precision, multiplier):
    edtrx__hkoo = np.int64(data)
    fhei__bobwb = data - edtrx__hkoo
    if precision:
        fhei__bobwb = np.round(fhei__bobwb, precision)
    return edtrx__hkoo * multiplier + np.int64(fhei__bobwb * multiplier)
...
def overload_freq_methods(method):
    ...
    def freq_overload(td, freq):
        iuuyg__cvp = ["freq == 'D'", "freq == 'H'",
            "freq == 'min' or freq == 'T'", "freq == 'S'",
            "freq == 'ms' or freq == 'L'", "freq == 'U' or freq == 'us'",
            "freq == 'N'"]
        qzfr__atq = [24 * 60 * 60 * 1000000 * 1000, 60 * 60 * 1000000 * 
            1000, 60 * 1000000 * 1000, 1000000 * 1000, 1000 * 1000, 1000, 1]
        lnkpv__oaklh = 'def impl(td, freq):\n'
        for hqivr__xkk, vfjp__uph in enumerate(iuuyg__cvp):
            yxpjz__pqhy = 'if' if hqivr__xkk == 0 else 'elif'
            lnkpv__oaklh += '    {} {}:\n'.format(yxpjz__pqhy, vfjp__uph)
            lnkpv__oaklh += '        unit_value = {}\n'.format(qzfr__atq[
                hqivr__xkk])
        lnkpv__oaklh += '    else:\n'
        lnkpv__oaklh += (
...
"""
            ...
            if method == 'round':
                lnkpv__oaklh += '    if unit_value == 1:\n'
                lnkpv__oaklh += '        value = td.value\n'
                lnkpv__oaklh += '    else:\n'
                lnkpv__oaklh += (
                    '        quotient, remainder = np.divmod(td.value, unit_value)\n'
                    )
                lnkpv__oaklh += """        mask = np.logical_or(remainder > (unit_value // 2), np.logical_and(remainder == (unit_value // 2), quotient % 2))
"""
                lnkpv__oaklh += '        if mask:\n'
                lnkpv__oaklh += '            quotient = quotient + 1\n'
                lnkpv__oaklh += '        value = quotient * unit_value\n'
            lnkpv__oaklh += '    return pd.Timestamp(value)\n'
        ssv__dbow = {}
        exec(lnkpv__oaklh, {'np': np, 'pd': pd}, ssv__dbow)
        impl = ssv__dbow['impl']
        return impl
...
def _install_freq_methods():
    kvzcg__ajnbl = ['ceil', 'floor', 'round']
    for method in kvzcg__ajnbl:
        sjtn__ecj = overload_freq_methods(method)
        overload_method(PDTimeDeltaType, method, no_unliteral=True)(sjtn__ecj)
...
def compute_pd_timestamp(totmicrosec, nanosecond):
    ...
    tuyyw__rrcr = totmicrosec // 1000000
    second = tuyyw__rrcr % 60
    dpivn__izuv = tuyyw__rrcr // 60
    minute = dpivn__izuv % 60
    payu__ktm = dpivn__izuv // 60
    hour = payu__ktm % 24
    fht__qiuj = payu__ktm // 24
    year, month, day = _ord2ymd(fht__qiuj)
...
def overload_sub_operator_timestamp(lhs, rhs):
    if lhs == pd_timestamp_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            hofks__sfj = lhs.toordinal()
            hhr__ffsy = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            qpq__xve = lhs.microsecond
            nanosecond = lhs.nanosecond
            afjbe__gng = rhs.days
            xtby__nuar = rhs.seconds
            zvha__hotqi = rhs.microseconds
            bhhi__iplm = hofks__sfj - afjbe__gng
            ccc__jpxx = hhr__ffsy - xtby__nuar
            gptmo__tzp = qpq__xve - zvha__hotqi
            totmicrosec = 1000000 * (bhhi__iplm * 86400 + ccc__jpxx
                ) + gptmo__tzp
...
def overload_add_operator_timestamp(lhs, rhs):
    ...
    if lhs == pd_timestamp_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            hhr__ffsy = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            qpq__xve = lhs.microsecond
            atim__ijwei = lhs.nanosecond
            zvha__hotqi = rhs.value // 1000
            lym__yik = rhs.nanoseconds
            gptmo__tzp = qpq__xve + zvha__hotqi
            totmicrosec = 1000000 * (hofks__sfj * 86400 + hhr__ffsy
                ) + gptmo__tzp
            bmqtr__luz = atim__ijwei + lym__yik
            return compute_pd_timestamp(totmicrosec, bmqtr__luz)
        return impl
...
def typeof_python_calendar(val, c):
    return types.Tuple([types.StringLiteral(xec__zspha) for xec__zspha in val])
