...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        nmm__ifk = []
        ixz__nlfpf = h5g_get_num_objs(obj_id)
        for cnj__avxo in range(ixz__nlfpf):
            reky__ojw = h5g_get_objname_by_idx(obj_id, cnj__avxo)
            nmm__ifk.append(reky__ojw)
        return nmm__ifk
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    hns__dnag = parse_dtype(dtype)
    szvp__azw = np.int32(numba_to_c_type(hns__dnag))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            sfuiy__qchu = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    vclor__dwvh = tuple_to_ptr(starts)
    fnj__vtd = tuple_to_ptr(counts)
    htpxa__gmz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, vclor__dwvh, fnj__vtd,
        is_parallel, out_arr.ctypes, htpxa__gmz)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    fnj__vtd = tuple_to_ptr(counts)
    htpxa__gmz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, vclor__dwvh, fnj__vtd,
        is_parallel, out_arr.ctypes, htpxa__gmz)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        sdz__vzlhu = types.Array(dtype, ndim, 'C')
        dyqx__frf = args[3]
        sdz__vzlhu = self.context.resolve_function_type(operator.getitem, [
            sdz__vzlhu, dyqx__frf], {}).return_type.copy(layout='C')
        return signature(sdz__vzlhu, *args)
...
def get_filter_read_indices(bool_arr):
    cjp__ltowt = bool_arr.nonzero()[0]
    sxmvp__zbce = bodo.libs.distributed_api.get_rank()
    exigs__nzq = bodo.libs.distributed_api.get_size()
    rkk__donjq = np.empty(exigs__nzq, np.int64)
    xnsu__ghmz = len(bool_arr)
    bodo.libs.distributed_api.allgather(rkk__donjq, xnsu__ghmz)
    fbwo__tspdr = rkk__donjq.cumsum()[sxmvp__zbce] - xnsu__ghmz
    cjp__ltowt += fbwo__tspdr
    cjmm__nxpo = bodo.libs.distributed_api.dist_reduce(len(cjp__ltowt), np.
        int32(sum_op))
    qbb__koh = bodo.libs.distributed_api.gatherv(cjp__ltowt)
    if sxmvp__zbce == 0:
        aljh__igkjm = qbb__koh
    else:
        aljh__igkjm = np.empty(cjmm__nxpo, cjp__ltowt.dtype)
    bodo.libs.distributed_api.bcast(aljh__igkjm)
    bdwa__hwkl = bodo.libs.distributed_api.get_start(cjmm__nxpo, exigs__nzq,
        sxmvp__zbce)
    cmfro__tsdz = bodo.libs.distributed_api.get_end(cjmm__nxpo, exigs__nzq,
        sxmvp__zbce)
    return aljh__igkjm[bdwa__hwkl:cmfro__tsdz]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        axhqo__fbf = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], axhqo__fbf)
        return builder.bitcast(axhqo__fbf, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    fnj__vtd = tuple_to_ptr(counts)
    htpxa__gmz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, vclor__dwvh,
        fnj__vtd, is_parallel, out_arr.ctypes, htpxa__gmz, read_indices.
