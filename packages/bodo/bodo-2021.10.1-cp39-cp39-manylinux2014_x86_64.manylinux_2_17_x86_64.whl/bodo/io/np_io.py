...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        spfu__ohfg = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, spfu__ohfg)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            ykjm__pywi = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(ykjm__pywi.dtype)
            file_write(unicode_to_utf8(fname), ykjm__pywi.ctypes, 
                dtype_size * ykjm__pywi.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            ykjm__pywi = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(ykjm__pywi.dtype)
            zvmw__vvpq = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                ykjm__pywi.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), ykjm__pywi.ctypes,
                start, count, zvmw__vvpq)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            iirv__zxyxn = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                iirv__zxyxn = min(iirv__zxyxn, count * dtype_size)
            if iirv__zxyxn < 0:
                return -1
            return iirv__zxyxn
        return impl
