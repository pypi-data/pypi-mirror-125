...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        fok__rxqpt, = args
        ypk__caoas = context.make_helper(builder, binary_array_type)
        ypk__caoas.data = fok__rxqpt
        context.nrt.incref(builder, data_typ, fok__rxqpt)
        return ypk__caoas._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        nsryw__voyv = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        rwemq__dntap = args[1]
        qnat__mezo = cgutils.create_struct_proxy(bytes_type)(context, builder)
        qnat__mezo.meminfo = context.nrt.meminfo_alloc(builder, rwemq__dntap)
        qnat__mezo.nitems = rwemq__dntap
        qnat__mezo.itemsize = lir.Constant(qnat__mezo.itemsize.type, 1)
        qnat__mezo.data = context.nrt.meminfo_data(builder, qnat__mezo.meminfo)
        qnat__mezo.parent = cgutils.get_null_value(qnat__mezo.parent.type)
        qnat__mezo.shape = cgutils.pack_array(builder, [rwemq__dntap],
            context.get_value_type(types.intp))
        qnat__mezo.strides = nsryw__voyv.strides
        cgutils.memcpy(builder, qnat__mezo.data, nsryw__voyv.data, rwemq__dntap
            )
        return qnat__mezo._getvalue()
...
def binary_arr_hex(arr):
    jleii__dwihg = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        rwemq__dntap = len(arr) * 2
        output = numba.cpython.unicode._empty_string(jleii__dwihg,
            rwemq__dntap, 1)
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        ifoau__dtlup = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        ywyu__cfq = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        lhj__wnmbq = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        fiff__isebs = cgutils.get_or_insert_function(builder.module,
            lhj__wnmbq, name='bytes_to_hex')
        builder.call(fiff__isebs, (ifoau__dtlup.data, ywyu__cfq.data,
            ywyu__cfq.nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            axn__xkbhf = arr._data[ind]
            return init_bytes_type(axn__xkbhf, len(axn__xkbhf))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != jleii__dwihg:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            fok__rxqpt = np.empty(len(hex_str) // 2, np.uint8)
            rwemq__dntap = _bytes_fromhex(fok__rxqpt.ctypes, hex_str._data,
                len(hex_str))
            result = init_bytes_type(fok__rxqpt, rwemq__dntap)
            return result
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        yzcvz__kmyb = lhs == binary_array_type
        dvbt__vrr = rhs == binary_array_type
        qmhd__oea = 'lhs' if yzcvz__kmyb else 'rhs'
        jer__omr = 'def impl(lhs, rhs):\n'
        jer__omr += '  numba.parfors.parfor.init_prange()\n'
        jer__omr += f'  n = len({qmhd__oea})\n'
        jer__omr += '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n'
        jer__omr += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        njis__rwkf = []
        if yzcvz__kmyb:
            njis__rwkf.append('bodo.libs.array_kernels.isna(lhs, i)')
        if dvbt__vrr:
            njis__rwkf.append('bodo.libs.array_kernels.isna(rhs, i)')
        jer__omr += f"    if {' or '.join(njis__rwkf)}:\n"
        jer__omr += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        jer__omr += '      continue\n'
        uiopc__bdaen = 'lhs[i]' if yzcvz__kmyb else 'lhs'
        ypr__xwd = 'rhs[i]' if dvbt__vrr else 'rhs'
        jer__omr += f'    out_arr[i] = op({uiopc__bdaen}, {ypr__xwd})\n'
        jer__omr += '  return out_arr\n'
        httk__zsl = {}
        exec(jer__omr, {'bodo': bodo, 'numba': numba, 'op': op}, httk__zsl)
        return httk__zsl['impl']
...
class BinaryArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self):
        hovr__dydx = 'iter(Bytes)'
        tcj__aslv = bytes_type
        super(BinaryArrayIterator, self).__init__(hovr__dydx, tcj__aslv)
...
class BinaryArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lqn__gzd = [('index', types.EphemeralPointer(types.uintp)), (
            'array', binary_array_type)]
        super(BinaryArrayIteratorModel, self).__init__(dmm, fe_type, lqn__gzd)
...
def iternext_binary_array(context, builder, sig, args, result):
    [dfv__kxcul] = sig.args
    [qdo__hnsbh] = args
    pee__bgnk = context.make_helper(builder, dfv__kxcul, value=qdo__hnsbh)
    qlrx__cnwr = signature(types.intp, binary_array_type)
    zpjzm__sov = context.compile_internal(builder, lambda a: len(a),
        qlrx__cnwr, [pee__bgnk.array])
    pefrw__iee = builder.load(pee__bgnk.index)
    hkiw__zij = builder.icmp(lc.ICMP_SLT, pefrw__iee, zpjzm__sov)
    result.set_valid(hkiw__zij)
    with builder.if_then(hkiw__zij):
        horz__xsd = signature(bytes_type, binary_array_type, types.intp)
        jrp__qhwgs = context.compile_internal(builder, lambda a, i: a[i],
            horz__xsd, [pee__bgnk.array, pefrw__iee])
        result.yield_(jrp__qhwgs)
        ctnya__zuwyo = cgutils.increment_index(builder, pefrw__iee)
        builder.store(ctnya__zuwyo, pee__bgnk.index)
