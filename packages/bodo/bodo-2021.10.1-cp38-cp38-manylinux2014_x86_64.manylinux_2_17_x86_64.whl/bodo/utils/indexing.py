...
def get_new_null_mask_bool_index(old_mask, ind, n):
    ole__uvv = n + 7 >> 3
    uihj__zoh = np.empty(ole__uvv, np.uint8)
    orqap__swjm = 0
    for hxar__vrmk in range(len(ind)):
        if ind[hxar__vrmk]:
            uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                hxar__vrmk)
            bodo.libs.int_arr_ext.set_bit_to_arr(uihj__zoh, orqap__swjm,
                uvdp__vsvv)
            orqap__swjm += 1
    return uihj__zoh
...
def array_getitem_bool_index(A, ind):
    ...
    vzdfi__tqge = A._data[ind]
    n = len(vzdfi__tqge)
    uihj__zoh = get_new_null_mask_bool_index(old_mask, ind, n)
    return vzdfi__tqge, uihj__zoh
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    orqap__swjm = 0
    for hxar__vrmk in range(len(ind)):
        uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind
            [hxar__vrmk])
        bodo.libs.int_arr_ext.set_bit_to_arr(uihj__zoh, orqap__swjm, uvdp__vsvv
            )
        orqap__swjm += 1
    return uihj__zoh
...
def array_getitem_int_index(A, ind):
    jdow__tzdsk = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    vzdfi__tqge = A._data[jdow__tzdsk]
    n = len(vzdfi__tqge)
    uihj__zoh = get_new_null_mask_int_index(old_mask, jdow__tzdsk, n)
    return vzdfi__tqge, uihj__zoh


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    dlto__rpv = numba.cpython.unicode._normalize_slice(ind, n)
    tro__flqo = numba.cpython.unicode._slice_span(dlto__rpv)
    ole__uvv = tro__flqo + 7 >> 3
    uihj__zoh = np.empty(ole__uvv, np.uint8)
    orqap__swjm = 0
    for hxar__vrmk in range(dlto__rpv.start, dlto__rpv.stop, dlto__rpv.step):
        uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            hxar__vrmk)
        bodo.libs.int_arr_ext.set_bit_to_arr(uihj__zoh, orqap__swjm, uvdp__vsvv
            )
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    vzdfi__tqge = np.ascontiguousarray(A._data[ind])
    uihj__zoh = get_new_null_mask_slice_index(old_mask, ind, n)
    return vzdfi__tqge, uihj__zoh
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for hxar__vrmk in range(n):
                A._data[idx[hxar__vrmk]] = val._data[hxar__vrmk]
                uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, hxar__vrmk)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    hxar__vrmk], uvdp__vsvv)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for hxar__vrmk in idx:
                A._data[hxar__vrmk] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            plvcg__psl = 0
            for hxar__vrmk in range(n):
                if not bodo.libs.array_kernels.isna(idx, hxar__vrmk) and idx[
                    hxar__vrmk]:
                    A._data[hxar__vrmk] = val._data[plvcg__psl]
                    uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, plvcg__psl)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        hxar__vrmk, uvdp__vsvv)
                    plvcg__psl += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    dlto__rpv = numba.cpython.unicode._normalize_slice(idx, n)
    plvcg__psl = 0
    for hxar__vrmk in range(dlto__rpv.start, dlto__rpv.stop, dlto__rpv.step):
        uvdp__vsvv = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            plvcg__psl)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, hxar__vrmk, uvdp__vsvv
            )
        plvcg__psl += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            dlto__rpv = numba.cpython.unicode._normalize_slice(idx, len(A))
            for hxar__vrmk in range(dlto__rpv.start, dlto__rpv.stop,
                dlto__rpv.step):
                A._data[hxar__vrmk] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    hxar__vrmk, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for hxar__vrmk in idx:
                    bodo.libs.array_kernels.setna(A, hxar__vrmk)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    ztbnt__kjr = bodo.libs.str_arr_ext.pre_alloc_string_array(n
                        , -1)
                    for hxar__vrmk in numba.parfors.parfor.internal_prange(n):
                        if idx[hxar__vrmk] or bodo.libs.array_kernels.isna(A,
                            hxar__vrmk):
                            ztbnt__kjr[hxar__vrmk] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(ztbnt__kjr,
                                hxar__vrmk)
                        else:
                            ztbnt__kjr[hxar__vrmk] = A[hxar__vrmk]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for hxar__vrmk in range(n):
                    if not bodo.libs.array_kernels.isna(idx, hxar__vrmk
                        ) and idx[hxar__vrmk]:
                        bodo.libs.array_kernels.setna(A, hxar__vrmk)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for hxar__vrmk in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, hxar__vrmk)
                        continue
                    A[hxar__vrmk] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        vwpt__wqyag = context.make_helper(builder, val_t, args[0])
        gbmq__jkaxh = vwpt__wqyag.data
        context.nrt.incref(builder, val_t.type, gbmq__jkaxh)
        return gbmq__jkaxh
