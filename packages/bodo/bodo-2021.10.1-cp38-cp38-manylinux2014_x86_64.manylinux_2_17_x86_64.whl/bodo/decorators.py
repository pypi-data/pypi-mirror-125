...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        nauty__dlw = self.overloads[sig]
        kzzl__opo = nauty__dlw.metadata.get('distributed_diagnostics', None)
        if kzzl__opo is None:
            dxga__kimy = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(dxga__kimy)
        kzzl__opo.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        cwg__fphvl = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(cwg__fphvl)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        llc__ahsvm = options.pop('distributed')
        pipeline_class = (pipeline_class if llc__ahsvm else bodo.compiler.
            BodoCompilerSeq)
    gnhuw__xoqp = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(gnhuw__xoqp, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(gnhuw__xoqp)
        else:
            return master_mode_wrapper(gnhuw__xoqp)
    else:
        return gnhuw__xoqp
