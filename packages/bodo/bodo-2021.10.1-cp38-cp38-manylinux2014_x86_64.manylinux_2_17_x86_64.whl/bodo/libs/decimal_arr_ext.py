...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    oaw__affia = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    ksz__gtab = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    zgdpv__dgxs = cgutils.get_or_insert_function(builder.module, ksz__gtab,
        name='decimal_to_str')
    builder.call(zgdpv__dgxs, [val, oaw__affia._get_ptr_by_name('meminfo'),
        oaw__affia._get_ptr_by_name('length'), scale])
    oaw__affia.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    oaw__affia.is_ascii = context.get_constant(types.int32, 1)
    oaw__affia.hash = context.get_constant(numba.cpython.unicode._Py_hash_t, -1
        )
    oaw__affia.data = context.nrt.meminfo_data(builder, oaw__affia.meminfo)
    oaw__affia.parent = cgutils.get_null_value(oaw__affia.parent.type)
    return oaw__affia._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, wcrob__roude, wcrob__roude = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    ksz__gtab = lir.FunctionType(lir.IntType(128), [lir.IntType(8).
        as_pointer()])
    zgdpv__dgxs = cgutils.get_or_insert_function(builder.module, ksz__gtab,
        name='str_to_decimal')
    vtcfc__umyi = builder.call(zgdpv__dgxs, [val])
    return vtcfc__umyi
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        qatjk__wmif = cgutils.alloca_once(builder, lir.ArrayType(lir.
            IntType(64), 2))
        builder.store(args[0], builder.bitcast(qatjk__wmif, lir.IntType(128
            ).as_pointer()))
        return builder.load(qatjk__wmif)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    lcidv__qxj = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, wcrob__roude = args
        ksz__gtab = lir.FunctionType(lir.IntType(1), [lir.IntType(128), lir
            .IntType(64), lir.IntType(128), lir.IntType(64)])
        zgdpv__dgxs = cgutils.get_or_insert_function(builder.module,
            ksz__gtab, name=lcidv__qxj)
        return builder.call(zgdpv__dgxs, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            lcidv__qxj = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, lcidv__qxj)
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    yhmu__ifr = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    oul__rwxe = [context.get_constant_generic(builder, types.int64, v) for
        v in yhmu__ifr]
    rjv__deddk = cgutils.pack_array(builder, oul__rwxe)
    qatjk__wmif = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(rjv__deddk, builder.bitcast(qatjk__wmif, lir.ArrayType(
...
def unbox_decimal(typ, val, c):
    ksz__gtab = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(128).as_pointer()])
    zgdpv__dgxs = cgutils.get_or_insert_function(c.builder.module,
        ksz__gtab, name='unbox_decimal')
    qatjk__wmif = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(zgdpv__dgxs, [val, qatjk__wmif])
    kqexg__mxou = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    nvxag__rngk = c.builder.load(qatjk__wmif)
    return NativeValue(nvxag__rngk, is_error=kqexg__mxou)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    ipnd__cujw = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    yuu__iws = c.pyapi.from_native_value(bodo.string_type, ipnd__cujw, c.
        env_manager)
    docf__fqyas = c.context.insert_const_string(c.builder.module, 'decimal')
    qksl__wst = c.pyapi.import_module_noblock(docf__fqyas)
    qatjk__wmif = c.pyapi.call_method(qksl__wst, 'Decimal', (yuu__iws,))
    c.pyapi.decref(yuu__iws)
    c.pyapi.decref(qksl__wst)
    return qatjk__wmif
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        svfb__fght = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, svfb__fght)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        gpd__ofkqu, jzl__zxwf, wcrob__roude, wcrob__roude = args
        yvs__quof = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        yvs__quof.data = gpd__ofkqu
        yvs__quof.null_bitmap = jzl__zxwf
        context.nrt.incref(builder, signature.args[0], gpd__ofkqu)
        context.nrt.incref(builder, signature.args[1], jzl__zxwf)
        return yvs__quof._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    tau__bvf = DecimalArrayType(precision, scale)
    pgvfg__mbp = tau__bvf(data, null_bitmap, precision_tp, scale_tp)
    return pgvfg__mbp, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    oiuer__pvhwb = context.get_constant(types.int64, n)
    ttig__ifgy = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [oiuer__pvhwb])
    umv__rxw = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for ghm__atwu, hinuu__jloe in enumerate(pyval):
        pcjtt__rzkv = pd.isna(hinuu__jloe)
        bodo.libs.int_arr_ext.set_bit_to_arr(umv__rxw, ghm__atwu, int(not
            pcjtt__rzkv))
        if not pcjtt__rzkv:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [ttig__ifgy._getvalue(), context.
                get_constant(types.int64, ghm__atwu), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), hinuu__jloe)])
    lneoa__uvn = context.get_constant_generic(builder, nulls_type, umv__rxw)
    yvs__quof = context.make_helper(builder, typ)
    yvs__quof.data = ttig__ifgy._getvalue()
    yvs__quof.null_bitmap = lneoa__uvn
    return yvs__quof._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    jeezy__csnzd = np.empty(n, dtype=int128_type)
    zsfi__edw = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(jeezy__csnzd, zsfi__edw, precision, scale)
...
def box_decimal_arr(typ, val, c):
    wci__urd = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    jeezy__csnzd = c.context.make_array(types.Array(int128_type, 1, 'C'))(c
        .context, c.builder, wci__urd.data)
    lrmta__fpfz = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, wci__urd.null_bitmap).data
    n = c.builder.extract_value(jeezy__csnzd.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    ksz__gtab = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    jnjvd__dtbqy = cgutils.get_or_insert_function(c.builder.module,
        ksz__gtab, name='box_decimal_array')
    agf__xqcy = c.builder.call(jnjvd__dtbqy, [n, jeezy__csnzd.data,
        lrmta__fpfz, scale])
...
def unbox_decimal_arr(typ, val, c):
    yvs__quof = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    rhfi__jrrrk = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(rhfi__jrrrk)
    c.pyapi.decref(rhfi__jrrrk)
    xnu__owdq = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(64
        ), 7)), lir.Constant(lir.IntType(64), 8))
    ttig__ifgy = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(int128_type, 1, 'C'), [n])
    knwd__ogv = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [xnu__owdq])
    ksz__gtab = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(64), lir.IntType(128).as_pointer(), lir.IntType(8).
        as_pointer()])
    zgdpv__dgxs = cgutils.get_or_insert_function(c.builder.module,
        ksz__gtab, name='unbox_decimal_array')
    c.builder.call(zgdpv__dgxs, [val, n, ttig__ifgy.data, knwd__ogv.data])
    yvs__quof.null_bitmap = knwd__ogv._getvalue()
    yvs__quof.data = ttig__ifgy._getvalue()
    kqexg__mxou = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(yvs__quof._getvalue(), is_error=kqexg__mxou)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            ynqfj__ejzs, qqwu__dywlh = array_getitem_bool_index(A, ind)
            return init_decimal_array(ynqfj__ejzs, qqwu__dywlh, precision,
                scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            ynqfj__ejzs, qqwu__dywlh = array_getitem_int_index(A, ind)
            return init_decimal_array(ynqfj__ejzs, qqwu__dywlh, precision,
                scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            ynqfj__ejzs, qqwu__dywlh = array_getitem_slice_index(A, ind)
            return init_decimal_array(ynqfj__ejzs, qqwu__dywlh, precision,
                scale)
        return impl_slice
    raise BodoError(
