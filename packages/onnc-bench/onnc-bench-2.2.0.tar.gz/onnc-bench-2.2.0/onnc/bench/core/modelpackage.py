...
class ModelPackage:
    ...
    def __init__(self, model: Union[object, str, Path, Model],
        ...
        self.__encapsulate_model(model_format, model_inputs,
                                 model_outputs)
    ...
    def __encapsulate_model(self, model_format, model_inputs, model_outputs):
        ...
        logger.info(self.model.format.name)
    ...
    def serialize(self, model_path: Union[str, Path, None] = None,
        ...
        self._src_model = model_serializer.serialize(self.model, Path(model_path))
        self._src_dataset = dataset_serializer.serialize(self.dataset,
                                                    Path(dataset_path))
        self.model = self._src_model
        self.dataset = self._src_dataset

        # we need to get the meta again after serialization
        self.__encapsulate_model(ModelFormat.NON_SPECIFIED, [], [])
