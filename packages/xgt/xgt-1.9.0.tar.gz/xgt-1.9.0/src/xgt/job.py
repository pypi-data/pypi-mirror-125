...
from .common import (XgtError, XgtInternalError,
                     _code_error_map)
...
class Job(object):
  ...
  def _get_job_data(self):
    ...
    if log.getEffectiveLevel() >= logging.DEBUG:
      ...
      if 'trace' in job_data:
        ...
        trace = ''
      msg = 'Job: {0} User: {1} Status: {2}'.format(str(job_id),
                                                    user,
                                                    job_status)
      if error != '':
        msg = msg + "\nError: \n" + error
  ...
  def _get_data_helper(self, offset, length):
    if 'rows' in self._data:
      ...
      try:
        ...
        raise XgtInternalError('Invalid JSON format: '+ str(ex))
      return jsn
    else:
      return None
  ...
  def __str__(self):
    txt = ('id:{0}, user:{1}, status:{2}').format(self.id, self.user, self.status)
    if len(self.error) > 0:
      txt = txt + (', nerror:{0}').format(self.error)
    return txt
