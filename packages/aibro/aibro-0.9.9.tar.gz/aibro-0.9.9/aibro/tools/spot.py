...
def _stop_spot(job_id: str, e: str):
    data = {"job_id": job_id, "error": e}
    aibro_client.connect_to_socket()
    aibro_client.post_with_json_data("v1/stop_spot", data)
    aibro_client.disconnect_socket()


def save_cancel_job_reason(job_id: str, reason: str):
    data = {"job_id": job_id, "reason": reason}
    aibro_client.post_with_json_data("v1/save_cancel_job_reason", data)
