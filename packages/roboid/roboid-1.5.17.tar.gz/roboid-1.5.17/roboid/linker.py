...
class Link(object):
    ...
    def handle_sensory(self, received):
        connection_state = received['connection']
    ...
    def handle_motoring(self):
        if self._roboid is not None:
            self._roboid.encode_motoring(self._motoring)
...
class Linker(object):
    ...
    @staticmethod
    def _get_or_create_link(group, module, index):
        index = str(index)
        key = module + index
    ...
    @staticmethod
    def register_roboid(group, module, index, tag, roboid):
        link = Linker._get_or_create_link(group, module, index)
        link.set_roboid(roboid)
        link.set_tag(tag)
    ...
    def _on_message(wsapp, message):
        try:
            ...
            if index >= 0:
                link = Linker._get_or_create_link(received['group'], received['module'], index)
                if link is not None:
