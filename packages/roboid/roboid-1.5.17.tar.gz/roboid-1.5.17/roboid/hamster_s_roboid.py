...
class HamsterSLinkRoboid(HamsterSRoboid):
    ...
    def encode_motoring(self, motoring):
        with self._thread_lock:
            map1 = 0xfc000000
            map2 = 0xc0000000
            motoring['module'] = 'hamsterS'
            motoring['index'] = self._index
            ...
            if self._write_serial_written:
                self._write_serial_written = False
                motoring['serial'] = self._write_serial_data
                map2 |= 0x04000000
    ...
    def decode_sensory(self, received):
        ...
        if (map2 & 0x00008000) != 0: self._free_fall_device._put_empty()
        if (map2 & 0x00004000) != 0: self._tap_device._put_empty()
        self._tilt_device._put(received['tilt'])
        if (map2 & 0x00002000) != 0: self._read_serial_device._put_array(received['serial'])
        self._pulse_count_device._put(received['pulseCount'])
        if (map2 & 0x00000800) != 0: self._wheel_state_device._put(received['wheelState'])
        if (map2 & 0x00000400) != 0: self._sound_state_device._put(received['soundState'])
        if (map1 & 0x00000010) != 0: self._line_tracer_state_device._put(received['lineTracerState'])
        self._battery_state_device._put(received['batteryState'])
        if (map2 & 0x00000200) != 0: self._serial_state_device._put_empty()