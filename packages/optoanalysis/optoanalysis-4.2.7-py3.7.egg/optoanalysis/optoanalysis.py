    ...
    def load_time_data(self, RelativeChannelNo=None, SampleFreq=None, PointsToLoad=-1, NormaliseByMonitorOutput=False):
        ...
        if FileExtension == "raw" or FileExtension == "trc":
            ...
            if data[15][1] == 'TDS1001B':
                ...
                self.SampleFreq = 1/dt
                self.voltage = voltage      
            elif data[0, 0] == '\ufeffTime': # data from picoscope
                column_names = data[0]
                units = data[1]
...
def extract_parameters(Pressure, PressureErr, A, AErr, Gamma0, Gamma0Err, method="chang"):
    ...
    conversionFactor = _np.sqrt(A * mass / (2 * kB * T0 * Gamma0))
...
def fit_data_2(freq_array, S_xx_array, AGuess, OmegaTrap, GammaGuess, make_fig=True, show_fig=True):
    ...
    if make_fig == True:
        ...
        PSDTheory_fit_initial = PSD_fitting_eqn2(p0[0],
                                                 p0[1],
                                                 p0[2],
                                                 datax)

        PSDTheory_fit = PSD_fitting_eqn2(Params_Fit[0],
                                         Params_Fit[1],
                                         Params_Fit[2],
                                         datax)
