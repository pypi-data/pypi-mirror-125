...
class TeamsCardFactory:
    ...
    def _add_message(args, card, message):
        ...
        trimmed = False
        if args.max_size:
            fragments = markdown_splitter.split_markdown(message, args.max_size//2)
            if len(fragments) >= 2:
                trimmed = True
                message = ''
                i = -1
                while len(message) < args.max_size:
                    message += fragments[i]
                    i += -1
        ...
        rendered = ''
        if trimmed:
            rendered += '> *Below message is trimmed. **See Pull Request** for full message... (or Jenkins Build)*\n\n'
        rendered += str(mc)

        card.add_section(rendered)
