...
class Sqllite_Manager(Data_Manager):
    ...
    def compare(self,local_db_path):
        ...
        self.cursor = self.conn.cursor()
        if not self.check_table_exists(self.conn, "last_deploy"):
            query = "SELECT DISTINCT path2 FROM Current_Dir"
            self.cursor.execute(query)
            new = self.cursor.fetchall()
            return new,[],[]
        query = "SELECT DISTINCT path2 FROM Current_Dir WHERE path NOT IN (SELECT DISTINCT path FROM last_deploy)"
        self.cursor.execute(query)
        new = self.cursor.fetchall()
        query = "SELECT DISTINCT path2 FROM last_deploy WHERE path NOT IN (SELECT DISTINCT path FROM Current_Dir)"
        self.cursor.execute(query)
        delete = self.cursor.fetchall()
        query = "SELECT DISTINCT path2 FROM Current_Dir WHERE path IN (SELECT DISTINCT path FROM last_deploy) AND sha NOT IN (SELECT DISTINCT sha FROM last_deploy)"
        self.cursor.execute(query)
