...
class SubClient(client.Client):
    ...
    def send_active_obj(self, startTime: int = None, endTime: int = None, optInAdsFlags: int = 2147483647, tz: int = -timezone // 1000, timers: list = None, timestamp: int = int(timestamp() * 1000)):
        """
        Send active time object.

        **Parameters**
            - **generate_sig_func** : Callback generator signature.
            - **startTime** : Time of start activity.
            - **endTime** : Time of end activity.
            - **optInAdsFlags** : Time of start activity.
            - **tz** : Timezone.
            - **timers** : Active time objects.
            - **timestamp** : Timestamp.
            - **endTime** : Time of end activity.
        ...
        **Returns**
        ...
        def send_active_obj_func(generate_sig_func):
            data = {
            ...
            headers["NDC-MSG-SIG"] = generate_sig_func(data)
    ...
    def generate_chat_bubble(self, bubble: BinaryIO = None, image: BinaryIO = None, config: Union[str, dict] = None, templateId: str = "71b41f1a-4c09-4e07-ac9e-18ef9bbbe65f"):
        """
        Generate chat bubble.

        **Parameters**
            - **bubble** : File bubble to generate chat bubble.
            - **image** : Bubble image.
            - **config** : Bubble congiguration.
        ...
        **Returns**
        ...
        if image is not None and config is not None:
            tcode = str(int(timestamp() * 1000))
            
            if isinstance(config, str):
                config = json.loads(config)

            with open(tcode + "/config.json", "w") as file:
                json.dump(config, file)
            
            with open(tcode + "/image.png", "wb") as file:
                file.write(image)
            
            with ZipFile(tcode + "/bubble.zip", 'w') as bubbleZip:
                bubbleZip.write(tcode + "/config.json", "config.json")
                bubbleZip.write(tcode + "/image.png", str(config.get("backgroundPath")))
            
            with open(tcode + "/bubble.zip", "rb") as f:
                data = f.read()
        ...
        else: raise exceptions.NoBubble()
    ...
    def edit_chat_bubble(self, bubbleId: str, bubble: BinaryIO = None, image: BinaryIO = None, config: Union[str, dict] = None):
        """
        Generate chat bubble.

        **Parameters**
            - **bubbleId** : ID of bubble to edit.
            - **bubble** : File bubble to edit chat bubble.
            - **image** : Bubble image.
            - **config** : Bubble congiguration.
