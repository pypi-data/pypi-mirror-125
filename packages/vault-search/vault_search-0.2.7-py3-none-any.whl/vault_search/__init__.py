...
def compute_url(VAULT_ADDR, PATH, VAULT_NS, VAULT_KV_PATH,
    ...
    if VAULT_OUTPUT == 'url':
        ...
        if TYPE == "section":
            return f"vault kv list {PATH.replace('/v1/' + VAULT_NS, '').replace(VAULT_KV_PATH + 'data/',VAULT_KV_PATH)}"
        else:
            return f"vault kv get -field={KEY} {PATH.replace('/v1/' + VAULT_NS, '').replace(VAULT_KV_PATH + 'data/',VAULT_KV_PATH)}"
    else:
        if TYPE == "section":
            return PATH.replace('/v1/' + VAULT_NS, '').replace(VAULT_KV_PATH + 'data/',VAULT_KV_PATH)
        else:
