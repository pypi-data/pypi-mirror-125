...
Run `flask db --help` to see the database commands help information.
...
```bash
flask user create --username <string> --admin <boolean> --enabled <boolean> --name <string> --email <string> [--password <string>]
```
...
Run `flask user --help` to see the user commands help information.

To run Notelist on a **local development** server (**not** for a production
environment) from the Python package installed, run one of the following
commands:
...
Alternatively, you can run the second command above from the source code
directory (`src/notelist`) :
...
```bash
docker container exec -it notelist flask db create
docker container exec -it notelist flask user create --username admin --admin y --enabled y
```

## How to run with Docker Compose

The easiest way to try Notelist is by using **Docker Compose** to run a
container for the API and another container for the MongoDB server for the data storage. To create and run the containers, run the following command from the
same directory of the `docker-compose.yml` file (the project directory):

```bash
docker compose up -d
```

Check the `docker-compose.yml` file to see how the containers are created.
