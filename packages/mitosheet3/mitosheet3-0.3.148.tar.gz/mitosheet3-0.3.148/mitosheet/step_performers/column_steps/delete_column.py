...
class DeleteColumnStepPerformer(StepPerformer):
    ...
    def transpile(
        ...
        column_headers = [f'\'{prev_state.column_ids.get_column_header_by_id(sheet_index, column_id)}\'' for column_id in column_ids]
        column_header_string = (", ").join(column_headers)

        return [f'{df_name}.drop([{column_header_string}], axis=1, inplace=True)']
    ...
    def describe(
        ...
        **params
        ...
        if df_names is not None:
            ...
            return f'Deleted column(s) {formated_column_ids} from {df_name}'
        return f'Deleted column(s) {formated_column_ids}'
