...
package AixCaliBuHAExamples
  model HeatPumpSystemCalibration
    ...
    Modelica.Blocks.Sources.RealExpression
                                     realExpression(y=heatPumpSystemWithInputs.vol.T)
                                                                    annotation (
        Placement(transformation(
          extent={{-22,-12},{22,12}},
          rotation=0,
          origin={66,-28})));
    Modelica.Blocks.Interfaces.RealOutput TAir "Value of Real output"
      annotation (Placement(transformation(extent={{100,-38},{120,-18}})));
  equation
    connect(TDryBulSource.y, heatPumpSystemWithInputs.TDryBul)
      annotation (Line(points={{-43,0},{-21,0}}, color={0,0,127}));
    connect(heatPumpSystemWithInputs.Pel, Pel) annotation (Line(points={{19.5,
            16.8},{86,16.8},{86,0},{110,0}}, color={0,0,127}));
    connect(realExpression.y, TAir)
      annotation (Line(points={{90.2,-28},{110,-28}}, color={0,0,127}));
    annotation (experiment(
        StopTime=3600,
        Interval=1,
        __Dymola_Algorithm="Dassl"));
  end HeatPumpSystemCalibration;

  model HeatPumpSystemWithInputs "Example for a heat pump system"
    ...
    Modelica.Blocks.Sources.RealExpression
                                     realExpression(y=heatPumpSystem.heatPump.sigBus.PelMea)
                                                                    annotation (
        Placement(transformation(
          extent={{-22,-12},{22,12}},
          rotation=0,
  ...
  equation
    ...
    connect(senT_a1.T, heatPumpSystem.TAct) annotation (Line(points={{77,-20},{
            -4,-20},{-4,-36},{5.95,-36},{5.95,-36.0714}},
                                                       color={0,0,127}));
    ...
    connect(TDryBul, heatPumpSystem.T_oda) annotation (Line(points={{-140,0},{
            -68,0},{-68,-45.0714},{5.95,-45.0714}}, color={0,0,127}));
    connect(TDryBul, TOut.T) annotation (Line(points={{-140,0},{-68,0},{-68,64},
            {-4,64}}, color={0,0,127}));
    connect(realExpression.y, Pel)
      annotation (Line(points={{104.2,112},{130,112}}, color={0,0,127}));
