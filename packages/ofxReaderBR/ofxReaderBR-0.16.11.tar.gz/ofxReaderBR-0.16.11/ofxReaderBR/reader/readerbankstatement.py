    ...
    def read(self):
        ...
        for stmt in ofx.statements:
            ...
            if len(origin.account_id) == 22:
                stmt.account.acctid = "O:" + str(nubank_id)
                nubank_id += 1
                account = stmt.account
                origin = Origin(account)
                if one_origin_error:
            ...
            asset_number = self._nubank_origin(created=created_origin, origin=origin.account_id) if\
                origin.account_type.name == "CREDITCARD" else self._bankaccount_origin(created=created_origin,
                                                                                       origin=origin.account_id)
            asset_balance = abs(float(stmt.balance.balamt))*-1 if origin.account_type.name == "CREDITCARD" else\
                float(stmt.balance.balamt)
            assets_obj = {
                "number": asset_number,
