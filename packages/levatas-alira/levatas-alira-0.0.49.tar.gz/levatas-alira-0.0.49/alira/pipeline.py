...
class Pipeline(object):
    ...
    def run(self, data: dict) -> Instance:
        ...
        for module in self.modules:
            if hasattr(module, Pipeline.METADATA_PIPELINE_MODULE_NAME):
                module_name = getattr(module, Pipeline.METADATA_PIPELINE_MODULE_NAME)
            else:
            ...
            if output is not None:
                instance.properties[module_name] = output
    ...
    def _load_modules(self):
        if self.model_directory is None:
            logging.info(
                "Configured a default pipeline since no model directory was specified."
            )
            return []

        pipeline_config_file = os.path.join(self.model_directory, PIPELINE_FILENAME)
        logging.info(f"Loading pipeline configuration from {pipeline_config_file}")

        try:
            with open(pipeline_config_file) as file:
                pipeline_configuration = yaml.load(file.read(), Loader=yaml.FullLoader)
        except Exception as e:
            logging.exception(e)
            raise ValueError(
                f"Unable to load pipeline configuration {pipeline_config_file}"
            )
        ...
        pipeline = pipeline_configuration["pipeline"]

        # If the pipeline defines a set of AWS credentials, we want to overwrite the
        # credentials that were passed in.
        self.aws_access_key = pipeline_configuration.get(
            "aws_access_key", self.aws_access_key
        )
        self.aws_secret_key = pipeline_configuration.get(
            "aws_secret_key", self.aws_secret_key
        )
        self.aws_region_name = pipeline_configuration.get(
        ...
        try:
            for module_configuration in pipeline:
                ...
                for argument_id, argument in module_configuration.items():
                    ...
                    if argument_id.endswith("_fn"):
                        module_path, _, fn_name = argument.rpartition(".")
                        function = getattr(import_module(module_path), fn_name)
                        arguments[argument_id] = function
                        continue

                    arguments[argument_id] = argument

                arguments["redis_server"] = self.redis_server
                arguments["aws_access_key"] = self.aws_access_key
                arguments["aws_secret_key"] = self.aws_secret_key
                arguments["aws_region_name"] = self.aws_region_name

                arguments.update(self.kwargs)
                modules.append(module(**arguments))

            return modules
        except ModuleNotFoundError as e:
            raise RuntimeError(f"Unable to load module {module_path}.{module_name}")
    ...
    def _initialize_metadata_store(self, store_configuration: dict):
        ...
        if store_configuration is None:
            ...
            connection_config.file_database.path = store_configuration["file"]
