...
class Automation:
    ...
    def register_task(api_inputs: ApiInputs, task: Task):
        ...
        base_class = ''
        allowed_base_classes = tuple([Task] + Task.__subclasses__())
        # allowed_base_classes = tuple(
        #     ['Task'] + [allowed_base_classes[i].__qualname__ for i in range(len(allowed_base_classes))])
        ...
        if base_class == '':
            ...
            return pandas.DataFrame()
        elif not set([base_class]).issubset({allowed_base_classes[i].__qualname__ for i in range(len(allowed_base_classes))}):
    ...
    def cancel_deployed_timer(api_inputs: ApiInputs, data_feed_id: uuid.UUID):
        """Cancel deployed timer for a given `data_feed_id`.
