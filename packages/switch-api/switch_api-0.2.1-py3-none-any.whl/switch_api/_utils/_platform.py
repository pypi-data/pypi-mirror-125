...
class Blob:
    """ """
    # TODO: Please review this Kate. The Account parameter needs to change to StorageAccountEnum parameter
    @staticmethod
    def list(api_inputs: ApiInputs, account: ACCOUNT, container: str, prefix: str = None):
        ...
        # TODO: Kate please review - new way of retrieving container client : no need to get the BlobServiceClient first since we have the Container SAS URI
    ...
    # TODO: Kate please review. Sets the default storage for upload to DataIngestion. ADX endpoints are looking in DataIngestion Storage instead of SwitchStorage.
    @staticmethod
...
def _get_ingestion_connection_string(api_inputs: ApiInputs):
    """
    Get connection string specific to Data Ingestion Storage
    ...
    -------
    str
        Data Ingestion connection string
    """
    headers = api_inputs.api_headers.default

    url = f"{api_inputs.api_projects_endpoint}/{api_inputs.api_project_id}/data-ingestion/data-feed/storage"
...
def __get_storage_options(account: ACCOUNT):
    """
    Get Storage Account Options. Currently the existing account literal doesn't match the Enum equivalent of storage options in the API, so we have this method
