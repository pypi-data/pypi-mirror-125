...
def upsert_discovered_records(df: pandas.DataFrame, api_inputs: ApiInputs, discovery_properties_columns: list,
    ...
    tuple[str, pandas.DataFrame]
        (response, response_data_frame) - Returns the response status and the dataframe containing the parsed response text.
    ...
    if discovery_properties_columns is not None and not set(discovery_properties_columns).issubset(data_frame.columns):
        ...
        return 'Integration.upsert_discovered_records(): data_frame must contain the following tag columns: ' + ', '.join(
            discovery_properties_columns)
    ...
    if device_tag_columns is not None and not set(tag_columns).issubset(data_frame.columns):
    ...
    if sensor_tag_columns is not None and not set(tag_columns).issubset(data_frame.columns):
    ...
    #     discovery_payload = data_frame.groupby(
    ...
    #     discovery_payload = discovery_payload[1:-1]  # remove outer [] from json
    ...
    #
    #         logger.info(f"Upserting discovery record group {name}")
    #         response = requests.post(url=url, headers=headers, json=discovery_payload )
    ...
    # return data_frame, discovery_payload
...
def upsert_device_sensors(df: pandas.DataFrame, api_inputs: ApiInputs, tag_columns: list = None,
    ...
    tuple[list, pandas.DataFrame]
        (response_status_list, upsert_response_df) - Returns the list of response statuses and the dataframe containing the parsed response text.
...
def upsert_timeseries_ds(df: pandas.DataFrame, api_inputs: ApiInputs, is_local_time: bool = True,
    ...
    -------
    tuple[str, pandas.DataFrame]
        (response_status, response_df) - Returns the response status and the dataframe containing the parsed response text.
    ...
    url = f"{api_inputs.api_projects_endpoint}/{api_inputs.api_project_id}/adx/time-series-operation?originalName={name}"
    logger.info("Sending request: POST %s", url)
...
# TODO : Kate please review this new helper method for upserting row count
def upsert_file_row_count(api_inputs: ApiInputs, row_count: int = None):
