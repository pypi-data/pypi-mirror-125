...
class ChangePointDetectorSession:
    ...
    #This is the main function    
        ...
        if  not sum([i > len(self.ts.data)-3 for i in ChangePoints]):
            ...
            PredictionVariance=self.TimeseriesModel.KalmanModelFit.get_prediction(0,len(self.data)+3).var_pred_mean
        else:
            Prediction=self.TimeseriesModel.KalmanModelFit.predict(0,len(self.ts.data))
            PredictionVariance=self.TimeseriesModel.KalmanModelFit.get_prediction(0,len(self.data)).var_pred_mean
