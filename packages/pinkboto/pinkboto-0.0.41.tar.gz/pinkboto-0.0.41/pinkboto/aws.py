...
import pinkboto
from pinkboto.utils import field_comparison, flat, path, unpack


class aws(object):
    def __init__(self, profile=None, region="us-east-1", cache=120, config=None):
        """
        Create connection.
        ...
        """
        self.profile = profile or environ.get("AWS_PROFILE", "default")
        self.region = region
        ...
        self.resources = yaml.safe_load(open(resources_file, "r"))

        self.cache = pinkboto.Cache(lifetime=cache)
        ...
        if "pagination_field" in resource["list"]:
            ...
            try:
                while len(path(list_property, step)) > 0 and path(pagin_field, step):
                    kwargs = {pagin_method_param: path(pagin_field, step)}
                    kwargs.update(method_params)
                    step = method(**kwargs)
                    results += path(list_property, step)
            except Exception:
                pinkboto.logger.exception(
                    f"pinkboto aws step malformation on step {step}"
                )
                raise
    ...
    def find(self, query=None, projection=None):
        ...
        if "parameters" in resource["list"]:
            ...
            method_params = dict([(k, v) for k, v in query.items() if k in parameters])
        ...
        if (
            ...
            and resource.get("client") != "sts"
            and resource.get("list", {}).get("method")
            not in [
        ...
        for k, v in query.items():
            results = [
                ...
                if field_comparison(path(k, result), v)
                or (isinstance(path(k, result), list) and v in path(k, result))
            ]

        if projection:
            if len(projection) > 1:
                output = [{k: path(k, obj) for k in projection} for obj in results]

            else:
                output = [path(k, obj) for k in projection for obj in results]
        else:
            output = results
        ...
        for obj in objs:
            ...
            if resource_method:
                delete_method = getattr(
                    resource_method(key), resource["delete"]["method"]
                )
            else:
