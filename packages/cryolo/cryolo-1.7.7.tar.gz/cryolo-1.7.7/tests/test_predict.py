...
class MyTestCase(unittest.TestCase):
    ...
    def test_run_prediction(self):
        ...
        config_path = os.path.join(os.path.dirname(__file__), "../resources/config_toxin.json")
        weights_path = os.path.join(os.path.dirname(__file__), "../resources/yolo_particle_toxin.h5")
        input_path = [os.path.join(os.path.dirname(__file__), "../resources/TcdA1-0018_frames_sum_filtered.mrc")]
        res = cryolo.predict.do_prediction(config_path=config_path, weights_path=weights_path, input_path=input_path, num_patches=2)
        self.assertEqual(93, len(res[0]["boxes"]),"crYOLO did not picked the expected number of particles")
    ...
    def test_find_not_fully_immersed_particles(self):
        ...
        boxes.append(cryolo.utils.BoundBox(1023 - 50, 1023 - 50, 50, 50))  # Fully immersed
