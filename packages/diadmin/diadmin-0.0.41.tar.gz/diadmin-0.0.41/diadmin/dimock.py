...
def main() :
    ...
    len_inports = len(opjson['inports'])
    if len_inports == 0:
        msgs = []
    elif len_inports == 1:
        msgs = ['msg']
...
'''
    ...
    for op in opjson['outports']:
        content +=f'\tatt = {att}\n'
        content +=f"\tmsg_{op['name']} = api.Message(attributes=att,body=data)\n"
        content +=f"\tapi.send('{op['name']}',msg_{op['name']})  # data type: {op['type']}\n\n\n"
...
'''
    ...
    content += '\n\n'
    for i,ip in enumerate(opjson['inports']) :
