...
def generate_userlist(userlist,
    ...
    if userlist :
        ...
        for i in range(1,num+1) :
            newuser = {'tenant':tenant,'user':user_prefix+str(i),'name':'user','role':role}
...
def main() :
    ...
    if args.generate :
        if args.userlist == 'nameless' :
            ...
            generate_userlist(userlist=None,
                              filename=params['USERLISTS']['LIST'],
                              num=params['USERLISTS']['NUMBER'],
                              pwd= params['USERLISTS']['PASSWORD'],
                              pwd_length= params['USERLISTS']['PWD_LENGTH'],
                              tenant=params['TENANT'],
                              role=params['USERLISTS']['DEFAULT_ROLE'])
        else :
            logging.info(f'Generate userlist from input userlist {args.userlist}')
            generate_userlist(userlist = args.userlist,
                              filename=params['USERLISTS']['LIST'],
                              format = params['USERLISTS']['FORMAT'],
                              pwd= params['USERLISTS']['PASSWORD'],
                              pwd_length= params['USERLISTS']['PWD_LENGTH'],
                              tenant=params['TENANT'])
    ...
    if args.add :
        di_login(params)
        users = read_userlist(params['USERLISTS']['USERLIST_FILE'])
        for u in users :
