...
class Device(Subject, YamlSerializable):
    ...
    session = None  # Holds current Session object
    ...
    command_encoding = "utf8"  # Default command encoding
    ...
    def __setitem__(self, key, value):
        ...
        !!! Be careful not to use self[POSITION] = 50m, this would set Attribute to just 50nm !!!
    ...
    def _init_attributes(self):
        ...
        Attributes;
            ...
            ATTR_STATUS: Current Device status can be user defined, but it's overwritten by connection methods
            ATTR_DESCRIPTION: Attribute's description, it's displayed in GUI on MouseHover
            ATTR_ENABLED: Meta flag for GUI to Enable / Disable edit of Value
            ATTR_LATENCY: Average Device response time to request
            ATTR_BUFFERED_COMMANDS: Number of buffered commands. Higher number can indicates ERROR.
            ATTR_HANGING_COMMANDS: Number of commands sent to Device and waiting response.
                                   Higher number can indicates ERROR.
    ...
    def add_attribute(self, name, attribute, group=None) -> Attribute:
        """
        Use this function to add existing attribute to Device.
        This is useful if Attribute from other Device has to be visible in this Device as well.
        !!! Use create_attribute when need to add new Attribute to this Device !!!
    ...
    def delete_attribute(self, name, group=None):
        ...
        :param str group: Optional group of the Attribute
        """
        attribute_path = name
        if group is not None:
            attribute_path = "{}.{}".format(group, attribute_path)
    ...
    def attach_attribute_callback(
        self, attribute, callback, max_update_rate=None, key_filter=None
    ):
        """
        ...
        :param Union[str, list, tuple] attribute: Attribute
        :param Callable callback: callable function or method
        :param int max_update_rate: millisecond(s)
        :param str key_filter: Callback's filter ie. VALUE, TYPE, ...
        """
    ...
    def _get(self, attribute) -> Union[Attribute, None]:
        """
        ...
        :param Union[list, tuple] attribute: list
        :return: mixed
        """
        try:
    ...
    def get_attribute(self, attribute) -> Union[Attribute, None]:
        """
        Get Attribute value.

        :param Union[str, list, tuple] attribute: Attribute
        :return: Union[Attribute, None]
        """
        try:
    ...
    def _set(self, attribute, value, callback=None) -> str:
        """
        ...
        :return str: Set token
        """
        attribute_object = get_from_dict(self.attributes, attribute[:-1])
        ...
        if allow_broadcast and kamzik3.session is not None:
            ...
            if (
                not is_equal(current_value, value)
                ...
                header = "{}.{}".format(attribute_object.attribute_id, attribute[-1])
                kamzik3.session.publisher.push_message(header, value)

        return set_token

    def set_attribute(self, attribute, value, callback=None) -> str:
        """
        This sets Device attribute.

        Re-implement it for client or any special use.

        :param Union[list, tuple, str] attribute: tuple, list, str
    ...
    def set_value(self, attribute, value, callback=None, key=VALUE) -> str:
        """
        Set attribute value.
        ...
        :param mixed value: mixed
        :param Callable callback: callable
        :param mixed key: Attribute's key
        :return str: token
        """
    ...
    def handle_readout_callback(self, callback, attribute, output):
        ...
        :param Callable callback: callable
        :param Union[list, tuple, str] attribute: tuple, list, str
        :param mixed output: Reply from Device
        """
        if callback is None:
            return
        else:
    ...
    def handle_connect(self):
        ...
        self.logger.info(
            u"Device connection took {} sec.".format(time.time() - self.init_time)
        )
    ...
    def handle_readout(self, readout_buffer) -> tuple:
        ...
        try:
            ...
            self.handle_response_error(
                u"Trying to pop from empty command buffer. Content of readout buffer is: {}".format(
                    readout_buffer
                )
            )
    ...
    def set_status(self, status):
        ...
        But STATUS_CONFIGURED server as a metadata indicating, that it was configured at least once.
    ...
    def in_statuses(self, statuses) -> bool:
        """
        Check list of statuses against current device status.

        :param Union[list, tuple] statuses: List of statuses
        :return bool: True if current status is in statuses
        """
    ...
    def command(self, command, callback=None, with_token=False, returning=True) -> int:
        ...
        !!! There are Devices, that silently executes command and in that case we are not waiting for response. !!!
        ...
        if not self.valid_command_format(command):
            raise CommandFormatException(
                u"Command '{}' form is invalid".format(command)
            )
    ...
    def send_command(self, commands) -> list:
        ...
        !!! Make sure to call it directly when it's needed, for example for Connection handshake.!!!
        ...
        try:
            ...
            if self.connected:
                ...
                self.push(
                    command_data
                    if self.command_encoding is None
                    else command_data.encode(self.command_encoding)
                )
            else:
                self.handle_response_error(u"Device is not connected")
        except IndexError:
            self.handle_connection_error(
                u"Device {} buffer error".format(self.device_id)
            )
        except (WriteException, serial.SerialException):
            self.handle_response_error(
                u"Device {} writing error".format(self.device_id)
            )
        finally:
            return commands
    ...
    def _expose_methods_to_clients(self):
        """This method is called to expose all decorated methods with @expose_method to the Client."""
    ...
    def wait_for_status(
        self, statuses, retry_timeout=1000, throw_exception=True, callback=None
    ) -> Union[bool, Thread]:
        """
        Wait until device change it's status in one defined in statuses.
        Method is non blocking, when callback is set.

        :param list statuses: List of one or multiple statuses to check
        :param int retry_timeout: Maximum time in ms to wait for status in ms
        :param bool throw_exception: Raise exception when True
        :param Callable callback: Function to call after check is finished
        :return bool: True if device is in one of statuses
        """
    ...
    def wait_until_value_set(
        self, attribute, value, retry_timeout=1000, throw_exception=True, callback=None
    ) -> Union[bool, Thread]:
        """
        ...
        :param int retry_timeout: Retry check timeout in ms
        :param bool throw_exception: Raise exception when True
        :param Callable callback: Function to call after check is finished
        :return bool: True if Attribute's value is equal
        """
