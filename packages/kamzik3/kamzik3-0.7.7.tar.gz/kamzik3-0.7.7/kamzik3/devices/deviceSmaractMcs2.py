...
class DeviceSmaractMcs2(DeviceSocket):
    ...
    emergency_stop_modes = frozenbidict({"Normal": 0, "Restricted": 1, "Auto release": 2})
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_EMERGENCY_STOP, default_type=self.emergency_stop_modes.keys(),
    ...
    def remove_poll_command(self, command, interval):
        ...
        command += ":SYST:ERR:NEXT?\r\n"
        super(DeviceSmaractMcs2, self).remove_poll_command(command, interval)
    ...
    def handle_readout(self, readout_buffer):
        ...
        if command == ":DEV:NOCH?":
            ...
            output = int(output)
            self.set_raw_value(ATTR_EMERGENCY_STOP, self.emergency_stop_modes.inv[output])
            ...
            if attribute == "STAT?":
                ...
                attribute, output = ATTR_REFERENCE_DIRECTION, self.safe_direction_modes.inv[int(output)]
            else:
                return
    ...
    def set_emergency_stop_mode(self, value, callback=None):
        self.logger.info(u"Set emergency stop mode to {}".format(value))
        value = self.emergency_stop_modes[value]
        # Set this mode as a default
        self.command(u":DEV:EST:MODE:DEF {}\r\n".format(value), callback, with_token=True)
        return self.command(u":DEV:EST:MODE {}\r\n".format(value), callback, with_token=True)
...
class DeviceSmaractMcs2Channel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_HOLDING, readonly=True, default_type=np.bool)
        ...
        self.create_attribute(ATTR_POSITIONER_CODE, readonly=False, default_type=np.int8, min_value=0, max_value=500,
        ...
        self.create_attribute(ATTR_REFERENCE_DIRECTION, default_type=self.safe_direction_modes.keys(),
    ...
    def subject_update(self, key, value, subject):
        ...
        if self.connected and isinstance(key, tuple) and key[0] == self.channel:
            ...
            if attribute == ATTR_STATE and self.get_value(ATTR_STATE) != value:
                ...
                holding = bool(value & 0x0004) and bool(value & 0x0008) and bool(value & 0x0008) and bool(
                    value & 0x0002) \
                          and not bool(value & 0x0001)
                self.set_raw_value(ATTR_HOLDING, holding)
    ...
    def enable_open_loop(self):
        ...
        self.set_attribute((ATTR_POSITION, UNIT),
                           default_unit if default_unit != None else self.position_attribute_copy[UNIT])
    ...
    def move_absolute(self, position):
        try:
            ...
            if move_mode == "Relative":
                position = position - self[ATTR_POSITION][VALUE]
    ...
    def _move_steps(self, value, callback=None):
        if self.get_value(ATTR_MOVE_MODE) != "Step":
            self.set_value(ATTR_MOVE_MODE, "Step")
        current_position = self.get_value(ATTR_POSITION)
        relative_steps = value - current_position
        n_factor, p_factor = self.get_value(ATTR_CLOSE_LOOP_FACTOR)
        relative_steps = relative_steps / (n_factor if relative_steps < 0 else p_factor)
        self.logger.info(f"Move by {relative_steps} steps")
        if not self.get_value(ATTR_SENSOR_PRESENT) or self.get_value(ATTR_SENSOR_MODE) == "Disabled":
            self.set_raw_value(ATTR_POSITION, value)
        token = self.command(u":MOVE{} {}\r\n".format(self.channel, relative_steps), callback, with_token=True)
        return token
    ...
    def set_sensor_mode(self, value, callback=None):
        ...
        out = self.command(u":CHAN{}:SENS:MODE {}\r\n".format(self.channel, value), callback, with_token=True)
        self.get_sensor_mode()
        return out
    ...
    def set_reference_direction(self, value, callback=None):
        self.logger.info(u"Set reference direction to {}".format(value))
        value = self.safe_direction_modes[value]
        return self.command(u":CHAN{}:REF:OPT {}\r\n".format(self.channel, value), callback, with_token=True)
