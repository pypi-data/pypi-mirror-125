...
from PyQt5.QtWidgets import QWidget, QColorDialog, QHeaderView, QShortcut, QButtonGroup, QPushButton, \
...
from kamzik3.devices.deviceSavedAttributes import UPDATE_INSERT_GROUPS, UPDATE_REMOVE_GROUP, UPDATE_INSERT_ROW, \
    UPDATE_VALUE, UPDATE_REMOVE_ROW
...
from kamzik3.snippets.snippetsWidgets import show_question_dialog, show_prompt_dialog, show_error_message
...
class SavedAttributesWidget(Ui_Form, DeviceWidget):
    ...
    def __init__(self, device=None, config=None, parent=None):
        ...
        self.cache_h5_file = os.path.join(kamzik3.session.get_value(ATTR_CACHE_DIRECTORY), "client_save.h5")
        ...
        self.manipulation_buttons = [self.button_set_value, self.button_change_color, self.button_remove]
        ...
        self.table_attributes.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
    ...
    def slot_set_status(self, value):
        if value in READY_DEVICE_STATUSES and not self.configured:
            ...
            self.device.attach_attribute_callback(ATTR_LAST_UPDATE, self.content_update, key_filter=VALUE)
            with open(self.cache_h5_file, "wb") as h5_fp:
                h5_fp.write(self.device.get_value(ATTR_CONTENT))
            self.device.attach_attribute_callback(ATTR_GROUPS, self.groups_changed, key_filter=VALUE)
        ...
        with self.update_lock:
            ...
            if values[0] == UPDATE_INSERT_GROUPS:
                ...
                for group, data in groups_data.items():
                    ...
                    df.to_hdf(self.cache_h5_file, key=group, format="table", complevel=1)
                ...
                if subgroup[1] == self.combo_preset.currentText() and subgroup[
                    2] == self.button_group.checkedButton().text():
    ...
    @pyqtSlot(str)
    def slot_refresh_group_table(self, group):
        self.preset_group_changed(self.combo_preset.currentText())
    ...
    def slot_value_update(self, row, column, value):
        ...
        for sub_group in self.groups[top_group]:
            ...
            self.device.update_value(group=group_path, row=row, column=column, value=value)
    ...
    def set_attribute_value(self):
        ...
        if result is not None:
            try:
                device_id, attribute_name, unit = result.group(1), result.group(2), result.group(3)
                self.logger.info(f"Set {device_id} {attribute_name} to value {attribute_value} {unit}")
                device = session.get_device(device_id)
                attribute_as_list = eval(f"[{attribute_name}]")
                value_in_device_units = device_units(device, attribute_as_list, units.Quantity(attribute_value, unit))
    ...
    def remove_rows(self):
        with self.update_lock:
            ...
            answer = show_question_dialog(f"Do You really want to remove row {row} ?", "Confirmation", self)
            if answer:
                top_group = self.combo_preset.currentText()
    ...
    def add_row(self):
        with self.update_lock:
            ...
            if user_comment:
                ...
                for subgroup in self.groups[top_group]:
                    group_path = os.path.join("/", top_group, subgroup)
                    df = pd.read_hdf(self.cache_h5_file, key=group_path)
                    table_row = ["#fff", time.strftime("%d-%m-%Y %H:%M:%S"), user_comment]
                    for column in df.columns[3:]:
                        ...
                        if result is not None:
                            try:
                                ...
                                attribute_as_list = eval(f"[{attribute_name}]")
                                device = session.get_device(device_id)
                                current_value = device.get_attribute(attribute_as_list).value().to(unit)
                    ...
                    self.device.insert_row(group=group_path, data=dict(zip(df.columns, table_row)), position=2)
    ...
    def add_preset_group(self):
        with self.update_lock:
            ...
            if group_name and group_name != "":
                ...
                for subgroup in self.groups[top_group]:
                    group_path = os.path.join("/", top_group, subgroup)
                    new_group_path = os.path.join("/", group_name, subgroup)
                    df = pd.read_hdf(self.cache_h5_file, key=group_path)
                    groups[new_group_path] = dict(zip(df.columns, [["None"]] * len(df.columns)))
    ...
    def remove_preset_group(self):
        with self.update_lock:
            ...
            answer = show_question_dialog(f"Do You really want to remove group {current_group} ?", "Confirmation", self)
            if answer:
    ...
    def slot_refresh_group_tables(self, top_group):
        ...
        while self.layout_subgroups_select.count() != 0:
            self.layout_subgroups_select.removeItem(self.layout_subgroups_select.itemAt(0))
        self.button_group = QButtonGroup()
        for button_index, subgroup in enumerate(self.groups[top_group]):
            button = QPushButton(subgroup)
            button.setStyleSheet("QPushButton {background-color:silver;} QPushButton:checked {background-color:#f6b442;font-weight:bold}")
            button.setCheckable(True)
            button.setAutoExclusive(True)
            button.setMinimumHeight(30)
            button.toggled.connect(lambda state, subgroup=subgroup: self.slot_refresh_group_table(state, subgroup))
            self.button_group.addButton(button, button_index)
            self.layout_subgroups_select.addWidget(button)
        self.layout_subgroups_select.addSpacerItem(QSpacerItem(0, 0, hPolicy=QSizePolicy.Expanding))
    ...
    def slot_refresh_group_table(self, state, subgroup, model_data=None):
        ...
        with self.update_lock:
            if model_data is None:
                model_data = pd.read_hdf(self.cache_h5_file, "/".join([top_group, subgroup]))
            row_count = len(model_data.index) - 1
            to_index = (row_offset + self.page_size) if row_count >= (row_offset + self.page_size) else row_count
            ...
            self.table_attributes.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
            self.table_attributes.horizontalHeader().setSectionResizeMode(1, QHeaderView.Interactive)
            self.table_attributes.selectionModel().selectionChanged.connect(self.current_selection_changed)
            self.input_page.blockSignals(True)
