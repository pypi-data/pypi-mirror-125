...
class DeviceTango(Device, DeviceProxy):
    ...
    def send_command(self, commands):
        for command in commands:
            ...
            self.set_raw_value(tango_attribute, polled_attribute.value)
...
class DeviceTangoLambdaDetector(DeviceTango):
    ...
    def handle_configuration(self):
        try:
            ...
            self.create_attribute("OperatingMode", group="Mode", default_value=current_mode, default_type=["ContinuousReadWrite", "TwentyFourBit"],
...
class DeviceTangoEigerDetector(DeviceTango):
    ...
    def __init__(self, path, file_writer, device_id=None, config=None):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_FRAME_COUNT, default_value=0, readonly=True,
                              description="Internal frame counter",
                              default_type=np.uint64, min_value=0)

    @expose_method(
        {"Name": "FilePrefix", "Exposure": "FrameTime", "Frames": "Nimages"})
    def acquire_frame(self, Name, Exposure, Frames):
        self.logger.info(f"Acquisition of {Frames} @ {Exposure} has started.")
        now = datetime.datetime.now()
        Frames = device_units(self, "Nimages", Frames).m
        self.set_attribute(["TriggerMode", VALUE], 3)
        path = os.path.join(self.get_value("shotDir"), f"{Name}_{self.device_id}_{now.strftime('%Y-%m-%d_%H:%M:%S')}")
    ...
    @expose_method(
        {"Exposure": "FrameTime", "Frames": "Nimages"})
    def acquire_scan_frame(self, Exposure, Frames):
        ...
        if Frames != self.get_value("Nimages"):
            ...
            self.set_attribute(["FileWriter", "NimagesPerFile", VALUE], int(Frames))
        self.set_attribute(["FileWriter", "Mode", VALUE], 0)
        self.set_attribute(["FileWriter", "NamePattern", VALUE],
                           self.get_value("scanFilenamePattern").replace("{$ATTR_FRAME_COUNT}",
                                                                         f"{self.get_value(ATTR_FRAME_COUNT):08d}"))
        self.Arm()
        self.Trigger()
        self.set_value(ATTR_FRAME_COUNT, self.get_value(ATTR_FRAME_COUNT) + int(Frames))
    ...
    @expose_method(
        {"Exposure": "FrameTime"})
    def live_view(self, Exposure):
        self.logger.info(f"Live view has started.")
        self.set_attribute(["TriggerMode", VALUE], 3)
    ...
    def stop(self):
        if self.get_value(ATTR_STATUS) == STATUS_BUSY:
            self.logger.info(f"Acquisition aborted")
            self.Abort()
