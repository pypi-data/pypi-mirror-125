...
class DevicePi(DeviceSocket):
        ...
        for line in output:
            ...
            if attribute is not None:
                ...
                if attribute == TRIG_OUTPUT_AXIS:
                    ...
                    self.create_attribute(attribute, trigger_group_name, default_value=int(value) - 1,
                                          default_type=np.uint16,
                                          set_function=set_attribute)
                elif attribute == TRIG_OUTPUT_MODE:
                    self.create_attribute(attribute, trigger_group_name,
                                          default_value=trigger_output_mode.inv.get(int(value)),
                                          default_type=list(
                                              trigger_output_mode.keys()),
                                          set_function=set_attribute)
                else:
                    self.create_attribute(attribute, trigger_group_name, default_value=float(value),
    ...
    def _set_wave_generators(self, command, output):
        ...
        for line in output:
            ...
            self.create_attribute(ATTR_PRESCAN_PIXELS, generator_group_name, default_value=0, default_type=np.uint,
                                  readonly=True)
            self.create_attribute(ATTR_POSTSCAN_PIXELS, generator_group_name, default_value=0, default_type=np.uint,
                                  readonly=True)
            self.create_attribute(ATTR_RECORDED_POSITIONS, generator_group_name, default_type=TYPE_ARRAY, readonly=True)
            ...
            self.create_attribute(ATTR_WAVETABLE_RATE, generator_group_name, default_type=np.uint32,
                                  set_function=set_function)
            ...
            self.create_attribute(ATTR_WAVETABLE_INTERPOLATION, generator_group_name,
                                  default_type=list(data_interpolation.keys()), set_function=set_function)
    ...
    def handle_readout(self, readout_buffer):
        ...
        if command == "IDN?\n":
            ...
            for line in lines:
                ...
                self.set_raw_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVE_TABLE],
                                   int(wave_table_id))
        elif "WTR?" in command:
            lines = output.split(" \n")
            for line in lines:
                ...
                if int(generator_id) == 0:
                    ...
                    self.set_raw_value(["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_RATE],
                                       int(rate))
                    self.set_raw_value(
                        ["Wave generator {}".format(int(generator_id) - 1), ATTR_WAVETABLE_INTERPOLATION],
                        interpolation)
        elif "SEP?" in command:
            lines = output.split(" \n")
            for line in lines:
            ...
            wto = self[f"Wave generator {generator_id}"][ATTR_WAVETABLE_OFFSET]

            recorded_data = []
            for i in range(0, len(lines[7:])):
                fixed_position = wto.remove_offset_factor(float(lines[7 + i]))
                recorded_data.append(fixed_position)
            ...
            self.set_raw_value(["Wave generator {}".format(generator_id), ATTR_RECORD_TABLE_DATA],
                               recorded_data)
            trigger_line = [int(i) for i in self.get_value([trigger_group_name, ATTR_TRIGGER_LINE_DATA]).split(",")]
            trigger_positions = [recorded_data[index] for index, trigger in enumerate(trigger_line) if trigger]
            self.set_raw_value(["Wave generator {}".format(generator_id), ATTR_RECORDED_POSITIONS], trigger_positions)


            channel = self.get_value([trigger_group_name, "Trigger axis"])
            self.notify((channel, ATTR_RECORD_TABLE_DATA), recorded_data)
            self.notify((channel, ATTR_RECORDED_POSITIONS), trigger_positions)
    ...
    def clear_wave_table_triggers(self):
        """
        Here comes a short description of clear_wave_table_triggers.
    ...
    def get_wave_generator_rate_and_interpolation_type(self, callback=None):
        """
        Here comes a short description of get_wave_generator_rate_and_interpolation_type.
    ...
    def get_wave_gen_output_offset(self, wave_generator_id=None, callback=None):
        """
        Here comes a short description of get_wave_gen_output_offset.
    ...
    def set_wave_gen_output_offset(self, wave_generator_id, offset, callback=None):
        """
        Here comes a short description of set_wave_gen_output_offset.
    ...
    def set_trigger_line(self, trigger_output, trigger_line_params, callback=None):
        """
        Here comes a short description of set_trigger_line.
    ...
    def set_record_table_rate(self, rate, callback=None):
        """
        Here comes a short description of set_record_table_rate.
    ...
    def set_recorded_configuration(self, table_id, source, option, callback=None):
        """
        Here comes a short description of set_recorded_configuration.
    ...
    def set_fly_scan(self, channel=0, center=50, step_size=1, pixels=10, dwell_time=100,
        ...
        wto = self[f"Wave generator {generator_id}"][ATTR_WAVETABLE_OFFSET]
        center = wto.remove_offset_factor(center)
        ...
        self.set_value(["Wave generator {}".format(generator_id), ATTR_PRESCAN_PIXELS], pre_scan_pixels)
        self.set_value(["Wave generator {}".format(generator_id), ATTR_POSTSCAN_PIXELS], post_scan_pixels)
...
class DevicePiChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_RECORD_TABLE_DATA, default_value=[], readonly=True)
        self.create_attribute(ATTR_RECORDED_POSITIONS, default_value=[], readonly=True)
