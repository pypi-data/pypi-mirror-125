...
class AttributeLogger(Device):
    ...
    def __init__(self, log_file_name, device_id=None, config=None):
        # Check if logged_attributes were already set (in case of Disconnected and Connected Device)
    ...
    def _set_interval(self, interval):
        """
        Private function used to set tick interval.

        :param int interval: interval in ms
        :return:
        """
        self.interval = interval * 1e-3
    ...
    def _get_logged_attributes(self) -> ValuesView:
        """
        Private function to collect all logged attributes.
        If new attribute is discovered, generate and write Header.
    ...
    def set_log_file_name(self, log_file_name) -> Logger:
        """
        This method is called when new logfile has to be created.
        INFO level is used for Attribute logging.
    ...
    def handle_interval_timeout(self):
        """
        This is callback for interval tick.
...
class AttributeLoggerTriggered(AttributeLogger):
    """
    This is different type of AttributeLogger.
    Instead of periodically adding new line, we log only when externally triggered.
    This Logger is used for Macros.
    """

    def start(self, flag=True):
        """
        Override Start method, because now we just generate Header and wait for external triggers.
        :param bool flag: Toggle flag
        """
        if flag:
            if self.is_status(STATUS_BUSY):
    ...
    def trigger(self):
        """
        Here we get a external trigger.
        This method is just a wrapper for handle_interval_timeout of Parent AttributeLogger.
        :return:
        """
