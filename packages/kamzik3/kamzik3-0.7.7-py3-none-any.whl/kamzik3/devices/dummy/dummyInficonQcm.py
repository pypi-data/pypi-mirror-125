...
class DummyInficonQcmSensorChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_Z_RATIO, default_value=0, default_type=float, readonly=True, decimals=9)
        self.create_attribute(ATTR_FUNDAMENTAL_FREQUENCY, unit="Hz", default_value=0, default_type=numpy.double,
                              readonly=True, decimals=9, factor=0.000873114913702011)
        self.create_attribute(ATTR_AHARMONIC_FREQUENCY, unit="Hz", default_value=0, default_type=numpy.double,
                              readonly=True, decimals=9, factor=0.000873114913702011)
        self.create_attribute(ATTR_SENSOR_THICKNESS, unit="nm", default_value=0, default_type=float,
                              readonly=True, decimals=9, factor=0.1)
        self.create_attribute(ATTR_CRYSTAL_LIFE, unit="%", default_value=100, default_type=int,
                              readonly=True)
        self.create_attribute(ATTR_RAW_RATE, unit="nm/s", default_value=0, default_type=float,
                              readonly=True, decimals=9, factor=0.1)
        ...
        self.create_attribute(ATTR_SHUTTER_OPEN, default_value=False, default_type=bool,
                              readonly=True)
        self.create_attribute(ATTR_LAYER_THICKNESS, unit="nm", default_value=0, default_type=float,
                              readonly=True, decimals=9, factor=0.1)
    ...
    def _sensor_simulation(self):
        while self.configured:
            ...
            self.set_value(ATTR_AHARMONIC_FREQUENCY, random.randint(10000, 100000) * factor)
            self.set_value(ATTR_FUNDAMENTAL_FREQUENCY, random.randint(10000, 100000) * factor)
