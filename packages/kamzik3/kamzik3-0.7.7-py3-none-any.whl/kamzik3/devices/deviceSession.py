...
class DeviceSession(Device):
    ...
    def get_device(self, device_id, search_master=True):
        try:
            return self.devices[device_id]
        except KeyError:
            if search_master:
                master_server, master_port = self.config.get("master_server", False), \
    ...
    def add_device(self, device):
        ...
        if device.device_id not in self.devices:
            ...
            raise DeviceError(f"Device {device.device_id} is already registered within session.")
    ...
    def set_console_log_directory(self, value):
        ...
        if not os.path.exists(log_directory):
            self.logger.info(f"Directory {log_directory} does not exists, trying to create it.")
...
def server_process(conf: str, stop_event: Event) -> Process:
    """
