...
class DeviceSmaractMcs1Channel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_SENSOR_MODE, default_value=self.device.get_value(ATTR_SENSOR_MODE),
                              readonly=True, default_type=self.sensor_modes.keys())
    ...
    def handle_configuration(self):
        ...
        def _finish_configuration(*_, **__):
            self._config_commands()
            self._config_attributes()
            self.set_value(ATTR_SENSOR_MODE, self.device.get_value(ATTR_SENSOR_MODE))
            self.set_value(ATTR_SENSOR_PRESENT, self.device.get_value(ATTR_SENSOR_PRESENT))
    ...
    def _move_steps(self, value, callback=None):
        ...
        return self.move_step(relative_steps, callback=callback)
    ...
    def enable_open_loop(self):
        ...
        if self.get_value(ATTR_POSITION) is None:
            self.set_raw_value(ATTR_POSITION, 0)
