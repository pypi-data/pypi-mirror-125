...
class DeviceClient(Device):
    ...
    def init(self):
        ...
        try:
            with self.client_comm_lock:
                self.socket.send_multipart([INSTRUCTION_INIT, self.device_id.encode()], copy=False)
                status, token, response = self.handle_server_response()
            if status == RESPONSE_OK:
                allow_proxy = self.config.get("allow_proxy", True)
                attributes, self.attributes_sharing_map, self.exposed_methods, self.qualified_name, device_proxy, device_publisher = json.loads(
    ...
    def set_attribute(self, attribute, value, callback=None):
        ...
        try:
            ...
            with self.client_comm_lock:
                self.socket.send_multipart([INSTRUCTION_SET, self.device_id.encode(), attribute_value.encode()], copy=False)
                status, token, response = self.handle_server_response()
            if status == RESPONSE_OK:
        ...
        try:
            with self.client_comm_lock:
                self.socket.send_multipart(
                    [INSTRUCTION_COMMAND, self.device_id.encode(), command.encode(), b"1" if callback else b"0"], copy=False)
                status, token, response = self.handle_server_response()
            if status == RESPONSE_OK:
    ...
    def method(self, method, attributes=None):
        ...
        try:
            ...
            with self.client_comm_lock:
                self.socket.send_multipart(
                    [INSTRUCTION_METHOD, self.device_id.encode(), str(method).encode(), dumped_attributes.encode()], copy=False)
                status, token, response = self.handle_server_response()
            if status == RESPONSE_OK:
