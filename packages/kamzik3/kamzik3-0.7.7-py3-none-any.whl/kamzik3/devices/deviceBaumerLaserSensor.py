...
class DeviceBaumerLaserSensorPort(DeviceSocket):
    ...
    Documentation is here: https://svnsrv.desy.de/desy/cfel-cxi/doc/manuals/Baumer/OADM_13T7480_S35A_manual.pdf
    ...
    def set_measuring_scale(self, value, callback=None) -> int:
        """
        Set measuring scale
        ...
        return self.command(
            "{{0S{}}}".format(MEASUREMENT_SCALE.inv[value]), callback, with_token=True
        )

    @expose_method()
    def laser_on(self, callback=None) -> int:
        """
        Turn on laser.
    ...
    @expose_method()
    def laser_off(self, callback=None) -> int:
        """
        Torn off laser.
        Stop measurement.
        :param Callable callback: Optional callback function
    ...
    @expose_method()
    def measure_distance(self, callback=None) -> int:
        """
        ...
        If You need in other use case constant measurement, just setup polling for '{0M}' command.
        :param Callable callback: Optional callback function
        :return int: token
        """
