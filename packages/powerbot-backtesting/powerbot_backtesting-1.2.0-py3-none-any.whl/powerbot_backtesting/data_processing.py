...
def get_orders(contract_hist_data: dict[str, pd.DataFrame],
    ...
    if append_all:
        ...
        for k, v in orders.items():
            v["time_range"] = k
            all_orders = all_orders.append(v)
        return all_orders
    ...
    if concurrent and len(contract_hist_data) >= 20:
        ...
        def process_multiple(contract_hist_data):
            for nr, (key, value) in enumerate(
                ...
                _process_orderbook(key, value, str(new_dir), timesteps, time_unit, timestamp[nr] if timestamp else None, from_timestamp,
        ...
        for nr, (key, value) in enumerate(tqdm(contract_hist_data.items(), desc="Processing Orders", unit="time periods", leave=False)):
            _process_orderbook(key, value, str(new_dir), timesteps, time_unit, timestamp[nr] if timestamp else None, from_timestamp, all_order_books,
                               use_cached_data)
