...
class PIDModel(object):
    def __init__(
        ...
        elif isinstance(lower_bounds, float):
            lower_bounds = [lower_bounds - self.steady_actions[i] for i in range(len_c)]
        if upper_bounds is None:
            upper_bounds = [upper_bounds for i in range(len_c)]
        elif isinstance(upper_bounds, float):
            upper_bounds = [upper_bounds - self.steady_actions[i] for i in range(len_c)]
