...
class AliRouteEntry():
    ...
    @classmethod
    def get(cls, vpn_id):
        result = vpn_client.describe_route({'VpnGatewayId': vpn_id})
        if result:
            return cls(result)
        return None
    ...
    def list(cls, vpn_id):
        result = vpn_client.describe_route({'VpnGatewayId': vpn_id})
        vpn_routes = result.get('VpnRouteEntries', {}).get('VpnRouteEntry', []) or []
        return [cls.get(ipsec['VpnConnectionId']) for vpn_route in vpn_routes]
