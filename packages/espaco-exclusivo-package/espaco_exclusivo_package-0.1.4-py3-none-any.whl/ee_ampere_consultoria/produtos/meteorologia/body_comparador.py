...
class BodyComparador:
    ...
    def set_periodo_analise_acumulada(self, data_inicio: datetime, data_fim: datetime, data_prev_base: datetime,
                                      data_prev_confrontante: datetime):
        """
        Colaboração: Rodrigo Sakata
        ~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        :param data_prev_base: data da previsão base
        :type data_prev_base: datetime
        :param data_prev_confrontante: data da previsão base
        :type data_prev_confrontante: datetime
        """
        ...
        # Rodrigo Sakata: Modificado a partir dessa linha de codigo
        # Rodrigo Sakata: Testa se a data de inicio da previsão é maior que a data de previsão de qualquer um dos modelos.
        if data_inicio <= data_prev_base or data_inicio <= data_prev_confrontante:
            raise Exception(
                "A data de inicio {} deve ser maior que as datas de previsão escolhidas.".format(data_inicio))

        if data_inicio > data_fim:
            raise Exception("A data fim {} deve ser maior que a data de inicio.".format(data_fim))

        # Rodrigo Sakata: Testa da data de previsao do modelo base
        if type(data_prev_confrontante) != datetime:
            raise Exception("Tipo {} não é um objeto date válido.".format(type(data_prev_confrontante)))

        if data_prev_confrontante > datetime.today():
            raise Exception("A data de previsão do modelo confrontante {} deve ser menor ou igual à hoje.".format(
                data_prev_confrontante))

        # Rodrigo Sakata: Testa da data de previsao do modelo confrontante
        if type(data_prev_base) != datetime:
            raise Exception("Tipo {} não é um objeto date válido.".format(type(data_prev_base)))

        if data_prev_base > datetime.today():
            raise Exception(
                "A data de previsão do modelo base {} deve ser menor ou igual à hoje.".format(data_prev_base))

        self.__data_inicial = data_inicio.timestamp()
        self.__data_final = data_fim.timestamp()
        self.__data_prev_confrontante = data_prev_confrontante.timestamp()
        self.__data_prev_base = data_prev_base.timestamp()
