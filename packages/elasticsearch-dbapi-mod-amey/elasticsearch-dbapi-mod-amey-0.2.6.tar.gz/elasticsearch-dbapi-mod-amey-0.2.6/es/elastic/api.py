...
class Connection(BaseConnection):
    ...
    def cursor(self) -> BaseCursor:
        ...
        if self.es:
            open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' create cursor '+ str(self.url) +'\n')
...
class Cursor(BaseCursor):
    ...
    def execute(
        ...
        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' execute called for '+ str(operation) +'\n')
        cursor = self.custom_sql_to_method_dispatcher(operation)
        if cursor:
            open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' execute returned from custom sql dispatcher'+ str(cursor) +'\n')
            return cursor
        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' execute returned null\n')

        re_table_name = re.match("SHOW ARRAY_COLUMNS FROM (.*)", operation)
        if re_table_name:
            return self.get_array_type_columns(re_table_name[1])

        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' creating query with ' + str(operation) + ' : ' + str(parameters) + '\n')
        query = apply_parameters(operation, parameters)
        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' created query |' + str(query) + '|\n')
        ...
        self._results = rows
        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' results from query |' + str(rows) + '|\n')
        # self._results = [('ASDF',)]
        self.description = get_description_from_columns(columns)
        open('/mnt/c/Users/PC/Desktop/TMP/sqlalch/log.log','a').write(str(time.time()//1) + ' returning |' + str(self) + '|\n')
        return self
