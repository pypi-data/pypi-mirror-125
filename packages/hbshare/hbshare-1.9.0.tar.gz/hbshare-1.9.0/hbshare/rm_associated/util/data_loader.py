...
class CtaNavAttributionLoader:
    ...
    def _load_fund_data(self):
        sql_script = "SELECT a.jjdm fund_id, b.jzrq tradeDate, b.fqdwjz as adj_nav from " \
                     ...
                     "and a.jjzt not in ('3') " \
                     "and a.jjdm = '{}' and b.jzrq >= {} and b.jzrq <= {} " \
                     "order by b.jzrq".format(self.fund_id, self.pre_date, self.end_date)
...
class MorningStarBoxStyleLoader:
    ...
    def _load_valuation_data(self, date):
        ...
        # 主板
        sql_script = "SELECT (select a.SecuCode from hsjy_gp.SecuMain a where a.InnerCode = b.InnerCode " \
                     "and rownum = 1 and a.SecuMarket in (83, 90) and a.SecuCategory = 1) SecuCode, " \
                     "b.TotalMV, b.NegotiableMV, b.PE, b.PB, b.PCFTTM, b.DividendRatio FROM " \
                     "hsjy_gp.LC_DIndicesForValuation b WHERE b.TradingDay = to_date('{}', 'yyyymmdd')".format(date)
        ...
        sql_script = "SELECT (select a.SecuCode from hsjy_gp.SecuMain a where a.InnerCode = b.InnerCode " \
                     "and rownum = 1 and a.SecuMarket in (83, 90) and a.SecuCategory = 1) SecuCode, " \
                     "b.TotalMV, b.NegotiableMV, b.PETTM, b.PB, b.PCFTTM, b.DividendRatio FROM " \
                     "hsjy_gp.LC_STIBDIndiForValue b WHERE b.TradingDay = to_date('{}', 'yyyymmdd')".format(date)
        res = hbs.db_data_query('readonly', sql_script)
        data_stib = pd.DataFrame(res['data'])
        data_stib.rename(
