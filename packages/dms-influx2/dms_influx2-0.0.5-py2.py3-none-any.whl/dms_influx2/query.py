...
def get_flux_query(bucket=None,
    ...
    if time_from is not None:
        ...
        if time_offset is not None:
            time_from = time_from - timedelta(hours=int(time_offset))
        start = time_from.strftime("%Y-%m-%dT%H:%M:%S.%fZ")

    if time_to is not None:
        if type(time_to) == str:
            time_to = parse(time_to)
        if time_offset is not None:
            time_to = time_to - timedelta(hours=int(time_offset))
        stop = time_to.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
