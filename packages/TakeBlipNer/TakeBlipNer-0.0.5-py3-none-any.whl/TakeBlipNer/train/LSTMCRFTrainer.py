...
from TakeBlipNer import utils, data, nermodel

import time


class LSTMCRFTrainer(object):
    def __init__(self, bilstmcrf_model: nermodel.LSTMCRF_NER, input_vocab, input_path,
                 label_vocab, postag_model, postag_label_vocab, save_dir, tensorboard,
                 ckpt_period, val, val_period, samples, pad_string, unk_string,
                 batch_size, shuffle, label_column, encoding, separator,
                 use_pre_processing, learning_rate, learning_rate_decay,
                 max_patience, max_decay_num, patience_threshold, val_path=None,
                 epochs=5, optimizer=optim.Adam, n_iter=5, azureml_logger=None):
        ...
        self.postag_label_vocab = postag_label_vocab
        self.postag_model = postag_model
        self.inputs_vocab = [input_vocab] + [postag_label_vocab]
    ...
    def load_data(self, train):
        ...
        dataloader = data.MultiSentWordDataLoader(
            dataset=dataset,
            vocabs=[self.input_vocab]+[self.postag_label_vocab]+[self.label_vocab],
    ...
    def display_samples(self, inputs, targets, lstms, crfs):
        ...
        inputs = [[utils.lexicalize(s, v) for s in input]
                  for input, v in zip(inputs, self.inputs_vocab)]
        ...
        col_names = [f'INPUT{i + 1:02d}' for i in range(len(inputs))] + \
                    ['LSTM', 'CRF', 'TARGETS']
        vocabs = self.inputs_vocab + [self.label_vocab] * 3
    ...
    def save_model(self, steps=None):
        save_path = os.path.join(self.save_dir, 'nermodel.pkl')
    ...
    def validate(self, steps=None):
        ...
        for i_idx, (batch, lens, _) in enumerate(val_data):
            ...
            sequence_batch_all = [
                utils.lexicalize_sequence(sequence, lens_s, vocab) for sequence, vocab in zip(
                    utils.tighten(sequence_batch, lens_s), self.inputs_vocab)]
