...
def version(
    ...
    minimal: bool = Option(
        False, help="Whether to show minimal output, this is enabled if there are less than 3 versions to show"
    ),
):
    ...
    latest_versions_length = len(latest_versions)
    if latest_versions_length <= 3:
        minimal = True

    if not minimal:
        output = f"{f'Top {limit} latest versions of ' if limit > 1 else 'Latest version of '}[green]{package_name}[/]{' (excluding pre-releases)' if no_pre_releases else ''}:\n"
    else:
    ...
    for n, version in enumerate(latest_versions, start=1):
        output += f" {f'[magenta]{n}.[/] ' if latest_versions_length > 1 else ''}[white]{version}[/]"
        try:
