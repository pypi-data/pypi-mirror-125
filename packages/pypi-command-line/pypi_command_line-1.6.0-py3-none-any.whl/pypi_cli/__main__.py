...
def version(
    ...
    output = f"Top {limit} latest versions of [green]{package_name}[/] ordered by version number{' (excluding pre-releases)' if no_pre_releases else ''}:\n"

    from datetime import timezone  # pylint: disable=import-outside-toplevel

    for n, version in enumerate(latest_versions, start=1):
        if no_pre_releases and version.is_prerelease:
            continue
        ...
        try:
            ...
            output += f" [red]{humanize.naturaltime(upload_time)}[/] [green]({upload_time.strftime('%a %b %d %H:%M:%S %Y')})[/]"
        except Exception as e:
            print(e)
        finally:
            output += "\n"
