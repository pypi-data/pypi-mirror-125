...
P6 = Point(3, 4, 0)
P7 = Point(0,0,0)
D1 = Dimensions(1., 1., -2., 0,0,0,0)
D2 = Dimensions(2., 2., -4., 0,0,0,0)
D3 = Dimensions(1., 2., -2., 0,0,0,0)
...
def test_add():
    ...
    assert vec.add(D2, D3) == Dimensions(kg=3.0, m=4.0, s=-6.0, A=0.0, cd=0.0, K=0.0, mol=0.0)
    assert vec.add(L1, L2) == tuple(range(1, 200, 2))
    
def test_subtract():
    assert vec.subtract(P1, P3) == Point(x=1.0, y=2.4, z=1.0)
    assert vec.subtract(D2, D3) == Dimensions(kg=1.0, m=0.0, s=-2.0, A=0.0, cd=0.0, K=0.0, mol=0.0)
    assert vec.subtract(L1, L2) == tuple(-1 for n in range(0,100))
    
def test_multiply():
    assert vec.multiply(P1, P3) == Point(x=2.0, y=3.4, z=0.0)
    assert vec.multiply(D2, D3) == Dimensions(kg=2.0, m=4.0, s=8.0, A=0.0, cd=0.0, K=0.0, mol=0.0)
...
def test_normalize():
    ...
    assert vec.normalize(D3) == Dimensions(kg=0.3333333333333333, m=0.6666666666666666, s=-0.6666666666666666, A=0.0, cd=0.0, K=0.0, mol=0.0)
