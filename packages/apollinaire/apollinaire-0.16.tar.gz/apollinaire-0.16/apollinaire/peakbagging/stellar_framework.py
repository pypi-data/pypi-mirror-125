...
def stellar_framework (freq, psd, r, m, teff, nwalkers=64, dnu=None, back=None,
                       ...
                       filename_back='background', back_mcmc=True, back_mle=True,
                       ...
                       fit_amplitude_ratio=False, frozen_harvey_exponent=False, 
  ...
  :param coeff_discard_back: coefficient used to compute the number of values
    to discard at the beginning of each MCMC sampled by the background step : total
    amount of sampled values will be divided by coeff_discard. Optional, default
    50.
  :type coeff_discard: int

  :param coeff_discard_pattern: coefficient used to compute the number of
    values to discard at the beginning of each MCMC sampled by the pattern step :
    total amount of sampled values will be divided by coeff_discard. Optional,
    default 50.
  :type coeff_discard: int

  :param coeff_discard_pkb: coefficient used to compute the number of values to
    discard at the beginning of each MCMC sampled by the peakbagging step : total
    amount of sampled values will be divided by coeff_discard. Optional, default
    50.
  :type coeff_discard: int
  ...
  :param num: number of point to consider in the degraded PSD if ``quickfit``
    is set to True. Optional, default 500.
  :type num: int
  ...
  if back is None :
    if back_mle :
      fitted_back, param_back = perform_mle_background (freq, psd, n_harvey=n_harvey, r=r, m=m, teff=teff, guess=guess_back, 
                                ...
                                show=False, show_guess=False, filename=filename_back+'.pdf', spectro=spectro,
    ...
    if back_mcmc :
      fitted_back, param_back, sigma_back = explore_distribution_background (freq, psd,
                                       ...
                                       frozen_harvey_exponent=frozen_harvey_exponent, low_cut=low_cut, fit_log=True, quickfit=quickfit,
                                       low_bounds=low_bounds_back, up_bounds=up_bounds_back, spectro=spectro, show=False, show_guess=False, show_corner=True,
  ...
  if a2z_in is None :
    ...
    if pattern_mcmc :
      df_a2z, pattern, sigma_pattern = explore_distribution_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, wdw=None,
                                                                     ...
                                                                     up_bounds=up_bounds_pattern, show=False,
  ...
  if not nopeakbagging :
    ...
    df_a2z, pkb = peakbagging (a2z_to_read, freq, psd, back=fitted_back,
                               wdw=None, spectro=spectro, nsteps_mcmc=nsteps_mcmc_peakbagging,
                               show_corner=True, store_chains=True, mcmcDir=mcmcDir, order_to_fit=None,
                               parallelise=parallelise, progress=progress, strategy=strategy,
                               nwalkers=nwalkers, normalise=False, instr=instr,
                               ...
                               fit_angle=fit_angle, fit_amplitude_ratio=fit_amplitude_ratio)
