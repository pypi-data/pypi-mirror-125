            ...
            try:
                ...
                if column.startswith("'") and column.endswith("'"):
                    ...
                    if column == "*":
                        ...
                        raise SensiSyntaxError("{} is not correct".format(column))
                else:
                    raise SensiSyntaxError("{} is not correct".format(column))

            except KeyError:
                raise SensiSyntaxError("{} is not a valid dataframe column".format(column))

            except IndexError:
                raise SensiSyntaxError("index {} is out of bounds column for the dataframe".format(column))
            ...
            try:
                ...
                if row.startswith("'") and row.endswith("'"):
                    ...
                    if row == "*":
                        ...
                        # TODO par Quincy: Throw error when incorrect column/row value
                        raise SensiSyntaxError("{} is not correct".format(row))
                else:
                    raise SensiSyntaxError("{} is not correct".format(row))

            except KeyError:
                raise SensiSyntaxError("{} is not a valid dataframe row".format(row))

            except IndexError:
                raise SensiSyntaxError("index {} is out of bounds row for the dataframe".format(row))
...
def apply_syntax_to_file(input_path, syntax, settings_json):
    ...
    if input_path and syntax and settings_json:
        ...
        raise SensiSyntaxError("Parameters can not be None")
