...
def model_type(model_name):
    ...
    methods = ModelType.Tree_type_1 + ModelType.Tree_type_2 + ModelType.Linear
    if tmp_name not in methods:
        pattern = re.compile("\w*")

        for item in [item for item in pattern.findall(str(model_name)) if item]:
            if item in methods:
                tmp_name = item
...
def feature_name(savedPath, StorageInfo, targetName, data=None):
    if data is not None:
        try:
            feature_name = list(data.keys())
        except AttributeError:
            raise TypeError('지원하지 않는 데이터 형식입니다. 현재 지원하는 데이터 형식은 json과 pandas.core.frame.DataFrame입니다.')
