...
def generate(exam, *, include_watermark, prefix, suffix, response=None):
    ...
    def write_question(question):
        ...
        if question_response and isinstance(question_response, TextQuestion):
            answer_text = (
                question_response.response.replace("_", r"\_")
                .replace("\n", r" \\ ")
                .replace("%", r"\%")
            )
        ...
        if question["type"] in ["short_answer", "short_code_answer"]:
            ...
            write(
                rf"\framebox[0.8\textwidth][c]{{\parbox[c][{30 * (question['options'])}px][t]{{0.75\textwidth}}{{{answer_text}}}}}"
            )
...
def render_latex(
    ...
    *,
    ...
    if include_watermark:
        ...
        subprocess.run(
            f"inkscape -D -z --file={tempdir}/watermark.svg --export-pdf={tempdir}/watermark.pdf",
            shell=True,
        ).check_returncode()
