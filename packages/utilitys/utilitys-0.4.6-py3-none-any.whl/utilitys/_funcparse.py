  ...
  *not required* since they can be inferred from the properly formatted docstring.
  ...
  try:
    ...
    try:
      ...
      # Many things can go wrong reading a badly-formatted docstring, so on failure treat the whole doc as a
      # description of the entire function
      return {'func-description': doc}
...
def _parseIniDocstring_docstringParser(doc):
  ...
  for param in parsed.params:
    ...
    try:
      ...
      # Many things can go wrong, so treat the whole description as a tip for this parameter
      tip = ' '.join(line.strip() for line in param.description.splitlines())
      update = {param.arg_name: {'tip': tip}}
...
class InteractiveFunction:
    ...
    def setParams(self, params=None):
        """Creates a dictionary of params for easier independent accessing"""
        if params is None:
            params = {}
...
def parseYamlDocstring(doc: str):
  from .fns import yamlLoad, YAMLError
  out = {}
  parsed = dp.parse(doc)
  out['func-description'] = '\n'.join([desc for desc in [parsed.short_description, parsed.long_description]
                                       if desc is not None])
...
def docParser(docstring: str, streamFormat='ini'):
  ...
  :param streamFormat: Format of the docstring, either yaml or ini
  """
