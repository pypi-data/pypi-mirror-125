...
class HeliosphereBaseHttpInstrumentor(HeliosphereBaseInstrumentor):
    ...
    def base_hook(span: Span, headers_attribute_name: str, payload_attribute_name: str,
        ...
        if payload and len(payload) <= HeliosphereBaseHttpInstrumentor.MAX_PAYLOAD_SIZE:
    ...
    def base_response_hook(span: Span, response_headers: Dict, response_payload: Optional[str]) -> None:
        HeliosphereBaseHttpInstrumentor.base_hook(span,
                                                  HeliosphereBaseHttpInstrumentor.HTTP_RESPONSE_HEADERS_ATTRIBUTE_NAME,
                                                  HeliosphereBaseHttpInstrumentor.HTTP_RESPONSE_BODY_ATTRIBUTE_NAME,
                                                  response_headers, response_payload)

    @staticmethod
    def base_request_hook(span: Span, request_headers: Dict, request_payload: Optional[str]) -> None:
        HeliosphereBaseHttpInstrumentor.base_hook(span,
                                                  HeliosphereBaseHttpInstrumentor.HTTP_REQUEST_HEADERS_ATTRIBUTE_NAME,
                                                  HeliosphereBaseHttpInstrumentor.HTTP_REQUEST_BODY_ATTRIBUTE_NAME,
