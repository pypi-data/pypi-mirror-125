...
from heliosphere.instrumentation import HeliosphereBaseInstrumentor
from heliosphere.instrumentation.base_http_instrumentor import HeliosphereBaseHttpInstrumentor


class HeliosphereFastAPIInstrumentor(HeliosphereBaseHttpInstrumentor):
    ...
    def fastapi_instrument_and_init(self, wrapped, instance, args, kwargs):
        ...
        if instance not in self.instrumented_apps:
            ...
            get_route_details = HeliosphereBaseInstrumentor.import_attribute(self.MODULE_NAME, '_get_route_details')
            instance.add_middleware(HelioshpereMiddleware,
                                    tracer=self.tracer_provider.get_tracer(__name__),
                                    get_span_details=get_route_details)
        return init_response


class HelioshpereMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        attributes[HeliosphereBaseHttpInstrumentor.HTTP_REQUEST_HEADERS_ATTRIBUTE_NAME] = json.dumps(request_headers)

        with self.tracer.start_as_current_span(
            f"Heliosphere fastapi {span_name}", kind=trace.SpanKind.SERVER,
        ) as span:
            span.set_attributes(attributes)

            async def wrapped_send(message):
                if 'headers' in message:
                    response_headers = HelioshpereMiddleware.extract_headers(message.get('headers'))
                    span.set_attribute(
                        HeliosphereBaseHttpInstrumentor.HTTP_RESPONSE_HEADERS_ATTRIBUTE_NAME,
                        json.dumps(response_headers))

                if 'body' in message:
                    span.set_attribute(
                        HeliosphereBaseHttpInstrumentor.HTTP_RESPONSE_BODY_ATTRIBUTE_NAME, message.get('body'))
            ...
            async def wrapped_receive():
                ...
                if 'body' in message:
                    span.set_attribute(
                        HeliosphereBaseHttpInstrumentor.HTTP_REQUEST_BODY_ATTRIBUTE_NAME, message.get('body'))
                return message
    ...
    def extract_headers(headers: Optional[List]) -> Optional[Dict]:
        if headers is None:
            return None
        return {
            HelioshpereMiddleware.bytes_to_str(key): HelioshpereMiddleware.bytes_to_str(value)
