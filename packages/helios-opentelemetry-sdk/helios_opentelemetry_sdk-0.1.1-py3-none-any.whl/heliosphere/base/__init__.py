...
from heliosphere.base.tracing.export.hooked_batch_span_processor import (
...
class HeliosphereBase(ABC):
    ...
    def __init__(self,
        ...
        _LOG.info('Heliosphere tracing initialized.')
    ...
    def init_span_exporter(self, span_exporter) -> SpanExporter:
        ...
        if not collector_endpoint:
            error_message = 'Missing Heliosphere collector endpoint.'
    ...
    def init_span_processor(self) -> SpanProcessor:
        ...
        return HookedBatchSpanProcessor(
            ...
            HookedBatchSpanProcessorConfig(
                flush_on_end=HeliosphereBase.is_span_triggered_by_heliosphere_test,
                on_start_hook=HeliosphereBase.on_start_hook
            )
        )
    ...
    def on_start_hook(span: Span, parent_context: Optional[Context]) -> None:
        if HeliosphereBase.is_context_of_heliosphere_test(parent_context):
            HeliosphereBase.set_span_as_triggered_by_heliosphere_test(span)
        HeliosphereBase.push_operation_name_attribute(span)
        HeliosphereBase.push_jaeger_attributes(span)

    @staticmethod
    def is_context_of_heliosphere_test(parent_context: Optional[Context]) -> bool:
        baggage_value = \
            get_baggage(HeliosphereTags.TEST_TRIGGERED_TRACE, parent_context)
        return baggage_value == HeliosphereTags.TEST_TRIGGERED_TRACE

    @staticmethod
    def is_span_triggered_by_heliosphere_test(span: ReadableSpan) -> bool:
        test_triggered_trace = HeliosphereTags.TEST_TRIGGERED_TRACE
        return span.attributes \
            and span.attributes.get(
                test_triggered_trace) == test_triggered_trace

    @staticmethod
    def set_span_as_triggered_by_heliosphere_test(span: Span) -> None:
        test_triggered_trace = HeliosphereTags.TEST_TRIGGERED_TRACE
    ...
    def _is_sdk_disabled(self) -> bool:
        if os.environ.get(self.HS_DISABLED_ENV_VAR):
            _LOG.info(f'Heliosphere instrumentation disabled by \
