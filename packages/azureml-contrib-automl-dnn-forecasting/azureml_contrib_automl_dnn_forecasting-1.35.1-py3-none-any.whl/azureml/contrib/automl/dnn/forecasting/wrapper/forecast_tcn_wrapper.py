...
class ForecastTCNWrapper(DNNForecastWrapper):
    ...
    def train(self, n_epochs: int, X: DataInputType = None, y: DataInputType = None,
              ...
              logger: logging.Logger = None, featurizer: TimeSeriesTransformer = None) -> None:
        """
        ...
        while True:
            ...
            try:
                ...
                train_data_sampler = self._get_data_sampler(ds_train)
                dataloader_train = self.create_data_loader(ds_train, batch_size, train_data_sampler)
    ...
    def _get_data_sampler(self, ds: TimeSeriesDataset) -> Optional[DistributedSampler]:
        """Build a distributed sampler for a dataset."""
        if hvd is None:
            return None
        return DistributedSampler(ds, num_replicas=hvd.size(), rank=hvd.rank())
    ...
    def _build_model_forecaster(self, run_update_callback: RunUpdateCallback,
        ...
        if hvd is not None:
            self.optimizer = hvd.DistributedOptimizer(
                self.optimizer, named_parameters=model.named_parameters(), compression=hvd.Compression.fp16)
    ...
    def _predict(
        ...
        if data_loader is None:
            data_loader = self.create_data_loader(ds)
