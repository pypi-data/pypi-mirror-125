...
class Report(AbstractRDAuthor):
    ...
    def last_order(self, field):
        """
        Получение следующего порядкового номера из списка
        """
        return field.aggregate(max_order=Coalesce(Max('order'), -1)).get('max_order') + 1
    ...
    def field_next_order(self):
        ...
        """
        return self.last_order(self.report_fields)
    ...
    def table_next_order(self):
        ...
        """
        return self.last_order(self.report_tables)
...
class ReportTableRelation(MPTTModel):
    ...
    def __str__(self):
        return f'Связь до таблицы {self.report_table.db_table.name} отчета {self.report_table.report}'
