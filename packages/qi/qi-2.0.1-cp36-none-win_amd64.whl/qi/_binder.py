...
class bind(object):
    ...
    def __init__(self, returnType=None, paramsType=None, methodName=None):
        ...
        elif isinstance(paramsType, AnyArguments) \
            or (inspect.isclass(paramsType) and
                issubclass(paramsType, AnyArguments)):
            self._sig = "m"
        else:
...
class multiThreaded(object):
    """This class decorator specifies that methods in the class are allowed to
       ...
       must garantee no concurrent access to it's internal data, usually by
