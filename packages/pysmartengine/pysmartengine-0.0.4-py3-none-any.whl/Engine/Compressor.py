...
#     """
#     k=k_exhaust_gu(Tt)
#     return (1-pow(pit, -(k - 1) / k))/(1-T0/Tt)
...
class TurbineSimple(object):
    def __init__(self,Te0=300,pe0=1.e5) -> None:
        """
        Te0:涡轮后温度
        pe0:涡轮后压力
        """
...
class Compressor:
    def __init__(self, mapfile, Tref=298, Pref=101325, MapType="real", order=2):
        ...
        self.Cmap=ICELib.Map()
        T=ArrayTable(4,0)
        for each in self.speeds:
            T.fromPandas(self.group.get_group(each))
            self.Cmap.addspeedmap(T.to_C())
    ...
    def massFlowrate(self, speed, pik):
        return self.Cmap.calculate(speed,pik)
...
class Turbine:
    def __init__(self, mapfile,maptype="Correct"):
    ...
    def calculate(self,speed,pi):
        # result=self.Cmap.calculate(speed,pi)
        return self.Cmap.calculate(speed,pi)

    def getSpeedMap(self,speed):
        return self.Cmap.getspeedmap(speed)
    ...
    def plot(self, showlegend=False, hightlight=None):
        ...
        if self.regTable is not None:
            ax[1].plot(self.regTable.table[0].data,self.regTable.table[1].data)

        plt.tight_layout()
        plt.show()

    def regressionWithValve(self,Tt=800,ptref=2.e5,alpha=1):
        from math import sqrt
        from .Valve import ValveSimple
        from .Cylinder import Volume
        VTur=Volume(1,ptref,Tt,alpha)
        ...
        def calerr(Area):
            ...
            if self.maptype==self.maptype:
                for i in range(len(self.map)):
                    p0=ptref/self.map.iloc[i][2]
                    # 声明涡轮前的状态
                    Venv=Volume(1,p0)
                    V.connect(VTur,Venv)
                    
                    Err+=(V.mass_flow_rate()*const-self.map.iloc[i][1])**2
                return Err
            else:
                for i in range(len(self.map)):
                    p0=ptref/self.map.iloc[i][2]
                    # 声明涡轮前的状态
                    Venv=Volume(1,p0)
                    # VTur=Volume(1,1.e5*self.map.iloc[i][2],Tt,alpha)
                    V.connect(VTur,Venv)
                    Err+=(V.mass_flow_rate()-self.map.iloc[i][1])**2
                return Err
        
        # 初始估计值
        from .GasProperty import Rg,k_exhaust_gu
        R1=Rg(alpha)
        k1=k_exhaust_gu(Tt)
        # temp=1.e5*self.map.iloc[0][2]/(R1*T)*pow(2/(k1+1),1/(k1-1))*sqrt(2*k1/(k1+1))
        temp=1./sqrt(R1)*pow(2/(k1+1),1/(k1-1))*sqrt(2*k1/(k1+1))
        areainit=self.map.iloc[0][1]/temp

        from .Algorithm.Optimization import multiopt
        opt=multiopt(calerr,[0],[10*areainit])
        opt.PSO(50,30)

        self.regTable=ArrayTable(2,0)
        V=ValveSimple(opt.best_x)
        for i in linspace(1,self.map.max()[2],50):
            # pt=ptref/i
            p0=ptref/i
            # 声明涡轮前的状态
            Venv=Volume(1,p0)
            # VTur=Volume(1,pt,Tt,alpha)
            V.connect(VTur,Venv)
            self.regTable.append([i,V.mass_flow_rate()*const])

        return opt.best_x


    def regressionWithValveSperate(self,speed,Ttref=800,ptref=2.e5,alpha=1):
        from math import sqrt
        from .Valve import ValveSimple
        from .Cylinder import Volume

        # 流量归一化常数
        constant=sqrt(Ttref)/ptref

        VTur=Volume(1,ptref,Ttref,alpha)

        # Venv=Volume(1,ptref)

        tempmap=self.group.get_group(speed)
        ...
        def calerr(Area):
            ...
            if self.maptype==self.maptype:
                for i in range(len(tempmap)):
                    p0=ptref/tempmap.iloc[i][2]
                    # 声明涡轮前的状态
                    # VTur=Volume(1,pt,Ttref,alpha)
                    Venv=Volume(1,p0)
                    V.connect(VTur,Venv)
                    
                    Err+=(V.mass_flow_rate()*constant-tempmap.iloc[i][1])**2
                return Err
            else:
                for i in range(len(tempmap)):
                    p0=ptref/tempmap.iloc[i][2]
                    # 声明涡轮前的状态
                    # VTur=Volume(1,1.e5*tempmap.iloc[i][2],Ttref,alpha)
                    Venv=Volume(1,p0)
                    V.connect(VTur,Venv)
                    Err+=(V.mass_flow_rate()-tempmap.iloc[i][1])**2
                return Err

        # 初始估计值
        from .GasProperty import Rg,k_exhaust_gu
        R1=Rg(alpha)
        k1=k_exhaust_gu(Ttref)
        # temp=1.e5*self.map.iloc[0][2]/(R1*T)*pow(2/(k1+1),1/(k1-1))*sqrt(2*k1/(k1+1))
        temp=1./sqrt(R1)*pow(2/(k1+1),1/(k1-1))*sqrt(2*k1/(k1+1))
        areainit=tempmap.iloc[0][1]/temp
        ...
        self.regTable=ArrayTable(2,0)
        V=ValveSimple(opt.best_x)
        for i in linspace(1,tempmap.max()[2],20):
            p0=ptref/i
            # VTur=Volume(1,pt,Ttref,alpha)
            Venv=Volume(1,p0)
            V.connect(VTur,Venv)
            self.regTable.append([i,V.mass_flow_rate()*constant])

        return opt.best_x

    def polyfit(self,order=2):
        from .Table import ArrayTable
        temptable=ArrayTable(2,0)
        temptable.fromPandas(self.map.iloc[:,[1,2]])
        temptable.append([0,1])
        temptable.doQuickSort(1)
        temp1,temp2=temptable.polyFit(coly=0,colx=1,_deg=order)
        self.regTable=temp1.selectColumns([0,2])
    ...
    def to_Cmap(self):
        ...
        for each in self.speeds:
            T.fromPandas(self.group.get_group(each))
            map.addspeedmap(T.to_C())

        # map.interpolatemap(2000)
        map.calculate(2000,1.5)
        map.showmap()
...
if __name__ == "__main__":
    ...
    Turb = Turbine("./data/TurbineMap.xlsx")
    Turb.plot(showlegend=True)
    Turb.regressionWithValve()
