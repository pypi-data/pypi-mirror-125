...
def ignitiondelay(p:float, T:float, Cig:float=1) -> float:
    """
    江少军给出的着火延迟公式，见内燃机工作过程仿真技术109页
    Parameters
    ----------
    p:in-cylinder pressure,Pa
    T:in-cylinder Temperature,K
    Cig:adjustment coefficient

    Returns
    -------
    when the integration 1/\tau=1,then the ignition begins
    """
    from math import exp
    return 0.1+1.194e-4*pow(p,-0.87)*exp(1967./T)*Cig
    return 0
...
class CylinderGeometry:
    def __init__(self, EngineType=None, bore=None, stroke=None, connecting_rod_length=None, compression_ratio=None,
        ...
        self.Cobj=ICELib.CylinderGeometry(bore,stroke,compression_ratio,connecting_rod_length)
        self.bore = bore
        self.stroke = stroke
    ...
    def TDCclearanceHeight(self):
        ...
        return self.Cobj.TDCclearanceHeight()
...
class HeatReleaseData(HeatRelease, ABC):
    ...
    def linearPlot(self,plot=True):
        # 参考哈工程博士论文，基于Wiebe方程的柴油机零维燃烧模型校准

        from math import log
        temp=ArrayTable(2,0)
        temp.setTableHeader(["$\ln \left( {\\varphi  - {\\varphi _0}} \\right)$","$\ln \left( { - \ln \left( {1 - {x_b}\left( \\varphi  \\right)} \\right)} \\right)$"])

        # 线性化
        for i in range(1,self.data.row-2):
            G=log(self.data.table[0].data[i]-self.SOC)
            H=log(-log(1-self.data.table[2].data[i]))
            temp.append([G,H])
    ...
    def plot(self):
        ...
        plt.subplot(121)
        plt.plot(self.data.table[0].data, self.data.table[1].data)
        plt.xlabel("Crank angle(°CA)",fontsize=23)
        plt.ylabel("Burn Rate (Normalized by Total Fuel Mass)",fontsize=23)
        plt.tight_layout()
        plt.grid()
        ...
        plt.plot(self.data.table[0].data, self.data.table[2].data)
        plt.xlabel("Crank angle(°CA)",fontsize=23)
        plt.ylabel("Burned Fuel (Fraction of Total Fuel Mass)",fontsize=23)
        plt.tight_layout()
...
class CylinderPressure:
    ...
    def FFTTrasform(self, n, tau=4.,plot=True):
        """
        傅里叶变换分析
        :param n:发动机转速，r/min
        :param tau:发动机冲程
    ...
    def FFTFilter(self, _cutOffFrequency, speed,amplitudeMutiplier=1):
        from numpy import fft
        import numpy as np

        transform=self.FFTTrasform(speed,plot=False)

        dofft = fft.fft(self.data.table[1].data)
        temp = dofft

        # 滤波,1 -1成对置为0
        for i in range(1,transform.row-1):
            if transform.table[0].data[i] > _cutOffFrequency:
                temp[i] = 0
                temp[-i]=0
        temp2 = [each * amplitudeMutiplier for each in temp]
        result = ArrayTable(3, 0)
        result.setTableHeader(["Crank angle","Cylinder pressure after LPF","Original cylinder pressure"])
        result.setTableUnit(["$^\circ CA$","bar","bar"])
        
        result.table[1].data = np.abs(fft.ifft(temp2))
        result.table[0].data = self.data.table[0].data
        result.table[2].data = self.data.table[1].data

        result.row=self.data.row
        result.col=3
        return result
    ...
    def HeatReleaseRate(self,speed,r=0,inj=None,ivc=None,TBeforeValeve=50 + 173.15,plot=True):
        from .dll import ICELib
        ht=ICELib.CylinderHeatTransfer(400,400,400,1)
        if ivc is None:
            ivc = -180 + 50  # 进气门迟后角为30~70

        # from GasProperty import Rg
        mc = self.data.linearInterpolate(ivc, 1) * self.CylGeo.V(ivc) / Rg() / chargeTemperature(TBeforeValeve)

        if inj is None:
            inj=mc/14.3/1.5

        mix=ICELib.DieselMixture(mc,r,inj)

        injob=ICELib.Injection(inj,14.3,42700e3)

        soc=self.startOfCombustion()
        print("Find start of combustion at {}".format(soc))

        result=ICELib.heatReleaseAfterSOC(speed,mix,injob,self.CylGeo.Cobj,self.data.slice(soc,soc+180).to_C(),0,ht)
        if plot:
            result.plot(18,0)
        return result
    ...
    def plot(self, ValveTiming=None):
        ...
        ax.annotate('maxium pressure %.5g bar \nat angle %.4g $^\circ$CA' % (
            self.data.linearInterpolate(maxpreCA, 1), maxpreCA),
                    ...
                    arrowprops=dict(arrowstyle="->"),fontsize=anotfontsize)
        ...
        plt.ylabel("Cylinder pressure(bar)",fontsize=23)
        plt.xlabel("Crank angle",fontsize=23)
        plt.tight_layout()
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.show()
    ...
    def LogP_LogV(self, CylGeo=None, ivc=-180 + 50, evo=180 - 10, phline=None, plot=False):
        ...
        if plot:
            ...
            ax.annotate('SOC %.2f $^\circ$CA' % soc,
                        ...
                        arrowprops=dict(arrowstyle="->"),fontsize=anafontsize)
            ...
            from math import exp
            ax.annotate('intake pressure %.2f bar' % (exp(indexintake[0]) / 1.e5),
            ...
            plt.xlabel("$log V$",fontsize=23)
            plt.ylabel("$log p$",fontsize=23)
...
# 计算能量平衡
class EnergyBalance:
    def __init__(self, bore, stroke, n, ge, Pe, Ts, ps, alpha=2, NOC=1, etav=1.4, piT=2, etaT=0.6, T0=20 + 273.15,
                 Hu=42496, L0=14.3, linkType=1):
        """
        :param bore:缸径,m
        :param stroke:冲程,m
        :param n: 发动机转速,r/min
        ...
        if linkType == 0:
            ...
            self.links = [
                ...
                {"source": self.nodes[4]["name"], "target": self.nodes[8]["name"],
                 "value": round(Qincool / self.Pf, 4)},
                {"source": self.nodes[4]["name"], "target": self.nodes[7]["name"],
                 "value": round((He0 - H0) / self.Pf, 4)},
...
class CylPreReconstruct:
    ...
    def do(self, pim=2.e5, Tim=300, xr=0.1, kc=1.35, phic=0.9, Rp=0.1, alpha=2, SOC=-10, ke=1.37, m=2, etaTK=0.45,
           speed=2100, plot=False):
