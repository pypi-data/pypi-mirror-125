...
def flowUnitArea(p1, T1, R1, k1, p2, T2=0, R2=0, k2=0):
    ...
    return ICELib.flowUnitArea(p1, T1, R1, k1, p2, T2, R2, k2)
    # if p1 < 0 or p2 < 0:
...
class ValveSimple(ValveABC, ABC):
    ...
    def airFlowExample(self, p1, T1, p2=None, T2=None, plot=True):
        ...
        for i in np.arange(0, p1 + step, step):
            table.append([i / p1, flowUnitArea(p1, T1, R1, k1, i)*self.flowArea])
    
        if p2 is not None and T2 is not None:
            k2 = k_Justi(T2)
            step2 = 0.001 * p2
            for i in np.arange(0.4 * p2, p2, step2):
                table.append([p2 / i, flowUnitArea(i, T1, R1, k1, p2, T2, R2, k2)*self.flowArea])
        ...
        if plot is True:
            ...
            print(choke1)
            massflow1 = flowUnitArea(p1, T1, R1, k1, p1 * choke1)*self.flowArea
            plt.scatter(choke1, massflow1)
    
            choke2 = pow(2 / (k2 + 1), k2 / (k2 - 1))
            print(1 / choke2)
            massflow2 = flowUnitArea(p1 * choke2, T1, R1, k1, p2, T2, R2, k2)*self.flowArea
