...
class JellyClient:
  def __init__ (self, workspace=None, project=None, process=None, version='v1', host='ws.web-jelly.com', port=1883):
  ...
  def make_topic (self, method, group='outbox', args=None):
    ...
    topic = "%s/%s/%s/%s/%s/%s/%s%s" % (
      self.version, 'jelly', 'client', self.client_id, 
      group, method, self.get_scope(), rest
    )
    return topic.lower()
