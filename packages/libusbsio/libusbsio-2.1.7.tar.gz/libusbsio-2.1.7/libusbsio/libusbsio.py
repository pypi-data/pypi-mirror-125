...
class LIBUSBSIO(object):
    ...
    def buffprint(buff) -> str:
        '''Internal buffer dump for logging purposes'''
        if not buff:
            return "None"
        elif(buff == buff[:1]*len(buff)):
            return "%s * %d" % (buff[:1], len(buff))
        else:
            return buff

    # I2C Port configuration information.
    class I2C_PORTCONFIG_T(Structure):
        _fields_ = [
    ...
    class I2C_FAST_XFER_T(Structure):
        _fields_ = [
    ...
    class SPI_PORTCONFIG_T(Structure):
        _fields_ = [
    ...
    class HIDAPI_DEVICE_INFO_T(Structure):
        class EX_T(Structure):
            _fields_ = [
                ("is_valid", c_uint16),
        ...
        def __getitem__(self, key):
            keys = [x[0] for x in self._fields_ ] + [ ('ex.'+x[0]) for x in self.ex._fields_ ]
            if key in keys:
    ...
    class I2C(PORT):
        ...
        def FastXfer(self, devAddr:int, txData:bytes=None, txSize:int=0, rxSize:int=0, ignoreNAK:bool=False, nackLastByte:bool=True) -> Tuple[bytes,int]:
            ...
            if(self.logger.isEnabledFor(logging.DEBUG)):
                self.logger.debug("I2C%d status=%d, received: %s" % (self._portNum, ret, rxData))
            return (rxData, ret)
    ...
    class SPI(PORT):
        ...
        def Transfer(self, devSelectPort:int, devSelectPin:int, txData:bytes, size:int=0, options:int=0) -> Tuple[bytes,int]:
            ...
            if(self.logger.isEnabledFor(logging.DEBUG)):
                self.logger.debug("SPI%d status=%d, received: %s" % (self._portNum, ret, rxData))
            return (rxData, ret)
