...
class Client:
    ...
    def get(self, symbol: bytes, data_type: bytes, end_timestamp: bytes):
        mget = message.Get(symbol, data_type, end_timestamp)
        self.send(mget)
        res, raw_res = [], []
        while True:
            mgetr = message.GetR()
        ...
        for raw_data in raw_res:
            data = yaml.safe_load(raw_data.get_field('data').decode("utf-8"))
            res.append(data)
        return res

    def sub(self, symbols: list, data_type: bytes, sub_handler = None):
        for symbol in symbols:
            msub = message.Sub(symbol, data_type)
            self.send(msub)
        ...
        while sub_handler:
            ...
            if error_code == 0:
                data = yaml.safe_load(msubr.get_field('data').decode("utf-8"))
