...
class Word(Seed, Glass):
    ...
    def elf(cls, word):
        ...
        if isinstance(word, Word):
            ...
            raise AngryElf(f'This is a Key, not a word. Idiot.')
...
class Key(Seed, Glass):
    ...
    def elf(cls, key):
        ...
        if isinstance(key, Key):
            ...
            if CLASS_SEPARATOR in key:
                raise AngryElf(f'This looks more like an Atom. Do I look '
                               f'like an Atom elf?')
...
class Atom(Doc, Seed, Glass):
    ...
    def to_dict(self,
        ...
        """
        di = super(Atom, self).to_dict(ignore_private=ignore_private,
                                       recursive=recursive)
        di['zulu'] = self.__zulu
        di['key'] = self.__key
    ...
    def __sub__(self, other):
        if isinstance(other, Folder):
            ...
            if str_self.startswith(str_other):
                ...
                raise FolderError(f'Cannot subtract folder which is not part of other; '
                                  f'This: {str_self};'
                                  f'Other: {str_other}; '
    ...
    def append(self, *args):
        ...
        if len(args) == 1:
            ...
            if isinstance(arg, list):
                ...
                return Folder.join(str(self),
                                   '/'.join([str(x) for x in arg.words()]))
            else:
