...
class ZeebeAdapterBase(object):
    ...
    def _create_channel(connection_uri: str, credentials: BaseCredentials = None,
        ...
        if credentials:
            return grpc.secure_channel(connection_uri,credentials.grpc_credentials, options=[('grpc.max_message_length', maxMsgLength),
                                         ('grpc.max_send_message_length', maxMsgLength),
                                         ('grpc.max_receive_message_length', maxMsgLength)])
        elif secure_connection:
            return grpc.secure_channel(connection_uri, grpc.ssl_channel_credentials(),options=[('grpc.max_message_length', maxMsgLength),
                                         ('grpc.max_send_message_length', maxMsgLength),
                                         ('grpc.max_receive_message_length', maxMsgLength)])
        else:
            return grpc.insecure_channel(connection_uri,options=[('grpc.max_message_length', maxMsgLength),
                                         ('grpc.max_send_message_length', maxMsgLength),
                                         ('grpc.max_receive_message_length', maxMsgLength)])
    ...
    def _check_connectivity(self, value: grpc.ChannelConnectivity) -> None:
        ...
        if value in [grpc.ChannelConnectivity.READY, grpc.ChannelConnectivity.IDLE]:
            ...
            if self._should_retry():
                ...
                time.sleep(10)
                if self._current_connection_retries > self._max_connection_retries/2:
                    time.sleep(20)
                self.retrying_connection = True
    ...
    def _common_zeebe_grpc_errors(self, rpc_error: grpc.RpcError):
        if self.is_error_status(rpc_error, grpc.StatusCode.RESOURCE_EXHAUSTED):
            if not "RESOURCE_EXHAUSTED" in str(rpc_error):
