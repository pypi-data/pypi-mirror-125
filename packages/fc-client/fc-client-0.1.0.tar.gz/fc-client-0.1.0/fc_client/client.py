...
class Client:
    ...
    def lock(args):
        ...
        if resource:
            os.system(f"labgrid-client reserve --wait name={resource}")
            os.system(f"labgrid-client -p {resource} lock")
        else:
                    ...
                    for k, v in reservations.items():  # pylint: disable=invalid-name
                        if k == f"Reservation '{token}'":
                            ...
                            if owner == me:
                                ...
                                os.system(f"labgrid-client cancel-reservation {token}")
                                os.system(f"labgrid-client -p {resource} unlock")
                            else:
