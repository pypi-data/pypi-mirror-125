...
from lightSOM.views.hexagons import generate_hex_lattice
...
class Nodes(object):
    ...
    def get_nodeDistance(self,node1, node2):
        ...
        if self.PBC == True:
            ...
            return np.min( [np.sqrt((node1[0] - node2[0]) * (node1[0] - node2[0]) + (node1[1] - node2[1]) * (node1[1] - node2[1])),
                 # right
                 np.sqrt((node1[0] - node2[0] + width) * (node1[0] - node2[0] + width) + (node1[1] - node2[1]) * (node1[1] - node2[1])),
                 # bottom
                 np.sqrt((node1[0] - node2[0] + offset) * ( node1[0] - node2[0] + offset) + (node1[1] - node2[1] + height / norm_constant) * ( node1[1] - node2[1] + height / norm_constant)),
                 # left
                 np.sqrt((node1[0] - node2[0] - width) * ( node1[0] - node2[0] - width) + (node1[1] - node2[1]) * (node1[1] - node2[1])),
                 # top
                 np.sqrt((node1[0] - node2[0] - offset) * (node1[0] - node2[0] - offset) + (node1[1] - node2[1] - height / norm_constant) * (node1[1] - node2[1] - height / norm_constant)),
                 # bottom right
                 np.sqrt((node1[0] - node2[0] + width + offset) * (node1[0] - node2[0] + width + offset) + (node1[1] - node2[1] + height / norm_constant) * (node1[1] - node2[1] + height / norm_constant)),
                 # bottom left
                 np.sqrt((node1[0] - node2[0] - width + offset) * (node1[0] - node2[0] - width + offset) + (node1[1] - node2[1] + height / norm_constant) * (node1[1] - node2[1] + height / norm_constant)),
                 # top right
                 np.sqrt((node1[0] - node2[0] + width - offset) * (node1[0] - node2[0] + width - offset) + (node1[1] - node2[1] - height / norm_constant) * (node1[1] - node2[1] - height / norm_constant)),
                 # top left
                 np.sqrt((node1[0] - node2[0] - width - offset) * (node1[0] - node2[0] - width - offset) + (node1[1] - node2[1] - height / norm_constant) * (node1[1] - node2[1] - width * 2 / norm_constant))])
