...
class somNet:
    ...
    def __init__(self, netHeight, netWidth, data, loadFile=None, PCI=0, PBC=0, n_jobs=-1):
        ...
        if loadFile==None:
            ...
            self.node_distances=[x[:] for x in [[0] * (self.width*self.height)] * (self.width*self.height)]
            for i, n1 in enumerate(self.nodeList):
                for j, n2 in enumerate(self.nodeList):
                    if i<j:
    ...
    def update_weights(self, train_ex, learn_rate, radius_sq, bmu, step=20):
        g, h = bmu.pos
        lattice_g, lattice_h= bmu.lattice_pos
        ...
        for i in range(int(max(0, g - step)), int(min(self.matrix.shape[0], g + step))):
            for j in range(int(max(0, h - step)), int(min(self.matrix.shape[1], h + step))):
                dist_sq=self.node_distances[i*self.width+j][g*self.width+h]
    ...
    def diff_graph(self, show=False, printout=True, returns=False, path='./'):
        ...
        for i, node in enumerate(self.nodeList):
            nodelist=[]
            for j, nodet in enumerate(self.nodeList):
                if node != nodet and self.node_distances[i][j] <= 1.001:
                    nodelist.append(nodet)
            neighbours.append(nodelist)

        diffs = []
        for node, neighbours in zip(self.nodeList, neighbours):
            diffs.append(np.sum(euclidean_distance(np.array([node.weights]), np.array([nb.weights for nb in neighbours]))))
