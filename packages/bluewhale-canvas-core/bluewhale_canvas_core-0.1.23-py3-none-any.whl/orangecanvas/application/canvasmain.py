...
class CanvasMainWindow(QMainWindow):
    ...
    def check_requires(self, fileobj: IO) -> bool:
        ...
        if requires:
            ...
            mb = QMessageBox(
                ...
                windowTitle=_("msg.install"),
                text=_("msg.install_text") + details + _("msg.install_message"),
                standardButtons=QMessageBox.Ok | QMessageBox.Abort |
                                QMessageBox.Ignore,
                informativeText=(_("msg.isntall_informativeText")),
            )
            mb.setDefaultButton(QMessageBox.Ok)
            bok = mb.button(QMessageBox.Ok)
            bok.setText(_("tip.install_addons"))
            bignore = mb.button(QMessageBox.Ignore)
            bignore.setText(_("tip.bignore"))
            bignore.setToolTip(_("tip.bignore_tip"))
            mb.setWindowModality(Qt.WindowModal)
    ...
    def install_requirements(self, requires: Sequence[str]) -> int:
        dlg = addons.AddonManagerDialog(
            parent=self, windowTitle=_("msg.install_packages"),
    ...
    def ask_load_swp(self):
        ...
        name = _("noun.canvas_core") or "Orange"
    ...
    def __handle_help_query_response(self, res: Optional[QUrl]):
        if res is None:
            mb = QMessageBox(
                text=self.tr(_("tip.found")),
                windowTitle=self.tr(_("tip.found_title")),
...
def scheme_requires(
    ...
    names = []
    data = unique(m.project_name for m in desc.nodes if m.project_name)
    for name in data:
        if name.startswith('Orange3'):
            names.append(name.replace('Orange3', 'BlueWhale3'))
        else:
            names.append(name)
    return names
