...
class PipelineRunOrchestrator:
    ...
    def __init__(self, workspace, input_dataset, pipeline_node_dict, component_execution_dict,
        ...
        :param pipeline_node_dict: dict[str, Component]
    ...
    @classmethod
    def generate_orchestrator_by_pipeline_component(cls, pipeline_component, pipeline_parameters=None,
        ...
        # Get dataset in the pipeline component to prepare for download.
        pipeline_input_dataset = \
            [item._dset for item in pipeline_component._input_ports.values()] + \
            list(pipeline_component._definition.get_dataset_in_registered_pipeline().values())

        return PipelineRunOrchestrator(workspace=pipeline_component.workspace, input_dataset=pipeline_input_dataset,
    ...
    def generate_orchestrator_by_pipeline(cls, pipeline, input_dataset, visualization_context=None,
        ...
        pipeline_node_dict = {node._id: node for node in pipeline.nodes}
        ...
        return PipelineRunOrchestrator(workspace=pipeline.workspace, input_dataset=input_dataset,
                                       pipeline_parameters={**pipeline._init_params, **pipeline_parameters},
                                       max_workers=max_workers, component_executor=component_executor,
                                       component_execution_dict=component_execution_dict,
                                       pipeline_node_dict=pipeline_node_dict,
    ...
    def _execute_nodes(self, execution_nodes, pipeline_run_info, execution_file):
        ...
        for node_id, node in execution_nodes.items():
            ...
            if node.type != ComponentType.PipelineComponent.value:
                ...
                run_future = self.pipeline_executor.submit(self._execute_sub_pipeline, node, node_working_dir,
                                                           child_tracker, pipeline_run_info)
            else:
                # Execute pipeline component in pipeline_executor.
                run_future = self.pipeline_executor.submit(self._execute_pipeline_component_node, node,
                                                           node_working_dir, child_tracker, pipeline_run_info)
    ...
    def _execute_pipeline_component_node(self, node, working_dir, tracker, pipeline_run_info):
        """
    ...
    def _execute_sub_pipeline(self, sub_pipeline, working_dir, child_tracker, pipeline_run_info):
        """
        ...
        try:
            orchestrator = PipelineRunOrchestrator.generate_orchestrator_by_pipeline(
                ...
                self.pipeline_parameters, component_executor=self.component_executor,
                component_execution_dict=self.component_execution_dict,
