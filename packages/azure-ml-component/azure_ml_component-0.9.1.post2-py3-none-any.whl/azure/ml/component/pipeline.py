...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def _ensure_registered_in_workspace(self, workspace):
        ...
        if unregister_component_definition:
            ...
            for node in nodes:
                pipeline_parameters = _build_pipeline_parameter(
                    node._parent._is_sub_pipeline, None, node._parent._init_params)
                ...
                if node._definition.type == ComponentType.ScopeComponent:
                    name_port_mapping = node._module_dto.module_python_interface.name_mapping
