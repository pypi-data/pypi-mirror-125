# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class AssetVersionPublishRequest(Model):
    """AssetVersionPublishRequest.

    :param asset_type: Possible values include: 'Component', 'Model',
     'Environment', 'Dataset'
    :type asset_type: str or ~designer.models.AssetType
    :param asset_source_type: Possible values include: 'Unknown', 'Local',
     'GithubFile', 'GithubFolder', 'DevopsArtifactsZip'
    :type asset_source_type: str or ~designer.models.AssetSourceType
    :param yaml_file:
    :type yaml_file: str
    :param source_zip_url:
    :type source_zip_url: str
    :param feed_name:
    :type feed_name: str
    :param set_as_default_version:
    :type set_as_default_version: bool
    :param referenced_assets:
    :type referenced_assets:
     list[~designer.models.AssetNameAndVersionIdentifier]
    """

    _attribute_map = {
        'asset_type': {'key': 'assetType', 'type': 'str'},
        'asset_source_type': {'key': 'assetSourceType', 'type': 'str'},
        'yaml_file': {'key': 'yamlFile', 'type': 'str'},
        'source_zip_url': {'key': 'sourceZipUrl', 'type': 'str'},
        'feed_name': {'key': 'feedName', 'type': 'str'},
        'set_as_default_version': {'key': 'setAsDefaultVersion', 'type': 'bool'},
        'referenced_assets': {'key': 'referencedAssets', 'type': '[AssetNameAndVersionIdentifier]'},
    }

    def __init__(self, *, asset_type=None, asset_source_type=None, yaml_file: str=None, source_zip_url: str=None, feed_name: str=None, set_as_default_version: bool=None, referenced_assets=None, **kwargs) -> None:
        super(AssetVersionPublishRequest, self).__init__(**kwargs)
        self.asset_type = asset_type
        self.asset_source_type = asset_source_type
        self.yaml_file = yaml_file
        self.source_zip_url = source_zip_url
        self.feed_name = feed_name
        self.set_as_default_version = set_as_default_version
        self.referenced_assets = referenced_assets
