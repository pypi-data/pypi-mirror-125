...
class InferenceV2(object):
    def __init__(self, dataset, model, patch_size, n_classes, mode = 'npy and nifti', step_size = [0.5, 0.5, 0.5], postprocessing = None, device = None,
    ...
    def save(self, image, path, index, mode = 'seg'):
        if 'npy' in self.mode:
            np.save(os.path.join(path, mode + '{0:03}.npy'.format(index)),image.numpy())
        if 'nifti' in self.mode:
                          ㅐ
    ...
    def valid_run(self):
        ...
        for data_index in range(len(self.dataset)):
            ...
            if self.save_path != None:
                ...
                self.save(image, self.image_path, data_index, mode = 'image')
