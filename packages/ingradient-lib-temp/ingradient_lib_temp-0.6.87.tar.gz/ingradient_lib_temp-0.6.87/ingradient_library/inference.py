...
class InferenceV2(object):
    ...
    def make_dir(self, save_path):
        self.save_path = os.path.join(save_path, r'' + str(datetime.datetime.now()))
        self.model_path = os.path.join(self.save_path, r'model')
        self.postprocessing_path = os.path.join(self.save_path, r'postprocessing')
    ...
    def save(self, image, path, index, mode = 'label'):
        if self.mode == 'npy':
            np.save(os.path.join(path, mode + '{0:03}.npy'.format(index), image))
    ...
    def valid_run(self):
        ...
        for data_index in range(len(self.dataset)):
            ...
            if self.save_path != None:
                self.save(result, self.model_path, data_index)
                self.save(image, image, data_index)
            if self.postprocessing != None:
                result = self.postprocessing(result.numpy()) 
                if self.save_path != None:
                    self.save(result, self.postprocessing_path, data_index)
