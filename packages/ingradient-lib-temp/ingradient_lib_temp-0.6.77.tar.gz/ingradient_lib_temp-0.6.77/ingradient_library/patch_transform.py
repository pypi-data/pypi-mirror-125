...
class Batch_Affine_3D(object):
    ...
    def pad_image_seg(self, images, patch_size): #patch size보다 전체 이미지 크기가 작을 경우 padding을 진
        image_shape = np.array((images.shape[-3], images.shape[-2], images.shape[-1]))
        patch_size = np.array(patch_size)
        if np.any(image_shape <patch_size):
            odd_pad = ((patch_size - image_shape) % 2 != 0).astype(int)
            even_pad = np.repeat(np.clip(((patch_size - image_shape)/2).astype(int), 0, np.inf), 2)
        ...
        print(image_shape, patch_size, images.shape)
        return images
    ...
    def set_random_scale_degree(self):
        degree = np.random.uniform(3) < 0.5
        if self.is_random:
...
class Batch_Low_Reesolution(object):
    ...
    def __calL__(self, images):
        ...
        for i in range(modality_size):
            if do_per_modality[i] > self.prob_per_modality:
                continue
            images[i] = F.interpolate(F.interpolate(images[i], scale_factor = 0.5, mode ='trilinear'), scale_factor = 2.0, mode ='trilinear')
