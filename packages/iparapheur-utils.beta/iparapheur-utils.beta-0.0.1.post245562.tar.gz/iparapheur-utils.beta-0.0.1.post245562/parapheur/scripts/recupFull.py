...
# On vérifie que les meta existent
...
if len(metas_list) != len(metas_default_values):
    ...
    exit(0)
## On vérifie que les métas de metas_tree sont bien dans metas_list
# print(metas_tree)
# valueToBeRemoved = ["PH_type","PH_soustype"]
# for valueRemoved in valueToBeRemoved:
#     print(valueRemoved)
#     metas_tree_to_check = [value for value in metas_tree if value != valueRemoved]
# print(metas_tree_to_check)
# check = all(item in metas_tree_to_check for item in metas_list)
#
# if check is False:
#     print("Erreur : Les métadonnées de metas_tree doivent être définies dans metas_list")
#     print("Metas_tree = " + str(metas_tree))
#     print("Metas_list = " + metas_list)
#     exit(0)
path_list = []
...
def get_full_folder_path_with_meta():
    ...
    for meta in metas_tree:
        if meta.startswith("___REMPLACE___"):
            ...
            try:
                ...
                # meta_replace = meta_replace.decode().encode('utf-8')
                meta_replace = cleanup_special_chars(meta_replace)
            print(meta_replace)
    ...
    full_path = full_path + "/"
    full_path = full_path + title_clean.encode('utf-8')
...
def generate_xml_archive():
    ...
    # created = etree.SubElement(archive, "created")
    # # created.attrib["valeur"] = str(dossier["created"])
    # created.attrib["valeur"] = datetime.datetime.fromtimestamp(dossier["created"]/1000).isoformat()
    ...
    for meta in liste_metas:
        ...
        # else:
        # try:
        #     x = cleanup_special_chars(str(x))
        # except:
        #     x = cleanup_special_chars(x)
        try:
            MetaDonnee.attrib["valeur"] = str(x)
        except:
            MetaDonnee.attrib["valeur"] = x.encode('ascii', 'ignore').decode('ascii')
        i += 1
    circuit = etree.SubElement(archive, "circuit")
    etape = etree.SubElement(circuit, "etape")
    # etape.attrib["horodatage"] = datetime.datetime.fromtimestamp(dossier["created"]/1000, tzinfo=timezone.utc).isoformat()
    etape.attrib["horodatage"] = str(dossier["created"])
...
if client.islogged:
    ...
    # print(dossiers_archive)
    # print(type(dossiers_archive))
    # exit(0)
    # Waiting delay
