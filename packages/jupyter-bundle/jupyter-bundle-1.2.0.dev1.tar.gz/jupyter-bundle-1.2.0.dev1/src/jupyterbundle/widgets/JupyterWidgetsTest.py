...
from jupyterbundle.widgets.JupyterWidgets import JupyterWidgets


class JupyterWidgetsMocked(JupyterWidgets):

    values: dict = {}

    @staticmethod
    def set_values(values):
        JupyterWidgetsMocked.values = values

    def get_value(self, name: str):
        widget = self._get_widget(name)

        if name in JupyterWidgetsMocked.values:
            value = JupyterWidgetsMocked.values[name]
            try:
                widget.value = value
            except TraitError as e:
                if str(e) == "Invalid selection: value not found":
                    if isinstance(value, list):
                        invalid_choice = (set(value) - set(widget.options)).pop()
                    else:
                        invalid_choice = value

                    choices_str = "', '".join(widget.options)
                    raise Exception(f"argument --{name}: invalid choice: '{invalid_choice}' (choose from '" + choices_str + "')")

        return super().get_value(name)


def set_input(**kwargs):
    JupyterWidgetsMocked.set_values(dict(kwargs.items()))


class JupyterWidgetsTest(unittest.TestCase):
    def setUp(self):
        self.__widgets = JupyterWidgetsMocked()
