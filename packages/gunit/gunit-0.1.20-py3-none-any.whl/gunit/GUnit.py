    ...
    def junit(self):
        ...
        for i, testcase in enumerate(cases[1:]):
            ...
            if testtype == "0" or testtype == "2":
                ...
                function = re.search(r"in (?!gunit_mono_hook|gunit_array|gunit_range_hook)(.*) \(.*?\) at", caseparts[1]).group(1)
            ...
            if success != "1":
                if fail_message == "":
                    # Only meant for 32-bit machines
                    expected_hex = f'0x{int(expected) & 0xFFFFFFFF:08X}'
                    result_hex = f'0x{int(result) & 0xFFFFFFFF:08X}'
                    if testtype == "2":
                        if int(expected) > int(result):
                            fail_message = f"Expected more than {expected} ({expected_hex}), received {result} ({result_hex})"

                        else:
                            fail_message = f"Expected less than {expected} ({expected_hex}), received {result} ({result_hex})"

                    elif expected != result:
                        fail_message = f"Expected {expected} ({expected_hex}), received {result} ({result_hex})"

                    else:
                        fail_message = f"Did not expect {expected} ({expected_hex})"
