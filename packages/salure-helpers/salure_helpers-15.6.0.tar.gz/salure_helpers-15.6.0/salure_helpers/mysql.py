...
class MySQL:
    ...
    def insert(self, table: str, dataframe: pd.DataFrame, ignore_duplicates=False, on_duplicate_key_update_columns: list=None):
        start = time.time()
        connection = pymysql.connect(host=self.host, user=self.user, password=self.password, database=self.database, port=self.port)
        cursor = connection.cursor()
        tableHeaders = ', '.join(list(dataframe))
        # Replace NA datatypes with None, which can be understood by the db as null/default
        dataframe = dataframe.where(pd.notnull(dataframe), None).copy()
        ...
        if ignore_duplicates:
            query = f"""INSERT IGNORE INTO {table} ({tableHeaders}) VALUES {values}"""
            ...
            print(on_duplicate_key_update_columns)
            query = f"""INSERT INTO {table} ({tableHeaders}) VALUES {values} ON DUPLICATE KEY UPDATE {on_duplicate_key_update_columns}"""
            print(query)
        else:
            query = f"""INSERT INTO {table} ({tableHeaders}) VALUES {values}"""
        cursor.execute(query)
        connection.commit()
        connection.close()
