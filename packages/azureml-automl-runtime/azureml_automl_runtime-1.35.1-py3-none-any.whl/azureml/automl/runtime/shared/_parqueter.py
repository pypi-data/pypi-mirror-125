...
class Parqueter:
    ...
    def load_pandas_dataframe(path: str) -> pd.DataFrame:
        ...
        try:
            ...
            ondisk_size_kb = os.path.getsize(path) / 1000.0
            logger.info(
                f"parquet to ndarray. Memory: {inmemory_size_kb} kb,"
                f"Disk: {ondisk_size_kb} kb"
            )
            return df
        except MemoryError as me:
            raise ResourceException._with_error(
    ...
    def load_numpy_array(path: str) -> np.ndarray:
        ...
        inmemory_size_kb = array.nbytes / 1000.0
        ondisk_size_kb = os.path.getsize(path) / 1000.0
        logger.info(
            f"parquet to ndarray. Memory: {inmemory_size_kb} kb,"
            f"Disk: {ondisk_size_kb} kb"
    ...
    def load_single_dim_numpy_array(path: str) -> np.ndarray:
        ...
        inmemory_size_kb = single_dim_array.nbytes / 1000.0
        ondisk_size_kb = os.path.getsize(path) / 1000.0
        logger.info(
            f"parquet to ndarray. Memory: {inmemory_size_kb} kb,"
            f"Disk: {ondisk_size_kb} kb"
    ...
    def load_sparse_matrix(path: str) -> sparse.spmatrix:
        ...
        ondisk_size_kb = os.path.getsize(path) / 1000.0
        logger.info(
            f"parquet to spmatrix. Memory: {inmemory_size_kb} kb,"
            f"Disk: {ondisk_size_kb} kb"
        )
        return csr
