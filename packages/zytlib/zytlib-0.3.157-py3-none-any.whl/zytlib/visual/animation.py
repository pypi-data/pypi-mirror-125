...
class TimeStamp(animation_content):
    ...
    def init(self):
        ...
        self.ax.legend()
        self.dots = self.ax.scatter(vector.zeros(self.N), self.curses.map(lambda x: x["content"][0]), color=self.curses.map(lambda x: x["color"]))

    def update_frame(self, frame):
        self.dots.set_offsets(vector.zip(vector.constant_vector(frame, self.N), self.curses.map(lambda x: x["content"][frame])))
