...
def TimeStamp(animation_content):
    ...
    def init(self):
        ...
        for index, curve in enumerate(self.curves):
            self.ax.plot(range(self.max_frame), curve["content"], color=curve["color"], linewidth=curve["linewidth"], label=None)
        self.ax.legend()
        self.dots = self.ax.scatter(vector.zeros(N), self.curses.map(lambda x: x["content"][0]), color=self.curses.map(lambda x: x["color"]))

    def update_frame(self, frame):
        self.dots.set_offsets(vector.zip(vector.constant_vector(frame, N), self.curses.map(lambda x: x["content"][frame])))
