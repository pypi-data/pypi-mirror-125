...
class TimeStamp(animation_content):
    ...
    def init(self):
        ...
        if self.vertical_line:
            self.vl = self.ax.plot([0, 0], [0, self.curves.map(lambda x: x["content"][0]).max()], linewidth=0.3, ls="--")
    ...
    def update(self, frame):
        ...
        if self.vertical_line:
            self.vl.set_data([0, 0], [0, self.curves.map(lambda x: x["content"][frame]).max()])
