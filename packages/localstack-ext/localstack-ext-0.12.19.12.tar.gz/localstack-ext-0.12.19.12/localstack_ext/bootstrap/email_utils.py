...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SjhWT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SjhWR(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SjhWs,from_name=SjhWs,smtp_host=SjhWs,smtp_user=SjhWs,smtp_pass=SjhWs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SjhWN(recipients,SjhWf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SjhWs):
 ...
 result["From"]=formataddr((SjhWR(Header(from_name,"utf-8")),from_email))
 return result
