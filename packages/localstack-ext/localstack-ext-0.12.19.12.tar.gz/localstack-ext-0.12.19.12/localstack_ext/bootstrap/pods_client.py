...
class PodInfo:
 def __init__(self,name=gNaPM,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(gNaPl):
 BACKEND="_none_"
 def __init__(self,pod_name=gNaPM,config=gNaPM):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise gNaPj("Not implemented")
 def push(self,comment:gNaPB=gNaPM)->PodInfo:
  raise gNaPj("Not implemented")
 def pull(self):
  raise gNaPj("Not implemented")
 def commit(self,message:gNaPB=gNaPM):
  raise gNaPj("Not implemented")
 def inject(self,version:gNaPz,reset_state:gNaPq):
  raise gNaPj("Not implemented")
 def list_versions(self)->List[gNaPB]:
  raise gNaPj("Not implemented")
 def version_info(self,version:gNaPz):
  raise gNaPj("Not implemented")
 def set_version(self,version:gNaPz,inject_version_state:gNaPq,reset_state:gNaPq,commit_before:gNaPq):
  raise gNaPj("Not implemented")
 def list_version_commits(self,version:gNaPz)->List[gNaPB]:
  raise gNaPj("Not implemented")
 def get_commit_diff(self,version:gNaPz,commit:gNaPz)->gNaPB:
  pass
 ...
 def get(cls,pod_name,pre_config=gNaPM):
  ...
  raise gNaPj('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=gNaPC
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=gNaPW)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=gNaPW
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise gNaPj("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @gNaPQ
 def get_state_zip_from_instance(get_content=gNaPC):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise gNaPj("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:gNaPB=gNaPM):
...
class CloudPodManagerCPVCS(CloudPodManager):
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=gNaPM)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def init(self):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def inject(self,version:gNaPz,reset_state:gNaPq):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=gNaPW)
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:gNaPB=gNaPM)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=gNaPW)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(gNaPs(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=gNaPs(zip_data_content)
  return result
 def pull(self):
  ...
  rm_rf(zip_path)
 @gNaPQ
 def presigned_url(pod_name:gNaPB,mode:gNaPB)->gNaPB:
  ...
  if response.status_code>=400:
   raise gNaPj("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  if res.status_code>=400:
   raise gNaPj("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:gNaPB=gNaPM):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=gNaPW)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise gNaPj('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=gNaPm(self,"clone_dir",gNaPM)
...
class PodConfigManagerMeta(gNaPh):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=gNaPM
   for manager in cls.CHAIN:
    try:
     tmp=gNaPm(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif gNaPK(tmp,gNaPo)and gNaPK(result,gNaPo):
       result.extend(tmp)
    except gNaPj:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not gNaPM:
    return result
   raise gNaPj('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(gNaPl,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @gNaPS
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise gNaPj('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(gNaPl):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=gNaPW).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=gNaPf([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=gNaPC):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except gNaPj:
   if safe:
    return{}
   raise gNaPj('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(gNaPl):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise gNaPj("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise gNaPj("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def commit_state(pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB],message:gNaPB=gNaPM,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:gNaPB,version:gNaPz,reset_state:gNaPq,pre_config:Dict[gNaPB,gNaPB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB],**kwargs)->List[gNaPB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:gNaPz,pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def set_version(version:gNaPz,inject_version_state:gNaPq,reset_state:gNaPq,commit_before:gNaPq,pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB],**kwargs)->gNaPq:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:gNaPz,pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB])->List[gNaPB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:gNaPz,commit:gNaPz,pod_name:gNaPB,pre_config:Dict[gNaPB,gNaPB])->gNaPB:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=gNaPM,squash_commits=gNaPC,comment=gNaPM,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,**kwargs):
 pre_config=kwargs.get("pre_config",gNaPM)
 if not pod_name:
  raise gNaPj("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state(reset_data_dir=gNaPC):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise gNaPj("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->gNaPB:
 try:
...
def get_persisted_resource_names(data_dir)->List[gNaPB]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and gNaPs(os.listdir(entry.path))>0:
    names.append(entry.name)
