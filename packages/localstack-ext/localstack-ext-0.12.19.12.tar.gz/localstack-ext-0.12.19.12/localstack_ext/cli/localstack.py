...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return yhqiF if e else yhqiH
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->yhqif:
...
@click.option("--revert",is_flag=yhqiF,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:yhqiA):
...
def _cpvcs_initialized(pod_name:yhqit)->yhqiA:
...
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_init(name:yhqit):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_commit(message:yhqit,name:yhqit):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--squash",is_flag=yhqiF,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_push(squash:yhqiA,message:yhqit,name:yhqit):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--version",default="-1",type=yhqiT,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=yhqiF,default=yhqiH,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_inject(version:yhqiT,reset:yhqiA,name:yhqit):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_versions(name:yhqit):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=yhqiF,type=yhqiT)
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_version_info(version:yhqiT,name:yhqit):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=yhqiF,type=yhqiT,help="The version the state should be set to")
@click.option("--inject",is_flag=yhqiF,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=yhqiF,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=yhqiF,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_set_version(version:yhqiT,inject:yhqiA,reset:yhqiA,commit_before:yhqiA,name:yhqit):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_commits(version:yhqiT,name:yhqit):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=yhqiF)
@click.option("--commit","-c",required=yhqiF)
@click.option("-n","--name",help="Name of the cloud pod",required=yhqiF)
def cmd_cpvcs_commit_diff(version:yhqiT,commit:yhqiT,name:yhqit):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(yhqif)
  status.stop()
...
def print_table(columns:Dict[yhqit,yhqit],rows:List[Dict[yhqit,Any]],formatters:Dict[yhqit,Callable[[Any],yhqit]]=yhqif):
 from rich.table import Table
 if formatters is yhqif:
  formatters=yhqiU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  yhqie(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->yhqif:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->yhqif:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->yhqif:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->yhqif:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
