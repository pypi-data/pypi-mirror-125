...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=Rqcpx(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not RqcpA
   return result
  except Rqcpb:
   props=self.props
   ...
   if parts[0]in candidates and RqcpE(parts)>1:
