...
def canonicalize_name(name:fbzmF)->fbzmF:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[fbzmW])[0]
 def get_physical_resource_id(self,attribute=fbzmW,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return fbzmY(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @fbzmP
