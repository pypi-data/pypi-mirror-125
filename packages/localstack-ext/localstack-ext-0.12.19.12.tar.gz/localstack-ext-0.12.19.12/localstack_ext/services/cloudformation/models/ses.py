...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[PRubU])[0]
 def get_physical_resource_id(self,attribute=PRubU,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if PRubp(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return PRubA(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @PRubF
 def add_defaults(resource,stack_name:PRubN):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return PRubA(SESReceiptRule,self).get_cfn_attribute(attribute)
 @PRubh
