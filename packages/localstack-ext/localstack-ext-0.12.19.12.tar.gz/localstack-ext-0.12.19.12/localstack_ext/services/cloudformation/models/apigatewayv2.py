...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[PCpam])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[PCpam])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[PCpam])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2IntegrationResponse(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[PCpam])[0]
 def get_physical_resource_id(self,attribute=PCpam,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return PCpaN(ApiGatewayV2IntegrationResponse,self).get_cfn_attribute(attribute_name)
 @PCpaW
 def get_deploy_templates():
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @PCpau
 def fetch_details(cls,api_id,PCpaz,method,uri=PCpam):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return PCpam
  integrations=[r for r in resp["Items"]if PCpaz==r.get("IntegrationType")and uri in[PCpam,r.get("IntegrationUri")]and method in[PCpam,r.get("IntegrationMethod")]]
  return(integrations or[PCpam])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or PCpam
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
