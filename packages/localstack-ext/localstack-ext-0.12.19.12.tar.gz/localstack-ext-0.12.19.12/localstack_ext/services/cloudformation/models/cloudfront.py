...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or FSvbL
 @FSvbQ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FSvbc(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @FSvba
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if FSvbt(origins,FSvbV)else origins.get("Items")
  origins_to_create=FSvbl([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if FSvbt(aliases,FSvbV)else aliases.get("Items")
  aliases_to_create=FSvbl(aliases)
  for distr in distrs:
   origins=FSvbl([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=FSvbl(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",FSvbH)
   config["Enabled"]=FSvbz(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if FSvbt(custom_errors,FSvbj)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=FSvbz(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":FSvbH}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=FSvbz(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=FSvbL):
    parent=config if parent is FSvbL else parent
    items=parent.get(key)or[]
    if FSvbt(items,FSvbj)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":FSvbT(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if FSvbt(cache_behavs,FSvbj)else cache_behavs)
