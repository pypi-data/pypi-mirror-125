...
SENT_EMAILS=[]
EMAIL_BLACKLIST=gkcXM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",gkcXF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=gkcXS,from_name=gkcXS,smtp_host=gkcXS,smtp_user=gkcXS,smtp_pass=gkcXS,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if gkcXY(recipients,gkcXd)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=gkcXS):
 ...
 result["From"]=formataddr((gkcXF(Header(from_name,"utf-8")),from_email))
 return result
