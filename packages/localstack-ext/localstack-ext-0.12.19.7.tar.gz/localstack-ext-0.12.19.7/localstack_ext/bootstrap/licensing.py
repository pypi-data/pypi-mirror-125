...
TEST_AUTH_HEADERS=zRfWC
class KeyActivationError(zRfWN):
 def __init__(self,message:zRfWc=zRfWC):
  self.message=message
class CachedKeyError(KeyActivationError):
 pass
class InvalidKeyError(KeyActivationError):
 pass
def read_api_key(raise_if_missing=zRfWp):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise zRfWN("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:zRfWc):
 return '"%s..."(%s)'%(api_key[:3],zRfWk(api_key))
def fetch_key():
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=zRfWx)
  ...
  if log_license_issues():
   api_key=zRfWc(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=zRfWc(zRfWU(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in zRfWn(zRfWk(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs["cached_key"]={"timestamp":zRfWU(timestamp),"key_hash":md5(api_key),"key":key_b64}
...
def load_cached_key(api_key):
 ...
 timestamp=zRfWc(cached_key["timestamp"])
...
def cleanup_environment():
 if not localstack_config.is_in_docker:
  return
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise zRfWN("Unable to fetch and validate API key from environment")
   if key!="test":
    decrypt_files(key)
...
def api_key_configured():
 return read_api_key(raise_if_missing=zRfWx)
def is_logged_in():
 configs=localstack_config.load_config_file()
...
def get_auth_headers():
 ...
 api_key=read_api_key(raise_if_missing=zRfWx)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise zRfWN("Please log in first")
