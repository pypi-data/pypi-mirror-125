...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or eQUcS
 @eQUcl
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return eQUcz(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @eQUcR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if eQUci(origins,eQUcN)else origins.get("Items")
  origins_to_create=eQUcI([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if eQUci(aliases,eQUcN)else aliases.get("Items")
  aliases_to_create=eQUcI(aliases)
  for distr in distrs:
   origins=eQUcI([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=eQUcI(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",eQUcF)
   config["Enabled"]=eQUcM(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if eQUci(custom_errors,eQUcf)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=eQUcM(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":eQUcF}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=eQUcM(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=eQUcS):
    parent=config if parent is eQUcS else parent
    items=parent.get(key)or[]
    if eQUci(items,eQUcf)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":eQUcX(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if eQUci(cache_behavs,eQUcf)else cache_behavs)
