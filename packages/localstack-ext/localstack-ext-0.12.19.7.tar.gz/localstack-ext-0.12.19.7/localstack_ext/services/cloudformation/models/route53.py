...
def canonicalize_name(name:vHUzF)->vHUzF:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[vHUza])[0]
 def get_physical_resource_id(self,attribute=vHUza,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return vHUzd(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @vHUzw
