...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[vdulW])[0]
 def get_physical_resource_id(self,attribute=vdulW,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if vdulw(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return vduly(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @vdulq
 def add_defaults(resource,stack_name:vduln):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return vduly(SESReceiptRule,self).get_cfn_attribute(attribute)
 @vdulB
