...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[rVYNT])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[rVYNT])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[rVYNT])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2IntegrationResponse(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[rVYNT])[0]
 def get_physical_resource_id(self,attribute=rVYNT,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return rVYNL(ApiGatewayV2IntegrationResponse,self).get_cfn_attribute(attribute_name)
 @rVYNc
 def get_deploy_templates():
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @rVYNz
 def fetch_details(cls,api_id,rVYNF,method,uri=rVYNT):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return rVYNT
  integrations=[r for r in resp["Items"]if rVYNF==r.get("IntegrationType")and uri in[rVYNT,r.get("IntegrationUri")]and method in[rVYNT,r.get("IntegrationMethod")]]
  return(integrations or[rVYNT])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or rVYNT
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
