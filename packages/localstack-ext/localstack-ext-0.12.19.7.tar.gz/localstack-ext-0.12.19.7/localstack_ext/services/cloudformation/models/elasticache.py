...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=jpHst(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not jpHsO
   return result
  except jpHsq:
   props=self.props
   ...
   if parts[0]in candidates and jpHsU(parts)>1:
