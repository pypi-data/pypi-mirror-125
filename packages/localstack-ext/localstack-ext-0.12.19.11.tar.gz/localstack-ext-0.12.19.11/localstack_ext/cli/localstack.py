...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return giPxA if e else giPxv
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->giPxV:
...
@click.option("--revert",is_flag=giPxA,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:giPxG):
...
def _cpvcs_initialized(pod_name:giPxr)->giPxG:
...
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_init(name:giPxr):
...
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_commit(message:giPxr,name:giPxr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--squash",is_flag=giPxA,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_push(squash:giPxG,message:giPxr,name:giPxr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--version",default="-1",type=giPxJ,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=giPxA,default=giPxv,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_inject(version:giPxJ,reset:giPxG,name:giPxr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_versions(name:giPxr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=giPxA,type=giPxJ)
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_version_info(version:giPxJ,name:giPxr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=giPxA,type=giPxJ,help="The version the state should be set to")
@click.option("--inject",is_flag=giPxA,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=giPxA,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=giPxA,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_set_version(version:giPxJ,inject:giPxG,reset:giPxG,commit_before:giPxG,name:giPxr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_commits(version:giPxJ,name:giPxr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=giPxA)
@click.option("--commit","-c",required=giPxA)
@click.option("-n","--name",help="Name of the cloud pod",required=giPxA)
def cmd_cpvcs_commit_diff(version:giPxJ,commit:giPxJ,name:giPxr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(giPxV)
  status.stop()
...
def print_table(columns:Dict[giPxr,giPxr],rows:List[Dict[giPxr,Any]],formatters:Dict[giPxr,Callable[[Any],giPxr]]=giPxV):
 from rich.table import Table
 if formatters is giPxV:
  formatters=giPxT()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  giPxf(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->giPxV:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->giPxV:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->giPxV:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->giPxV:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
