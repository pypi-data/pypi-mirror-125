...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[KIUpu])[0]
 def get_physical_resource_id(self,attribute=KIUpu,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if KIUpP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return KIUpG(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @KIUpd
 def add_defaults(resource,stack_name:KIUpR):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return KIUpG(SESReceiptRule,self).get_cfn_attribute(attribute)
 @KIUpO
