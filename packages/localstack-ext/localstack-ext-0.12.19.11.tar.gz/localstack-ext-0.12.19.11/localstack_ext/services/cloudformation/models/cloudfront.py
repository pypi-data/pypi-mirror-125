...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or LsFBS
 @LsFBm
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return LsFBb(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @LsFBO
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if LsFBq(origins,LsFBu)else origins.get("Items")
  origins_to_create=LsFBA([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if LsFBq(aliases,LsFBu)else aliases.get("Items")
  aliases_to_create=LsFBA(aliases)
  for distr in distrs:
   origins=LsFBA([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=LsFBA(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",LsFBQ)
   config["Enabled"]=LsFBV(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if LsFBq(custom_errors,LsFBk)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=LsFBV(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":LsFBQ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=LsFBV(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=LsFBS):
    parent=config if parent is LsFBS else parent
    items=parent.get(key)or[]
    if LsFBq(items,LsFBk)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":LsFBY(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if LsFBq(cache_behavs,LsFBk)else cache_behavs)
