...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=WLlGv(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not WLlGD
   return result
  except WLlGX:
   props=self.props
   ...
   if parts[0]in candidates and WLlGN(parts)>1:
