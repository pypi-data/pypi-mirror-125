...
SENT_EMAILS=[]
EMAIL_BLACKLIST=BAgyl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",BAgyO(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=BAgyj,from_name=BAgyj,smtp_host=BAgyj,smtp_user=BAgyj,smtp_pass=BAgyj,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if BAgyU(recipients,BAgyR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=BAgyj):
 ...
 result["From"]=formataddr((BAgyO(Header(from_name,"utf-8")),from_email))
 return result
