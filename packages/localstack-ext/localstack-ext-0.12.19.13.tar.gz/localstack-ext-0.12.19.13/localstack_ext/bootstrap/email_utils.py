...
SENT_EMAILS=[]
EMAIL_BLACKLIST=AuKIa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",AuKIS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=AuKIE,from_name=AuKIE,smtp_host=AuKIE,smtp_user=AuKIE,smtp_pass=AuKIE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if AuKIm(recipients,AuKIn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=AuKIE):
 ...
 result["From"]=formataddr((AuKIS(Header(from_name,"utf-8")),from_email))
 return result
