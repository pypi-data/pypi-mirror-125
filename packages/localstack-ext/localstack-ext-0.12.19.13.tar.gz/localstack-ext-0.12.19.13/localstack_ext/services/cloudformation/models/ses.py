...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[lIMBJ])[0]
 def get_physical_resource_id(self,attribute=lIMBJ,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if lIMBY(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lIMBV(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @lIMBj
 def add_defaults(resource,stack_name:lIMBF):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lIMBV(SESReceiptRule,self).get_cfn_attribute(attribute)
 @lIMBK
