...
def canonicalize_name(name:iFXsY)->iFXsY:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[iFXsl])[0]
 def get_physical_resource_id(self,attribute=iFXsl,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return iFXsq(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @iFXsw
