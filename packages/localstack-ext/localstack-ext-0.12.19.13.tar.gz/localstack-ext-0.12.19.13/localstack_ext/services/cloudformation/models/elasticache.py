...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=GBzae(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not GBzaM
   return result
  except GBzaP:
   props=self.props
   ...
   if parts[0]in candidates and GBzax(parts)>1:
