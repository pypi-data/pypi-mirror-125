...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or OemHg
 @OemHG
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return OemHR(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @OemHC
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if OemHF(origins,OemHu)else origins.get("Items")
  origins_to_create=OemHx([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if OemHF(aliases,OemHu)else aliases.get("Items")
  aliases_to_create=OemHx(aliases)
  for distr in distrs:
   origins=OemHx([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=OemHx(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",OemHA)
   config["Enabled"]=OemHI(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if OemHF(custom_errors,OemHV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=OemHI(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":OemHA}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=OemHI(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=OemHg):
    parent=config if parent is OemHg else parent
    items=parent.get(key)or[]
    if OemHF(items,OemHV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":OemHq(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if OemHF(cache_behavs,OemHV)else cache_behavs)
