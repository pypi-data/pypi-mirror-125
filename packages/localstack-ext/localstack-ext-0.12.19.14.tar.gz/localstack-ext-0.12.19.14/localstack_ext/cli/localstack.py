...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ubCqF if e else ubCqI
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ubCqf:
...
@click.option("--revert",is_flag=ubCqF,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ubCqi):
...
def _cpvcs_initialized(pod_name:ubCqa)->ubCqi:
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_init(name:ubCqa):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=ubCqF,default=ubCqI)
def cmd_cpvcs_delete(name:ubCqa,remote:ubCqi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_register_remote(name:ubCqa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=ubCqF)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=ubCqF)
def cmd_cpvcs_rename(name:ubCqa,new_name:ubCqa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_commit(message:ubCqa,name:ubCqa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=ubCqF,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_push(squash:ubCqi,message:ubCqa,name:ubCqa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=ubCqV,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=ubCqF,default=ubCqI,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_inject(version:ubCqV,reset:ubCqi,name:ubCqa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=ubCqF,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=ubCqF,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:ubCqa,inject:ubCqi,reset:ubCqi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=ubCqF,default=ubCqI)
def cmd_cpvcs_list_pods(remote:ubCqi):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_versions(name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ubCqF,type=ubCqV)
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_version_info(version:ubCqV,name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=ubCqV,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_version_metamodel(version:ubCqV,name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ubCqF,type=ubCqV,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=ubCqF,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=ubCqF,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=ubCqI,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_set_version(version:ubCqV,inject:ubCqi,reset:ubCqi,commit_before:ubCqi,name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_commits(version:ubCqV,name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=ubCqF)
@click.option("--commit","-c",required=ubCqF)
@click.option("-n","--name",help="Name of the cloud pod",required=ubCqF)
def cmd_cpvcs_commit_diff(version:ubCqV,commit:ubCqV,name:ubCqa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(ubCqf)
  status.stop()
...
def print_table(columns:Dict[ubCqa,ubCqa],rows:List[Dict[ubCqa,Any]],formatters:Dict[ubCqa,Callable[[Any],ubCqa]]=ubCqf):
 from rich.table import Table
 if formatters is ubCqf:
  formatters=ubCqk()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ubCqg(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ubCqf:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ubCqf:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ubCqf:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ubCqf:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
