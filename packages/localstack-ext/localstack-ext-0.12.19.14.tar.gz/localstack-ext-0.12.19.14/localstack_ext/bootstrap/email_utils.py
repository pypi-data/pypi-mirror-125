...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OgIen()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OgIer(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OgIeV,from_name=OgIeV,smtp_host=OgIeV,smtp_user=OgIeV,smtp_pass=OgIeV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OgIeK(recipients,OgIeo)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OgIeV):
 ...
 result["From"]=formataddr((OgIer(Header(from_name,"utf-8")),from_email))
 return result
