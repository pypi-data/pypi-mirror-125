...
from localstack_ext.constants import API_PATH_PODS,API_STATES_DIR,PODS_NAMESPACE_DELIM
...
class PodInfo:
 def __init__(self,name=OKyGE,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(OKyGb):
 BACKEND="_none_"
 def __init__(self,pod_name=OKyGE,config=OKyGE):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise OKyGz("Not implemented")
 def delete(self,remote:OKyGa)->OKyGa:
  raise OKyGz("Not implemented")
 def push(self,comment:OKyGN=OKyGE)->PodInfo:
  raise OKyGz("Not implemented")
 def pull(self,inject_version_state:OKyGa=OKyGs,reset_state_before:OKyGa=OKyGs,fetch_all:OKyGa=OKyGE):
  raise OKyGz("Not implemented")
 def commit(self,message:OKyGN=OKyGE):
  raise OKyGz("Not implemented")
 def inject(self,version:OKyGp,reset_state:OKyGa):
  raise OKyGz("Not implemented")
 def list_versions(self)->List[OKyGN]:
  raise OKyGz("Not implemented")
 def version_info(self,version:OKyGp):
  raise OKyGz("Not implemented")
 def version_metamodel(self,version:OKyGp):
  raise OKyGz("Not implemented")
 def set_version(self,version:OKyGp,inject_version_state:OKyGa,reset_state:OKyGa,commit_before:OKyGa):
  raise OKyGz("Not implemented")
 def list_version_commits(self,version:OKyGp)->List[OKyGN]:
  raise OKyGz("Not implemented")
 def get_commit_diff(self,version:OKyGp,commit:OKyGp)->OKyGN:
  raise OKyGz("Not implemented")
 def register_remote(self,pod_name:OKyGN)->OKyGa:
  raise OKyGz("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->OKyGa:
  raise OKyGz("Not implemented")
 def list_pods(self,fetch_remote:OKyGa)->Set[OKyGN]:
  raise OKyGz("Not implemented")
 ...
 def get(cls,pod_name,pre_config=OKyGE):
  ...
  raise OKyGz('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=OKyGs
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=OKyGk)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=OKyGk
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise OKyGz("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @OKyGr
 def get_state_zip_from_instance(get_content=OKyGs):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise OKyGz("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:OKyGN=OKyGE):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @OKyGr
 def parse_pod_name_from_qualifying_name(qualifying_name:OKyGN)->OKyGN:
 ...
 @OKyGr
 def _get_max_version_for_pod_from_platform(pod_name:OKyGN,auth_headers):
  ...
  remote_max_ver=OKyGp(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with OKyGw(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with OKyGw(meta_archive,"rb")as meta_archive_content:
   ...
   with OKyGw(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=OKyGE)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:OKyGN):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:OKyGN=OKyGE)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:OKyGa=OKyGs,reset_state_before:OKyGa=OKyGs,fetch_all:OKyGa=OKyGk):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=OKyGs):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(OKyGY(lambda ver:OKyGN(ver),OKyGj(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:OKyGp,reset_state:OKyGa):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=OKyGk,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:OKyGa)->Set[OKyGN]:
  ...
  return result
 @OKyGS
 def upload_content(cls,presigned_url:OKyGN,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise OKyGz("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @OKyGr
 def create_platform_url(request:OKyGN)->OKyGN:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:OKyGN=OKyGE)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=OKyGk)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(OKyGP(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=OKyGP(zip_data_content)
  return result
 def pull(self,inject_version_state:OKyGa=OKyGs,reset_state_before:OKyGa=OKyGs,fetch_all:OKyGa=OKyGE):
  ...
  rm_rf(zip_path)
 @OKyGr
 def presigned_url(pod_name:OKyGN,mode:OKyGN)->OKyGN:
  ...
  if response.status_code>=400:
   raise OKyGz("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise OKyGz("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:OKyGN=OKyGE):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=OKyGk)
 ...
 def pull(self,inject_version_state:OKyGa=OKyGs,reset_state_before:OKyGa=OKyGs,fetch_all:OKyGa=OKyGE):
  ...
  if remote_ref not in remote_refs:
   raise OKyGz('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=OKyGU(self,"clone_dir",OKyGE)
...
class PodConfigManagerMeta(OKyGA):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=OKyGE
   for manager in cls.CHAIN:
    try:
     tmp=OKyGU(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif OKyGn(tmp,OKyGH)and OKyGn(result,OKyGH):
       result.extend(tmp)
    except OKyGz:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not OKyGE:
    return result
   raise OKyGz('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(OKyGb,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @OKyGS
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise OKyGz('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(OKyGb):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=OKyGk).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=OKyGM([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=OKyGs):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except OKyGz:
   if safe:
    return{}
   raise OKyGz('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(OKyGb):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise OKyGz("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise OKyGz("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:OKyGN,remote:OKyGa,pre_config:Dict[OKyGN,OKyGN])->OKyGa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs)->OKyGa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:OKyGN,new_pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:OKyGa,pre_config:Dict[OKyGN,OKyGN],**kwargs)->List[OKyGN]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],message:OKyGN=OKyGE,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:OKyGN,version:OKyGp,reset_state:OKyGa,pre_config:Dict[OKyGN,OKyGN],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs)->List[OKyGN]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:OKyGp,pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:OKyGp,pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:OKyGp,inject_version_state:OKyGa,reset_state:OKyGa,commit_before:OKyGa,pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN],**kwargs)->OKyGa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:OKyGp,pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN])->List[OKyGN]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:OKyGp,commit:OKyGp,pod_name:OKyGN,pre_config:Dict[OKyGN,OKyGN])->OKyGN:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=OKyGE,squash_commits=OKyGs,comment=OKyGE,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=OKyGs,reset_state_before=OKyGs,**kwargs):
 pre_config=kwargs.get("pre_config",OKyGE)
 if not pod_name:
  raise OKyGz("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=OKyGs,exclude_from_reset:List[OKyGN]=OKyGE):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise OKyGz("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->OKyGN:
 try:
...
def get_persisted_resource_names(data_dir)->List[OKyGN]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and OKyGP(os.listdir(entry.path))>0:
    names.append(entry.name)
