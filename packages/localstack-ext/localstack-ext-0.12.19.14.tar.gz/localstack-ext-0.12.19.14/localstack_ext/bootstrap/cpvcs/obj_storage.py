...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=HKOSh
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:HKOSY)->HKOSY:
  pass
 @HKOSr
 def get(cls,requested_file_locator:HKOSY):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=HKOSh
 @HKOSr
 def get(cls,state_file_locator:HKOSY,requested_storage:HKOSY,serializers:Dict[HKOSY,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[HKOSY,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:HKOSY)->HKOSY:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:HKOSY)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:HKOSY)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:HKOSY)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:HKOSY)->HKOSY:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[HKOSY]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:HKOSY,new_key:HKOSY,referenced_by_version:HKOSY=HKOSh):
  pass
 @abc.abstractmethod
 def version_exists(self,key:HKOSY)->HKOSW:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:HKOSY,key:HKOSY):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:HKOSY,new_key:HKOSY,base_path:HKOSY)->HKOSW:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[HKOSY]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[HKOSY,HKOSY],remote_location:HKOSY=HKOSh)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[HKOSY]:
  ...
  for cpvcs_obj in args:
   if HKOSd(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif HKOSd(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[HKOSY,HKOSY],remote_location:HKOSY=HKOSh)->List[Union[Revision,Version]]:
