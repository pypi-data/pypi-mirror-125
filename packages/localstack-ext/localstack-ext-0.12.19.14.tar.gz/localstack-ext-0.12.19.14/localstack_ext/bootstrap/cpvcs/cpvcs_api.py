...
from localstack.utils.common import json_safe,mkdir,new_tmp_dir,rm_rf,short_uid
...
from localstack_ext.bootstrap.cpvcs.utils.hash_utils import(compute_file_hash,compute_node_hash,compute_version_archive_hash,random_hash)
...
from localstack_ext.bootstrap.state_utils import api_states_traverse,load_persisted_object
...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=otAYh,state_files=otAYu(),version_number=0)
 ...
 with otAYj(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with otAYj(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with otAYj(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:otAYU,meta_archives:Dict[otAYU,otAYU],state_archives:Dict[otAYU,otAYU],remote_info:Dict[otAYU,otAYU],pod_name:otAYU):
...
def merge_from_remote(version_space_archive:otAYU,meta_archives:Dict[otAYU,otAYU],state_archives:Dict[otAYU,otAYU]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->otAYk:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:otAYU):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:otAYU)->otAYk:
 if config_context.pod_exists_locally(new_pod_name):
  LOG.warning(f"{new_pod_name} already exists locally")
  return otAYy
 config_context.rename_pod_name(new_pod_name)
 return otAYn
def list_locally_available_pods(show_remote_or_local:otAYk=otAYn)->Set[otAYU]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:otAYU,file_name:otAYU,service:otAYU,region:otAYU)->otAYU:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->otAYU:
 ...
 return key
def _get_state_file_path(key:otAYU)->otAYU:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=otAYu(otAYx(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:otAYU)->Optional[otAYU]:
...
def get_version_info(version_no:otAYF)->Dict[otAYU,otAYU]:
...
def read_file_from_archive(archive_path:otAYU,file_name:otAYU):
 try:
...
def commit(message:otAYU=otAYh)->Revision:
 ...
 curr_expansion_point_parent_state_files=otAYu()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=otAYh
...
def zip_directories(zip_dest:otAYU,directories:List[otAYU])->otAYU:
 ...
 return zip_dest
def create_version_space_archive()->otAYU:
...
def get_max_version_no()->otAYF:
 with otAYj(config_context.get_max_ver_path())as fp:
  return otAYF(os.path.basename(fp.readline()))
...
def push(comment:otAYU=otAYh)->Version:
 ...
 return new_version
def get_version_meta_archive(version_no:otAYF)->otAYU:
...
def get_version_state_archive(version_no:otAYF)->otAYU:
...
def get_version_metamodel(version_no:otAYF)->otAYU:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:otAYk=otAYy):
 ...
 with otAYj(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with otAYj(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:otAYF,state_file_refs:Set[StateFileRef],delete_files=otAYy,archive=otAYn):
...
def set_active_version(version_no:otAYF,commit_before=otAYy)->otAYk:
 ...
 return otAYy
def _set_active_version(key:otAYU):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=otAYu(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:otAYF)->Version:
 versions=load_version_references()
 version_ref=otAYL((version[1]for version in versions if version[0]==version_no),otAYh)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[otAYF,otAYU]]:
 result={}
 with otAYj(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with otAYj(symlink,"r")as sp:
    result[otAYF(os.path.basename(symlink))]=sp.readline()
 return otAYc(result.items(),key=lambda x:x[0],reverse=otAYn)
def list_versions()->List[otAYU]:
 ...
 return result
def list_version_commits(version_no:otAYF)->List[otAYU]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:otAYF,commit_no:otAYF)->otAYU:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->otAYU:
 with otAYj(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->otAYU:
 with otAYj(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->otAYU:
 with otAYj(config_context.get_known_ver_path(),"a")as fp:
...
def _merge_two_state_dir_func(**kwargs):
 def _is_special_case(obj)->otAYk:
  return otAYS(obj,(Queue,FakeBucket))
...
def _merge_expansion_point_with_max():
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=otAYy)
...
def _infer_backend_init(clazz,sf):
 if otAYS(clazz,otAYH):
  backend={}
 else:
  constructor=otAYv(clazz,"__init__",otAYh)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->otAYU:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[otAYU,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,otAYh)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(otAYh,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=otAYr(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=otAYr(backend1)
