...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or LpRPb
 @LpRPr
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return LpRPB(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @LpRPT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if LpRPN(origins,LpRPl)else origins.get("Items")
  origins_to_create=LpRPW([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if LpRPN(aliases,LpRPl)else aliases.get("Items")
  aliases_to_create=LpRPW(aliases)
  for distr in distrs:
   origins=LpRPW([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=LpRPW(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",LpRPF)
   config["Enabled"]=LpRPa(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if LpRPN(custom_errors,LpRPI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=LpRPa(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":LpRPF}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=LpRPa(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=LpRPb):
    parent=config if parent is LpRPb else parent
    items=parent.get(key)or[]
    if LpRPN(items,LpRPI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":LpRPO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if LpRPN(cache_behavs,LpRPI)else cache_behavs)
