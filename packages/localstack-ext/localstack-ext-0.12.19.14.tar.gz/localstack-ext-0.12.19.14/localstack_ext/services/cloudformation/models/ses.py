...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[dcwPN])[0]
 def get_physical_resource_id(self,attribute=dcwPN,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if dcwPt(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dcwPV(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @dcwPv
 def add_defaults(resource,stack_name:dcwPK):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dcwPV(SESReceiptRule,self).get_cfn_attribute(attribute)
 @dcwPM
