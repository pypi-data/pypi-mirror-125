...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=cSglv(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not cSglC
   return result
  except cSgly:
   props=self.props
   ...
   if parts[0]in candidates and cSglu(parts)>1:
