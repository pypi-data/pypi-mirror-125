...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Hniqj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",Hniqd(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Hniqx,from_name=Hniqx,smtp_host=Hniqx,smtp_user=Hniqx,smtp_pass=Hniqx,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Hniqu(recipients,HniqL)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Hniqx):
 ...
 result["From"]=formataddr((Hniqd(Header(from_name,"utf-8")),from_email))
 return result
