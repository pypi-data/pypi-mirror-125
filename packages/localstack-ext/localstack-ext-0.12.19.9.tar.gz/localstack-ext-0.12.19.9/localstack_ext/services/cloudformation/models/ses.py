...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[UYWmL])[0]
 def get_physical_resource_id(self,attribute=UYWmL,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if UYWmi(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return UYWmw(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @UYWms
 def add_defaults(resource,stack_name:UYWmC):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return UYWmw(SESReceiptRule,self).get_cfn_attribute(attribute)
 @UYWmq
