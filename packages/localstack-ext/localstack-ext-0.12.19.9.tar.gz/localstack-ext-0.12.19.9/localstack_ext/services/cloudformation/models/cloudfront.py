...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or cuTxY
 @cuTxU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return cuTxw(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @cuTxt
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if cuTxH(origins,cuTxJ)else origins.get("Items")
  origins_to_create=cuTxK([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if cuTxH(aliases,cuTxJ)else aliases.get("Items")
  aliases_to_create=cuTxK(aliases)
  for distr in distrs:
   origins=cuTxK([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=cuTxK(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",cuTxi)
   config["Enabled"]=cuTxW(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if cuTxH(custom_errors,cuTxm)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=cuTxW(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":cuTxi}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=cuTxW(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=cuTxY):
    parent=config if parent is cuTxY else parent
    items=parent.get(key)or[]
    if cuTxH(items,cuTxm)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":cuTxP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if cuTxH(cache_behavs,cuTxm)else cache_behavs)
