...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=rzFoV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not rzFop
   return result
  except rzFom:
   props=self.props
   ...
   if parts[0]in candidates and rzFoA(parts)>1:
