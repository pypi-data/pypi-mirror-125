...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=oyVIF(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not oyVIH
   return result
  except oyVIj:
   props=self.props
   ...
   if parts[0]in candidates and oyVId(parts)>1:
