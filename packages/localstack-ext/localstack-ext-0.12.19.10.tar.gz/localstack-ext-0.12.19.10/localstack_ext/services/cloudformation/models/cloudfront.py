...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or AcsBV
 @AcsBE
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return AcsBR(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @AcsBl
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if AcsBm(origins,AcsBi)else origins.get("Items")
  origins_to_create=AcsBG([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if AcsBm(aliases,AcsBi)else aliases.get("Items")
  aliases_to_create=AcsBG(aliases)
  for distr in distrs:
   origins=AcsBG([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=AcsBG(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",AcsBq)
   config["Enabled"]=AcsBb(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if AcsBm(custom_errors,AcsBj)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=AcsBb(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":AcsBq}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=AcsBb(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=AcsBV):
    parent=config if parent is AcsBV else parent
    items=parent.get(key)or[]
    if AcsBm(items,AcsBj)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":AcsBa(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if AcsBm(cache_behavs,AcsBj)else cache_behavs)
