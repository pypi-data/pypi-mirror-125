...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[gwvzb])[0]
 def get_physical_resource_id(self,attribute=gwvzb,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if gwvzG(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return gwvzm(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @gwvzS
 def add_defaults(resource,stack_name:gwvzA):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return gwvzm(SESReceiptRule,self).get_cfn_attribute(attribute)
 @gwvzh
