...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return KeUAH if e else KeUAi
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->KeUAV:
...
@click.option("--revert",is_flag=KeUAH,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:KeUAu):
...
def _cpvcs_initialized(pod_name:KeUAx)->KeUAu:
...
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_init(name:KeUAx):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_commit(message:KeUAx,name:KeUAx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--squash",is_flag=KeUAH,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_push(squash:KeUAu,message:KeUAx,name:KeUAx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--version",default="-1",type=KeUAJ,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=KeUAH,default=KeUAi,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_inject(version:KeUAJ,reset:KeUAu,name:KeUAx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_versions(name:KeUAx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=KeUAH,type=KeUAJ)
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_version_info(version:KeUAJ,name:KeUAx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version",required=KeUAH,type=KeUAJ,help="The version the state should be set to")
@click.option("--inject",is_flag=KeUAH,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=KeUAH,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=KeUAH,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_set_version(version:KeUAJ,inject:KeUAu,reset:KeUAu,commit_before:KeUAu,name:KeUAx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_commits(version:KeUAJ,name:KeUAx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=KeUAH)
@click.option("--commit","-c",required=KeUAH)
@click.option("-n","--name",help="Name of the cloud pod",required=KeUAH)
def cmd_cpvcs_commit_diff(version:KeUAJ,commit:KeUAJ,name:KeUAx):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(KeUAV)
  status.stop()
...
def print_table(columns:Dict[KeUAx,KeUAx],rows:List[Dict[KeUAx,Any]],formatters:Dict[KeUAx,Callable[[Any],KeUAx]]=KeUAV):
 from rich.table import Table
 if formatters is KeUAV:
  formatters=KeUAT()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  KeUAf(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->KeUAV:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->KeUAV:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->KeUAV:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->KeUAV:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
