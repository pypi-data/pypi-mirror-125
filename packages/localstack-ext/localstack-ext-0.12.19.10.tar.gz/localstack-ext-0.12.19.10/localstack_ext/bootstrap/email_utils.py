...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PQjIq()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PQjIX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PQjIU,from_name=PQjIU,smtp_host=PQjIU,smtp_user=PQjIU,smtp_pass=PQjIU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PQjIf(recipients,PQjIs)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PQjIU):
 ...
 result["From"]=formataddr((PQjIX(Header(from_name,"utf-8")),from_email))
 return result
