...
def init(creator:fdoys="Unknown",pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=creator,comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=fdoyJ,state_files=fdoyD(),version_number=0)
 ...
 with fdoyw(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with fdoyw(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with fdoyw(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def set_pod(pod_name:fdoys):
 config_context.set_pod_context(pod_name)
def create_state_file_from_fs(path:fdoys,file_name:fdoys,service:fdoys,region:fdoys)->fdoys:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->fdoys:
 ...
 return key
def _get_state_file_path(key:fdoys)->fdoys:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=fdoyD(fdoyN(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:fdoys)->Optional[fdoys]:
...
def get_version_info(version_no:fdoyM)->List[fdoys]:
 version_node=get_version_by_number(version_no)
 if not version_node:
  return[]
 return fdoyH(fdoyn(lambda state_file:state_file.metadata(),version_node.state_files))
def commit(message:fdoys=fdoyJ)->Revision:
 ...
 curr_expansion_point_parent_state_files=fdoyD()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=fdoyJ
...
def get_max_version_no()->fdoyM:
 with fdoyw(config_context.get_max_ver_path())as fp:
  return fdoyM(os.path.basename(fp.readline()))
...
def push(comment:fdoys=fdoyJ)->Version:
 ...
 new_active_revision=Revision(hash_ref=random_hash(),state_files=fdoyD(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 if head_version.version_number!=max_version.version_number:
  ...
  new_version_state_files:Set[StateFileRef]=fdoyD()
  ...
  for state_file in max_version_regular_sf:
   ...
   match=expansion_point_file_paths_refs.pop(qualifier,fdoyJ)
   if match:
  ...
  newly_added_file_refs=fdoyD(expansion_point_file_paths_refs.values())
  newly_added_files=fdoyD(fdoyN(lambda leftover:leftover.hash_ref in newly_added_file_refs,expansion_point_regular_sf))
 ...
 new_version=Version(hash_ref=fdoyJ,state_files=new_version_state_files,parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=max_version.version_number+1)
 ...
 return new_version
def get_version_state_pod(version_no:fdoyM):
 version_path=os.path.join(config_context.get_cpvcs_root_dir(),f"version_{version_no}.zip")
 if os.path.isfile(version_path):
  return version_path
def _create_commit_history(version_node:Version,delete_reference:fdoyL=fdoyi):
 ...
 rm_rf(history_dir)
def _create_state_zip(version_number:fdoyM,state_file_refs:Set[StateFileRef],delete_files=fdoyi):
 ...
 rm_rf(version_dir)
def set_active_version(version_no:fdoyM,commit_before=fdoyi)->fdoyL:
 ...
 return fdoyi
def _set_active_version(key:fdoys):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=fdoyD(),parent_ptr=NIL_PTR,creator="Unknown",rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:fdoyM)->Version:
 versions=load_version_references()
 version_ref=fdoyQ((version[1]for version in versions if version[0]==version_no),fdoyJ)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[fdoyM,fdoys]]:
 result={}
 with fdoyw(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=symlink.rstrip()
   with fdoyw(symlink,"r")as sp:
    result[fdoyM(os.path.basename(symlink))]=sp.readline()
 return fdoyc(result.items(),key=lambda x:x[0],reverse=fdoyz)
def list_versions()->List[fdoys]:
 ...
 return result
def list_version_commits(version_no:fdoyM)->List[fdoys]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:fdoyM,commit_no:fdoyM)->fdoys:
 try:
...
def _update_head(new_head_ver_no,new_head_key)->fdoys:
 with fdoyw(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->fdoys:
 with fdoyw(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->fdoys:
 with fdoyw(config_context.get_known_ver_path(),"a")as fp:
...
def _push_remote():
 pass
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->fdoys:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[fdoys,StateFileRef]:
 ...
 def _infer_backend_init(clazz,sf):
  if fdoyP(clazz,fdoyu):
   backend={}
  else:
   constructor=fdoyh(clazz,"__init__",fdoyJ)
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(_infer_backend_init,state_file_from,fdoyJ)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(_infer_backend_init,fdoyJ,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(_infer_backend_init,sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=fdoyT(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=fdoyT(backend1)
