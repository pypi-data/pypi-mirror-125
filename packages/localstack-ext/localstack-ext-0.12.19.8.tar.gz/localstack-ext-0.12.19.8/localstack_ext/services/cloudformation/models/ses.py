...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[mrukx])[0]
 def get_physical_resource_id(self,attribute=mrukx,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if mrukb(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return mrukg(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @mrukR
 def add_defaults(resource,stack_name:mruks):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return mrukg(SESReceiptRule,self).get_cfn_attribute(attribute)
 @mrukH
