...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or LqxCW
 @LqxCb
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return LqxCY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @LqxCc
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if LqxCM(origins,LqxCu)else origins.get("Items")
  origins_to_create=LqxCw([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if LqxCM(aliases,LqxCu)else aliases.get("Items")
  aliases_to_create=LqxCw(aliases)
  for distr in distrs:
   origins=LqxCw([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=LqxCw(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",LqxCK)
   config["Enabled"]=LqxCv(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if LqxCM(custom_errors,LqxCN)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=LqxCv(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":LqxCK}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=LqxCv(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=LqxCW):
    parent=config if parent is LqxCW else parent
    items=parent.get(key)or[]
    if LqxCM(items,LqxCN)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":LqxCm(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if LqxCM(cache_behavs,LqxCN)else cache_behavs)
