...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ciARQ if e else ciARq
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ciARC:
...
@click.option("--revert",is_flag=ciARQ,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ciARw):
...
def _cpvcs_initialized(pod_name:ciARK)->ciARw:
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_init(name:ciARK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=ciARQ,default=ciARq)
def cmd_cpvcs_delete(name:ciARK,remote:ciARw):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_register_remote(name:ciARK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=ciARQ)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=ciARQ)
def cmd_cpvcs_rename(name:ciARK,new_name:ciARK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_commit(message:ciARK,name:ciARK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=ciARQ,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_push(squash:ciARw,message:ciARK,name:ciARK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=ciARF,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=ciARQ,default=ciARq,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_inject(version:ciARF,reset:ciARw,name:ciARK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=ciARQ,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=ciARQ,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:ciARK,inject:ciARw,reset:ciARw):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=ciARQ,default=ciARq)
def cmd_cpvcs_list_pods(remote:ciARw):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_versions(name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ciARQ,type=ciARF)
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_version_info(version:ciARF,name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=ciARF,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_version_metamodel(version:ciARF,name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ciARQ,type=ciARF,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=ciARQ,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=ciARQ,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=ciARq,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_set_version(version:ciARF,inject:ciARw,reset:ciARw,commit_before:ciARw,name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_commits(version:ciARF,name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=ciARQ)
@click.option("--commit","-c",required=ciARQ)
@click.option("-n","--name",help="Name of the cloud pod",required=ciARQ)
def cmd_cpvcs_commit_diff(version:ciARF,commit:ciARF,name:ciARK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(ciARC)
  status.stop()
...
def print_table(columns:Dict[ciARK,ciARK],rows:List[Dict[ciARK,Any]],formatters:Dict[ciARK,Callable[[Any],ciARK]]=ciARC):
 from rich.table import Table
 if formatters is ciARC:
  formatters=ciARb()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ciARv(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ciARC:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ciARC:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ciARC:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ciARC:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
