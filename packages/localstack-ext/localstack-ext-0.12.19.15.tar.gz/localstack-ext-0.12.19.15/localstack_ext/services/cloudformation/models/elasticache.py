...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=SFmEP(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not SFmEJ
   return result
  except SFmEk:
   props=self.props
   ...
   if parts[0]in candidates and SFmEn(parts)>1:
