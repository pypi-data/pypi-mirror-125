...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[VwtOD])[0]
 def get_physical_resource_id(self,attribute=VwtOD,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if VwtOX(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return VwtOF(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @VwtOC
 def add_defaults(resource,stack_name:VwtOy):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return VwtOF(SESReceiptRule,self).get_cfn_attribute(attribute)
 @VwtOx
