...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or zaRMh
 @zaRMr
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return zaRMo(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @zaRMH
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if zaRMI(origins,zaRML)else origins.get("Items")
  origins_to_create=zaRMv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if zaRMI(aliases,zaRML)else aliases.get("Items")
  aliases_to_create=zaRMv(aliases)
  for distr in distrs:
   origins=zaRMv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=zaRMv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",zaRMN)
   config["Enabled"]=zaRMQ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if zaRMI(custom_errors,zaRMT)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=zaRMQ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":zaRMN}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=zaRMQ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=zaRMh):
    parent=config if parent is zaRMh else parent
    items=parent.get(key)or[]
    if zaRMI(items,zaRMT)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":zaRMc(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if zaRMI(cache_behavs,zaRMT)else cache_behavs)
