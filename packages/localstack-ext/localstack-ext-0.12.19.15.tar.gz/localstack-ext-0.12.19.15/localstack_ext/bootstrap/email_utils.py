...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xFKRa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",xFKRv(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xFKRg,from_name=xFKRg,smtp_host=xFKRg,smtp_user=xFKRg,smtp_pass=xFKRg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xFKRA(recipients,xFKRi)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xFKRg):
 ...
 result["From"]=formataddr((xFKRv(Header(from_name,"utf-8")),from_email))
 return result
