...
def random_hash()->pBPKD:
 return pBPKr(random.getrandbits(160))
def compute_file_hash(file_path:pBPKD,accum=pBPKH)->Optional[pBPKD]:
 try:
  with pBPKw(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:pBPKQ,state_archive:pBPKD)->pBPKD:
 ...
 m.update(pBPKD(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->pBPKD:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=pBPKA(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with pBPKw(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except pBPKV as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if pBPKs(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(pBPKD(cpvcs_node.revision_number).encode("utf-8"))
 elif pBPKs(cpvcs_node,Version):
  m.update(pBPKD(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
