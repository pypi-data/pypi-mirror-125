...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=oONfM,state_files=oONfE(),version_number=0)
 ...
 with oONfI(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with oONfI(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with oONfI(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:oONfu,meta_archives:Dict[oONfu,oONfu],state_archives:Dict[oONfu,oONfu],remote_info:Dict[oONfu,oONfu],pod_name:oONfu):
...
def merge_from_remote(version_space_archive:oONfu,meta_archives:Dict[oONfu,oONfu],state_archives:Dict[oONfu,oONfu]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->oONfq:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:oONfu):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:oONfu)->oONfq:
 ...
 return oONfW
def list_locally_available_pods(show_remote_or_local:oONfq=oONfW)->Set[oONfu]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:oONfu,file_name:oONfu,service:oONfu,region:oONfu)->oONfu:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->oONfu:
 ...
 return key
def _get_state_file_path(key:oONfu)->oONfu:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=oONfE(oONfw(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:oONfu)->Optional[oONfu]:
...
def get_version_info(version_no:oONfK)->Dict[oONfu,oONfu]:
...
def read_file_from_archive(archive_path:oONfu,file_name:oONfu):
 try:
...
def commit(message:oONfu=oONfM)->Revision:
 ...
 curr_expansion_point_parent_state_files=oONfE()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=oONfM
...
def zip_directories(zip_dest:oONfu,directories:List[oONfu])->oONfu:
 ...
 return zip_dest
def create_version_space_archive()->oONfu:
...
def get_max_version_no()->oONfK:
 with oONfI(config_context.get_max_ver_path())as fp:
  return oONfK(os.path.basename(fp.readline()))
...
def push(comment:oONfu=oONfM)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=oONfE(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=oONfE(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:oONfK)->oONfu:
...
def get_version_state_archive(version_no:oONfK)->oONfu:
...
def get_version_metamodel(version_no:oONfK)->oONfu:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:oONfq=oONfj):
 ...
 with oONfI(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with oONfI(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:oONfK,state_file_refs:Set[StateFileRef],delete_files=oONfj,archive=oONfW):
...
def set_active_version(version_no:oONfK,commit_before=oONfj)->oONfq:
 ...
 return oONfj
def _set_active_version(key:oONfu):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=oONfE(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:oONfK)->Version:
 versions=load_version_references()
 version_ref=oONfg((version[1]for version in versions if version[0]==version_no),oONfM)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[oONfK,oONfu]]:
 result={}
 with oONfI(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with oONfI(symlink,"r")as sp:
    result[oONfK(os.path.basename(symlink))]=sp.readline()
 return oONfV(result.items(),key=lambda x:x[0],reverse=oONfW)
def list_versions()->List[oONfu]:
 ...
 return result
def list_version_commits(version_no:oONfK)->List[oONfu]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:oONfK,commit_no:oONfK)->oONfu:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->oONfu:
 with oONfI(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->oONfu:
 with oONfI(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->oONfu:
 with oONfI(config_context.get_known_ver_path(),"a")as fp:
...
def _merge_two_state_dir_func(**kwargs):
 def _is_special_case(obj)->oONfq:
  return oONfs(obj,(Queue,FakeBucket))
...
def _merge_expansion_point_with_max():
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=oONfj)
...
def _infer_backend_init(clazz,sf):
 if oONfs(clazz,oONfi):
  backend={}
 else:
  constructor=oONfU(clazz,"__init__",oONfM)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->oONfu:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[oONfu,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,oONfM)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(oONfM,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=oONfh(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=oONfh(backend1)
