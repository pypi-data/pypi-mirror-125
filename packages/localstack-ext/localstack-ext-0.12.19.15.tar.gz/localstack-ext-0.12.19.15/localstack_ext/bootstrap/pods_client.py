...
class PodInfo:
 def __init__(self,name=zFWfT,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(zFWft):
 BACKEND="_none_"
 def __init__(self,pod_name=zFWfT,config=zFWfT):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise zFWfX("Not implemented")
 def delete(self,remote:zFWfr)->zFWfr:
  raise zFWfX("Not implemented")
 def push(self,comment:zFWfB=zFWfT)->PodInfo:
  raise zFWfX("Not implemented")
 def pull(self,inject_version_state:zFWfr=zFWfK,reset_state_before:zFWfr=zFWfK,fetch_all:zFWfr=zFWfT):
  raise zFWfX("Not implemented")
 def commit(self,message:zFWfB=zFWfT):
  raise zFWfX("Not implemented")
 def inject(self,version:zFWfN,reset_state:zFWfr):
  raise zFWfX("Not implemented")
 def list_versions(self)->List[zFWfB]:
  raise zFWfX("Not implemented")
 def version_info(self,version:zFWfN):
  raise zFWfX("Not implemented")
 def version_metamodel(self,version:zFWfN):
  raise zFWfX("Not implemented")
 def set_version(self,version:zFWfN,inject_version_state:zFWfr,reset_state:zFWfr,commit_before:zFWfr):
  raise zFWfX("Not implemented")
 def list_version_commits(self,version:zFWfN)->List[zFWfB]:
  raise zFWfX("Not implemented")
 def get_commit_diff(self,version:zFWfN,commit:zFWfN)->zFWfB:
  raise zFWfX("Not implemented")
 def register_remote(self,pod_name:zFWfB)->zFWfr:
  raise zFWfX("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->zFWfr:
  raise zFWfX("Not implemented")
 def list_pods(self,fetch_remote:zFWfr)->Set[zFWfB]:
  raise zFWfX("Not implemented")
 ...
 def get(cls,pod_name,pre_config=zFWfT):
  ...
  raise zFWfX('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=zFWfK
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=zFWfY)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=zFWfY
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise zFWfX("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @zFWfQ
 def get_state_zip_from_instance(get_content=zFWfK):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise zFWfX("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:zFWfB=zFWfT):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @zFWfQ
 def parse_pod_name_from_qualifying_name(qualifying_name:zFWfB)->zFWfB:
 ...
 @zFWfQ
 def _get_max_version_for_pod_from_platform(pod_name:zFWfB,auth_headers):
  ...
  remote_max_ver=zFWfN(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with zFWfw(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with zFWfw(meta_archive,"rb")as meta_archive_content:
   ...
   with zFWfw(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=zFWfT)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:zFWfB):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:zFWfB=zFWfT)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:zFWfr=zFWfK,reset_state_before:zFWfr=zFWfK,fetch_all:zFWfr=zFWfY):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=zFWfK):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(zFWfo(lambda ver:zFWfB(ver),zFWfU(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:zFWfN,reset_state:zFWfr):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=zFWfY,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:zFWfr)->Set[zFWfB]:
  ...
  return result
 @zFWfO
 def upload_content(cls,presigned_url:zFWfB,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise zFWfX("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @zFWfQ
 def create_platform_url(request:zFWfB)->zFWfB:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:zFWfB=zFWfT)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=zFWfY)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(zFWfR(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=zFWfR(zip_data_content)
  return result
 def pull(self,inject_version_state:zFWfr=zFWfK,reset_state_before:zFWfr=zFWfK,fetch_all:zFWfr=zFWfT):
  ...
  rm_rf(zip_path)
 @zFWfQ
 def presigned_url(pod_name:zFWfB,mode:zFWfB)->zFWfB:
  ...
  if response.status_code>=400:
   raise zFWfX("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise zFWfX("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:zFWfB=zFWfT):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=zFWfY)
 ...
 def pull(self,inject_version_state:zFWfr=zFWfK,reset_state_before:zFWfr=zFWfK,fetch_all:zFWfr=zFWfT):
  ...
  if remote_ref not in remote_refs:
   raise zFWfX('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=zFWfE(self,"clone_dir",zFWfT)
...
class PodConfigManagerMeta(zFWfj):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=zFWfT
   for manager in cls.CHAIN:
    try:
     tmp=zFWfE(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif zFWfe(tmp,zFWfV)and zFWfe(result,zFWfV):
       result.extend(tmp)
    except zFWfX:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not zFWfT:
    return result
   raise zFWfX('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(zFWft,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @zFWfO
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise zFWfX('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(zFWft):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=zFWfY).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=zFWfC([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=zFWfK):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except zFWfX:
   if safe:
    return{}
   raise zFWfX('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(zFWft):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise zFWfX("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise zFWfX("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:zFWfB,remote:zFWfr,pre_config:Dict[zFWfB,zFWfB])->zFWfr:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs)->zFWfr:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:zFWfB,new_pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:zFWfr,pre_config:Dict[zFWfB,zFWfB],**kwargs)->List[zFWfB]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],message:zFWfB=zFWfT,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:zFWfB,version:zFWfN,reset_state:zFWfr,pre_config:Dict[zFWfB,zFWfB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs)->List[zFWfB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:zFWfN,pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:zFWfN,pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:zFWfN,inject_version_state:zFWfr,reset_state:zFWfr,commit_before:zFWfr,pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB],**kwargs)->zFWfr:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:zFWfN,pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB])->List[zFWfB]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:zFWfN,commit:zFWfN,pod_name:zFWfB,pre_config:Dict[zFWfB,zFWfB])->zFWfB:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=zFWfT,squash_commits=zFWfK,comment=zFWfT,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=zFWfK,reset_state_before=zFWfK,**kwargs):
 pre_config=kwargs.get("pre_config",zFWfT)
 if not pod_name:
  raise zFWfX("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=zFWfK,exclude_from_reset:List[zFWfB]=zFWfT):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise zFWfX("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->zFWfB:
 try:
...
def get_persisted_resource_names(data_dir)->List[zFWfB]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and zFWfR(os.listdir(entry.path))>0:
    names.append(entry.name)
