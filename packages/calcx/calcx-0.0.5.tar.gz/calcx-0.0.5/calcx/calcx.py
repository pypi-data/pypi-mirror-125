...
def mdd_list(df):
    ...
    if df.empty:
        ...
        df['mdd'] = 1 - (df['sawnav'] / df['sawnav'].expanding(min_periods=1).max())
        df_rl['date'] = df['date']
        df_rl['mdd'] = df['mdd']
        return df_rl
...
class CalcxTarget:
    def __init__(self, df, date_column='date', nv_column='sawnav', index_column=None, compute_index=False, 
        ...
        period = [pd.to_datetime(date(1990,1,1)), pd.to_datetime(datetime.today().date())] if period is None else period
