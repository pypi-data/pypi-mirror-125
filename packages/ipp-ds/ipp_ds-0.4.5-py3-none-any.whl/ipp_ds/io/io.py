...
def to_any(byte_stream, uri, encoding='utf-8', conn_type='gen2', timeout=DEFAULT_TIMEOUT):
        ...
        try:
            if conn_type == 'gen2':
                if isinstance(byte_stream, BytesIO):
                    file_client.upload_data(byte_stream, overwrite=True, chunk_size=DEFAULT_CHUNK_SIZE, timeout=timeout)
                else:
                    file_client.upload_data("".join(byte_stream.readlines()), overwrite=True, encoding=encoding, chunk_size=DEFAULT_CHUNK_SIZE, timeout=timeout)

            if conn_type == 'blob':
                if isinstance(byte_stream, BytesIO):
                    file_client.upload_blob(byte_stream, overwrite=True, timeout=timeout)
                else:
                    file_client.upload_blob("".join(byte_stream.readlines()), overwrite=True, encoding=encoding, timeout=timeout)
            break

        except Exception as e:

            print(f'Retrying to upload (loop {i+1}). Error:{e}')
...
def read_cols(uri, conn_type = 'gen2', **kwargs):
