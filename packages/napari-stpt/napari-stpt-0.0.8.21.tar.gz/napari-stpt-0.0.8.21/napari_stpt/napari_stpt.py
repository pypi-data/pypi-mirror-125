...
class NapariSTPT:
    ...
    def Load(self, text):
        ...
        with napari.gui_qt():
            if self.cb_C1.isChecked():
                print("loading C1")
                volume_1_temp = (ds.sel(channel=1, type='mosaic', z=0).to_array().data * bscale + bzero).astype(dtype=np.float32)

                if self.start_slice.text() == "":
                    start_slice_number = 0
                else:
                    start_slice_number = int(self.start_slice.text())

                if self.end_slice.text() == "":
                    end_slice_number = volume_1_temp.shape[0]-1
                else:
                    end_slice_number = int(self.end_slice.text())
                ...
                for optical_slice in range(1, self.optical_slices):
                    ...
                    volume_1_temp = (ds.sel(channel=1, type='mosaic', z=optical_slice).to_array().data * bscale + bzero).astype(dtype=np.float32)
                ...
                print("aligning C1")
                self.aligned_1 = self.Align(volume_1, resolution, output_resolution, start_slice_number)
            ...
            if self.cb_C2.isChecked():
                ...
                volume_2 = np.zeros(
                    (self.optical_slices*number_of_slices, size_y, size_z), dtype=np.float32)
                volume_2[0::self.optical_slices, :, :] = volume_2_temp[start_slice_number:end_slice_number+1,
                ...
                for optical_slice in range(1, self.optical_slices):
                    ...
                    volume_2[optical_slice::self.optical_slices, :, :] = volume_2_temp[start_slice_number:end_slice_number+1,
                                                                                       start_y:start_y+size_y, start_z:start_z+size_z]
                ...
                self.aligned_2 = self.Align(
                    volume_2, resolution, output_resolution, start_slice_number)
            ...
            if self.cb_C3.isChecked():
                ...
                volume_3 = np.zeros(
                    (self.optical_slices*number_of_slices, size_y, size_z), dtype=np.float32)
                volume_3[0::self.optical_slices, :, :] = volume_3_temp[start_slice_number:end_slice_number+1,
                                                                       start_y:start_y+size_y, start_z:start_z+size_z]
                ...
                for optical_slice in range(1, self.optical_slices):
                    ...
                    volume_3[optical_slice::self.optical_slices, :, :] = volume_3_temp[start_slice_number:end_slice_number+1,
                                                                                       start_y:start_y+size_y, start_z:start_z+size_z]
                ...
                self.aligned_3 = self.Align(
                    volume_3, resolution, output_resolution, start_slice_number)
            ...
            if self.cb_C4.isChecked():
                ...
                volume_4 = np.zeros(
                    (self.optical_slices*number_of_slices, size_y, size_z), dtype=np.float32)
                volume_4[0::self.optical_slices, :, :] = volume_4_temp[start_slice_number:end_slice_number+1,
                                                                       start_y:start_y+size_y, start_z:start_z+size_z]
                ...
                for optical_slice in range(1, self.optical_slices):
                    ...
                    volume_4[optical_slice::self.optical_slices, :, :] = volume_4_temp[start_slice_number:end_slice_number+1,
                                                                                       start_y:start_y+size_y, start_z:start_z+size_z]
                ...
                self.aligned_4 = self.Align(
                    volume_4, resolution, output_resolution, start_slice_number)
    ...
    def add_polygon(self):
        ...
        new_shape = np.array(new_shape)
        output_resolution = float(self.pixel_size.text())
        shapes_layer = self.viewer.add_shapes(new_shape, shape_type='polygon', name = "Shapes", scale=(15, output_resolution, output_resolution),)
