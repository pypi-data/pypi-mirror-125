...
from typing import Optional, Callable, List, Coroutine, Any
...
class IngeniumAPI:
    ...
    _detected_sock_fn: Optional[Callable[[int, str], None]] = None
    ...
    def set_sock_detection(self, detected_fn: Callable[[int, str], Coroutine[Any, Any, None]], detected_socks: List[int]):
        self._detected_sock_fn = detected_fn
    ...
    async def load(self, just_login=False, hass_integration=False, debug=False):
        ...
        for o in login_result.get("devices"):
            ...
            if ctype.is_actuator():
                ...
                self._objects.append(IngNoiseSensor(self, self._is_knx, address, ctype, components))
