    ...
    async def update_state(self, p: Package) -> bool:
        ...
        if self.mode == ACTUATOR_BLIND:
            ...
            if p.data1 == 1:
                ...
                # Average
                #self.voltage = sum(self.voltages) // len(self.voltages)
                # Median
                self.voltage = sorted(self.voltages)[len(self.voltages)//2]
                #print("######### ", self.components[0].label, "NEW VOLTAGE: ", self.voltage, self.voltages, flush=True)
                ...
                if (time_diff > 100 and value_diff > 1) or value_diff > 3 or time_diff > 300:
                    ...
                    #print("################# ", self.components[0].label, "VOLTAGE SENT: ", self.voltage, self.voltages, flush=True)
                    return True
                return False
