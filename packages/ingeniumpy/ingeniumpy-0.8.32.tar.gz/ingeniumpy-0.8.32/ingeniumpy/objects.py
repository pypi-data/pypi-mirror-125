    ...
    async def maybe_detect_sock(self):
        if not self.is_sock:
            ...
            if self.api._detected_sock_fn is not None:
                await self.api._detected_sock_fn(self.address, self.components[0].label)
        ...
        if self.mode == ACTUATOR_BLIND:
            for c in self.components:
                if c.output < 2 and p.data1 == 4:
                    old_value = self.blind1
                    self.blind1 = p.data2
                    return old_value != self.blind1
                elif 2 <= c.output < 4 and p.data1 == 5:
                    old_value = self.blind2
                    self.blind2 = p.data2
                    return old_value != self.blind2
                elif 4 <= c.output < 6 and p.data1 == 6:
                    old_value = self.blind3
                    self.blind3 = p.data2
                    return old_value != self.blind3
                elif 6 <= c.output < 8 and p.data1 == 7:
                    old_value = self.blind4
                    self.blind4 = p.data2
                    return old_value != self.blind4
                return False
            ...
            if p.data1 == 1:
                ...
                self.voltages.append(p.data2)  
                if len(self.voltages) > 8:     
                    self.voltages.pop(0)
                ...
                # Average
                #self.voltage = int(round(sum(self.voltages) / len(self.voltages)))
                # Median
                #self.voltage = sorted(self.voltages)[len(self.voltages)//2]
                print("######### ", self.components[0].label, "NEW VOLTAGE: ", self.voltage_ema, self.voltage, flush=True)
                ...
                if (time_diff > 100 and value_diff > 1) or value_diff > 3 or time_diff > 300:
                    ...
                    print("################# ", self.components[0].label, "VOLTAGE SENT: ", self.voltage_ema, self.voltage, flush=True)
                    return True
                return False
