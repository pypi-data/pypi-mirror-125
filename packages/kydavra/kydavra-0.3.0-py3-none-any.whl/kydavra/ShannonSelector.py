...
class ShannonSelector:
    ...
    def select(self, dataframe : 'pd.DataFrame', target : str) -> list:
        ...
        X_columns = [col for col in dataframe.columns if col != target]

        # The number of sample for every class in the data set
        target_dict = dict(dataframe[target].value_counts())

        # The tuple with the names of every class name in the data set
        unique_classes_names = tuple(dataframe[target].unique())
        ...
        for column in X_columns:
            ...
            nan_class_prob = dict(dataframe[dataframe[column].isna()][target].value_counts())
            ...
            no_nan_class_prob = dict(dataframe[dataframe[column].notna()][target].value_counts())
