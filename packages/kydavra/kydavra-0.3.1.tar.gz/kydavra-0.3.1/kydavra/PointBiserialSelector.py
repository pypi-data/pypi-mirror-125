...
from .errors import NotBetweenZeroAndOneError, NonNumericDataError, NoSuchColumnError, MissingDataError
    ...
    def select(self, dataframe : 'pd.DataFrame', target : str) -> list:
        ...
        for numeric_col in numeric_cols_names:
            if numeric_col != target:
                corr_dict[numeric_col] = self.pointBiserialCorr(dataframe, numeric_col, target)
