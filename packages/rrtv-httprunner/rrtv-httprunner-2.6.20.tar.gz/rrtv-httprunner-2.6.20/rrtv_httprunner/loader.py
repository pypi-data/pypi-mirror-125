...
def load_custom_functions_default(start_path: Text = None, test_path: Text = None) -> Dict[Text, Callable]:
    """ load project custom module functions
    """
    try:
        batch_custom_functions = {}
        ...
        path = self_py_dir.replace("/", ".").split(".py")[0]
        mported_module = importlib.import_module(path)
        batch_custom_functions.update(load_module_functions(mported_module))
    except Exception as ex:
        logger.error(f"error occurred in custom.py: {ex}")
...
def load_project_meta(test_path: Text, reload: bool = False) -> ProjectMeta:
    ...
    if custom_path:
        ...
        if custom_functions == {}:
            custom_functions = load_custom_functions_default(project_root_directory, test_path)
        else:
            custom_functions.update(load_custom_functions_default(project_root_directory, test_path))
