...
class PipelineRunWrapper(SermosTool):
    ...
    def save_to_cache(self):
        ...
        # If we set a ttl to max task time then timeout will cause redis to drop
        # config w/ retry count before we get a chance to retry.
        # Here we set the pipeline config to expire at the max length of all the
        # tasks plus a buffer so we can properly handle error timeouts
