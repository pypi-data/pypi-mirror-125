...
def get_trade_days(region, start_date=None, end_date=None, count=None):
...
def get_security_info(region, types=None, code=None, date=None):
    if not settings.pg_static:
        raise Exception('auth failed')
    if region == 'cn':
        source = 'jq'
    ...
    if not date:
        if not code:
            sql = f'select * from {table_name} and {type_sql};'
