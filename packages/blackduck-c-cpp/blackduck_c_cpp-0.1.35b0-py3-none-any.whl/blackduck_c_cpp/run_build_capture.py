...
class RunBuildCapture(object):
    ...
    def __init__(self, my_args, log_path, blackduck_output_dir):
        ...
        self.cov_home = self.my_args.args.coverity_root
        ...
        self.run_modes = list(map(lambda mode: mode.lower(), map(str.strip, self.my_args.args.modes.split(","))))
    ...
    def __enter__(self):
        ...
        if self.use_offline_files and (self.offline_mode or not self.my_args.args.skip_build):
    ...
    def get_phase_status_dict(self):
        ...
        for p in phases:
            if 'Blackuck-c-cpp Status Report' not in p:
                ...
                if error_matches:
                    phase_dict[p] = 'ERROR - SEE LOGS FOR DETAILS'.format('\n'.join(error_matches))
        return phase_dict
    ...
    def run_build(self, cov_emit_link_path, cov_build_path):
        ...
        conf_cmd += conf_key + util.conf_cmd("--template --compiler *g++* --comptype gcc", self.cov_bin, cov_conf)

        conf_cmd += conf_key + util.conf_cmd("--template --compiler *g++* --comptype g++", self.cov_bin, cov_conf)

        conf_cmd += conf_key + util.conf_cmd("--template --compiler ld --comptype gcc", self.cov_bin, cov_conf)
        conf_cmd += conf_key + util.conf_cmd("--template --compiler *-ld --comptype gcc", self.cov_bin, cov_conf)
        conf_cmd += conf_key + util.conf_cmd("--template --compiler ^ld-* --comptype gcc", self.cov_bin, cov_conf)
        ...
        if os.path.exists(cov_emit_link_path):
            self.bld_cmd = '"{}" --emit-link-units -c "{}" --dir "{}" "{}"'.format(cov_build_path, os.path.join(cov_conf, "bld.xml"), self.cov_dir, sh_path)
        else:
            self.bld_cmd = '"{}" -c "{}" --dir "{}" "{}"'.format(cov_build_path, os.path.join(cov_conf, "bld.xml"), self.cov_dir, sh_path)
    ...
    def run_get_package_manager_bom(self, emit_cov_header_files, header_files_set, linker_files_set, executable_files_set):
        """
    ...
    def run_build_capture(self):
        ...
        # COV BUILD
        if not self.my_args.args.skip_build:
...
def run():
    ...
    """
    my_args = C_Parser()
    blackduck_output_dir = my_args.args.output_dir if my_args.args.output_dir else os.path.join(expanduser("~"), ".synopsys", "blackduck-c-cpp", "output", my_args.args.project_name)
