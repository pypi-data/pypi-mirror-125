...
class BuiltinMacro:
    ...
    @staticmethod
    def comment_macro(tree_slice: list, ev_func, identity_token) -> list:
        """
        ...
        :param identity_token: in case we need to wrap in Identifier
        :return: converted slice of tree (as this would be expected)
        """

        # TODO: fake for now, it just returns an 'identity' function

        return [identity_token, identity_token]

    @staticmethod
    def fn_macro(tree_slice: list, ev_func, identity_token) -> list:
        """
        ...
        parameters_vector, function_body = tree_slice
    ...
    @staticmethod
    def if_macro(tree_slice: list, ev_func, identity_token) -> list:
        """
        ...
        if not isinstance(condition, list):
            condition = [identity_token, condition]
        if not isinstance(true_branch, list):
            true_branch = [identity_token, true_branch]
        if not isinstance(else_branch, list):
            else_branch = [identity_token, else_branch]
...
class Builtin:
    ...
    def plus_func(args: (List[datatypes.IntegerNumber]
                         or List[datatypes.FloatNumber])) -> (datatypes.IntegerNumber or datatypes.FloatNumber):
        """
        Builtin '+' function implementation
...
class Library:
    ...
    macro: dict = {'if': {'macro_name': 'if', 'macro_reference': BuiltinMacro.if_macro},
                   'fn': {'macro_name': 'fn', 'macro_reference': BuiltinMacro.fn_macro},
...
class Help:
    ...
    docstrings: List[str] = [
        ...
        '(fn [... function parameters vector ...] ... function body ...) -> Function: result',
