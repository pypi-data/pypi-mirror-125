...
from nanamilang.shortcuts import ASSERT_TYPE_OF_A, ASSERT_NOT_EMPTY_COLLECTION
...
class Formatter:
    ...
    def __init__(self, tokenized: List[Token]) -> None:
        ...
        ASSERT_TYPE_OF_A(tokenized, list)
    ...
    def format(self) -> str:
        ...
        def space(idx: int, token: Token) -> str:
            if idx > 0:
                if (token.type() in datatypes.DataType.simple_types
                    and self._tokenized[idx-1].type() in datatypes.DataType.simple_types) or \
                        (token.type() is Token.ListBegin
                         and self._tokenized[idx-1].type() in datatypes.DataType.simple_types) or \
                        (token.type() == Token.ListBegin
                         and self._tokenized[idx-1].type() == Token.ListEnd) or \
                        (token.type() in datatypes.DataType.simple_types
                         and self._tokenized[idx-1].type() in Token.ListEnd):
                    return' '
            return ''
        ...
        def symbol(idx: int, token: Token) -> str:
            ...
            elif token.type() in datatypes.DataType.simple_types:
                if token.dt().name == datatypes.Undefined.name:
                    return token.dt().reference()
