...
from nanamilang.shortcuts import (
    ASSERT_TYPE_OF_A, ASSERT_NOT_EMPTY_STRING,
...
class Tokenizer:
    ...
    _function_name_pattern: str = r'^\D\S*$'
    ...
    def _value_dispatched_or_invalid(self, value: str) -> Token:
        ...
        # Ok, then it must be a Token.Function?
        elif re.match(self._function_name_pattern, value):
            return Token(Token.Function, value)
    ...
    def _get_next_token(self) -> List[Token]:
        ...
        # Matching a Token.ListBegin and Token.Function (make-vector)
        elif self._curr_symbol() == '[':
            return [Token(Token.ListBegin), Token(Token.Function, 'make-vector')]
        # Matching a Token.ListEnd
        elif self._curr_symbol() == ']':
            return [Token(Token.ListEnd)]
        # Matching a Token.ListBegin and Token.Function (make-hashmap)
        elif self._curr_symbol() == '{':
            return [Token(Token.ListBegin), Token(Token.Function, 'make-hashmap')]
        # Matching a Token.ListEnd
        elif self._curr_symbol() == '}':
            return [Token(Token.ListEnd)]
        # Matching a Token.ListBegin and Token.Function (make-set)
        elif self._curr_sym_is('#'):
            if self._has_next_symbol():
                if self._next_sym_is('{'):
                    self._pointer += 1
                    return [Token(Token.ListBegin), Token(Token.Function, 'make-set')]
