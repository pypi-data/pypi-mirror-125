...
from nanamilang.shortcuts import (
    ASSERT_IN, ASSERT_TYPE_OF_A, ASSERT_NOT_EMPTY_STRING
)
...
class Token:
    ...
    ListEnd: str = 'ListEnd'
    Function: str = datatypes.Function.name
    ...
    _dt_types: List[str] = datatypes.DataType.simple_types

    _valid_types: List[str] = [Invalid, ListBegin, ListEnd] + _dt_types
    ...
    def __init__(self,
        ...
        if _type in self._dt_types:
            if _type != Token.Function:
                self._dt_instance = (
                    datatypes.DataType.resolve(_type)(_value)
                )
            else:
                resolved = Builtin.resolve(_value)
                self._dt_instance = (
                    datatypes.Function(resolved)
                    if resolved else datatypes.Undefined(_value)
                )
        # Validate and store passed values for other Token private fields
        if _valid:
            ASSERT_TYPE_OF_A(_valid, bool)
            self._valid = _valid
        if _reason:
            ASSERT_TYPE_OF_A(_reason, str)
            ASSERT_NOT_EMPTY_STRING(_reason)
            self._reason = _reason
        if _position:
            ASSERT_TYPE_OF_A(_position, int)
            self._position = _position
        if _raw_symbol:
            ASSERT_TYPE_OF_A(_raw_symbol, str)
