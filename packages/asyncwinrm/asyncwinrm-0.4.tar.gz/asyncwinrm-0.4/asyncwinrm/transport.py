...
from asyncwinrm.exceptions import InvalidCredentialsError, WinRMError, WinRMTransportError
from asyncwinrm.encryption import Encryption
...
HAVE_KERBEROS = False
try:
    from asyncwinrm.vendor.requests_kerberos import HTTPKerberosAuth, REQUIRED
...
class Transport(object):
    ...
    def build_session(self):
        ...
        if hasattr(self, 'verify') and session.verify:
            if self.ca_trust_path == 'legacy_requests' and settings['verify'] is not None:
                ...
                if not DISPLAYED_CA_TRUST_WARNING and session.verify is not True:
                    ...
                    if os.environ.get('REQUESTS_CA_BUNDLE') is not None:
                        message += " REQUESTS_CA_BUNDLE contains {ca_path}".format(
                            ca_path=os.environ.get('REQUESTS_CA_BUNDLE'))
        ...
        # TODO: ssl is not exactly right here- should really be client_cert
            ...
            session.auth = BasicAuth(username=self.username, password=self.password)
    ...
    async def send_message(self, message):
        ...
        if self.encryption:
            ...
            prepared_request = self.session.build_request('POST', self.endpoint, data=message)
            # request = requests.Request('POST', self.endpoint, data=message)
            # prepared_request = self.session.prepare_request(request)

        response = await self._send_message_request(prepared_request, message)
        return self._get_message_response_text(response)

    async def _send_message_request(self, prepared_request, message):
        try:
            response = await self.session.send(prepared_request, timeout=self.read_timeout_sec)
