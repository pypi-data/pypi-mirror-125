...
class SegmentationAlbumentationsDataLoader:
    ...
    def __init__(self, dataset_path, precache=False, train_augmentations=None, val_augmentations=None, test_augmentations=None, images_dir='images', masks_dir='annotations', width=512, height=512, batch_size=16, num_classes=2, mask_downsample=1, train_val_test_split=[0.8, 0.1, 0.1], buffer_size=4, label_shift=0, normalization='standard'):
    ...
    def open_images(self, image, label):
        ...
        lbl = tf.cast(tf.image.resize(
            lbl, [self.height/self.mask_downsample, self.width/self.mask_downsample]), np.uint8)[:, :, 0]
    ...
    def set_shapes(self, img, label):
        ...
        if self.normalization == 'standard':
            img = img / 255.
        elif self.normalization == 'mobilenet':
            img = img / 127.5 - 1
