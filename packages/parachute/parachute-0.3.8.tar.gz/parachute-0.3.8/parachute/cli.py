    ...
    with TablePrinter("Parameter name", backup_name, backup2_name) as p:
        ...
        if not only_craft:
            ...
            for parameter_name in all_parameters - seen_parameters:
                p(parameter.name, backup.parameters[parameter_name].value, "-")
...
@option_decorator
def restore(
    backup_file: TextIO, filter: Optional[re.Pattern], socket: str, baud_rate: int
):
...
@option_decorator
def get_params(params: Iterable[str], socket: str, baud_rate: int):
    click.echo("Retrieving parameters...", err=True)
    craft = CraftCommunication(socket, baud_rate)
...
def set_params(params: Iterable[str], socket: str, baud_rate: int):
    ...
    with TablePrinter("Parameter name", "Old value", "New value") as p:
        for param in params:
            if "=" not in param:
                click.echo(
                    f"Wrong parameter syntax: {param} (should be NAME=VALUE).", err=True
                )
                continue
