...
class ExtendedModel(AbstractObject):
    ...
    def print_summary(self, suppress_print:bool=False, save_as:Optional[str]=None):
        summary_str = ""
        # workspace
        ...
        summary_str += f"Datasets:\n"
        summary_str += "\t" + ", ".join([d.GetName() for d in datasets]) + "\n"
        # categories
        if isinstance(self.pdf, ROOT.RooSimultaneous):
            channel_cat = self.pdf.indexCat()
            n_cat = len(channel_cat)
            summary_str += f"Categories:\n"
            for i in range(n_cat):
                channel_cat.setIndex(i)
                label = channel_cat.getLabel()
                summary_str += f"\t{label}\n"
        ...
        unconstrained_nps = self.get_unconstrained_nuisance_parameters()
        param_strs     = ['POIs', 'Constrained NPs', 'Unconstrained NPs']
        param_sets     = [pois, constrained_nps, unconstrained_nps]
        for pstr, pset in zip(param_strs, param_sets):
            ...
            for p in pset:
                ...
                vmin   = vrange[0]
                vmax   = vrange[1]
                const_str = "C" if p.isConstant() else "F"
