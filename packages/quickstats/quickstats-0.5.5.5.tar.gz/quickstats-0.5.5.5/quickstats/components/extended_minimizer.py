...
class ExtendedMinimizer(object):
    ...
    def __init__(self, minimizer_name:str, pdf:"ROOT.RooAbsPdf", data:"ROOT.RooAbsData",
        ...
        _PRINT_.info('INFO: Created ExtendedMinimizer("{}") instance'.format(self.name))
    ...
    def minimize(self, nll=None, **kwargs):
        ...
        if not perform_minimization:
            _PRINT_.info('INFO: ExtendedMinimizer::minimize("{}") no floating parameters found'
        ...
        if self.config['save']:
            ...
            _PRINT_.info('INFO: ExtendedMinimizer::minimize("{}") saving results as {}'.format(self.name, save_name))
    ...
    def robust_minimize(self):
        ...
        while ((status != 0) and (status!=1) and (strategy<2) and (retry>0)):
            strategy += 1
            retry -= 1
            _PRINT_.error('ExtendedMinimizer::robust_minimize("{}") fit failed with status {}. '
                          'Retrying with strategy {}'.format(self.name, status, strategy))
            self.minimizer.setStrategy(strategy)
            status = self.minimizer.minimize(minimizer_type, minimizer_algo)
        
        if status not in [0, 1]:
            _PRINT_.error('ExtendedMinimizer::robust_minimize("{}") fit failed with status {}'.format(self.name, status))
    ...
    def use_limits(self, par:"ROOT.RooRealVar", val:float):
        if (val < par.getMin()):
            _PRINT_.warning('ExtendedMinimizer::use_limits("{}") {} = {} limited by minimum at {}'.format(
                            self.name, par.GetName(), val, par.getMin()))
            return par.getMin()
        elif (val > par.getMax()):
            _PRINT_.warning('ExtendedMinimizer::use_limits("{}") {} = {} limited by maximum at {}'.format(
    ...
    def find_sigma(self):
        ...
        for v in self.scan_set:
            ...
            _PRINT_.info('ExtendedMinimizer::minimize("{}")'.format(self.name))        
            _PRINT_.info(v)
        ...
        for i in range(max_iter):
            ...
            for guess in guess_to_corr:
                if (abs(guess - val_pre) < direction*val_pre*0.02):
                    damping_factor *= 0.8
                    _PRINT_.info('{} Changing damping factor to {}'.format(front_str, damping_factor))
                    n_damping += 1
                    if n_damping > 10:
                        n_damping = 1
                        damping_factor = 1.0
            ...
            _PRINT_.info('{} {} {:.3f} (MLE {:.3f}) -> {:.3f} (MLE {:.3f}), '
            ...
            _PRINT_.info('{} NLL:                 {}'.format(front_str, nll))
            _PRINT_.info('{} delta(NLL):          {}'.format(front_str, nll - nll_min))
            _PRINT_.info('{} nsigma*sigma(pre):   {}'.format(front_str, abs(val_pre - val_mle)))
            _PRINT_.info('{} sigma(guess):        {}'.format(front_str, sigma_guess))
            _PRINT_.info('{} par(guess):          {}'.format(front_str, val_guess + corr))
            _PRINT_.info('{} best-fit val:        {}'.format(front_str, val_mle))
            _PRINT_.info('{} tmu:                 {}'.format(front_str, t_mu))
            _PRINT_.info('{} Precision:           {}'.format(front_str, direction*val_guess*precision))
            _PRINT_.info('{} Correction:          {}'.format(front_str, -corr))
            _PRINT_.info('{} nsigma*sigma(guess): {}'.format(front_str, abs(val_guess-val_mle)))
            
            if abs(delta) <= rel_precision:
                break
        if i >= max_iter:
            _PRINT_.error('ERROR: find_sigma failed after {} iterations'.format(i+1))
            return ROOT.TMath.QuietNan()
        
        err = val_guess - val_mle
        _PRINT_.info('{} {} {}sigma = {:.3F} at -2lnL = {:4f} after {} iterations'.format(
