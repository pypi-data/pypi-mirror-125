...
def is_package_available(p):
    ...
    try:
        ...
        # print2("Warning: cannot import %r: %s" % (p, e))
        return False
...
for module, info in list(index["packages"].items()):
    for requirement, options in list(info["requires"].items()):
        if not any([is_package_available(op) for op in options]):
            print2(("For package %r, dependency %r cannot be satisfied." % (module, requirement)))
...
if missing:
    print2("\n\n")
    print2("Dependency search problems summary")
    print2("----------------------------------\n")
    print2("I could not find the following packages installed:\n")
...
for p in packages:
    ...
    if not p in index["packages"]:
        print2(("Warning: packages %r is not in the autogenerated index." % p))
