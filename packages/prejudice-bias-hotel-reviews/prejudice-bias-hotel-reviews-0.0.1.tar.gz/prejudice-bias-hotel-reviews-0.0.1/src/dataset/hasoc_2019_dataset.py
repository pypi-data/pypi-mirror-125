...
class HASOC2019Dataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 SEED=42):
        """
        Initializes the HASOC2019Dataset dataset.
        ...
        # load the dataset
        self.df_train = pd.read_csv(self.dataset_path / 'german_dataset.tsv',
                                    sep='\t')
        self.df_train = self.df_train.drop(columns=['text_id', 'task_2'])
        self.df_test = pd.read_csv(self.dataset_path /
                                   'hasoc_de_test_gold.tsv',
                                   sep='\t')
        self.df_test = self.df_test.drop(columns=['text_id', 'task_2'])
