...
class TrajectoriesAPI(BaseAPI):
    ...
    def list_data(self, requests: List[TrajectoryDataRequest]) -> TrajectoryDataList:
        """Get multiple trajectory data by a list of TrajectoryDataRequest

        Args:
            requests (List[TrajectoryDataRequest]): list of trajectory data requests
        Returns:
            TrajectoryDataList: List of TrajectoryData objects
        """
        trajectory_data_request_items = TrajectoryDataRequestItems(items=requests)
    ...
    def interpolate(
        ...
        *,
        ...
        The interpolation uses the minimum curvature metho also between
        trajectory points.

        Args:
            meausured_depths (List[float]): List of measured depths.
