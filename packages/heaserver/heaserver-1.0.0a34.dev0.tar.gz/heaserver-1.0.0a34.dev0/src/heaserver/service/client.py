...
from heaserver.service.aiohttp import StreamReaderWrapper, SupportsAsyncRead
...
async def get_streaming(request: web.Request, url: Union[str, URL], headers: Optional[Dict[str, str]] = None) -> web.StreamResponse:
    ...
    async with session.get(url, headers=headers, raise_for_status=False) as response_:
        if response_.status == 200:
            ...
            r = web.Response(status=response_.status)
            r.raise_for_status()
            return r
...
async def get_resource_url(app: web.Application, type_or_type_name: Union[str, Type[root.HEAObject]]) -> Optional[str]:
    ...
    url = URL(app[appproperty.HEA_REGISTRY]) / 'components' / 'bytype' / type__
