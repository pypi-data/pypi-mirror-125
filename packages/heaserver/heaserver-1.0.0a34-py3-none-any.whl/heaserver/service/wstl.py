...
properties:
{
  ...
  HEA prefixes custom relation values with "hea" and defines the following custom values:
    ...
    hea-opener-editable: a link to the content of an openable HEADesktopObject in a media type that supports editing.
    Include MIME types in the link relation values to indicate the media types that are available. For most openables,
    there will be one media type (the storage format).
    hea-opener-view: a link to read-only views of an openable HEADesktopObject. Include MIME types in the link
    relation values to indicate the media types of the views that are available. An example is a PDF view of a
    text document. There would be a hea-opener-editable link to the actual text document, and a hea-opener-view link
    to a PDF of it. Some openables might only have read-only views and no editable form.
}
