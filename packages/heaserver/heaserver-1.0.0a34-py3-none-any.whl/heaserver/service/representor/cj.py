...
from typing import Mapping, Any, Union, Dict, List, Optional, Type, Tuple, Generator, Callable
...
def _get_links(actions: Dict[str, Any], tvars: Dict[str, Any]=None, link_callback: Callable[[int, Link], None]=None):
    """
    Get top-level links.
...
def _get_queries(actions: Dict[str, Any]):
    rtn = []
...
def _get_template(actions: Dict[str, Any], tvars: Dict[str, Any]):
    rtn = {}
    for action in actions:
...
def _get_item_links(actions: Dict[str, Any], tvars: Dict[str, Any], link_callback: Callable[[int, Link], None]=None):
    coll = []
...
def _get_item_link(actions: Dict[str, Any], link_callback: Callable[[int, Link], None]=None):
    rtn = {}
    for i, action in enumerate(actions):
        target = action['target']
