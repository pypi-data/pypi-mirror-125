...
from coegil.Helpers import put_s3_object, get_s3_object
...
def generate_temporary_credentials(duration_hours: int):
    result = api_save('PUT', '/public/management/key/temporary', {
...
def invoke_schedule(asset_id: str, job_name: str, schedule_name: str, variable_override: str):
    api_save('PUT', '/public/schedule/queue', {
        'asset_id': asset_id,
        'job_name': job_name,
...
@click.option('--asset_id', required=True, help="The asset's unique identifier.")
def get_schedule_status(asset_id: str):
    result = api_get('/public/schedule', {
        'asset_id': asset_id
    })
    _write_to_std_out(result)
...
def download_file(asset_id: str):
    artifacts = api_get('/public/artifact', {
        'asset_id': asset_id
    })
    _write_to_std_out(artifacts)
...
def download_file(asset_id: str, file: str, artifact_name: str):
    ...
    directory = os.path.join(os.path.dirname(file))
    if not os.path.exists(directory):
        os.makedirs(directory)
    with open(file, 'wb+') as f:
        contents = get_s3_object(s3_bucket, s3_key, credential_override=credential_override)
        f.write(contents)
