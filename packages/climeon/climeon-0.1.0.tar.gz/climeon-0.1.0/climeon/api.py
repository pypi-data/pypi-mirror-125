...
OFFLINE_FOLDER = path.join(getenv("APPDATA"), "ClimeonLive")
OFFLINE_NAME = "%s_%s_%s_%s_%s.parquet"
INVALID_CHARS = ""
...
class Client():
    ...
    def login(self):
        ...
        if self.user and self.passwd:
            ...
            self.logger.debug("Silent login with user %s", self.user)
        ...
        if not result:
            ...
            self.user = result["id_token_claims"]["emails"][0]
        if "error" in result:
    ...
    def tsi_query(self, machine_id, date_from, date_to, variables, interval):
        ...
        body = {
            "aggregateSeries": {
                ...
                "interval": interval,
                "projectedVariables": list(variables.values())
            }
        }
    ...
    def get_log_file(self, machine_id, date):
        ...
        if response.status_code == 404:
            error = f"No logfile found for {machine_id} and date {date.date()}"
    ...
    def download_logfile(self, machine_id, date, directory):
        ...
        log_path = f"{directory}/{machine_id}_{date}.csv"
        with open(log_path, mode="w+", encoding="utf-8") as file_stream:
            file_stream.write(log_file)
        return log_path

    def get_log_data(self, machine_id, date_from, date_to, variables=None):
        ...
        date_list = [date_from + timedelta(days=d) for d in range(diff.days + 1)]
...
def offline_name(machine_id, date_from, date_to, variables, interval):
    ...
    var = "".join(c for c in variables)
    name_raw = OFFLINE_NAME % (machine_id, date_from, date_to, var, interval)
    filename = "".join(c for c in name_raw if c not in "' []-:/%")
