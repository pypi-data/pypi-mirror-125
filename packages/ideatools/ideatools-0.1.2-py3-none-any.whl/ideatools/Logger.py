...
class Logger(object):
    ...
    def __init__(self, func=None, path=None, file_name=None,
        ...
        if logger_out_path == None:
            if path == "" or path == None:
                default_file_name = os.path.basename(__file__)
                now_time = datetime.datetime.now().strftime('%Y-%m-%d')
                path = str(__file__).replace(default_file_name, "")
