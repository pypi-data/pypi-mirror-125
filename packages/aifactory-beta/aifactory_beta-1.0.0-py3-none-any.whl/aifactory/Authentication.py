...
class AFAuth():
    ...
    def pack_submit_key(self, params: dict):
        submit_key = None
        if self.encrypt_mode:
            submit_key = self.crypt.encrypt_aes(self.submit_key, VERSION)
    ...
    def _check_submit_key_validity_(self):
        params = {AUTH_REQUEST_KEYS.AUTH_METHOD: self.auth_method,
                  AUTH_REQUEST_KEYS.VERSION: VERSION}
    ...
    def get_submit_key(self, num_trial=0, refresh=False):
        ...
        params = {AUTH_REQUEST_KEYS.AUTH_METHOD: self.auth_method, AUTH_REQUEST_KEYS.VERSION: VERSION,
        ...
        if response.text == AUTH_RESPONSE.KEY_NOT_VALID:
            ...
            self.submit_key = self.crypt.decrypt_aes(submit_key_response[AUTH_REQUEST_KEYS.SUBMIT_KEY], VERSION)
