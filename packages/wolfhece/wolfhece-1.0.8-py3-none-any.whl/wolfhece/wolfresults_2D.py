...
class Wolfresults_2D(object):
    ...
    def __init__(self,fname = None,mold = None):
        ...
        if fname is not None:
            self.filename = fname
            wolfpy.r2d_init(fname.ljust(255))
    ...
    def read_oneresult(self,which=-1):
        for i in range(self.nb_blocks):
            ...
            ma.masked_equal(self.myblocks['block'+str(i+1)].waterdepth.array,0.)
            ma.masked_where(self.myblocks['block'+str(i+1)].waterdepth.array==0.,self.myblocks['block'+str(i+1)].qx.array)
            ma.masked_where(self.myblocks['block'+str(i+1)].waterdepth.array==0.,self.myblocks['block'+str(i+1)].qy.array)
