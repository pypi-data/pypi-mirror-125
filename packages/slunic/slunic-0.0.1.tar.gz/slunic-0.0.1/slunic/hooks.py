...
_slunic_hooks = {}
_searched_for_slunic_hooks = False
...
def register(hook_name, fn=None, order=0):
    ...
    if hook_name not in _slunic_hooks:
        _slunic_hooks[hook_name] = []
    _slunic_hooks[hook_name].append((fn, order))


def search_for_hooks():
    global _searched_for_slunic_hooks
    if not _searched_for_slunic_hooks:
        list(get_app_submodules("slunic_hooks"))
        _searched_for_slunic_hooks = True


def get_hooks(hook_name):
    """Return the hooks function sorted by their order."""
    search_for_hooks()
    hooks = _slunic_hooks.get(hook_name, [])
