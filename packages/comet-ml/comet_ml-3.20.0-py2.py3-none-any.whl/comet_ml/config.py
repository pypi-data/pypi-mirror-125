...
DEFAULT_WS_JOIN_TIMEOUT = 10

DEFAULT_PARAMETERS_UPDATE_INTERVAL_SECONDS = 60
...
CONFIG_MAP = {
    ...
    "comet.internal.file_upload_worker_ratio": {"type": int, "default": 4},
    # Deprecated:
...
def _init(directory="~/", should_prompt_user=None, settings=None):
...
def init_onprem(force=False):
    ...
    if root_url == "https://www.comet.ml/" or force:
        LOGGER.info(
            "For help on these settings, please see: https://www.comet.ml/docs/onprem/"
        )

        client_url = _input_user("Please enter your onprem COMET_URL_OVERRIDE: ")

        optimizer_url = _input_user("Please enter your onprem COMET_OPTIMIZER_URL: ")

        # Set the environment variables, config.save will save them in the local config file
        os.environ["COMET_OPTIMIZER_URL"] = optimizer_url
        os.environ["COMET_URL_OVERRIDE"] = client_url

    directory = "~/"

    # TODO: We currently force the user confirmation but it might hangs
    config.save(directory=directory, _prompt_user_confirmation=True, force=force)
