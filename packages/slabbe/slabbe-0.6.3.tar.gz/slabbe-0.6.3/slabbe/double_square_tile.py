...
class DoubleSquare(SageObject):
    ...
    def tikz_tiling(self, nx=2, ny=2, kind=1, rectangle=None, clip=None):
        r"""
...
# Christoffel tiles
###############################
def christoffel_tile(p, q):
    r"""
    Returns the `(p,q)` Christoffel Tile [BBGL2011]_.
    ...
    w = words.ChristoffelWord(p, q, alphabet=[0,1])
    w = WP(list(w))
