...
class YouTubeMusicAPI():
    ...
    def __init__(self, query : str) -> None:
        ...
        self.base_url = f"https://music.youtube.com/search?q={query}"
        self.playlist_url = f"https://www.youtube.com/results?search_query={query}&sp=EgIQAw%253D%253D"
        
    def track(self):
        ps = get(self.base_url, headers=self.user_agent).content.decode("unicode_escape")
        id = search('"videoId":".{11}"', ps)
    ...
    def playlist(self):
        ps = get(self.playlist_url, headers=self.user_agent).content.decode("utf-8")
        id = search('"playlistId":".{34}"', ps)
