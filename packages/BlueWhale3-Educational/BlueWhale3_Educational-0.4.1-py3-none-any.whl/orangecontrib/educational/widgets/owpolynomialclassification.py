...
class OWPolynomialClassification(OWBaseLearner):
    ...
    def add_main_layout(self):
        box = gui.widgetBox(self.controlArea, __("box.var"))
        gui.comboBox(
        ...
        gui.comboBox(
            ...
            callback=self._on_attr_changed)
        gui.spin(
            box, self, value='degree', label=__("row.polynomial_expansion"),
            minv=1, maxv=5, step=1, alignment=Qt.AlignRight, controlWidth=70,
            callback=self._on_degree_changed)
        gui.comboBox(
            box, self, 'target_class', label=__("row.target"),
        ...
        gui.checkBox(
            box, self.graph, 'show_legend', __("checkbox.show_legend"),
            callback=self.graph.update_legend_visibility)
        gui.checkBox(
            box, self, 'contours_enabled', label=__("checkbox.show_contour"),
    ...
    def select_data(self):
        ...
        new_class = DiscreteVariable(
            old_class.name + "'",
            values=(values[1 - target_idx] if len(values) == 2 else __("msg.others"),
                    self.target_class),
    ...
    def send_report(self):
        ...
        name = "" if self.degree == 1 \
            else __("report.polynomial_expansion").format(self.degree)
