...
class OWGradientDescent(OWWidget):
    ...
    class Error(OWWidget.Error):
        num_features = Msg(__("msg.num_features"))
        no_class = Msg(__("msg.no_class"))
        no_class_values = Msg(__("msg.no_class_values"))
        no_nonnan_data = Msg(__("msg.no_nonnan_data"))
        same_variable = Msg(__("msg.same_variable"))
    ...
    def __init__(self):
        ...
        self.auto_play_button_text = [__("btn.run"), __("btn.stop")]
        self.auto_play_thread = None

        self.var_model = DomainModel(valid_types=ContinuousVariable,
                                     order=DomainModel.ATTRIBUTES)

        self.options_box = gui.widgetBox(self.controlArea, __("box.variables"))
        opts = dict(
        ...
        gui.comboBox(value='target_class', label=__("row.target"),
        ...
        self.stochastic_checkbox = gui.checkBox(
            ...
            label=__("row.stochastic_descent"))
        ...
        self.step_button = gui.button(
            hbox, self, callback=self.step, label=__("btn.step"), default=True)
        self.step_back_button = gui.button(
            hbox, self, callback=self.step_back, label=__("btn.step_back"))
    ...
    def help_event(self, event):
        ...
        if opt_points.size:
            ...
            if hovereds:
                ...
                label.setHtml(__("caption.cost").format(hovered_line.value))
            else:
                cost = self.learner.j(np.array([xc, yc]))
                label.setHtml(__("caption.cost_xy").format(xc, yc, cost))
    ...
    def _format_label(self, x, y, step):
        return __("caption.cost_step").format(step, x, y, self.learner.j(np.array([x, y])))
