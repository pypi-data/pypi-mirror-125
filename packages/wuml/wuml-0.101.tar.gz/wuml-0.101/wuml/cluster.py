	...
	def plot_scatter_result(self, dimension_reduction_method='all'):
		...
		if dimension_reduction_method == 'all':
			for i, m in enumerate(methods):
				title = '%d Clusters via %s'%(self.n_clusters, m)
				ϰ = wuml.dimension_reduction(self.X, method=m)
				if i == 0:
					plts = wplotlib.plot_clusters(ϰ, self.labels, title=title, subplot=131+i, figsize=(10,3))
				else:
					plts = wplotlib.plot_clusters(ϰ, self.labels, title=title, subplot=131+i)
