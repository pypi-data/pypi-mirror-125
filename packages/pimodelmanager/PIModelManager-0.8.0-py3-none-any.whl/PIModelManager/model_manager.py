...
class ModelManager:
        ...
        for tenant_name in cls._tenant_configs:
            for model_name in cls._tenant_configs[tenant_name]:
                if model_name in cls._model_class_mapping:
                    ...
                    logger.info(f"class mapping not present for model {model_name}")
            ...
            for language in cls._models[model_name]:
                for version_model in cls._models[model_name][language]:
                    ...
                    if not used:
                        ...
                        models_to_delete.append([model_name, language, version_model])
    ...
    def _initialize_model_from_config(cls, config: Dict[str, str]) -> None:
        ...
        if not cls._models[model_name][language][version]:
            ...
            logger.info(f"Finished initializing {model_name} model with {config}")

    @classmethod
    def set_credentials(
        cls,
        grant_type: Union[str, None] = os.environ.get("MODEL_MANAGER_GRANT_TYPE"),
        client_id: Union[str, None] = os.environ.get("MODEL_MANAGER_CLIENT_ID"),
        client_secret: Union[str, None] = os.environ.get("MODEL_MANAGER_CLIENT_SECRET"),
        scope: Union[str, None] = os.environ.get("MODEL_MANAGER_SCOPE"),
        container: Union[str, None] = os.environ.get("MODEL_MANAGER_CONTAINER"),
        login_url: Union[str, None] = os.environ.get("MODEL_MANAGER_LOGIN_URL"),
        get_files_url: Union[str, None] = os.environ.get("MODEL_MANAGER_GET_FILES_URL"),
    ...
    async def get_model(
        ...
        **kwargs,
        ...
        if path:
            return await cls.get_model_path(
            ...
            try:
                if isinstance(
                    ...
                    return await model.remote(**kwargs)
                else:
        ...
        if model["model_type"] == "spacy":
            ...
            if name and version:
                if cls.__pip_package_is_available(name, version):
                    ...
                    logger.info(f"Downloading package {name}-{version} from {url}")
                    ...
                    if isinstance(class_instance, ray.serve.api.Deployment):
                        logger.info(f"Model {model} obtained through Ray deployment.")
                    else:
    ...
    def upload_model(
        ...
        if path.exists():
            ...
            files = {"file": (final_path.name, open(final_path, "rb"), mimetype)}
