from src.replifactory import read_csv_tail
import numpy as np
...
def calculate_growth_rate(time_values, od_values):
    ...
    """
    assert 1 < time_values[-1] < 1e10  # must be time in seconds since 1970-01-01 00:00:00

    if len(time_values) < 6:  # at least 6 data points required for reasonable measurement.
        return np.nan, np.nan, np.nan

    if np.diff(time_values).max() > 30*60:  # half an hour gap in the data, major problem.
        return np.nan, np.nan, np.nan

    # denoise, trim
    if len(time_values) >= 10:
        median_window = int(len(time_values) / 8)+1
        od_values = pd.Series(od_values).rolling(median_window * 2, center=True).median()
...
def calculate_last_growth_rate(t, od):
    ...
    for _ in range(4):
        ...
        new_window_size = int(abs(td) * 60 * 0.5)  # optimum window size [minutes] is ~half the doubling time
...
def adaptive_window_doubling_time(t, od, dilution_timepoints):
    timepoints, growth_rates, gr_errors = adaptive_window_growth_rate(t, od, dilution_timepoints)
    t_doubling = np.log(2) / growth_rates
    t_doubling_error = t_doubling * gr_errors / growth_rates
    return timepoints, t_doubling, t_doubling_error
