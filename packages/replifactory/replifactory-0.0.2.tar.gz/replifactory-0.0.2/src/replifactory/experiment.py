import os
import schedule
from src.replifactory.loading import load_object
from src.replifactory import ExperimentWorker
_experiment = None
...
class Experiment:
    def __init__(self, directory=None, connect=False):
        ...
        if "_experiment" in globals().keys():
            if _experiment is not None:
                print(_experiment)
                print("An experiment is already initialized in this thread: '%s'" % _experiment.directory)
            ...
            if self.directory is not None:
                if os.path.exists(os.path.join(self.directory, "device_config.yaml")):
                    if dev.directory != self.directory:
                        raise RuntimeError("Device config %s already exists." % self.directory)
    ...
    def run(self):
        ...
        device.stirrers.set_speed_all(speed=1)
