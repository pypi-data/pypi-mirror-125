...
from src.replifactory import MorbidostatCulture
from src.replifactory import TurbidostatCulture
from src.replifactory import EnduranceStressCulture
from src.replifactory.culture.blank import BlankCulture
from src.replifactory import BatchCulture
from src.replifactory import PatientCulture
from src.replifactory import Experiment

import os
import glob


class ProgramViewerTab:
    title = "Program Viewer"

    def __init__(self, main_gui):
        self.main_gui = main_gui
        self.title = "Experiment config"
        ...
        self.widget = VBox([self.selector, self.vials])
    ...
    def make_vial_widgets(self):
        ...
        title = widgets.HTML('<b>Selected experiment directory:   %s:</b>' % self.main_gui.experiment.directory,
...
class VialConfigWidget:
    def __init__(self, program_viewer_tab, vial_number):
        ...
        culture_options = [BlankCulture, BatchCulture, TurbidostatCulture, MorbidostatCulture, EnduranceStressCulture,
                           PatientCulture]
        culture_options_names = [str(klass)[8:-2] for klass in culture_options]
        culture_options += [type(None)]
        culture_options_names += ["None"]
        ...
        if self.culture is not None:
            ...
            left_panel = VBox([HBox([title, self.blank, self.inoculate]), algorithm, description_box])
            self.widget = HBox([left_panel, self.culture.widget], layout=Layout(border='solid'))
        else:
