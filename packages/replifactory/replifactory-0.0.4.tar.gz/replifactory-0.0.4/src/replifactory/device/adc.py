...
class Photodiodes:
    ...
    def connect(self):
        try:
            self.adc_port = self.device.i2c.get_port(self.device.PORT_ADC)  # photodiodes ADC
    ...
    def measure(self, gain=8, bitrate=16, continuous_conversion=False):
        ...
        bitrate_bits = {12: 0b0000,
                        14: 0b0100,
                        16: 0b1000,
                        18: 0b1100}
        bitrate_to_samples_per_second = {12: 240,   # from MCP3421 datasheet
                                         14: 60,
                                         16: 15,
                                         18: 3.75}
        gain_bits = {1: 0b00,
        ...
        write_byte = ready_bit | conversion_mode | bitrate_bits[bitrate] | gain_bits[gain]
        ...
        for _ in range(100):  # try reading until conversion ready
            ...
            new_data_is_ready = config[0] < 128  # if bit 7 (ready bit) is 1: NOT READY YET!
        ...
        digital_signal = sum([2 ** i for i in range(len(data_bits)) if data_bits[::-1][i] == "1"])
        if sign_bit == "1":
        ...
        lsb_mv = 2.048 * 2 / 2 ** bitrate * 1000 / gain  # least significant bit millivolts
