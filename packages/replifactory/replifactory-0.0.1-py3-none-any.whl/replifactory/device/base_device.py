...
from src.replifactory import Photodiodes
from src.replifactory import Lasers
from src.replifactory import OdSensor, measure_od_all
from src.replifactory.device.pwm import PwmController
from src.replifactory.device.stirrers import Stirrers
from src.replifactory import Pump, calibrate_pump
from src.replifactory.device.thermometers import Thermometers
from src.replifactory import Valves
from src.replifactory import make_dilution
from src.replifactory import CultureDict
from src.replifactory.loading import save_object, load_config, load_object
from src.replifactory import QueueWorker
...
class BaseDevice:
    ...
    PORT_THERMOMETER_VIALS = 0x48  # ADT 75
    PORT_THERMOMETER_BOARD = 0x4C  # ADT 75
    ...
    def __init__(self, ftdi_address='ftdi://ftdi:2232h', connect=False):
        ...
        self.calibration_mv_to_od = {1: {}, 2: {}, 3: {}, 4: {}, 5: {}, 6: {}, 7: {}}
        ...
        self.delay_tube_wash_hrs = 24
        self.pump1_flush_volume = 1
        self.pump2_flush_volume = 0.1
        self.pump3_flush_volume = 0
    ...
    def connect(self, ftdi_address=None, fit_calibration=False):
    ...
    def prime_tubing(self, vial_number, pump_number, volume):
        ...
        for i in range(20):
            q = input("%.2f ml of air available in pump %d tubing to vial %d?" % (volume,pump_number,vial_number))
            if q == "":
    ...
    def check_parameters(self):
        assert self.pump2.stock_concentration
        assert self.pump1.stock_volume
        assert self.pump2.stock_volume
        assert self.pump4.stock_volume
        for v in self.cultures.keys():
            if self.cultures[v] is not None:
                assert callable(self.od_sensors[v].calibration_function)
        assert callable(self.pump1.calibration_function)
        assert callable(self.pump2.calibration_function)
        # assert callable(self.device.pump3.calibration_function)
        assert callable(self.pump4.calibration_function)
        print("Calibration functions OK")
