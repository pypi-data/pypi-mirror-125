...
class Cloudformation(o7lib.aws.base.Base):
    ...
    def ValidateTemplate(self, filePath, stack):
        """Validate and Load a CFN template"""
        ...
        if filePath is not None :
            ...
            with open(filePath, encoding='utf-8') as fileobj:
                templateContent = fileobj.read()

            try:
                response = self.cfn.validate_template(TemplateBody=templateContent)
        ...
        print('=' * 50)
        print(f'Name :  {COLOR_OK}{stack["StackName"]} {COLOR_END}')
        print('')
        ...
        if newDesc == curDesc:
            print(f'Description: {COLOR_OK}{curDesc}{COLOR_END}')
        else :
            print(f'Current Description -> {COLOR_FAIL}{curDesc}{COLOR_END}')
            print(f'New Description     -> {COLOR_FAIL}{newDesc}{COLOR_END}')
            if o7lib.util.input.IsItOk(f'{COLOR_WARNING}Do you Accept a New Description ?{COLOR_END}') is False:
                return None
        ...
        for newParam in response.get('Parameters',[]):
            ...
            tempsParam = {
                ...
                'comment' : f'{COLOR_FAIL}NEW using default !{COLOR_END}',
        ...
        for curParam in stack.get('Parameters',[]):
            ...
            if missing:
                tempParameters.append({
                    ...
                    'comment' : f'{COLOR_FAIL}MISSING, WILL BE REMOVE{COLOR_END}'
                })
        print('')
        params = {
            'columns' : [
                ...
                {'title' : 'Comment'    , 'type': 'str',  'dataName': 'comment'},
                {'title' : 'Description', 'type': 'str',  'dataName': 'description'}
            ]
        }
        while True:
            o7lib.util.displays.Table(params, tempParameters)
            keyType, key = o7lib.util.input.InputMulti(f'{COLOR_WARNING}Are Those Parameters Acceptable (y/n) Edit Parameter(int): {COLOR_END}')
            ...
            if keyType == 'int' and key > 0 and key <= len(tempParameters):
                ...
                tempParameters[key-1]['comment'] = f'Before-> {tempParameters[key-1]["currentValue"]}'
        ...
        if len(capabilities) > 0:
            ...
            if o7lib.util.input.IsItOk(f'{COLOR_WARNING}Do you Accept Required Capabilities ?{COLOR_END}') is False:
                return None
        ...
        if filePath is not None :
            ret['TemplateBody'] = templateContent
        else:
    ...
    def DisplayEvents(self, events, maxEvent = None):
        ...
        self.ConsoleTitle(left='Cloudformation Events in Stack: TBD')
        print('')
    ...
    def DisplayResources(self, resources):
        ...
        self.ConsoleTitle(left='Cloudformation Resources for Stack: TBD')
        print('')
        ...
        while True :
            ...
            if keyType == 'str':
                ...
                if key.lower() == 'u':
                    ...
                    filePath = o7lib.util.file_explorer.FileExplorer().SelectFile()
                    if filePath is None:
