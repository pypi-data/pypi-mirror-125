...
class FinlabDataFrame(pd.DataFrame):
    ...
    def reshape(df1, df2):
        ...
        d1_index_freq = df1.get_index_str_frequency() if isinstance(df1, FinlabDataFrame) else None
        d2_index_freq = df2.get_index_str_frequency() if isinstance(df2, FinlabDataFrame) else None
        
        if (d1_index_freq or d2_index_freq) and (d1_index_freq != d2_index_freq):
    ...
    def _index_str_to_date_month(self):
        ...
        if not (self.index.str.find('M') != -1).all():
          loggin.warn('FinlabDataFrame: invalid index, cannot format index to monthly timestamp.')
          return self
    ...
    def _index_str_to_date_season(self, detail=True):
      ...
      df = pd.DataFrame({
        'value': self.unstack().values,
        'disclosures': disclosure_dates.unstack().values,
      }, index=disclosure_dates.unstack().index).reset_index()

      return FinlabDataFrame(df.pivot('disclosures', 'stock_id')['value'].ffill())
