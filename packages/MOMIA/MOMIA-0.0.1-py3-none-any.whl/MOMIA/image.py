    ...
    def cluster_segmentation(self,
        ...
        if not boundary_annotation:
            if predictor == 'default':
                boundary_model = pk.loads(importlib.resources.read_binary("MOMIA.configurations", "boundary_MLP.pk"))
            else:
                try:
                    boundary_model = pk.load(open(predictor, 'rb'))
                except:
                    print('Unable to load user provided boundary predictor, use default instead.')
                    boundary_model = pk.loads(importlib.resources.read_binary("MOMIA.configurations",
                                                                              "boundary_MLP.pk"))

            if normalizer == 'default':
                boundary_normalizer = pk.loads(importlib.resources.read_binary("MOMIA.configurations",
                                                                               "normalizer.pk"))
            else:
                try:
                    boundary_normalizer = pk.load(open(normalizer, 'rb'))
                except:
                    print('Unable to load user provided boundary data normalizer, use default instead.')
                    boundary_normalizer = pk.loads(importlib.resources.read_binary("MOMIA.configurations",
                                                                                   "normalizer.pk"))

            self.boundary_prediction_model = boundary_model
            self.boundary_normalizer = boundary_normalizer
