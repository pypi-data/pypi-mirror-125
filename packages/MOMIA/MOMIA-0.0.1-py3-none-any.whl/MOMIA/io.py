...
def process_single_input(imagefile,
    ...
    if configfile == 'default':
        _print_handle(verbose, 'Use default configurations.')
        ...
        try:
            ...
            _print_handle(verbose,
                          'Use user specified confiturations "{}".'.format(configfile.split('/')[-1][:-4]))
        except:
    ...
    _print_handle(verbose, 'Imported file {}.'.format(imagefile.split('/')[-1]))
    ...
    _print_handle(verbose, 'segmentation complete.')
    # retrieve cells
    cell_list = obj.get_cells(branched=save_branched,
                              discarded=save_discarded,
                              return_list=True)
    t2 = timeit.default_timer()
    _print_handle(verbose, "Analysis complete, {} seconds used.".format(round(t2 - t1), 2))
    if save_input:
    ...
    if save_cells:
        ...
        _print_handle(verbose, 'cell data saved as {}{}_cells.pk'.format(dest_folder,header))
    if return_obj:
        return obj
...
def process_folder(source_folder,
                   ...
                   save_discarded=False,
                   save_branched=False,
                   boundary_classifier='./MOMIA/configurations/boundary_MLP.pk',
                   boundary_normalizer='./MOMIA/configurations/normalizer.pk',
                   n_cores=2):
    """
...
def _print_handle(do_print, text):
    if do_print:
        print(text)