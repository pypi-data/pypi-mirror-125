...
def make_thumbnail(cell, channel,
                   fixed_cap_length=True,
                   reorientation=True,
                   nrows=20,
                   ncols=50,
                   npole=4,
    ...
    if fixed_cap_length:
        resized  = _pole_aware_resize(data, length,
...
def batch_make_thumbnailes(celllist, channel, **kwargs):
    thumbnails = []
    lengths = []
    for cell in celllist:
        thumbnail, length = make_thumbnail(cell, channel, **kwargs)
        thumbnails.append(thumbnail)
        lengths.append(length)
    return np.array(thumbnails),np.array(lengths)


def _pole_aware_resize(data,
                       length,
                       reorientation=True,
                       nrows=20,
                       ncols=50,
    ...
    if flatten:
        return stitched.flatten()
    else:
        return stitched


def length_based_stack(thumbnails,
                       lengths,
                       n=10, pad=0,
                       remove_extreme_perc=0.05):
    extrema = max(5, int(len(lengths) * remove_extreme_perc))
    sorted_stack = thumbnails.copy()[np.argsort(lengths)][extrema:-extrema]
