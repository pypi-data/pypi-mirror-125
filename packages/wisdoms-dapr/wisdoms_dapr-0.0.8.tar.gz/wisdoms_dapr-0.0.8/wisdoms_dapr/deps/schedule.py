...
class ScheduleSubscribeInfoSchema(BaseModel):
    """调度订阅信息"""
...
class ScheduleTaskRegister(object):
    ...
    def __init__(
        ...
        *,
        ...
        schedule_pubsub_name: str = 'pubsub',
        schedule_subscribe_topic_name: str = 'submit_schedule_tasks',
        schedule_subscribe_tasks_register_method_name: str = 'subscribe_tasks_register_event',
        ...
        :param schedule_subscribe_topic_name: 调度微服务发布的注册调度任务的事件名称
        :param schedule_subscribe_tasks_register_method_name: 订阅调度微服务发布的注册调度任务事件的处理路由方法名称
        ...
        self.subscribe_info: list[ScheduleSubscribeInfoSchema] = [
            ScheduleSubscribeInfoSchema(
                pubsubname=schedule_pubsub_name,
                topic=schedule_subscribe_topic_name,
                route=schedule_subscribe_tasks_register_method_name,
            ),
        ]  # 所有的订阅信息
        ...
        # 注册订阅注册调度任务事件的路由
        router.post('/' + schedule_subscribe_tasks_register_method_name)(self.register)
    ...
    def get_schedule_tasks(self) -> list[dict[str, typing.Any]]:
        ...
        for r in self.router.routes:
            for d in r.dependencies:
                if not isinstance(d.dependency, ScheduleDependency):
                    continue

                # 添加调度依赖
                info = d.dependency.schedule_info.dict()
