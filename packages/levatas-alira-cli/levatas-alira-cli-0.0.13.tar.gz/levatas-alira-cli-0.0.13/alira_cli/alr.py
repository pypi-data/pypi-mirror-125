...
class CLI(object):
    ...
    def setup(self, folder, skip_installation_check=False):
        ...
        if not skip_installation_check and self.installed:
            click.echo(
                "The application is already installed. Use `alr upgrade` "
    ...
    def upgrade(self, folder):
        ...
        if not self.installed:
            click.echo(
                "The application is not currently installed. Use `alr setup` "
                "to install the available packages."
            )
            return
    ...
    def remove(self):
        ...
        try:
            ...
            logging.exception(exception)
            click.echo(
                f"Unexpected error ocurred when removing network {NETWORK_NAME}..."
            )
    ...
    def _start_container(self, descriptor, local, ports_mapping, docker_params):
        ...
        if len(docker_params):
            ...
            try:
                ...
                logging.exception(exception)
                click.echo(
                    f"Unexpected error occurred starting {descriptor['name']}..."
                )
...
@click.group()
@click.option("--volume", "-v", help=HELP_TEXT_VOLUME, envvar='ALIRA_VOLUME', default=DEFAULT_VOLUME_PATH)
@click.pass_context
def cli(ctx, volume):
