...
class CoordinateAdjuster:
    ...
    def fit(self,data):
        '''
        Builds cdf objects for each one neede in computation from in data as well as creating total CDFs
        ...
        for team in df['awayTeam']:
            self.aTeamCDFS[team] = {}
            self.aTeamCDFS[team]['y'] = ECDF(df[(df['awayTeam'] == team)&(df['isAway']==1)]['yCord'].values.reshape(-1,1))
            self.aTeamCDFS[team]['x'] = ECDF(df[(df['awayTeam'] == team)&(df['isAway']==1)]['xCord'].values.reshape(-1,1))
        for arena in df['arenaId'].unique().tolist():
            self.rinkCDFS[arena] = {}
            self.rinkCDFS[arena]['y'] = ECDF(df[df['arenaId'] == arena]['yCord'].values.reshape(-1,1))
            self.rinkCDFS[arena]['x'] = ECDF(df[df['arenaId'] == arena]['xCord'].values.reshape(-1,1))

            for team in df['awayTeam'].unique().tolist():
                self.raCDFS[f"{team}{arena}"] = {}
                self.raCDFS[f"{team}{arena}"]['y'] = ECDF(df[(df['arenaId'] == arena)&(df['awayTeam'] == team)&(df['isAway']==1)]['yCord'].values.reshape(-1,1))
                self.raSDFS[f"{team}{arena}"]['x'] = ECDF(df[(df['arenaId'] == arena) & (df['awayTeam'] == team) & (df['isAway'] == 1)][
                                  'xCord'].values.reshape(-1, 1))
            self.raCDFS[f"{arena}"] = {}
            self.raCDFS[f"{arena}"]['y'] = ECDF(df[(df['arenaId'] == arena)&(df['isAway'] == 0)]['yCord'].values.reshape(-1,1))
            self.raCDFS[f"{arena}"]['x'] = ECDF(df[(df['arenaId'] == arena) & (df['isAway'] == 0)]['xCord'].values.reshape(-1, 1))
    ...
    def upd_row(self, r):
        ...
        if r['awayTeam'] in self.aTeamCDF.keys():
        ...
        if r['arenaId'] in self.rCDF.keys():
            rCDF = self.rCDF[r['arenaId']]
        else:
            raise ValueError("arenaId column (3rd column) has id that did not exist in data that was originally passed to CoordinateAdjuster")
        if r['isAway'] == 0:
