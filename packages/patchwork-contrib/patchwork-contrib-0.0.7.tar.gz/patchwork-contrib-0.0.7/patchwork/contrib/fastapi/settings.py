...
class PublisherConfig(BaseModel):
    ...
    :cvar engine:
        A path to class which is a publisher engine
    :cvar options:
        Optional mapping with options for publisher
    """
    engine: ClassPath
    options: Dict = {}
...
def register_patchwork(app: FastAPI, api_settings: PatchworkFastAPISettings):
    global settings
    ...
    if not is_starsessions_installed:
        return
