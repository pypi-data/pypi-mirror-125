...
class EmailSender:
    ...
    def send_text(self, to_carrier, to_number, message):
        logger = get_logger()
        to_number = '{}@{}'.format(to_number, to_carrier.value)
        logger.debug("sending to:{}, message:{}".format(to_number, message))

        try:
            server = smtplib.SMTP(self.email_connection.smtp_host, self.email_connection.smtp_port)
            server.starttls()
            server.login(self.email_connection.username, self.email_connection.password)
            complete_message = ('From:{}\r\nTo:{}\r\nSubject:{}'.format(self.email_connection.smtp_host,
                                                                        to_number,
                                                                        message))
            server.sendmail(from_addr=self.email_connection.smtp_host,
                            to_addrs=to_number,
...
def main():
    ...
    mail_from_cfg.send_text(EmailSender.Carrier.T_MOBILE, '3217778377', 'testing from thompcoutils')
