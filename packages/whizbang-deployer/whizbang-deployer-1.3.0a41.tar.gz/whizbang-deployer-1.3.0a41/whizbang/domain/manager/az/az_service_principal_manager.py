...
class AzServicePrincipalManager(AzManagerBase):
    def __init__(self, repository: AzServicePrincipalRepository, az_app_registration_manager: AzAppRegistrationManager):
        ...
        self._repository: AzServicePrincipalRepository = self._repository

    def create_for_rbac(self, service_principal: ServicePrincipal, scopes: List[str], years: int = 1) -> ServicePrincipal:
        return self._repository.create_for_rbac(service_principal=service_principal, scopes=scopes, years=years)
