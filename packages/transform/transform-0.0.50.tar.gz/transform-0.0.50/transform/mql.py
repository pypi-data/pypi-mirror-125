...
class MQLClient:
    ...
    def list_queries(self, active_only: bool, limit: Optional[int] = None) -> Dict[str, Any]:  # type: ignore[misc]
    ...
    def get_dimension_values(
        self, metric_name: str, dimension_name: str, model_key_id: Optional[int] = None
    ) -> List[str]:
        """Retrieves a list of dimension values given a [metric_name, dimension_name]."""
        model_key = self.get_model_key(model_key_id) if model_key_id else None
        return self.context.mql_client.get_dimension_values(metric_name, dimension_name, model_key=model_key)
