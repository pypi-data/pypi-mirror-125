...
@exception_handler
def get_dimension_values(config: CLIContext, dimension_name: str, metric_name: str) -> None:
    """List all dimension values that are queryable through materialized tables."""
    model_key = config.resolve_query_model_key()
    try:
        dim_vals = sorted(
            config.mql.get_dimension_values(metric_name, dimension_name, model_key.id if model_key else None)
        )
    except Exception as e:
        click.echo(
            textwrap.dedent(
...
def list_dimensions(config: CLIContext) -> None:
    ...
    if not dimensions:
        click.echo("List of dimensions unavailable.")

    click.echo(f"🌱 We've found {len(dimensions)} unique dimensions for the {config.org.name} organization.")
    for d in dimensions:
