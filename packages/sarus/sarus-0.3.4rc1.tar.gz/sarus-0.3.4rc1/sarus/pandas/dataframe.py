        ...
        **kwargs,
        ...
        Args:
            ...
            target_epsilon (float):
                Maximum privacy to consume for the mean computation. If greater
                than zero, send a query on the remote private dataset.
                Otherwise, execute on the synthetic data.
        Returns:
            A pandas.Series.
        """
        if target_epsilon == 0.0:
            ...
            values = [f'AVG("{column}")' for column in numeric_columns]
            ...
            if response["status"] == "SUCCESS":
                ...
                result = {name_mapping[col]: result[idx] for idx, col in enumerate(API_columns)}
    ...
    def std(
        ...
        **kwargs,
        ...
        if target_epsilon == 0.0:
            ...
            # Build query
            # Need to be careful with quotes ordering: put " " inside ' '
            values = [f'STDDEV("{column}")' for column in numeric_columns]
            query = f'SELECT {", ".join(values)} FROM {self.dataset.name}'
            if verbose:
                logging.info(query)
            response = self.dataset.client.query(
    ...
    def sum(
        ...
        **kwargs,
        ...
        if target_epsilon == 0.0:
            ...
            # Build query
            # Need to be careful with quotes ordering: put " " inside ' '
            values = [f'SUM("{column}")' for column in numeric_columns]
            query = f'SELECT {", ".join(values)} FROM {self.dataset.name}'
            if verbose:
                logging.info(query)
            response = self.dataset.client.query(
