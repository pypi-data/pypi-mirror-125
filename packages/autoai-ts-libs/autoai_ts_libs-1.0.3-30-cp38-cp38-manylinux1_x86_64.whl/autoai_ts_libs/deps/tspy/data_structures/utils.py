...
class SeriesWithKeyUnaryMapfunctionToSeries(object):
    ...
    def evaluate(self, obj_j_pair):
        py_obs_coll = ObservationCollection(self._tsc, obj_j_pair.right())
...
class Interpolator(object):
    ...
    def interpolate(self, history, future, timestamp):
        py_history = ObservationCollection(self._tsc, history)
        py_future = ObservationCollection(self._tsc, future)
...
class SeriesFilterFunction(object):
    ...
    def apply(self, j_series):
        py_series = ObservationCollection(self._tsc, j_series)
...
class SeriesUnaryMapFunction(object):
    ...
    def evaluate(self, j_obs_coll):
        py_obs_coll = ObservationCollection(self._tsc, j_obs_coll)
...
class JavaMultiDataSink:
    ...
    def dump(self, observations_map):
        self._python_multi_data_sink.dump({k: ObservationCollection(self._tsc, v) for k, v in observations_map.items()})
...
class JavaDataSink:
    ...
    def dump(self, observations):
        self._python_data_sink.dump(ObservationCollection(self._tsc, observations))
...
class JavaTimeSeriesWriteFormat:
    ...
    def write(self, observations, encode_value, options):
        self._python_time_series_writer.write(
            ObservationCollection(self._tsc, observations),
...
class JavaMultiTimeSeriesWriteFormat:
    ...
    def write(self, observations_dict, encode_key, encode_value, options):
        self._python_time_series_writer.write(
            {k: ObservationCollection(self._tsc, observations) for k, observations in observations_dict.items()},
    ...
    if obj_type is None:
        if isinstance(obj, JavaObject):
            ...
            if is_instance_of(gateway, obj, "com.ibm.research.time_series.core.utils.Segment"):
                return Segment(tsc, obj.observations(), obj.start(), obj.end()), 2

            elif is_instance_of(gateway, obj, "com.ibm.research.time_series.core.utils.ObservationCollection"):
                return ObservationCollection(tsc, obj), 3
...
def cast_to_j_if_necessary(obj, tsc):
    ...
    elif isinstance(obj, ObservationCollection):
