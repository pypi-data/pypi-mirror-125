...
class Resolver(BaseModel, T.Generic[NodeType]):
    ...
    def gql_fields_str(self) -> str:
        ...
        fields = get_fields_and_nested_fields(self.node)
    ...
    def make_add_mutation_str(self) -> str:
        ...
        s = f"""
            mutation Add{typename}($input: [{add_model_name}!]!, $upsert: Boolean) {{
                {add_function_name}(input: $input, upsert: $upsert) {self.make_add_query_str()}
            }}
        """
        return s
    ...
    async def query(self) -> T.List[NodeType]:
        ...
        res = await execute.gql(query_str=s, url=self.node.GQL.url)
