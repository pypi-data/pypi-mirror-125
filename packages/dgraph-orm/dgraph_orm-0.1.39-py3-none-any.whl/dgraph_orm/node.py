...
class Node(BaseModel):
    ...
    async def _add(
        ...
        *,
        ...
        if not isinstance(input, cls.GQL.add_model):
            input = cls.GQL.add_model.parse_obj(input)
        ...
        variables = {"input": jsonable_encoder(input_d), "upsert": upsert}
        j = await gql(query_str=query_str, variables=variables, url=cls.GQL.url)
        node_d = j["data"][f"{cls.GQL.add_function_name}"][cls.GQL.payload_node_name][0]
    ...
    async def update_from_set_remove_ds(
        ...
        *,
        ...
        j = await gql(query_str=query_str, variables=variables, url=self.GQL.url)
