...
async def add(cls, *, input: {self.node_config.add_model}, resolver: {self.resolver_name} = None{upsert_str}) -> {self.node_name}:
    return (await cls.add_many(inputs=[input], resolver=resolver, upsert=upsert))[0]

@classmethod
async def add_many(cls, *, inputs: List[{self.node_config.add_model}], resolver: {self.resolver_name} = None{upsert_str}) -> List[{self.node_name}]:
    return await cls._add(inputs=inputs, given_resolver=resolver, upsert={"upsert" if has_upsert else "None"})
        """
...
def build_from_app_config(
    ...
    s = build_all(
        schema=schema, node_configs=node_configs, include_strawberry=include_strawberry
    )
