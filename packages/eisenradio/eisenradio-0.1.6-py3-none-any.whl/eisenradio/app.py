...
def index_posts_clicked(post_request):
    ...
    pay_load = post_request['data']
    button_list = pay_load.split('_')
    table_id = button_list[1]
    button = button_list[0]

    if button == 'Listen':  # turn values, update button status dict

        if status_listen_btn_dict[int(table_id)]:
            status_listen_btn_dict[int(table_id)] = 0
            EisenApp.dispatch_master(int(table_id), 'Listen', 0)  # now idle
            # ask GIni for url also 'playlist'
            if not last_btn_id == table_id:  # auto pressed
                return jsonify(
                    {'result': 'auto_click_jquery'})
        else:
            status_listen_btn_dict[int(table_id)] = 1
            if last_btn_id is None:
            ...
            if not last_btn_id == table_id:  # other btn pressed
                if listen_btn:  # was playing before actual btn
                    ...
                    last_btn_id = table_id
                    str_url = query_radio_url(table_id)  # radio url
                    last_url = str_url
        ...
        if listen_btn:
            return jsonify(
                {'former_button_to_switch': False, 'buttons': button, 'query_url': str_url,
                 'result': 'activate_audio'})
        else:
            return jsonify(
                {'former_button_to_switch': False, 'buttons': button, 'query_url': str_url,
...
class EisenApp:
    ...
    def dispatch_record_title(table_id, record, title):
        ...
        try:
            ...
            if title:
                thread = threading.Thread(target=eR.GRecorder.get_metadata_from_stream_loop, args=(str_url, str_radio))
                ...
                thread = threading.Thread(name=table_id, target=eR.record, args=(str_radio, str_url))
                thread.setDaemon(True)
                EisenApp.eisen_threads.append(thread)
                thread.start()

            thread = threading.Thread(target=EisenApp.display_write_db, args=(str_radio, table_id))
            thread.setDaemon(True)  # non block
            EisenApp.eisen_threads.append(thread)
            thread.start()
            return True
...
def get_environment():
    ...
    try:
        if os.environ["SNAP"]:
            print('Eisen App runs in Ubuntu Snap Container, check environment:')
            print('')
...
if __name__ == "__main__":
    main()
    app.run(host='localhost', port='5050', threaded=True, debug=True, use_reloader=False)
