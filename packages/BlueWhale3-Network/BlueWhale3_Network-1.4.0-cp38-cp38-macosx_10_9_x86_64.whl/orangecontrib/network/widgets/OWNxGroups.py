...
def __(key):
    return i18n.t('network.OWNxGroups.' + key)
...
class OWNxGroups(OWWidget):
    ...
    class Inputs:
        ...
        data = Input('Data', Table, label=i18n.t("network.common.data"))

    class Outputs:
        network = Output('Network', Network, default=True, label=i18n.t("network.common.network"))
        data = Output('Data', Table, label=i18n.t("network.common.data"))

    class Warning(OWWidget.Warning):
        no_graph_found = Msg(__("msg.network_miss_data"))
        no_discrete_features = Msg(__("msg.miss_discrete_feature"))

    class Error(OWWidget.Error):
        data_size_mismatch = Msg(__("msg.data_length_nodes_not_match"))
    ...
    weight_labels = \
        [__("label.no_weight"), __("label.number_connections"), __("label.sum_connection_weights")]
    ...
    def __init__(self):
        ...
        info_box = gui.widgetBox(self.controlArea, __("box.info"))
    ...
    def _set_input_label_text(self):
        if self.network is None:
            self.input_label.setText(__("text.input_no_data"))
        else:
            self.input_label.setText(
                __("text.input_data").format(self.network.number_of_nodes(), self.network.number_of_edges()))

    def _set_output_label_text(self, output_network):
        if output_network is None:
            self.output_label.setText(__("text.output_no_data"))
            self.out_nodes = self.out_edges = None
        else:
            self.out_nodes = output_network.number_of_nodes()
            self.out_edges = output_network.number_of_edges()
            self.output_label.setText(
                __("text.output_data").format(self.out_nodes, self.out_edges)
            )
    ...
    def send_report(self):
        ...
        self.report_items(__("report.input_network"), [
            (__('report.number_vertices'), self.network.number_of_nodes()),
            (__('report.number_edges'), self.network.number_of_edges())])
        self.report_data(__("report.input_data"), self.effective_data)
        self.report_items(__("report.Settings"), [
            (__('report.group_by'), self.feature.name),
            (__("report.weights"), self.weight_labels[self.weighting].lower() +
             (", normalized by geometric mean" if self.normalize else ""))
        ])
        if self.out_nodes is not None:
            self.report_items(__("report.output_network"), [
                (__("report.number_vertices"), self.out_nodes),
                (__("report.number_edges"), self.out_edges)])
