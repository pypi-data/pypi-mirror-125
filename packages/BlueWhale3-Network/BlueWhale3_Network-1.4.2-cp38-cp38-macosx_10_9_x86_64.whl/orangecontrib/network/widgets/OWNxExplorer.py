...
def __(key):
    return i18n.t('network.OWNxExplorer.' + key)
...
class OWNxExplorer(OWDataProjectionWidget):
    ...
    class Inputs:
        node_data = Input("Node Data", Table, label=i18n.t("network.common.node_data"))
        node_subset = Input("Node Subset", Table, label=i18n.t("network.common.node_subset"))
        network = Input("Network", Network, default=True, label=i18n.t("network.common.network"))
        node_distances = Input("Node Distances", Orange.misc.DistMatrix, label=i18n.t('network.common.node_distance'))

    class Outputs(OWDataProjectionWidget.Outputs):
        subgraph = Output("Selected sub-network", Network, label=i18n.t('network.common.select_sub-network'))
        unselected_subgraph = Output("Remaining sub-network", Network, label=i18n.t('network.common.select_sub-network'))
        distances = Output("Distance matrix", Orange.misc.DistMatrix, label=i18n.t('network.common.distance_matrix'))

    UserAdviceMessages = [
        widget.Message(__("msg.connect_component"),
    ...
    class Warning(OWDataProjectionWidget.Warning):
        distance_matrix_mismatch = widget.Msg(__("msg.distance_matrix_mismatch"))
        no_graph_found = widget.Msg(__('msg.no_graph_found'))

    class Error(OWDataProjectionWidget.Error):
        data_size_mismatch = widget.Msg(__("msg.data_size_mismatch"))
        network_too_large = widget.Msg(__('msg.network_too_large'))
        single_node_graph = widget.Msg(__('msg.single_node_graph'))
    ...
    def _add_info_box(self):
        info = gui.vBox(self.controlArea, box=__("box_layout"))
        gui.label(
            info, self,
            __("node_detail"))
        gui.label(
            info, self, __('edge_detail'))
        lbox = gui.hBox(info)
        self.relayout_button = gui.button(
            lbox, self, __("btn.improve"), callback=self.improve, autoDefault=False,
            tooltip=__("btn.improve_tip"))

        self.stop_button = gui.button(
            lbox, self, __('btn.stop'), callback=self.stop_relayout, autoDefault=False,
            hidden=True)
        self.randomize_button = gui.button(
            lbox, self, __("btn.re_layout"), callback=self.restart, autoDefault=False,
            tooltip=__("btn.re_layout_tip"))
        gui.hSlider(info, self, "layout_density", minValue=1, maxValue=50,
                    label=__("label_gravity"), orientation=Qt.Horizontal,
                    callback_finished=self.improve,
                    tooltip=__("label_gravity_tip"))
        gui.checkBox(info, self, "observe_weights",
                     label=__("label_observe_weights"),
                     callback=self.improve)

    def _add_effects_box(self):
        gbox = self.gui.create_gridbox(self.controlArea, box=__("box_size"))
        self.gui.add_widget(self.gui.PointSize, gbox)
        gbox.layout().itemAtPosition(1, 0).widget().setText(__('row.node_size'))
        self.gui.add_control(
            gbox, gui.hSlider, __("row.edge_width"),
    ...
    def _add_mark_box(self):
        ...
        def text_line():
            ...
            return gui.lineEdit(
                gui.hBox(vbox), self, "mark_text", label=__("label_text"),
        ...
        self.mark_criteria = [
            (__("gbox.mark_criteria"), None, lambda: np.zeros((0,))),
            (__("gbox.mark_label_starts"), text_line(), mark_label_starts),
            (__("gbox.mark_label_contains"), text_line(), mark_label_contains),
            (__("gbox.mark_text"), text_line(), mark_text),
            (__("gbox.mark_reachable"), None, mark_reachable),

            (__("gbox.mark_close"),
             spin("mark_hops", __("number_hops"), 1, 20),
             mark_close),

            (__("gbox.mark_from_input"), None, mark_from_input),

            (__("gbox.mark_connection"),
             spin("mark_max_conn", __("max_connection"), 0, 1000),
             lambda: np.flatnonzero(self.network.degrees() <= self.mark_max_conn)),

            (__("gbox.mark_min_conn"),
             spin("mark_min_conn", __("min_connection"), 1, 1000),
             lambda: np.flatnonzero(self.network.degrees() >= self.mark_min_conn)),

            (__("gbox.mark_most_connections"),
             spin("mark_most_conn", __("mark_number"), 1, 1000),
             mark_most_connections),

            (__("gbox.mark_more_than_any_neighbour"), None,
             mark_more_than_any_neighbour),

            (__("gbox.mark_more_than_average_neighbour"), None,
        ...
        self.btselect = gui.button(
            hbox, self, __("btn.select"), callback=self.select_marked)
        self.btadd = gui.button(
            hbox, self, __("btn.select_add_marked"), callback=self.select_add_marked)
        self.btgroup = gui.button(
            hbox, self, __("btn.select_as_group"), callback=self.select_as_group)
    ...
    def update_selection_buttons(self):
        ...
        if not len(selection) or np.max(selection) == 0:
            ...
            self.btadd.setText(__("msg.add_select"))
            self.btadd.show()
            self.btgroup.hide()
        else:
            self.btadd.setText(__("msg.add_group"))
            self.btadd.show()
            self.btgroup.show()
