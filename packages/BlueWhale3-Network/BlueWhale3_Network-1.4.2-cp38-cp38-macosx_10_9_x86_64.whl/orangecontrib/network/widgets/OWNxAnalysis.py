...
def __(key):
    return i18n.t('network.OWNxAnalysis.' + key)
...
METHODS = (
    ...
    ("degrees", lambda network: network.degrees(), __("btn.degree"), NODELEVEL, GENERAL),

    ("number_of_nodes", lambda n: n, __("btn.number_node"), GRAPHLEVEL, GENERAL),
    ("number_of_edges", lambda e: e, __("btn.number_edge"), GRAPHLEVEL, GENERAL),
    ("average_degree", lambda network: avg_degree(network), __("btn.average_degree"), GRAPHLEVEL, GENERAL),
    ("density", lambda network: density(network), __("btn.density"), GRAPHLEVEL, GENERAL),

    ("shortest_paths", shortest_paths_nan_diag, __("btn.shortest_paths")),
    ("diameter",
     lambda shortest_paths: np.nanmax(shortest_paths), __("btn.diameter"), GRAPHLEVEL, GENERAL),
    ("radius",
     lambda shortest_paths: np.nanmin(np.nanmax(shortest_paths, axis=1)),
     __("btn.radius"), GRAPHLEVEL, GENERAL),
    ("average_shortest_path_length",
     lambda shortest_paths: np.nanmean(shortest_paths),
     __("btn.average_shortest_path_length"), GRAPHLEVEL, GENERAL),

    ("number_strongly_connected_components", lambda network:
    csgraph.connected_components(network.edges[0].edges, False)[0],
     __("btn.number_strongly_connect_component"), GRAPHLEVEL, DIRECTED),
    ("number_weakly_connected_components", lambda network:
    csgraph.connected_components(network.edges[0].edges, True)[0],
     __("btn.number_weakly_connect_component"), GRAPHLEVEL, DIRECTED),

    ("in_degrees", lambda network: network.in_degrees(), __("btn.in_degree"), NODELEVEL, GENERAL),
    ("out_degrees", lambda network: network.out_degrees(), __("btn.out_degree"), NODELEVEL, GENERAL),
    ("average_neighbour_degrees",
     ...
     __("btn.average_neighbor_degree"), NODELEVEL, GENERAL),
    ("degree_centrality",
     lambda degrees, n: n and degrees / (n - 1) if n > 1 else 0,
     __("btn.degree_centrality"), NODELEVEL, GENERAL),
    ("in_degree_centrality",
     lambda in_degrees, n: in_degrees / (n - 1) if n > 1 else 0,
     __("btn.in_degree_centrality"), NODELEVEL, GENERAL),
    ("out_degree_centrality",
     lambda out_degrees, n: out_degrees / (n - 1) if n > 1 else 0,
     __("btn.out_degree_centrality"), NODELEVEL, GENERAL),
    ("closeness_centrality",
     lambda shortest_paths: 1 / np.nanmean(shortest_paths, axis=1),
     __("btn.closeness_centrality"), NODELEVEL, GENERAL)
)
...
class OWNxAnalysis(widget.OWWidget):
    ...
    class Inputs:
        network = Input("Network", Network, label=i18n.t("network.common.network"))
        items = Input("Items", Table, i18n.t("network.common.items"))

    class Outputs:
        network = Output("Network", Network, label=i18n.t("network.common.network"))
        items = Output("Items", Table, i18n.t("network.common.items"))

    class Information(widget.OWWidget.Information):
        computing = Msg(__('msg_compute'))
    ...
    def __init__(self):
        ...
        graph_indices = gui.createTabPage(tabs, __("box.graph-level_index"),
                                          orientation=Qt.Horizontal)
        node_indices = gui.createTabPage(tabs, __("box.node-level_index"),
                                         orientation=Qt.Horizontal)
    ...
    def run_more_jobs(self):
        ...
        for method in needed:
            if method.name not in self.running_jobs:
                setattr(self, "lbl_" + method.name, __("status.pend"))
        ...
        for method in doable[:free]:
            ...
            if not method.level == INTERNAL:
                setattr(self, "lbl_" + method.name, __("status.run"))
        self.show_computing()
    ...
    def cancel_job(self, name=None):
        ...
        for name in to_stop:
            job = self.running_jobs[name]
            job.wait()
            setattr(self, "lbl_" + name, __("status.terminate"))
