...
def __(key):
    return i18n.t('network.OWNxEmbeddings.' + key)
...
class OWNxEmbedding(OWWidget):
    ...
    def __init__(self):
        ...
        gui.spin(box, self, "p", 0.0, 10.0, 0.1, label=__("label.p"),
                 spinType=float, **kwargs)
        gui.spin(box, self, "q", 0.0, 10.0, 0.1, label=__("label.q"),
                 spinType=float, **kwargs)
        gui.spin(box, self, "walk_len", 1, 100_000, 1, label=__("label.walk_len"),
                 **kwargs)
        gui.spin(box, self, "num_walks", 1, 10_000, 1, label=__("label.num_walks"),
                 **kwargs)
        gui.spin(box, self, "emb_size", 1, 10_000, 1, label=__("label.emb_size"),
                 **kwargs)
        gui.spin(box, self, "window_size", 1, 20, 1, label=__("label.window_size"),
                 **kwargs)
        gui.spin(box, self, "num_epochs", 1, 100, 1, label=__("label.num_epochs"),
                 **kwargs)

        gui.auto_commit(self.controlArea, self, "auto_commit", __("btn_commit"),
                        checkbox_label=__("btn_auto_commit"), orientation=Qt.Horizontal)
        commit()
