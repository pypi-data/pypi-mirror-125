...
def __(key):
    return i18n.t('network.OWNxSave.' + key)
...
class OWNxSave(OWSaveBase):
    ...
    class Error(OWSaveBase.Error):
        multiple_edge_types = Msg(__('msg_save_error'))
    ...
    def __init__(self):
        ...
        self.label_model = DomainModel(
            placeholder=__("placeholder_none"), valid_types=(StringVariable,))
        box = gui.hBox(None)
        gui.widgetLabel(box, __('node_label'))
        gui.comboBox(
            box, self, "label_variable",
            tooltip=__("node_label_tip"),
