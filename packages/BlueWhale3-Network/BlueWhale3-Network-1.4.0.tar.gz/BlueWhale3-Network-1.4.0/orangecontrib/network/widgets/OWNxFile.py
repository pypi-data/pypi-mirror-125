...
def __(key):
    return i18n.t('network.OWNxFile.' + key)
...
demos_path = next(
    get_entry_point("BlueWhale3-Network", "orange.data.io.search_paths", "network")
    ())[1]
...
class OWNxFile(OWWidget):
    ...
    class Information(OWWidget.Information):
        auto_annotation = Msg(
            __('msg.node_annotated_data_name'))
        suggest_annotation = Msg(
            __('msg.add_optional_data'))

    class Error(OWWidget.Error):
        io_error = Msg(__('msg.read_file_io_err'))
        error_parsing_file = Msg(__('msg.read_file_parse_err'))
        auto_data_failed = Msg(__('msg.read_file_data_err'))
        mismatched_lengths = Msg(__('msg.data_size_not_match_node'))
    ...
    def __init__(self):
        ...
        gui.button(
            hb, self, __('btn_reload'), callback=self.reload,
            icon=self.style().standardIcon(QStyle.SP_BrowserReload),
            sizePolicy=(QSizePolicy.Maximum, QSizePolicy.Fixed))

        self.label_model = VariableListModel(placeholder=__("placeholder_match"))
        self.label_model[:] = [None]
        gui.comboBox(
            self.controlArea, self, "label_variable", box=True,
            label=__("label_variable"), orientation=Qt.Horizontal,
    ...
    def populate_comboboxes(self):
        ...
        for file in self.recentFiles or (__("none"),):
            self.filecombo.addItem(path.basename(file))
        self.filecombo.addItem(__("browse_documentation_network"))
    ...
    def browse_net_file(self, browse_demos=False):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, __('dialog_open_file'), startfile,
