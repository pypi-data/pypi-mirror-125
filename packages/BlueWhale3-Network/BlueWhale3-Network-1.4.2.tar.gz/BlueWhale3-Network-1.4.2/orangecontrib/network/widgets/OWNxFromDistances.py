...
def __(key):
    return i18n.t('network.OWNxFromDistances.' + key)
...
class OWNxFromDistances(widget.OWWidget):
    ...
    class Inputs:
        distances = Input("Distances", DistMatrix, label=i18n.t("network.common.distance"))

    class Outputs:
        network = Output("Network", Network, label=i18n.t("network.common.network"))
        data = Output("Data", Table, label=i18n.t("network.common.data"))
        distances = Output("Distances", DistMatrix, label=i18n.t("network.common.distance"))
    ...
    class Warning(widget.OWWidget.Warning):
        kNN_too_large = \
            Msg(__('msg.knn_large_than_supply_distance'))
        large_number_of_nodes = \
            Msg(__('msg.performance_hinder'))
        invalid_number_of_items = \
            Msg(__('msg.number_data_items_nodes'))

    class Error(widget.OWWidget.Error):
        number_of_edges = Msg(__('msg.edges_high'))
    ...
    def __init__(self):
        ...
        # info
        boxInfo = gui.widgetBox(self.controlArea, box=__("box.info"))
        self.infoa = gui.widgetLabel(boxInfo, __("label.no_data"))
    ...
    def addHistogramControls(self):
        ...
        grid = QGridLayout()
        gui.widgetBox(self.controlArea, box=__("box.edges"), orientation=grid)
        self.spin_high = gui.doubleSpin(self.controlArea, self, 'spinUpperThreshold',
                                        ...
                                        controlWidth=60,
                                        addToLayout=False)
        grid.addWidget(QLabel(__('label.distance_threshold')), 0, 0)
        ...
        spin = gui.doubleSpin(self.controlArea, self, "percentil", 0, 100, PERCENTIL_STEP,
                      label=__("label.percentile"), orientation=Qt.Horizontal,
                      callback=self.setPercentil,
                      callbackOnReturn=1, controlWidth=60)
        grid.addWidget(QLabel(__("label.percentile")), 1, 0)
        grid.addWidget(spin, 1, 1)

        knn_cb = gui.checkBox(self.controlArea, self, 'include_knn',
                              label=__('label.include_closest_neighbors'),
        ...
        ribg = gui.radioButtonsInBox(self.controlArea, self, "node_selection",
                                     box=__("box.node_selection"),
                                     callback=self.generateGraph)
        grid = QGridLayout()
        ribg.layout().addLayout(grid)
        grid.addWidget(
            gui.appendRadioButton(ribg, __("box.keep_all_nodes"), addToLayout=False),
            0, 0
        )
        ...
        grid.addWidget(
            gui.appendRadioButton(ribg, __("row.component_contain_nodes"),
                                  addToLayout=False), 1, 0)
        grid.addWidget(exclude_limit, 1, 1)

        grid.addWidget(
            gui.appendRadioButton(ribg, __("row.largest_connect_component"),
                                  addToLayout=False), 2, 0)

        ribg = gui.radioButtonsInBox(self.controlArea, self, "edge_weights",
                                     box=__("box.edge_weights"),
                                     callback=self.generateGraph)
        hb = gui.widgetBox(ribg, None, addSpace=False)
        gui.appendRadioButton(ribg, __("row.proportional_distance"), insertInto=hb)
        gui.appendRadioButton(ribg, __("row.inverted_distance"), insertInto=hb)
    ...
    def generateGraph(self, N_changed=False):
        ...
        if self.matrix is None:
            if hasattr(self, "infoa"):
                self.infoa.setText(__("text.no_data_load"))
        ...
        if hasattr(self, "infoa"):
            self.infoa.setText(__("text.input_data_item") + '%d' % self.matrix.shape[0])
        if hasattr(self, "infob"):
            self.infob.setText(__("text.network_nodes") + '%d (%3.1f%%)' % (self.pconnected,
                                                                            self.pconnected / float(
                                                                                self.matrix.shape[0]) * 100))
        if hasattr(self, "infoc"):
            self.infoc.setText(__('text.network_edges') + '%d (%.2f edges/node)' % (
