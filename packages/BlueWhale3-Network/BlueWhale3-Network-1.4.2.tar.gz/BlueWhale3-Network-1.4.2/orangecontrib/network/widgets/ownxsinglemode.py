...
def __(key):
    return i18n.t('network.ownxsinglemode.' + key)
...
class OWNxSingleMode(OWWidget):
    ...
    class Error(OWWidget.Error):
        no_data = Msg("msg.network_additional_data")
        no_categorical = Msg("msg.no_categorical_feature")
        same_values = Msg("msg.modes_value_not_same")
    ...
    def __init__(self):
        ...
        gui.widgetBox(self.controlArea, box=__("box.mode_indicator"), orientation=form)
        form.addRow(__("row.feature"), gui.comboBox(
            None, self, "variable", model=VariableListModel(),
            callback=self.indicator_changed))
        form.addRow(__("row.connect"), gui.comboBox(
            None, self, "connect_value",
            callback=self.connect_combo_changed))
        form.addRow(__("row.by"), gui.comboBox(
            None, self, "connector_value",
            callback=self.connector_combo_changed))

        gui.comboBox(
            self.controlArea, self, "weighting", box=__("box.edge_weights"),
            items=[x.name for x in twomode.Weighting],
            callback=self.update_output)

        self.lbout = gui.widgetLabel(gui.hBox(self.controlArea, __("box.output")), "")
        self._update_combos()
    ...
    def _set_output_msg(self, out_network=None):
        if out_network is None:
            self.lbout.setText(__('text.no_network_output'))
        else:
            self.lbout.setText(
                __('text.node_edge').format(out_network.number_of_nodes(), out_network.number_of_edges()))

    def send_report(self):
        if self.network:
            self.report_items("", [
                (__('report.input_network'),
                 __('report.edge_weights').format(
                ...
                (__("report.output_network"), self.lbout.text())
            ])
