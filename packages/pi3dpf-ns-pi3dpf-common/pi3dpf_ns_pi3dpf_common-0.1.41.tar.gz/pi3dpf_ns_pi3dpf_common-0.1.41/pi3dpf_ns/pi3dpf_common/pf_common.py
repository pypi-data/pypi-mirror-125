    ...
    for secrets_fname in secrets_path.split(':'):
        ...
        try:
            with open(secrets_fname) as sf:
                ...
                secrets = list(
                    filter(lambda x: re.match('^' + secret_name + ':\s*(.*)', x), secrets_content.split('\n')))
                if len(secrets) == 1:  # make duplicate fail
                    ...
                    _log.error(
                        "Found {:d} password entries in file '{}', which exceeds the expected number of one.".format(
                            len(
...
def get_config_param(config, parameter):
    ...
    try:
        ...
        _log.error(
            "NoOptionError - get_config_param(): neither [{}] nor [DEFAULT] define '{}'".format(this_host, parameter))
        raise

    # resolve environment variables HOME, HOSTNAME, PF_ROOT and THIS_DIR

    #  for env_sym in [r'\$HOME', r'\$HOSTNAME', r'\$PF_ROOT', r'\$THIS_DIR']:
    #    for e in range(0, len(elements)):
    #      # x = re.search(r'\\*\$(HOME|HOSTNAME|PF_ROOT|THIS_DIR)', elements[e])
    #      if isinstance(elements[e], str):
    #        m = re.search(env_sym, elements[e])
    #        if m:
    #          if env_sym == '\\$THIS_DIR':
    #            try:
    #              substitute = os.path.dirname(config.cfg_fname[0])
    #              elements[e] = elements[e].replace(env_sym[1:], substitute)
    #              _log.info("{} $THIS_DIR was resolved using {}".format(parameter, substitute))
    #            except AttributeError:
    #              import pdb
    #              pdb.set_trace()
    #          elif os.path.expandvars(env_sym[1:]) == env_sym[1:] and env_sym[1:] == '$HOSTNAME':
    ...
    for e in range(0, len(elements)):
        if isinstance(elements[e], str):
            m = re.search(r'\\*\$(HOME|HOSTNAME|PF_ROOT|THIS_DIR)', elements[e])
            if m:
                env_sym = m.group(1)
                env_sym_expanded = os.path.expandvars('${}'.format(env_sym))
                if env_sym == 'THIS_DIR':
                    cfg_fname = config.cfg_fname[0] if isinstance(config.cfg_fname, list) else config.cfg_fname
                    substitute = os.path.dirname(cfg_fname)
                    elements[e] = re.sub(r'\\*\${}'.format(env_sym), substitute, elements[e])
                    _log.info("{} $THIS_DIR was resolved using {}".format(parameter, substitute))
                elif env_sym_expanded == '$' + env_sym and env_sym == 'HOSTNAME':
                    elements[e] = elements[e].replace(env_sym, os.uname()[1])
                    _log.debug("Symbol $HOSTNAME was resolved using os.uname()")
                else:
                    # elements[e] = elements[e].replace(env_sym, env_sym_expanded)
                    elements[e] = re.sub(r'\\*\${}'.format(env_sym), env_sym_expanded, elements[e])
