...
from .recommendation import Recommendation, TaskRecommendation
...
class Protocol(metaclass=DeclarativeFieldsMetaclass):
    ...
    def revision(cls) -> str:
        # Dpreecated - use Class.version instead.
        raise NotImplementedError('revision must be overridden')

    @property
    def references(self) -> List[str]:
        raise NotImplementedError('references must be overridden')
...
class NotificationProtocol(Protocol):
    """
    A protocol that sends a payload to an endpoint
    """
    def __init__(self, patient: Patient, change_types: List = None, **kwargs):
        super().__init__(patient=patient, **kwargs)
...
class ClinicalQualityMeasure(Protocol):
    ...
    def impacted_by_changes(self, change_types: Optional[List]) -> bool:
        ...
        if not change_types or not self.compute_on_change_types:
            return True

        for change in self.compute_on_change_types:
    ...
    def screening_interval_context(self) -> str:
        if hasattr(self, 'default_display_interval_in_days'):
            abc = self.friendly_time_duration(self.default_display_interval_in_days)
