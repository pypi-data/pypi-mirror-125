...
def plot(
    ...
    with tempfile.NamedTemporaryFile('w') as f:
        ...
        if share_y_axis:
            # Make multi-line title numbered list aligned by left
            datetimeline.css = [..., 'inline:.plot_title { text-anchor: start !important; }']

        assert pid_series_list and (share_y_axis or len(pid_series_list) <= 2)
        for i, pid_series in enumerate(pid_series_list):
            for pid, points in pid_series.items():
                if share_y_axis:
                    datetimeline.add(f'{i + 1}. PID {pid}', points)
                else:
                    datetimeline.add(f'PID {pid}', points, secondary=bool(i))
