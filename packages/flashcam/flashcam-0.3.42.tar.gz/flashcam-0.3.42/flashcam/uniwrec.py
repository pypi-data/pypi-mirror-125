...
def display2(videodev, save = False,
    ...
    def get_stream():
        ...
        ok = False
        try:
            stream = urllib.request.urlopen(request, timeout=3)
            ok = True
            print("D... stream ok3")
        except:
            ok = False
...
#----------------------------------------------------------------
        ...
        if save:
            ...
            sfilenamea = f"{sme}{sname}_{sfilenamea}.avi"
                        ...
                        while ret_val == 0:
                            for i8 in range(1): # I decimate and remove delay
                                ...
                                if a != -1 and b != -1:
                                    ...
                                    frame = cv2.imdecode(np.frombuffer(jpg, dtype=np.uint8), cv2.IMREAD_COLOR)
                                    ret_val=1
                                    print("got  Size={:6.0f}kB {:6.0f}kB/s {} {}".format(
                                        len(bytex)/1024,
                                        BPSlast,
