...
class Stream_Enhancer:
   ...
   4 columns MAX
    ...
    expo= (3,0) # 3 may be left??
    gain= (4,0) # 3 may be left??
    gamma= (5,0) # 3 may be left??
    ...
    def RC_COLOR(self,row,col):
        ...
        # left side
        if row==3 and col==0:       return (0,0,0) #
        if row==4 and col==0:       return (50,50,50) #
        if row==5 and col==0:       return (80,80,80) #
...
#-------------------------------------------- ENDOF LABELS----------
    ...
    def translate(self, speedx = -0.35,  speedy = -0.015 ):
        ...
        dwidth = -0.35 * self.camera_age  #negative up
        dheight = -0.015 * self.camera_age  # positive down
