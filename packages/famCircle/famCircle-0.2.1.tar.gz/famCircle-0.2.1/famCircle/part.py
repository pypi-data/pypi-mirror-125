...
class part():
    ...
    def make_plot(self,lens,gff,family,blast,ks,family_dic):
        ...
        plt.axis('off')
        self.pair_index(axes1,lens,gff,family,blast,ks,gene_average,chro_lingth,chro_order,family_dic)
    ...
    def pair_index(self,axes1,lens,gff,family,blast,ks,gene_average,chro_lingth,chro_order,family_dic):
        fam_dic = {}
        for i in family:
        ...
        for i in cluster:
            for j in range(len(i)):
                ...
                id0 = i[j]
                clusterx[id0] = j
                self.map2(axes1,((gff[id0]['end']/chro_lingth)*0.8)+0.1,(j)*0.005+0.32+(j)*0.001,0.002,family_dic[id0])
        ratio = 0.5
        for i in ks.keys():
            id1,id2 = i.split('_')[0],i.split('_')[1]
            if id1 in family and id2 in family and self.cluster0(cluster,id1,id2):
                if self.parameter == 'True':
                    ...
                    if ks_v >= float(self.ks_concern.split(',')[0]) and ks_v <= float(self.ks_concern.split(',')[1]):
                        ...
                        col = "#d4dcda"
                    self.plot_bez_Ks2(((gff[id1]['end']/chro_lingth)*0.8)+0.101, (clusterx[id1]) * 0.006+0.32, ((gff[id2]['end']/chro_lingth)*0.8)+0.101, (clusterx[id2])*0.006+0.32, col, ratio)
                else:
                    self.plot_bez_Ks2(((gff[id1]['end']/chro_lingth)*0.8)+0.101, (clusterx[id1]) * 0.006+0.32, ((gff[id2]['end']/chro_lingth)*0.8)+0.101, (clusterx[id2])*0.006+0.32, 'red', ratio)

    def run(self):
        lens,chr_list = read_lens(self.lens)# 染色体字典
        gff = read_gff(self.gff)# 基因字典
        family,family_dic = read_family1(self.genefamily)# 家族列表
        ...
        self.make_plot(lens,gff,family,blast,ks,family_dic)
