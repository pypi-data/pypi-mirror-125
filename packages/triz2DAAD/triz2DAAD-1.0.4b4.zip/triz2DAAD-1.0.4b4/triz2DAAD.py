...
def imprimeDEF():
        ...
        if daadReady:
                print(';', file=f)
                print('; Definiciones específicas para DAAD Ready 0.4', file=f)
                print(';', file=f)
                if english:
...
def imprimePRO0_DR():
        if english:
                print('DAAD Ready adds some definitions of its own.', file=f)
        else:
                print('; DAAD Ready añade además un par de definiciones propias', file=f)
        print('', file=f)
        print('>', file=f)
        print('_       _       AT 0', file=f)
        print('                #ifdef "canBoostSpeed"', file=f)
        ...
        print('', file=f)
        if english:
                print('; Some Triz2DAAD definitions to make sure DAAD Ready groups are mutually exclusive', file=f)
        else:
                print('; Definiciones de Triz2DAAD para asegurar que algunos grupos de DAAD Ready sean mutuamente excluyentes.', file=f)
        print('', file=f)
        print('#ifdef "hasSplitMode"', file=f)
        print('#define noretryImage 0', file=f)
        print('#endif', file=f)
        print('', file=f)
        print('#ifdef "next"', file=f)
        print('#define noretryImage 0', file=f)
        ...
        print('', file=f)
        if english:
                print('ZX Spectrum tape code. DAAD Ready takes for granted that in this mode the text window (WINDOW 1) will always cover the full screen.', file=f)
        else:
                print('; Código para ZX Spectrum en cinta. Para DAAD Ready en este modo se da por hecho que la ventana de texto (WINDOW 1) ocupa siempre toda la pantalla.', file=f)
        print('', file=f)
        print('#ifdef "tape"', file=f)
        print('>', file=f)
        print('_       _       WINDOW 1', file=f)
        ...
        print('', file=f)
        if english:
                print('; 16 bit and PCW mode. It uses PICTURE as a condition to select text window (WINDOW 1) ubication.', file=f)
        else:
                print('; Modo 16 bits y PCW. Utiliza PICTURE como condición para decidir donde colocar la ventana de texto (WINDOW 1).', file=f)
        print('', file=f)
        # En este momento estamos pendientes de que se resuelva el bug de locno para los condactos XPICTURE y PICTURE.
        # Mientras tanto no hay soporte para el gráfico 255 en modo nativeraster.
        soporte255Nativeraster = False
        if soporte255Nativeraster:
                print('#ifdef "nativeraster"', file=f)
                print('>', file=f)
                ...
                print('#ifdef "nativeraster"', file=f)
                if english:
                        print('; At this point there\'s no support for graphic nº 255 in nativeraster mode.', file=f)
                        print('; This could change really soon.', file=f)
                else:
                        print('; Por el momento no hay soporte para el gráfico 255 en modo nativeraster.', file=f)
                        print('; Esto puede cambier en breves.', file=f)
                print('>', file=f)
                print('_       _       CLS', file=f)
                print('                ZERO fDarkF', file=f)
                print('                PICTURE @fPlayer', file=f)
                ...
                print('', file=f)
        if english:
                print('; Amstrad CPC, C64 and ZX Uno modes. Meant to avoid the text window invade the split screen area (always at 14,0).', file=f)
        else:
                print('; Modos Amstrad CPC, C64 y ZX Uno. Pensados para evitar que la ventana de texto invada el area de la split-screen (siempre en 14,0).', file=f)
        print('', file=f)
        print('#ifdef "hasSplitMode"', file=f)
        print('>', file=f)
        ...
        print('', file=f)
        if english:
                print('; ZX Next mode. Just like "VGA256" mode with an extra XNEXTCLS condact', file=f)
        else:
                print('; Modo ZX Next. Igual al modo "VGA256" con un condacto XNEXTCLS extra.', file=f)
        print('', file=f)
        print('#ifdef "next"', file=f)
        print('>', file=f)
        print('_       _       CLEAR 20', file=f)
        ...
        print('', file=f)
        if english:
                print('; Rest of target platforms (including CPC, C64 and UNO if NOSPLIT mode is selected). If not dark, loads and displays the maluva graphic file. If it\'s dark OR there\'s no file (including a filler 255 graphic), text window is located at 0,0. If neither the case, it goes to 14,0. ', file=f)
        else:
                print('; Resto de plataformas (incluyendo CPC, C64 y Uno si se elige modo NOSPLIT). Si no está oscuro, carga el fichero gráfico de maluva y lo muestra. Si está oscuro O no hay gráfico (ni tampoco un gráfico 255 de relleno) la ventana de texto se coloca en 0,0. Si no es ninguno de los tres casos, va a 14,0.', file=f)
        print('', file=f)
...
print()
print('Triz2DAAD versión 1.0.4b4 211102 (c) 2019-21 Pedro Fernández')     
...
if daadReady:
        if english:
                print('-Exporting to a DAADReady 0.4 compatible template.')
        else:
                print('-Expoartando a una plantilla comnpatible con DAADReady 0.4')
        dsf=True
        if statusLine:
                if english:
