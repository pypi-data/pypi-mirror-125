...
class LocalContext(object):
    ...
    def add_event_to_event_buffer(self, event_group_id, name, meta):
        log.debug("adding event for event group %s to event buffer", event_group_id)
        self.event_buffer[event_group_id].append({"name": name, "meta": meta})
        if len(self.event_buffer) >= TracerConfig.event_buffer_size:
    ...
    def flush_event_buffer(self):
        log.debug("submitting to Lariat: events %s", self.event_buffer.keys())
        payload = json.dumps({"events": dict(self.event_buffer)})
        try:
            _make_request(TracerConfig.endpoint + TracerConfig.ingest_url, payload)
        except Exception as e:
            log.error("failed to submit events: %s", e)

        self.event_buffer.clear()

    def start(self):
        """Start a fresh context: should be called at the start of synchronous requests
        on long-running programs such as web servers
        """
        self.trace_id = random.getrandbits(32)
