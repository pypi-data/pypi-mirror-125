...
baseurl = "https://api.nextdns.io/configurations"
...
class account:
    ...
    def list(header):
        ...
        return confs
    def createconfig(name, header):
        name = {"name":name}
        new = requests.post(f"{baseurl}", headers=header, json=json)
        new = new.json()
        return new

class settings:
    def listsettings(config,header):
        list = requests.get(f"{baseurl}/{config}/settings", headers=header)
    ...
    def setup(config,header):
        setup = requests.get(f"{baseurl}/{config}/setup", headers=header)
    ...
    def clearlogs(config,header):
        logs = requests.delete(f"{baseurl}/{config}/logs", headers = header)
    ...
    def rename(name, config, header):
        nname = {"name":name}
        rename = requests.patch(f"{baseurl}/{config}/settings", headers = header, json=nname)
        if rename.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return f"Config renamed to {name}"
    def delete(config,header):
        dconfig = requests.delete(f"{baseurl}/{config}", headers = header)
    ...
    def logclientips(bool, config, header):
        ...
        logcips = requests.patch(f"{baseurl}/{config}/settings", headers = header, json=logcips)
    ...
    def logdomains(bool, config, header):
        ...
        logdom = requests.patch(f"{baseurl}/{config}/settings", headers = header, json=logdom)
    ...
    def blockpage(bool,config, header):
        if bool == True:
            ...
            bool = "false"
        bp = {"blockPage":bool}
        bp = requests.patch(f"{baseurl}/{config}/settings", headers = header, json=bp)
        if bp.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return bp.text
    def adddnsrewrite(domain:str, answer:str, config, header):
        new = {"name":domain,"answer":answer}
        rewrite = requests.post(f"{baseurl}/{config}/settings/rewrites", headers = header, json = new)
        if rewrite.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return rewrite.text
    def deletednsrewrite(id, config, header):
        delete = requests.delete(f"{baseurl}/{config}/settings/rewrites/{id}", headers=header)
...
class security:
    def list(config, header):
        settings = requests.get(f"{baseurl}/{config}/security", headers = header)
    ...
    def threatintelligencefeeds(bool, config, header):
        ...
        setting = requests.patch(f"{baseurl}/{config}/security", headers = header, json=setting)
    ...
    def addtld(tld, config, header):
        ...
        hex = f"hex:{hex}"
        put = requests.put(f"{baseurl}/{config}/security/blocked_tlds/{tld}", headers = header)
    ...
    def removetld(tld, config, header):
        hex = tld.encode('utf-8').hex()
        hex = f"hex:{hex}"
        remove = requests.delete(f"{baseurl}/{config}/security/blocked_tlds/{tld}", headers = header)
...
class privacy:
    def list(config, header):
        settings = requests.get(f"{baseurl}/{config}/privacy", headers = header)
        if settings.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            settings = settings.json()
    ...
    def blockdisguised(bool, config, header):
        ...
        setting = requests.patch(f"{baseurl}/{config}/privacy", headers = header, json=setting)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    ...
    def blocknative(native, config, header):
        ...
        if hex in nativetrackinghex:
            hex = f"hex:{hex}"
            put = requests.put(f"{baseurl}/{config}/privacy/natives/{hex}", headers = header)
            if put.text == "Not Found":
                raise ConfigNotFound(config)
            else:
                return put.text
    def unblocknative(native, config, header):
        ...
        if hex in nativetrackinghex:
            hex = f"hex:{hex}"
            delete = requests.delete(f"{baseurl}/{config}/privacy/natives/{hex}", headers = header)
            if delete.text == "Not Found":
                raise ConfigNotFound(config)
            else:
                return delete.text
...
class parental:
    def list(config, header):
        settings = requests.get(f"{baseurl}/{config}/parentalcontrol", headers = header)
        if settings.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            settings = settings.json()
    ...
    def porn(bool, config, header):
        if bool == True:
            setting = requests.put(f"{baseurl}/{config}/parentalcontrol/categories/hex:706f726e", headers = header)
        else:
            setting = requests.delete(f"{baseurl}/{config}/parentalcontrol/categories/hex:706f726e", headers = header)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    def gambling(bool, config, header):
        if bool == True:
            setting = requests.put(f"{baseurl}/{config}/parentalcontrol/categories/hex:67616d626c696e67", headers = header)
        else:
            setting = requests.delete(f"{baseurl}/{config}/parentalcontrol/categories/hex:67616d626c696e67", headers = header)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    def dating(bool, config, header):
        if bool == True:
            setting = requests.put(f"{baseurl}/{config}/parentalcontrol/categories/hex:646174696e67", headers = header)
        else:
            setting = requests.delete(f"{baseurl}/{config}/parentalcontrol/categories/hex:646174696e67", headers = header)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    def piracy(bool, config, header):
        if bool == True:
            setting = requests.put(f"{baseurl}/{config}/parentalcontrol/categories/hex:706972616379", headers = header)
        else:
            setting = requests.delete(f"{baseurl}/{config}/parentalcontrol/categories/hex:706972616379", headers = header)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    def socialnetworks(bool, config, header):
        if bool == True:
            setting = requests.put(f"{baseurl}/{config}/parentalcontrol/categories/hex:736f6369616c2d6e6574776f726b73", headers = header)
        else:
            setting = requests.delete(f"{baseurl}/{config}/parentalcontrol/categories/hex:736f6369616c2d6e6574776f726b73", headers = header)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return setting.text
    def safesearch(bool, config, header):
        ...
        setting = requests.patch(f"{baseurl}/{config}/parentalcontrol", headers = header, json=setting)
        if setting.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return settings
    ...
    def blocksite(site, config, header):
        ...
        put = requests.put(f"{baseurl}/{config}/parentalcontrol/services/hex:{sitehex}", headers = header)
        if put.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return put.text
    def unblocksite(site, config, header):
        sitehex = siteshex[f"{site.lower()}"]
        delete = requests.delete(f"{baseurl}/{config}/parentalcontrol/services/hex:{sitehex}", headers = header)
        if delete.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return delete.text
...
class denylist:
    def list(config, header):
        list = requests.get(f"{baseurl}/{config}/denylist", headers=header)
        if list.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            list = list.json()
    ...
    def blockdomain(domain, config, header):
        ...
        put = requests.put(f"{baseurl}/{config}/denylist/hex:{hex}", headers = header)
        if put.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return put.json()
    def unblockdomain(domain, config, header):
        hex = domain.encode('utf-8').hex()
        delete = requests.delete(f"{baseurl}/{config}/denylist/hex:{hex}", headers = header)
        if delete.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return delete.json()
...
class allowlist:
    def list(config, header):
        settings = requests.get(f"{baseurl}/{config}/allowlist", headers = header)
        if settings.text == "Not Found":
            raise ConfigNotFound(config)
        else:
    ...
    def add(domain, config, header):
        hex = domain.encode('utf-8').hex()
        hex = f"hex:{hex}"
        put = requests.put(f"{baseurl}/{config}/allowlist/hex:{hex}", headers = header)
        if put.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return put.json()
    def remove(domain, config, header):
        hex = domain.encode('utf-8').hex()
        delete = requests.delete(f"{baseurl}/{config}/allowlist/hex:{hex}", headers = header)
        if delete.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            return delete.json()
...
class analytics:
    def counter(config, header):
        count = requests.get(f"{baseurl}/{config}/analytics/counters", headers = header)
    ...
    def topresolveddomains(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_domains/resolved", headers = header)
    ...
    def topblockeddomains(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_domains/blocked", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
    ...
    def toplists(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_lists", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
    ...
    def topdevices(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_devices", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
    ...
    def topclientips(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_client_ips", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
    ...
    def toprootdomains(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/top_root_domains", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
    ...
    def gafam(config, header):
        gafam = requests.get(f"{baseurl}/{config}/analytics/gafam", headers = header)
    ...
    def trafficdest(config, header):
        top = requests.get(f"{baseurl}/{config}/analytics/traffic_destination_countries", headers = header)
        if top.text == "Not Found":
            raise ConfigNotFound(config)
        else:
            top = top.json()
