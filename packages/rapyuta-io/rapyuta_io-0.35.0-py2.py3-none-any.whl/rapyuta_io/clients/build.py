...
class Build(PartialMixin, ObjDict):
    ...
    def __init__(self, buildName, strategyType, repository, architecture, rosDistro='',
        ...
        if rosDistro is not '' and rosDistro not in list(ROSDistro.__members__.values()):
    ...
    def poll_build_till_ready(self, retry_count=120, sleep_interval=5):
        ...
        :param retry_count: Number of retries which will be used for polling.
        :type retry_count: int
        :param sleep_interval: sleep interval between simultaneous retries.
        :type sleep_interval: int
        :raises: :py:class:`BuildFailed`: Returns if the build fails.
        :raises: :py:class:`RetriesExhausted`: Returns If number of polling retries exhausted before the build
        ...
        url = self._host + '/build/{}'.format(self.guid)
        headers = create_auth_header(self._auth_token, self._project)
        rest_client = RestClient(url).method(HttpMethod.GET).headers(headers)
