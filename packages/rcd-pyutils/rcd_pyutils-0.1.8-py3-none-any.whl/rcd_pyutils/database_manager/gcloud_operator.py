...
def upload_to_gcloud(local_folder_path: str, uuid: str, bucket: str = os.environ.get("OPEN_DATA_SOURCE_BUCKET")):
    """
    ...
    lst_blob = [path for blob in gcs.list_blob(prefix=uuid) if (path := blob.name.removeprefix(f"{uuid}/")) != ""]
    print(f"💧{len(lst_blob)} blob from `{bucket}/{uuid}` detected.")
    lst_local_file = [path for file_path in detect_all_files(local_folder_path, full_path=True)
                      if not (path := file_path.removeprefix(f"{local_folder_path}{os.path.sep}")).startswith(".")]
    print(f"📑{len(lst_local_file)} local files from `{local_folder_path}` detected.")
    lst_upload = sorted([file_path for file_path in set(lst_local_file) - set(lst_blob)])
    print(f"📤{len(lst_upload)} local file need to be uploaded as gcloud blob.")

    for file_path in lst_upload:
        gcs.upload_blob(local_file_path=os.path.join(local_folder_path, file_path),
                        blob_path=os.path.join(uuid, file_path))


@timeit(program_name="download data folder to gcloud")
def download_from_gcloud(local_folder_path: str, uuid: str, bucket: str = os.environ.get("OPEN_DATA_SOURCE_BUCKET")):
    """
    ...
    lst_local_file = [path for file_path in detect_all_files(local_folder_path, full_path=True)
                      if not (path := file_path.removeprefix(f"{local_folder_path}{os.path.sep}")).startswith(".")]
    print(f"📑{len(lst_local_file)} local files from `{local_folder_path}` detected.")
    lst_download = sorted([file_path for file_path in set(lst_blob) - set(lst_local_file)])
    print(f"📥{len(lst_download)} gcloud blob need to be downloaded as local file.")

    for file_path in lst_download:
        gcs.download_blob(local_file_path=os.path.join(local_folder_path, file_path),
                          blob_path=os.path.join(uuid, file_path))
...
class GcloudOperator:
    ...
    def download_blob(self, local_file_path: str, blob_path: str) -> None:
        ...
        dir_path = f"{os.path.sep}".join(local_file_path.split(os.path.sep)[:-1])
        detect_path(dir_path)
