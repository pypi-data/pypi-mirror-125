...
class Binding:
    ...
    Can be enabled and disable separately from creation. Default to starting
    ...
    .. automethod:: __bool__
    '''
    def __init__(self, widget: 'mixin.MixinWidget', sequence: str, action: typing.Callable, *, immediate: bool = False, description: typing.Optional[str] = None, _bind_all: bool = False):
        assert isinstance(widget, (tk.Widget, tk.Tk)), f'{widget} is not a valid widget'
        # `_bind_all` is not documented on purpose, use `BindingGlobal` always.
        self.obj = None
        self.widget = widget
        self.sequence = sequence
        self.action = action
        self._fun = {
            False: self.widget.bind,
            True: self.widget.bind_all,
        }[_bind_all]
        if _bind_all is False and sequence in widget.wroot._bindings_global:
    ...
    def enable(self):
        '''Enable the binding.'''
        if self.obj is None:
            self.obj = self._fun(self.sequence, self.action, add=True)
        else:
    ...
    def __repr__(self):
        return '%s("%s" @ %s)  # %s' % (self.__class__.__name__, self.sequence, self.widget, self.description)


class BindingGlobal(Binding):
    ...
    def __init__(self, widget: 'mixin.MixinWidget', sequence: str, *args, **kwargs):
        ...
        super().__init__(root_widget, sequence, *args, _bind_all=True, **kwargs)
...
class TreeElement:
    ...
    Args:
        ...
        children: An optional list of `TreeElement`, to be shown as children of
            this `TreeElement`. This can recurse without limit.
        data: Arbitrary data to store on the element. Defaults to `None`.
    '''
    label: str
...
@dataclass
class TextElement_span(TextElement):
    '''Text Element: A text span, with optional tags.

    This is the most basic text element, just some string.
...
class TextElement_br(TextElement):
    ...
    This is equivalent to a `TextElement_span` with ``"\\n"`` as text.
    '''
    def __post_init__(self):
        self.text = '\n'
        self.atags = tuple()
