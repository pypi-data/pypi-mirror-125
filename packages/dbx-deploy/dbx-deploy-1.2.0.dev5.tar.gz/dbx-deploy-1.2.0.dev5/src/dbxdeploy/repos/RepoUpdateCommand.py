...
class RepoUpdateCommand(ConsoleCommand):
    ...
    def configure(self, argument_parser: ArgumentParser):
        ...
        required_choice_args.add_argument("--checkout-branch", dest="checkout_branch", help="Branch to checkout")
        required_choice_args.add_argument("--checkout-tag", dest="checkout_tag", help="Tag to checkout")
        required_args.add_argument("--repo-name", dest="repo_name", help="Project repo name")

    def run(self, input_args: Namespace):
        if not (input_args.repo_url and input_args.repo_name) or not (input_args.checkout_branch or input_args.checkout_tag):
            raise Exception("Missed required arguments. Check -h for the list of required arguments.")
        if input_args.checkout_branch and input_args.checkout_tag:
            raise Exception("Can't pick both --checkout-branch and --checkout-tag. Check -h for details.")

        self.__repo_manager.create_or_update(
            input_args.repo_url,
            input_args.checkout_branch,
            input_args.checkout_tag,
