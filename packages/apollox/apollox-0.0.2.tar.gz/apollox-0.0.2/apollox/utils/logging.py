...
apollox_dev_log_handler = handlers.TimedRotatingFileHandler(
...
apollox_dev_log_handler.setFormatter(_dev_formatter)
apollox_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

apollox_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

apollox_usr_log_handler.setLevel(logging.INFO)
...
apollox_usr_log_handler.setFormatter(_usr_formatter)
...
apollox_console_log_handler = logging.StreamHandler(sys.stdout)
apollox_console_log_handler.setFormatter(_console_formatter)
apollox_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    apollox_usr_log_handler.addFilter(warning_filter)
    apollox_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    apollox_usr_log_handler.removeFilter(warning_filter)
    apollox_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    apollox_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(apollox_dev_log_handler)

    log.addHandler(apollox_console_log_handler)

    log.addHandler(apollox_usr_log_handler)
