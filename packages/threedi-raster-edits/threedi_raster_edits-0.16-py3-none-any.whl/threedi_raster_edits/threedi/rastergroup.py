...
class ThreediRasterGroup(RasterGroup):
    ...
    def fix_table(self):
        ...
        if hasattr(self, "ct_lu"):
            ...
            for key, value in self.ct_lu.items():
                ...
                # fix value types
                new_values = [
                    CT_LU_TYPES[CT_LU_KEYS.index(key)](v) for v in value[1:] if v
                ]
        ...
        if hasattr(self, "ct_soil"):
            ...
            for key, value in self.ct_soil.items():
                ...
                # fix value types
                new_values = [
                    CT_SOIL_TYPES[CT_SOIL_KEYS.index(key)](v) for v in value[1:] if v
                ]
    ...
    def generate_infiltration(self):
        ...
        output = self.soil.copy(shell=True)
        pbar = Progress(len(output), "Generating infiltration")
            ...
            for raster in self:
                if raster.name in alignment_check["to_be_aligned"]:
                    ...
                    self[raster.name] = raster.align(
                        self.dem, idw=False, fill_value=True, nodata_align=True
                    )
