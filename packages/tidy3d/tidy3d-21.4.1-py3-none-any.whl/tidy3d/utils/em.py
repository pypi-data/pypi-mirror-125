...
def expand_syms(sim, span_inds, E, H, eps):
...
def get_eps_yee(sim, span_inds, freqs, file_in, file_out, dset_name):
...
def set_eps_yee(mode_plane, sim, file_in, file_out, dset_name):
    ...
    eps_sub = get_eps_yee(sim, span_inds, freqs, file_in, file_out, dset_name)
    mode_plane._set_yee_arr(eps_sub)


def eps_python_cpp(sim, mplane, tidy3d_path, subpixel=False):
    ...
    sim.fdtd_path = tidy3d_path
    subprocess_cmd("mkdir -p tmp")
    sim.fjson = 'tmp/fdtd3d.json'
    sim.export_json('tmp/fdtd3d.json')
    mplane._set_yee_sim(sim)
    eps_sim = np.stack([mplane.eps_ex, mplane.eps_ey, mplane.eps_ez])
    set_eps_yee(mplane, sim, "tmp/mplane_in.hdf5", "tmp/mplane_in.hdf5", "src_test")
