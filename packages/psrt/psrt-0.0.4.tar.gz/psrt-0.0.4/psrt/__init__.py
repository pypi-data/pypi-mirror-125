...
# logger for background processes (pinger etc.)
...
OP_PUBLISH_NO_ACK = b'\x12\x7F'  # priority hard-coded until supported
OP_SUBSCRIBE = b'\x02'
...
def pub_udp(target, topic, message, need_ack=True, **kwargs):
    ...
    if need_ack:
        ...
        print(server, target)
        if server != target:
            raise RuntimeError('Invalid responder')
        code = data[0]
