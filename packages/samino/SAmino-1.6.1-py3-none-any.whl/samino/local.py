...
class Local:
    ...
    def get_video_rep_info(self, chatId: str):
        req = requests.get(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/avchat-reputation", headers=self.headers)
    ...
    def claim_video_rep(self, chatId: str):
        ...
        req = requests.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/avchat-reputation", headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
