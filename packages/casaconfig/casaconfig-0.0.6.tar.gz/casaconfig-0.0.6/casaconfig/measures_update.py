...
def measures_update(path=None, version=None, force=False, logger=None):
    ...
    if not force:
        if ((version is not None) and (version == current)) or ((version is None) and (updated == datetime.today().strftime('%Y-%m-%d'))):
            if logger is None:
                ...
                logger.post('CASACONFIG: current measures detected, not updating', 'INFO')
            return

    if logger is None:
        print('connecting to ftp.astron.nl ...')
    else:
        logger.post('CASACONFIG: connecting to ftp.astron.nl ...', 'INFO')
    ...
    if target not in files:
        if logger is None:
            ...
            logger.post('CASACONFIG: cant find specified version', 'ERROR')
        return
    ...
    with open(os.path.join(path,'measures.ztar'), 'wb') as fid:
        if logger is None:
            ...
            logger.post('CASACONFIG: downloading data from ASTRON server ...', 'INFO')
