...
class ComponentConfig(Generic[ComponentType]):
    ...
    def dependency_validator(cls, v, field: ModelField):
        ...
        engine, error = engine_field.validate(v.get('engine'), {}, loc='engine')
        if error:
            raise ValidationError(error, cls)

        options_field = getattr(engine.type_, 'Settings', None)
        if options_field is not None:
            opts = options_field.validate(v.get('options', {}))
        else:
            opts = {}
        return cls(engine=engine, options=opts)
