...
def get_next_monthly_recurring_date(input_date_format, output_date_format):
    current_date = get_today_date(input_date_format)

    current_date_obj = datetime.strptime(current_date, input_date_format)

    current_date_time_obj = datetime.strptime(current_date, input_date_format)
    ...
    last_date_of_current_month = datetime(current_year, current_month, rng[1]).strftime(input_date_format)
    if current_date_str == last_date_of_current_month:
        if current_month != 2:
            next_monthly_recurring_date_obj = get_next_recurring_date(current_date_obj, 'M')
            if current_day > next_monthly_recurring_date_obj.day:
                ...
                if current_date is not None and current_date_time_obj.day == 31:
                    month_range = calendar.monthrange(next_monthly_recurring_date_obj.year,
                                                      next_monthly_recurring_date_obj.month)
                    last_date_of_next_month = datetime(next_monthly_recurring_date_obj.year,
                                                       next_monthly_recurring_date_obj.month,
                    ...
                    current_day_of_next_month = datetime(next_monthly_recurring_date_obj.year,
                                                         next_monthly_recurring_date_obj.month,
                                                         current_day).strftime(output_date_format)
            ...
            if current_date is not None:
                next_month_recurring_date = datetime(next_monthly_recurring_date_obj.year,
                                                   next_monthly_recurring_date_obj.month,
                                                   current_date_time_obj.day).strftime(output_date_format)
        ...
        next_monthly_recurring_date_obj = get_next_recurring_date(current_date_obj, 'M')
        if next_monthly_recurring_date_obj.month == 2 and (current_day == 29 or current_day == 30 or current_day == 31):
            month_range = calendar.monthrange(next_monthly_recurring_date_obj.year,
                                              next_monthly_recurring_date_obj.month)
