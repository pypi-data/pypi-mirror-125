...
def cli():
    ...
    ** how to use **
    ...
    dsf-cli create-cloud-eval-files
...
def read_template_file(filename):
    with open(os.path.join(__location__, 'cli/' + filename + '_template.py'), 'r') as file:
        data = file.read()
        return data
...
def copy_cloud_eval_files(main_dir):
    ...
    dir = os.path.join(__location__, 'cli/tester/cloud_eval/')
    listOfFiles = list()
    conflicted_files = []
    for (dirpath, dirname, filenames) in os.walk(dir):
