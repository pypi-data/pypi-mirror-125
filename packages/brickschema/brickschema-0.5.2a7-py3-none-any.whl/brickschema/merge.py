...
def _merge_features(fields, g1_features, g2_features):
    ...
    if len(linked_entities) != len(g1_features) or len(linked_entities) != len(g2_features):
...
def merge_type_cluster(g1, g2, namespace, similarity_threshold=0.9, merge_types=None):
    ...
    for etype, cluster in clusters.items():
        ...
        while True:
            pprint(g1_features)
            pprint(g2_features)
            linked_records, leftover_g1, leftover_g2 = _merge_features(fields, g1_features, g2_features)
            for link in linked_records:
                (e1, e2), similarity = link
        ...
        if use_previous:
            ...
            try:
                ...
                while True:
                    ...
                    if len(set([x['uri'] for x in record_pair]).intersection(mapped_items)) > 0:
