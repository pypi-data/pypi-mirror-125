...
class ReportingService:
    ...
    def start_test(self, report: TestReport, item: Item) -> None:
        ...
        if zebrunner_context.test_is_active:
            references = [(str(mark.args[0]), str(mark.args[1])) for mark in item.iter_markers("artifact_reference")]
            models = [ArtifactReferenceModel(name=r[0], value=r[1]) for r in references]
            self.api.send_artifact_references(models, zebrunner_context.test_run_id, zebrunner_context.test_id)
