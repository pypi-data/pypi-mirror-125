...
class Command(click.Command):
    ...
    def __init__(self, *args, description, example, version, **kwargs):
        ...
        # Make sure that relations are specified properly and implicitely add them to
        # both related options if not already the case. For excluded options, also check
        # if no defaults are specified in the respective options.
        for name, option in self.option_map.items():
            for attr in ["requires", "excludes"]:
                for relation in getattr(option, attr):
                    if name == relation:
                        raise ValueError(
                            f"Option '{name}' cannot require or exclude itself."
                        )
                    ...
                    if related_option is None:
                        raise ValueError(
                            f"There is no option '{relation}' that option '{name}' can"
                            " relate to."
                        )

                    if attr == "excludes" and (
                        option.default is not None or related_option.default is not None
                    ):
                        raise ValueError(
                            f"Option '{name}' and option '{relation}' exclude each"
                            " other, but at least one option specifies a default value."
                        )

                    if name not in getattr(related_option, attr):
                        getattr(related_option, attr).append(name)

    def invoke(self, ctx):
        # Check the requirements and exclusions of all options.
        for name, option in self.option_map.items():
            for requirement in option.requires:
                related_option = self.option_map[requirement]

                if ctx.params[related_option.name] is None:
                    raise UsageError(
                        f"Option '{name}' requires option '{requirement}'."
                    )

            for exclusion in option.excludes:
                if ctx.params[option.name] is not None:
                    related_option = self.option_map[exclusion]

                    if ctx.params[related_option.name] is not None:
                        raise UsageError(
                            f"Option '{name}' excludes option '{exclusion}'."
                        )
