...
class LoxIntelligentRoomControllerV2:
    ...
    def set_value(self, stateName, value):
        if self._device_type == "IRoomControllerV2" and stateName == "tempActual":
            _LOGGER.debug("id:'{0}', name:'{1}', [Current Temp] - state={2}".format(self._id, self._name, value))
            self._current_temp = value

            self.async_update()

        if self._device_type == "IRoomControllerV2" and stateName == "tempTarget":
            _LOGGER.debug("id:'{0}', name:'{1}', [Target Temp] - state={2}".format(self._id, self._name, value))
            self._target_temp = value

            self.async_update()

        if self._device_type == "IRoomControllerV2" and stateName == "demand":
            _LOGGER.debug("id:'{0}', name:'{1}', [Demand] - state={2}".format(self._id, self._name, value))
