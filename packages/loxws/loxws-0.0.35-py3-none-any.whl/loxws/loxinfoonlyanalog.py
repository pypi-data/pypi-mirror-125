...
class LoxInfoOnlyAnalog:
    ...
    def format(self):
        ...
        if 'format' in self._details:
            control_format = self._details["format"]
            _LOGGER.debug("id:'{0}', name:'{1}', format:'{2}'".format(self._id, self._name, control_format))
    ...
    def set_value(self, stateName, value):
        if self._device_type == "InfoOnlyAnalog" and stateName == "value":
            _LOGGER.debug("id:'{0}', name:'{1}', [SetValue] state='{2}'".format(self._id, self._name, value))
            self._state = value
