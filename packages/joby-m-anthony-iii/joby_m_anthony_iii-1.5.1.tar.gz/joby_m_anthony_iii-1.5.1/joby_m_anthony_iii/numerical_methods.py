...
def condition_number(A, norm_type="l_two"):
...
class MultiVariableIteration:
	def __init__(self, A, x0, b, power=-6, N=100, norm_type="l_infinity"):
		...
		if N <= 0 or not isinstance(N, int): raise ValueError(f"ERROR! {bad_N}")
		if norm_type != "l_infinity" and norm_type != "l_two": raise ValueError(f"ERROR! {bad_type}")
		n = len(x0)
		self.A = A
		self.x0 = x0 # np.reshape(x0,(n,1))
		self.b = b # np.reshape(b,(n,1))
