...
# common error messages or error message expressions
...
not_square = lambda matrix_name: f"Matrix, {matrix_name} must be square!"
...
def diagonality(A):
	...
	"""
	sym_A = varname(A)
	good_matrix = lambda matrix_name: f"Information: Matrix, {matrix_name} is strictly, diagonally dominant."
	bad_matrix = lambda matrix_name: f"Warning! Matrix, {matrix_name} is not strictly, diagonally dominant. Solution may be inaccurate."
	A = np.array(A)
	if not(np.sum(np.shape(A)) - np.shape(A)[0] == np.shape(A)[0]):
		raise IndexError(not_square(sym_A))
...
def eigen_values(A):
	...
	sym_A = varname(A)
	if not(np.sum(np.shape(A)) - np.shape(A)[0] == np.shape(A)[0]):
		raise ValueError(f"ERROR! {not_square(sym_A)}")
	A = np.array(A)
	sym_r = sp.Symbol("r")
...
class norms:
	def l_infinity(x, x0=[]):
		...
		if x0.shape[0] == 0:
			...
			raise IndexError("ERROR! 'x', and 'x0' must be the same size!")
	...
	def l_two(x, x0=[]):
		...
		if x0.shape[0] == 0:
			...
			raise IndexError("ERROR! 'x', and 'x0' must be the same size!")
		return norm
# preceded by norms.l_two and norms.l_infinity
def condition_number(A, norm_type="l_infinity"):
	...
	See Also
	--------
	norms.l_two() : Yields the l_2 norm.

	norms.l_infinity() : Yields the l_infinity norm.
	"""
	sym_A = varname(A)
	A = np.array(A)
	...
	if norm_type == "l_infinity":
		norm, abnorm = norms.l_infinity(A), norms.l_infinity(A_inv)
	elif norm_type == "l_two":
		norm, abnorm = norms.l_two(A), norms.l_two(A_inv)
	k = norm*abnorm
	print("Information: Condition Number K(" + sym_A + ") = ", k)
	return k

def make_array(X, f, variable=sp.Symbol("x")):
	...
	f : expression
		Equation that maps the domain to range.
	...
	if isinstance(f, (FunctionType, sp.Expr)):
		sym_f = sp.N(sp.sympify(f(variable)))
		f = sp.lambdify(variable, sym_f)
		print(func_func)
	X = np.array(X)
...
def symmetry(A):
	...
	"""
	sym_A = varname(A)
	symmetric = lambda matrix_name: f"Matrix, {matrix_name} is symmetric."
	asymmetric = lambda matrix_name: f"Matrix, {matrix_name} is not symmetric."
	if not(np.sum(np.shape(A)) - np.shape(A)[0] == np.shape(A)[0]):
		raise IndexError(f"ERROR! {not_square(sym_A)}")
	A = np.array(A)
	...
	for ai in A:
		...
		for aj in ai:
			if aj == At[i][j]: is_symmetric = True
				...
				is_symmetric = False
				print(f"Warning! {asymmetric(sym_A)}")
				return is_symmetric
			j += 1
		i += 1
	if is_symmetric: print(f"Information: {symmetric(sym_A)}")
	return is_symmetric
...
def tridiagonality(A):
	...
	"""
	sym_A = varname(A)
	bad_matrix = lambda matrix_name: f"Warning! Matrix, {matrix_name} is not tridiagonal."
	good_matrix = lambda matrix_name: f"Information: Matrix, {matrix_name} is tridiagonal."
	if not(np.sum(np.shape(A)) - np.shape(A)[0] == np.shape(A)[0]):
		raise IndexError(f"ERROR! {not_square(sym_A)}")
	A = np.array(A)
...
class SingleVariableIteration:
	def __init__(self, function, a, b, power, variable=sp.Symbol("x"), iter_guess=True, k=0):
		"""
		Parameters
		----------
		function : expression
		...
		self.k : float, optional
			Absolute maximum slope of `f`. Assumed `0` if not defined.
	...
	def find_k(self):
		...
		----------
		self : object
			Use instanced object parameters.
	...
	def max_iterations(self, method, p0=0):
		...
		Warnings
		--------
		number_of_iter : string
			Informs user of maximum number of iterations.
		...
		bad_method = "I am sorry. The desired method must be: `bisection`, `fixed_point`, `newton_raphson`, `secant_method`, or `false_position`."
		...
		else: raise ValueError(f"ERROR! {bad_method}")
		self.iter_max = N_max
		print(f"Information: {number_of_iter}{N_max}")
		return N_max
	...
	def bisection(self):
		...
		if isinstance(f, (FunctionType, sp.Expr)):
			sym_f = sp.N(sp.sympify(f(self.variable)))
			f = sp.lambdify(self.variable, sym_f)
			...
			if f(a)*f(b) < 0:
				...
				else: raise ValueError(f"ERROR! {bad_iter}")
				...
				while error >= tol and i <= N:
					...
					errors.append(error); iterations.append(i)
					i += 1				  # iterate to i + 1
				if i < N: print(f"Congratulations! {solution_found}")
				else: print(f"Warning! {solution_not_found}")
				self.iterations = iterations
				self.approximations = approximations
				self.errors = errors
			# abort if f(a) is not opposite f(b)
			else: raise ValueError(f"ERROR! {opposite_signs}")
		# abort if not expression
		else: raise TypeError(f"ERROR! {must_be_expression}")
	...
	def fixed_point(self, p0):
		...
		if isinstance(f, (FunctionType, sp.Expr)):
			...
			while error >= tol and i <= N:
				p = f(p0)			   # new value, p
				approximations.append(p)
				error = abs((p - p0)/p0)	 # error of new value, p
				errors.append(error); iterations.append(i)
	...
	def newton_raphson(self, p0):
		...
		if isinstance(f,(FunctionType, sp.Expr)):
			...
			f = sp.lambdify(self.variable, sym_f)
			# determine form of derivative
			df = sp.lambdify(self.variable, sp.diff(sym_f))
			# initialize lists
			iterations, approximations, errors = [], [], []
			if iter_guess == True and k == 0:
				# if left unassigned, guess
				...
				N = iter_guess
			# else, break for bad assignment
			else: raise ValueError("ERROR! " + bad_iter)
			i, error = 0, tol*10		# initialize
			# exit by whichever condition is TRUE first
			while error >= tol and i <= N:
				fp0 = f(p0)
		...
		if isinstance(f, (FunctionType, sp.Expr)):
			...
			if f(p0)*f(p1) < 0:
				...
				if iter_guess == True and k == 0:
					...
					N = (iter_guess)
				# else, break for bad assignment
				else: raise TypeError(f"ERROR! {bad_iter}")
				i, error = 0, tol*10		# initialize
				# exit by whichever condition is TRUE first
				while error >= tol and i <= N:
					q0, q1 = f(p0), f(p1)
...
class MultiVariableIteration:
	def __init__(self, A, x0, b, power=-6, N=100, norm_type="l_infinity"):
		"""
		Parameters
		----------
		self : object
		...
		Warnings
		--------
		Not recommended to use `eigen_values()` to find eigenvalues of characteristic matrix, A; therefore, do not use if matrix, A is a large, sparse matrix if desiring quick calculations.

		See Also
		--------
		spectral_radius() : finds the spectral radius of characteristic matrix, A.
		...
		bad_x0 = lambda vector_name: f"Systems vector, {vector_name} must be n x 1 or 1 x n array!"
		bad_b = lambda vector_name: f"Input vector, {vector_name} must be n x 1 or 1 x n array!"
		bad_N = "Maximum iterations, N must be an integer greater than zero."
		bad_type = "Desired norm type was not understood. Please choose 'l_infinity' or 'l_two'."
		A, x0, b = np.array(A), np.array(x0), np.array(b)
		if np.sum(A.shape[0]) != np.sum(A.shape[1]): raise IndexError(f"ERROR! {not_square(sym_A)}")
		if np.sum(x0.shape) > np.sum(x0.shape[0]): raise IndexError(f"ERROR! {bad_x0(sym_x0)}")
		if np.sum(b.shape) > np.sum(b.shape[0]): raise IndexError(f"ERROR! {bad_b(sym_b)}")
		if N <= 0 or not isinstance(N, (int, float)): raise ValueError(f"ERROR! {bad_N}")
		if norm_type != "l_infinity" and norm_type != "l_two": raise ValueError(f"ERROR! {bad_type}")
		n = len(x0)
		self.A = np.array(A)
		self.x0 = np.array(x0) # np.reshape(x0,(n,1))
		self.b = np.array(b) # np.reshape(b,(n,1))
	...
	def find_omega(self, omega=0):
		...
		will_converege : string
			If 0 < omega < 2, then method will converge regardless of choice for `x0`.
		...
		non_triad = lambda matrix_name: f"Matrix, {matrix_name} is not tridiagonal."
		incalculable = lambda matrix_name: f"I could not determine if matrix, {matrix_name} was positive definite."
		...
		self.symmetry = symmetry(A)
		if self.symmetry and state: theorem_6_22 = True
		...
		if theorem_6_22 or theorem_6_25:
			if 0 < omega and omega < 2: print(will_converge)
			...
			if self.is_tridiagonal:
				...
				print(f"Warning! {non_triad(sym_A)}")
				print(f"Assigning supplied omega, {omega} as `self.best_omega`.")
		else:
			print(f"Warning! {incalculable(sym_A)}")
			print(f"Assigning supplied omega, {omega} as `self.best_omega`.")
	...
	def gauss_seidel(self):
		...
		Warnings
		--------
		solution_found : Inform user that solution was indeed found.

		solution_not_found : If initial guess or tolerance were badly defined.
		...
		k, n = 0, len(x0)
		iterations, approximations, errors = [k], [x0], [norm]
		while norm > tol and k <= N:
			...
			while i < n:
				...
				i += 1
			if norm_type == "l_infinity":
				norm = norms.l_infinity(xi, x0)
			elif norm_type == "l_two":
				norm = norms.l_two(xi, x0)
			iterations.append(k)
			approximations.append(xi)
			errors.append(norm)
			x0 = xi
			k += 1
		if k < N: print(f"Congratulations! {solution_found}")
		else: print(f"Warning! {solution_not_found}")
	...
	def newton_raphson(f, symbols, x0, powers, N, norm_type=None):
		"""Given f(x) and initial guess, `p0` in [`a`,`b`], find x within tolerance, `tol`.
		...
		f, x0, b, norm = self.A, self.x0, self.b, self.tol*10
		xi = np.zeros_like(x0)
		X0, error = [], []
		k, n = 0, len(x0)
		for each in symbols:
		...
		for k in range(1, N):
			...
			if norm_type == "l_two":
				...
				if tol_exit == 0:
					return pd.DataFrame(data={"Iterations": range(0, k),"x": X0, "Error": error})
				else: x0 = xk
			else: raise ValueError("")
		return x0
	...
	def successive_relaxation(self, omega=None):
		...
		"""
		calculate_omega = lambda w: f"omega was not given; therefore, I attempted to choose one, {w}."
		optimal_omega = lambda omega, w: f"omega = {omega} given. Which is not optimum: {w}"
		...
		while norm > tol and k <= N:
			...
			while i < n:
				...
				i += 1
			if norm_type == "l_infinity":
				norm = norms.l_infinity(xi, x0=x0)
			elif norm_type == "l_two":
				norm = norms.l_two(xi, x0=x0)
			iterations.append(k)
			approximations.append(xi)
			errors.append(norm)
			x0 = xi
	...
	def clamped(self, variable=sp.Symbol("x"), fp=0):
		...
		def algorithm(g, gp):
			...
			while i <= n:
				...
				i += 1
			return Y, A, B, C, D
		sym_X, sym_f, sym_fp = varname(self.X), varname(self.f), varname(fp)
	...
	def natural(self, variable=sp.Symbol("x")):
		...
		def algorithm(g):
			...
			while i <= n:
				...
				i += 1
			return Y, A, B, C, D
		sym_X, sym_f = varname(self.X), varname(self.f)
		bad_X = "Input domain, " + sym_X + " was neither an n x 1 nor a 1 x n array."
		bad_f = "Input range, " + sym_f + " was neither function nor expression and not an n x 1 or 1 x n array."
		bad_data = "Arrays " + sym_X + " and " + sym_f + " must be of equal length."
		X, f = np.array(self.X), self.f
		if np.sum(X.shape) > np.sum(X.shape[0]): raise ValueError("ERROR! " + bad_X)
		if not isinstance(f, (FunctionType, sp.Expr)):
			f = np.array(f)
			if np.sum(f.shape) > np.sum(f.shape[0]): raise ValueError("ERROR! " + bad_f)
	...
	def linear(X_i, Y_i, n):
		...
		def poly(X):
			...
			for x_i in X_i:
				...
				i += 1
			return p, err
		sym_X_i, sym_Y_i = varname(X_i), varname(Y_i)
...
def richard_extrapolation(function, x0, h, order, direction=0, variable=sp.Symbol("x")):
	...
	if isinstance(function,(FunctionType, sp.Expr)):
		sym_f = sp.N(sp.sympify(function(variable)))
		function = sp.lambdify(variable, sym_f)
		print(func_func)
...
class ode:
	...
	def runge_kutta(f, t0, w0, tn, N):
		...
		if not isinstance(f,(FunctionType, sp.Expr)): raise TypeError("ERROR! " + bad_f)
		if not isinstance(N,(int)) or N == 0: raise ValueError("ERROR!\n" + bad_N)
