...
class Hangman:
    ...
    async def command(self, ctx: commands.Context):
        ...
        embed.add_field(name='Word', value=''.join(revealed_message.split(' ')), inline=False)
        msg = await ctx.send(embed=embed)

        while True:
            embed = discord.Embed(color=discord.Color.blurple())
            embed.add_field(name='Word', value=''.join(f':regional_indicator_{i}:' for i in word), inline=False)
            message: discord.Message = await ctx.bot.wait_for('message', check = lambda m: m.author == ctx.author and m.channel == ctx.channel)
            if len(message.content.lower()) > 1:
                ...
                await ctx.send(f"Invalid Syntax: {message.content.lower()} is not a letter")
