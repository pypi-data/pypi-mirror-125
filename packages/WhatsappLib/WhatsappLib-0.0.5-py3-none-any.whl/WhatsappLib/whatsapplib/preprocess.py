...
from WhatsappLib.whatsapplib.utils import util
...
class analysis(util) :
    """
    This class contains more than 15 methods which will help you to extract very useful insights from the chat.
    Just create an object of the analysis class and pass the location of text file (chat file) and then use the object of the analysis class to use different-different
    ...
    * saveDatframe
    ...
    For more detail checkout documentation of respective method, Each method having short & beautiful summary.

    Made with ❤ by ronil
    """
    def __init__(self, filepath, save_data = 'N') :
        self.df = util._processing(filepath, save_data)
    ...
    def wordCloud(self, save_fig = 'N') :
        ...
        """
        df1 = self.df
        text = analysis._preprocess_text(df1)
    ...
    def maxEmojiUsers(self, save_fig = 'N') :
        ...
        """
        df1 = self.df
        df = pd.DataFrame(analysis._emoji_helper(df1).items(), columns=['Users', 'No. of Emojies']).sort_values('No. of Emojies',
    ...
    def topEmojis_G(self) :
        ...
        """
        df1 = self.df
        return analysis._emoji_help(df1)
    ...
    def topEmojis_I(self, user) :
        ...
        if user in df1['Author'].unique() :
            return analysis._emoji_extractor(user, df1)
        else :
            raise UserNotPresentException('User is not a member of respective group.')
