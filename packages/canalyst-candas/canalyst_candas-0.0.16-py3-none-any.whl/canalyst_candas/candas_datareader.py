...
def plot_eps_surprise_data(df, alpha_days="alpha_10_days"):
    ...
    plt.title(ticker + " earnings suprise and alpha correlation:" + str(i_correlation))
...
def rolling_r_squared(
    ...
    ax1.plot(df["slope"], label="Regression Slope", color=colors["blue"])
    ax1.legend(loc="upper left")

    ax2.plot(df["rsquared"], label="R Squared", color=colors["blue"])
    ax2.legend(loc="upper left")

    ax3.plot(df["value"], label="Time Series Value", color=colors["blue"])
...
def remove_stopwords(sentence):
    my_stopwords = ["in", "Change", "YoY", ",", "%", "(Calculated)", "Total", "growth"]
    tokens = sentence.split(" ")
    tokens_filtered = [word for word in tokens if not word in my_stopwords]
    tokens_filtered = [
        "".join(c for c in s if c not in string.punctuation) for s in tokens_filtered
    ]
