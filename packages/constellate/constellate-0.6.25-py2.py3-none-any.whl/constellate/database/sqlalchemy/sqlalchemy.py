...
from typing import Dict, Tuple, List, Iterator, AsyncGenerator, Optional, Union
...
async def session_scope(
    ...
    if config is not None:
        session_maker.configure(bind=config.engine_config.engine)
    elif len(configs) == 1:
...
class SQLAlchemy:
    ...
    async def _setup_engine_session_maker(self, instance: SQLAlchemyDBConfig = None):
        instance.session_maker = await self._create_session_maker(
            type=SessionType.MONO_ENGINE,
            bind=instance.engine_config.engine,
            autoflush=True,
            future=True,
        )
    ...
    async def session_scope(
        ...
        if bind is not None:
            kwargs.update({"bind": bind})

        async for session in self._session_scope_generator(
            auto_flush=auto_flush, expire_on_commit=expire_on_commit, twophase=twophase, **kwargs
        ):
            yield session
    ...
    async def _session_scope_generator(
        ...
        session_maker = await self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit, twophase=twophase, **kwargs
        )
    ...
    async def session_scope2(
        self, auto_flush=True, expire_on_commit=True, twophase=False
    ) -> AsyncSession:
        """Provide a transactional scope around a series of database operations."""
        session_maker = await self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit, twophase=twophase
        )
