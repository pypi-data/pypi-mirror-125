...
logger = logging.getLogger('matchlight-sdk')

logFormatter = logging.Formatter("%(asctime)s [%(threadName)-12.12s] [%(levelname)-5.5s]  %(message)s")

fileHandler = logging.FileHandler("matchlight_sdk.log")
...
class Connection(object):
    ...
    def _pub_request(self, method, url, data=None, params=None, headers=None, **kwargs):
        try:
            ...
            logger.info("URL: {0}, Response: {1}".format(response.url, response.json()))
