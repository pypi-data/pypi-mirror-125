...
def create_training_job(local_data, source_code, weights, hyper_param):
    ...
    if local_data is not None:
        ...
        if local_data.startswith('s3://'):
            ...
            with open("./s3_public_dataset.txt", 'wb') as s3_public:
                s3_public.write(local_data)
            __upload_one_file(job_id, FileType.Traindata.name, s3_public)
            print(f"use s3 public dataset{local_data}")
        else:
        #*****haiyan*****
