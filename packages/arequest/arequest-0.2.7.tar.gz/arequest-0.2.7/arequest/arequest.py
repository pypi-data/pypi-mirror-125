...
__all__ = ["get", "post", "head", "put", "delete", "options", "patch", "Session", "ConnectionError", "TimeoutError", "nameservers"]
...
class Session:
    ...
    async def _request(self, method, url, params=None, data=None, headers=None,
        ...
        if self.keepAlive and self.writer:
            ...
            if ipRegex.match(url.hostname):
                ip = url.hostname
            else:
                resolver = aiodns.DNSResolver(nameservers=nameservers, loop=asyncio.get_running_loop())
                ip = (await resolver.gethostbyname(url.hostname, socket.AF_INET)).addresses[0]

            if url.scheme == "https":
                if not verify:
                    context = ssl.SSLContext(ssl.PROTOCOL_TLS)
                reader, writer = await asyncio.open_connection(
                    ip, url.port or 443, ssl=verify or context, server_hostname=sslHostname or url.hostname)
        ...
        if self.keepAlive and r.headers.get("Connection") == "keep-alive":
