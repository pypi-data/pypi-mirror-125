...
def get_current_commit_hash(repo):
    ...
    try:
        ...
        try:
            ...
            return "Hermes was unable to retrieve this information"
...
class Experiment:
    ...
    def new(cls, script_name, execution_file, job_idx, experiment_name):
        ...
        **This method is used internally by CLI Hermes and not intended to be used outside of that scope.**
        ...
        commit_hash = get_current_commit_hash(
            repo
        )  # TODO this should be script specific
    ...
    def load(cls, path_to_experiment):
        ...
        script_name, experiment_name, identifier = p.parts[-3:]
        info = Experiment.read_info(path_to_experiment)
        commit_hash = info["commit_hash"]
        try:
            start_time = time.strptime(info["start_time"], Experiment.TIME_FMT)
        except:
            try:
                start_time = float(info["start_time"])
                logging.info(
                    "Failed to load experiment start time as time.struct_time, loaded as floating point number instead."
                )
            except:
                start_time = None
        ...
        if hasattr(self, "_exec_args_dict"):
            ...
            for arg in self.exec_args:
                ...
                if len(aux) <= 1:
                    ...
                    logging.warning("Unknown exec argument format: %s" % arg)
                    key = aux[0]
                    vals = aux[1:]
                    try:
    ...
    def load_network_class(self, keys=["--neural_network_file", "-nnf"], default="fcn"):
        ...
        for key in keys:
            ...
            possible_paths = [nnf]
            try:
                nnf_string_splitted = nnf_string.split(".")
    ...
    def store_times(self, start_time, end_time):
        ...
        self._add_to_info(
            Experiment.START_TIME_N, time.strftime(Experiment.TIME_FMT, start_time)
        )
        self._add_to_info(
            Experiment.END_TIME_N, time.strftime(Experiment.TIME_FMT, end_time)
        )
    ...
    def _fetch_new_files(self):
        ...
        for part in dir_list:
            ...
            if not self.identifier in part:
                continue
            self.move_file(part)
