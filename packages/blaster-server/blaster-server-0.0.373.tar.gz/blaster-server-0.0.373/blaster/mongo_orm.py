...
class Model(object):
			...
			if(is_shard_key_changed or force):
				...
				if(_secondary_collection and _before_id != None):
					...
					IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="deleting from secondary",
						seconday_pk=_secondary_pk, model=str(secondary_Model), collection=COLLECTION_NAME(_secondary_collection)
					)
				...
				if(_after_shard_key != None and after_doc["_id"]):
					...
					IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="deleting and inserting new secondary",
						...
						model=str(secondary_Model), collection=COLLECTION_NAME(_secondary_collection)
					)
				...
				if(_secondary_values_to_set or _secondary_values_to_unset):
					...
					IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="updating secondary",
						model=str(cls), collection=COLLECTION_NAME(_secondary_collection),
					...
					if(not secondary_shard_update_return_value):
						LOG_WARN("MONGO", description="secondary couldn't be propagated, probably due to concurrent update",
								model=str(cls), collection=COLLECTION_NAME(_secondary_collection), secondary_model=str(secondary_Model), secondary_pk=str(_secondary_pk),
	...
	def update(self, _update_query, more_conditions=None, **kwargs):
		...
		while(retries > 0):
			...
			IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="update before and query",
				model=str(cls), collection=COLLECTION_NAME(primary_collection_shard), query="_query", original_doc=str(self._original_doc), update_query=str(_update_query)
			)
			...
			IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="after update",
				model=str(cls), collection=COLLECTION_NAME(primary_collection_shard),
		...
		**kwargs
		...
		def query_collection(_Model, _collection, _query, offset=None):
			...
			IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO",
				description="querying", model=str(_Model), collection=COLLECTION_NAME(_collection),
			...
			if(_Model._is_secondary_shard):
				...
				def batched_requery_iter(ret, n=200):
					for docs in batched_iter(ret, n):
						...
						IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="requerying", model=str(cls), query=str(_query))
						...
						for _id in _ids_order:
							...
							if(not item):
								IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="missing from primary", model=str(cls), _id=str(_id))
								continue
							yield item
	...
	def commit(self, force=False, ignore_exceptions=None):
		...
		if(self.__is_new): # try inserting
			...
			IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="new object values",
				model=str(cls), collection=COLLECTION_NAME(_collection_shard), set_query=str(self._set_query_updates)
			)
			try:
	...
	def delete(self):
		...
		IS_DEV and MONGO_DEBUG_LEVEL > 1 and LOG_SERVER("MONGO", description="deleting from primary",
			model=str(_Model), collection=COLLECTION_NAME(collection_shard), delete_query=str(_delete_query)
		)
		if(_Model.__cache__):
		...
		if(Model._is_secondary_shard):
			...
			if(to_add_attrs or to_remove_attrs):
				try:
					...
					if(not getattr(Model, "_is_migrating_", False)):
						raise Exception("There is already a pending job to update attributes, it needs to be finished or set _is_migrating_ = True inside the " + str(Model) + " class")
					Model._attrs_to_backfill_ = {}
					for _attr_name in to_remove_attrs:
