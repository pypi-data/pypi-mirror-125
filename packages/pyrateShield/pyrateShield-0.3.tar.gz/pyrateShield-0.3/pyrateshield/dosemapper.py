...
from pyrateshield import labels, Logger
...
class Worker(multiprocessing.Process, Logger):
    ...
    def update_project(self):        
        if self.project.dosemap.engine == labels.PYSHIELD:
            #self.barriers = pyshield.Barriers.from_pyrateshield(self.project)
    ...
    def get_dosemap(self, source):
        if self.project.dosemap.engine == labels.PYSHIELD:
            ...
            # return pyshield.Sources.from_pyrateshield(
            #             model=self.project,
            #             source=source, 
...
class Dosemapper(Logger):
    def __init__(self, processes=None):
        ...
        self.max_cpu_count = psutil.cpu_count(logical=True)
    ...
    def get_sources_critical_points(self, project):
        sources = list(project.sources_nm) + list(project.sources_ct) + list(project.sources_xray)
        ...
        for crit_point in crit_points:
            for source in sources:
                ...
                reports += [self.get_source_critical_point_result(
    ...
    def get_critical_points(self, project):
        ...
        summed_report = pd.pivot_table(report, index=CRITICAL_POINT_NAME, aggfunc=sum)
