...
def validate_instance_type(
    instance_type_list: list, provider_name: Optional[str] = None
):
    ...
    if provider_name:
        allowed_instance_types = [*all_instances[provider_name]]
    else:
    ...
    for instance_type in instance_type_list:
        if provider_name:
            if instance_type not in allowed_instance_types:
                error_msg = (
                    f"Instance type '{instance_type}' is not a valid instance type "
                    f"for the cloud provider '{provider_name}'. Please check your "
                    f"account backend options. Allowed instances are: {allowed_instance_types}"
                )
                raise InstanceTypeError(error_msg)
        else:
            if instance_type not in allowed_instance_types:
