...
class Cluster(distributed.deploy.Cluster, Generic[IsAsynchronous]):
    ...
    worker_gpu_type
        Type of GPU to use for this cluster. For AWS and Azure GPU types are tied to the instance type, but for
    ...
    async def _start(self):
        ...
        with console.status(
            ...
            if should_create:
                provider_name = await cloud.get_account_provider_name(self.account)
                if self.worker_vm_types:
                    validate_instance_type(self.worker_vm_types, provider_name)
                if self.scheduler_vm_types:
                    validate_instance_type(self.scheduler_vm_types, provider_name)
            ...
            backend_options = await cloud.get_backend_options(account=self.account)
            public_http_enabled = backend_options["options"].get(
                "enable_public_http", True
            )

            public_ingress_disabled = backend_options["options"].get(
                "disable_public_ingress", False
            )

            try:
                if not public_ingress_disabled and public_http_enabled:
