...
async def test_normalize_name_uppercase_account(cloud, cleanup):
    ...
    assert cloud._normalize_name(name=cluster_name) == (
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
def test_setting_backend_options_private_address(cloud, sample_user, mocker):
    mocker.patch("coiled.utils.boto3")
    account = sample_user.account
    ...
    # disabling http and ssh should be reflected in the backend options
    coiled.set_backend_options(
        backend="aws",
        aws_region=settings.AWS_DEFAULT_USER_REGION,
        create_vpc=True,
    ...
    # We want aws_public_http == False and aws_public_ssh == False
    assert "enable_public_http" in account.options.keys()
    assert "enable_public_ssh" in account.options.keys()
    assert "disable_public_ingress" in account.options.keys()
    assert account.options.get("enable_public_http") is False
    assert account.options.get("enable_public_ssh") is False
    assert account.options.get("disable_public_ingress") is True
    ...
    assert kwargs_options.enable_public_ssh is False
    assert kwargs_options.enable_public_http is False

    # default values should reflect public ingress being enabled
    #    enable_public_http=True
    #    enable_public_ssh=True
    #    disable_public_ingress=False
    coiled.set_backend_options(
    ...
    assert account.options.get("enable_public_http") is True
    assert account.options.get("enable_public_ssh") is True

    assert "disable_public_ingress" in account.options.keys()
    assert account.options.get("disable_public_ingress") is False
