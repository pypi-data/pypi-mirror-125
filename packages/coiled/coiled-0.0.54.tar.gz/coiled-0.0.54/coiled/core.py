...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def get_backend_options(self, account: str = None) -> dict:
        ...
        account = account or self.default_account
        response = await self._do_request(
            # Account needs to be lower case or an error is thrown
            "GET",
            self.server + f"/api/v1/{account.lower()}/backend_options/",
        )
        if response.status >= 400:
            await handle_api_exception(response)
...
def set_backend_options(
    ...
    enable_public_http: bool = True,
    enable_public_ssh: bool = True,
    disable_public_ingress: bool = False,
    ...
    **kwargs,
    ...
    enable_public_http
        This argument determines if public ingress for HTTP is enabled.

    enable_public_ssh
        This argument determines if public ingress for SSH is enabled. Useful for debugging and viewing
        cluster logs.

    disable_public_ingress
        Determines if any incoming traffic from outside the VPC is enabled. Overrides other arguments.
    ...
    backend_options = {
        ...
        "options": {
            ...
            "enable_public_ssh": enable_public_ssh,
...
def list_instance_types(backend: str = "", json: bool = False) -> Optional[Dict]:
    ...
    ----------
    backend
        Relevant cloud provider to get a list of allowed instance types. If not provided
        the list will show all instances for AWS, GPU and Azure.
    json
...
def list_gpu_types(json: bool = False) -> Optional[Dict]:
    ...
    For AWS and Azure the GPU types are tied to the instance type, but for GCP you can
