...
class StringMatch():
    ...
    def tokenize(self, analyzer='char_wb', n=3):
        ...
        self.tfidf_vect  = TfidfVectorizer(vocabulary=self.vocab, analyzer=analyzer, ngram_range=(n, n))#ACREDITO QUE QUERIA USAR O TFIDTRANSFORMER... OU O TFIDVECTORIZER
...
def select_by_scoremin(match_df, score_min):
    """
    ...
    """
    df = match_df.loc[match_df.Score > score_min]
    return df

def do_string_matching(source_data, target_data, score_min: float = 0.0, column: str = 'end_completo',
    ascending: bool = True):
    

    if ((column in source_data) and (column in target_data)):
        source_addresses = source_data[column].to_list()
        target_addresses = target_data[column].to_list()
    else:
        print("É necessario duas colunas com a mesma identificação")
    

    titlematch = StringMatch(source_addresses, target_addresses)
    titlematch.tokenize()
    match_df = titlematch.match()

    match_df = select_by_scoremin(match_df, score_min)
