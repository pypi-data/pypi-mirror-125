...
class Postgres(object):
    ...
    def over_insert_df(self, df, table_name, text_column, date_columns=None):
        conf = self.conf
        conn = psycopg2.connect(database=conf['database'],
                                user=conf['user'],
                                password=conf['password'],
        ...
        c0 = df.columns.tolist()
        data_list = df.values.tolist()
        data_list.insert(0, c0)

        if date_columns:
            text_column.extend(date_columns)
        ...
        for row in data_list:
            if count == 0:
                ...
                if not text_column:
                    text_column = column_list
                sql_str = ','.join(['"{}" float'.format(j.replace(" ", "_")) if j not in text_column
                                    else '"{}" text'.format(j.replace(" ", "_"))
                                    for j in column_list])
                if not date_columns:
                    date_columns = []
    ...
    def over_insert_df_pro(self, df, table_name, text_column, date_columns=None):
        conf = self.conf
        conn = psycopg2.connect(database=conf['database'],
                                user=conf['user'],
                                password=conf['password'],
        ...
        if not self.is_table_exist(table_name):
            ...
            cursor.execute('create table {} ({});'.format(table_name, sql_str))
        cursor.copy_from(f, table_name,
                         columns=columns,
                         sep=',', null='', size=16384)  # 默认sep和null 都是none
