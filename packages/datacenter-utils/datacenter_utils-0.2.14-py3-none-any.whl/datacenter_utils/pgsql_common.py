    ...
    def over_insert_df(self, df, table_name, text_column, date_columns=None):
        ...
        for row in data_list:
            if count == 0:
                ...
                sql_str = ','.join([f'"{j.replace(" ", "_")}" float' if j not in text_column
                ...
                # 自定义数据类型
                if not self.is_table_exist(table_name):
                    cursor.execute(f'create table {table_name} ({sql_str});')
                    print('create table', sql_str)
            else:
                sql = f"INSERT INTO {table_name} VALUES ({','.join(['%s' for i in range(len(row))])})"
                for i in range(len(row)):
