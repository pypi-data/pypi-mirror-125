...
class KlatApi(KlatApiABC):
    ...
    def __init__(self, socket, dom="klattalk.com", init_nick=None):
        ...
        self.klat_ready.wait(30)
        if not self.klat_ready.is_set():
    ...
    def conversation_users(self) -> list:
        ...
        if time.time() - self._users_last_updated > self._users_list_cache or not self._users:
            ...
            self._await_users_list.wait(self._get_users_timeout)
            if not self._await_users_list.is_set():
    ...
    def logout_klat(self):
        ...
        self._await_logout.wait(30)
        if not self._await_logout.is_set():
            self.socket.disconnect()
    ...
    def register_klat(self, username, password):
        ...
        self._await_login.wait(30)
        if not self._await_login.is_set():
            self.socket.disconnect()
    ...
    def change_domain(self, new_dom: str):
        ...
        self._await_change_conversation.wait(30)
        if not self._await_change_conversation.is_set():
            self.socket.disconnect()
    ...
    def create_conversation(self, dom: str, title: str, goto: bool = False,
        ...
        self.socket.emit("create conversation", (dom, self._nick, self._cid, title, "", "", None, "",
                                                 first_shout, None))
        self._await_new_conversation.wait(30)
        if not self._await_new_conversation.is_set():
            self.socket.disconnect()
    ...
    def get_conversation_log(self, follows: int = 10, context: int = 20,
        ...
        self._await_log.wait(30)
        if not self._await_log.is_set():
            self.socket.disconnect()
