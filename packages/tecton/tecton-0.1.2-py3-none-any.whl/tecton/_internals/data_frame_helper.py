...
class _FeatureSetQueryBuilder(object):
    ...
    def build(self) -> DataFrame:
        ...
        return self._join(parent_df, feature_df)
    ...
    def _validate(self):
        ...
        self.use_materialized_data = self.fd._validate_and_return_use_materialized_data(self.use_materialized_data)
    ...
    def _rename_fv_cols_to_fs_names(self, feature_df):
        ...
        for spine_column_name, package_column_name in self.override_join_keys.items():
            feature_df = feature_df.withColumnRenamed(package_column_name, spine_column_name)
        return feature_df
...
def _sort_key_for_spine(package_and_config):
    ...
    fv = package_and_config.feature_definition
    if fv.is_on_demand:
        return 3
...
def get_features_for_spine(
    ...
    for package_and_config in feature_set_config._get_feature_definitions_and_join_configs():
        if from_source and is_bfc_mode_single(package_and_config.feature_definition):
        ...
        for spine_key, fp_key in package_and_config.join_keys:
            if spine_key not in spine_columns:
                if fp_key == package_and_config.feature_definition.wildcard_join_key:
                    wildcard_key_not_in_spine = True
                    wildcard_join_key = package_and_config.feature_definition.wildcard_join_key
                else:
    ...
    for package_and_config in sorted(
        ...
        definition = package_and_config.feature_definition
        parent_df = df
...
def _sort_key(package_and_config):
    ...
    fd = package_and_config.feature_definition
...
def _get_feature_dataframe_with_limits(
    ...
    if not use_materialized_data or not fd.materialization_enabled:
        tc._register_temp_views_for_feature_view(
            fd=fd,
            register_stream=False,
            raw_data_time_limits=raw_data_time_limits,
        )
    use_materialized_data = fd._validate_and_return_use_materialized_data(use_materialized_data)
...
def get_features_for_preview(
    ...
    for package_and_config in sorted(feature_set_config._get_feature_definitions_and_join_configs(), key=_sort_key):
        ...
        if spine_time_key is None:
            spine_time_key = package_and_config.feature_definition.timestamp_key
        df = _FeatureSetQueryBuilder(
            spark=spark,
            spine_df=None,
            parent_df=df,
