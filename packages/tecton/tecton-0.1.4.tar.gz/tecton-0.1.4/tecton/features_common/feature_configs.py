...
class ExistingClusterConfig:
    ...
    def _to_cluster_proto(self) -> feature_package_pb2.ClusterConfig:
        return feature_package_pb2.ClusterConfig(existing_cluster=self._to_proto())
...
class EMRClusterConfig:
    ...
    def _to_cluster_proto(self) -> feature_package_pb2.ClusterConfig:
        return feature_package_pb2.ClusterConfig(new_emr=self._to_proto())
...
class DatabricksClusterConfig:
    ...
    def _to_cluster_proto(self) -> feature_package_pb2.ClusterConfig:
        return feature_package_pb2.ClusterConfig(new_databricks=self._to_proto())
...
class DynamoConfig:
    ...
    This class describes the attributes of DynamoDB based online feature store storage for the feature definition.
    """

    def _to_proto(self):
        store_config = feature_package_pb2.OnlineStoreConfig()
        store_config.dynamo.no_op = True
...
class RedisConfig:
    ...
    This class describes the attributes of Redis-based online feature store storage for the feature definition.
    ...
    def _to_proto(self):
        store_config = feature_package_pb2.OnlineStoreConfig()
        store_config.redis.no_op = True
        if self.redis_connection_string is not None:
            store_config.redis.primary_endpoint = self.redis_connection_string
            store_config.redis.reader_endpoint = self.redis_connection_string
        else:
