...
class FeatureView(FeatureDefinition):
    ...
    def get_feature_dataframe(
        ...
        if use_materialized_data and not self.type == "OnDemand" and not utils.is_live_workspace(self.workspace):
    ...
    def run(
        ...
        **mock_inputs: Union[pandas.DataFrame, DataFrame],
        ...
        :param aggregate_tiles: Only available for BatchWindowAggregateFeatureView and StreamWindowAggregateFeatureView.
            ...
            If unset, default to True for supported FVs.
        ...
        Examples:
            ...
            1) :py:mod:`fv.run()` Use inputs from the linked DataSources for both ds1 and ds2. feature_end_time is
            defaulted to time now, and feature_start_time is set to time now minus a materialization schedule interval.

            2) :py:mod:`fv.run(feature_start_time=datetime(2021, 6, 21), feature_end_time=datetime(2021, 6, 22))`
            feature_start_time and feature_end_time set by users.
