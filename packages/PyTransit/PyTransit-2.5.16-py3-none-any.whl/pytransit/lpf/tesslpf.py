...
from ..orbits import as_from_rhop, i_from_ba, epoch
from ..param import GParameter, UniformPrior as UP, NormalPrior as NP
from ..utils.tess import read_tess_spoc
from .loglikelihood import CeleriteLogLikelihood
from .lpf import BaseLPF, map_ldc
from ..utils.keplerlc import KeplerLC
from ..utils.misc import fold  #TODO: Change to ..orbits.fold
...
class TTVLPF(TESSLPF):
    ...
    def _init_p_orbit(self):
        ...
        for i in range(self.nlc):
            porbit.append(GParameter(f'tc{i:02d}', f'zero_epoch_{i:02d}', 'd', NP(0.0, 0.1), (-inf, inf)))
        self.ps.add_global_block('orbit', porbit)

    def transit_model(self, pv, copy=True):
        pv = atleast_2d(pv)
        ldc = map_ldc(pv[:, self._sl_ld])
        zero_epoch = pv[:, 3:3 + self.nlc] - self._tref
        period = pv[:, 0]
        smaxis = as_from_rhop(pv[:, 1], period)
        inclination = i_from_ba(pv[:, 2], smaxis)
        radius_ratio = sqrt(pv[:, self._start_k2:self._start_k2 + 1])
