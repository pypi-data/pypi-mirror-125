...
class ImageResizerConfig:
    def __init__(self, src_dir, dest_dir, hash_dir=None):
        ...
        """
        for _dir in [src_dir, dest_dir, hash_dir]:
            if _dir[-1] != "/":
                _dir = _dir + "/"
        ...
        self.COMP_LIST = [
            CompressMethod("w", 0, 400),
            CompressMethod("h", 1, 200),
            CompressMethod("half", 2, 0.5),
        ]
...
class ImageResizer:
    def __init__(
        self,
        src_dir, dest_dir, hash_dir=None
    ):
        self.config = ImageResizerConfig(src_dir, dest_dir, hash_dir)
    ...
    def perform(self):
        ...
        for folder, file_name in changed_list:
            src_file = Path(folder) / file_name

            hash_folder = self.get_dest_folder_path(folder, self.config.HASH_DIR)
            hash_file = hash_folder / file_name

            try:
                changed_img = Img(Path(folder) / file_name)

                for method in self.config.COMP_LIST:
                    dest = f"{self.config.COMP_DIR}{method.name}/"
                    dest_folder = self.get_dest_folder_path(folder, dest)
                    changed_img.compress_img_to(dest_folder, method)

            except UnsupportedExtensionError:
                pass
            HashAgent.hash_src_save_to_dest(src_file, hash_file)
    ...
    def get_dest_folder_path(self, folder: str, dest_folder: str):
        dest_dir = Dir(dest_folder + "/" + folder.replace(self.path_static, ""))
        if not dest_dir.exists():
            dest_dir.mkdir()

        return dest_dir.get_path()