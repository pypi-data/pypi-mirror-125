...
class MainThreadWorker():
	...
	def interrupt_handler(sig, frame):
		timeout = False
		getAliveThreadWithInterrupt = lambda: any([ThreadWorker.allWorkers[key].is_alive for key in ThreadWorker.allWorkers if ThreadWorker.allWorkers[key].enable_keyboard_interrupt])
	...
	@staticmethod
	def __exitThread():
		@MainThreadWorker.worker("systemExit",keyboard_interrupt=False)
		def go():
			while True:
				time.sleep(0.1)
				if keyboard.is_pressed("CTRL+Z"):
					while all([w.is_alive for w in MainThreadWorker.allWorkers.values()]):
						for key in MainThreadWorker.allWorkers:
							if "systemExit" not in key:
								MainThreadWorker.allWorkers[key].abort()
						time.sleep(0.1)
						break
					break
			MainThreadWorker.abort_all_thread()
		MainThreadWorker.__systemExitThread = go()

	@staticmethod
	def enableKeyboardInterrupt():
		if not MainThreadWorker.keyboard_interrupt_handler_status:
			signal.signal(signal.SIGINT, ThreadWorker.interrupt_handler)
			MainThreadWorker.keyboard_interrupt_handler_status = True
			MainThreadWorker.__exitThread()

	@staticmethod
	def disableKeyboardInterrupt():
		if MainThreadWorker.keyboard_interrupt_handler_status:
			signal.signal(signal.SIGINT, signal.default_int_handler)
			MainThreadWorker.keyboard_interrupt_handler_status = False
			if MainThreadWorker.__systemExitThread:
				MainThreadWorker.__systemExitThread.abort()
