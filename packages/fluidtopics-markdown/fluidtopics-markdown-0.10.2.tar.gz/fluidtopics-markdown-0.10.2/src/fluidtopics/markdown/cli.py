...
from .metadata import FT_ORIGIN_ID, FT_LANG, get_md_metas
    ...
    for file in sorted(curdir.iterdir(), key=lambda f: "    README.md" if f.name == "README.md" else f.name):
        ...
        if file.is_dir():
            ...
            if dirdesc.exists():
                ...
                dirtitle = dirmetas.get(SemanticMetadata.TITLE, file.name)
                ...
                if with_content:
                    content[relf_posix] = {
                        "value": {
                            ...
                            SemanticMetadata.TITLE: dirtitle,
                            "metas": dirmetas,
                        },
                        "childs": dir_content(
                            file,
            ...
            if meta_include_list:
                if audience is None:
                    ...
                    continue
            if metas.get(SemanticMetadata.TITLE, None) is not None:
                content[relf_posix] = {
                    "value": {
                        "href": relf_posix,
                        SemanticMetadata.TITLE: metas[SemanticMetadata.TITLE],
                        "metas": metas,
                    }
                }
            else:
...
def do_collect(
    ...
    if root_readme:
        ...
        metas, root_content = get_md_metas(root_readme)
    if implicit_meta:
    ...
    if SemanticMetadata.TITLE not in metas:
    ...
    ftmap = FTMap(
        title=metas[SemanticMetadata.TITLE],
