...
def init():
    ...
    global location1
    location1 = Location()
    if not os.path.isfile("data/src/main/resources/plugin.yml"):
        os.makedirs("data/src/main/resources")
    if not os.path.isfile("data/src/main/java/Main/Main.java"):
...
def make(server,**setting):
    ...
    for code in lines:
        ...
        if getvalue == True:
            ...
            code = code.replace("    ","")
            code = code.replace("\n","")
            if code.__contains__("if"):
                raise Exception("I'm Sorry you can't use 'if' instead you can use 'while when():'"+'"'+filename+'", line '+str(long)+',')
            elif code.__contains__("elif") or code.__contains__("else"):
                raise Exception("I'm Sorry who can't use elif or else"+'"'+filename+'", line '+str(long)+',')
            elif m == 'init':
                del check_list[0] 
                javacode = space.getspace(javacode)
