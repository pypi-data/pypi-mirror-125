...
parser.add_option("-s", "--strict mode", dest="strict_mode",
                  ...
                  help="use strict mode, keywords ctf, flag and key are required")
...
def color_print_result(result_list):
    keywords = [
        'flag', 'ctf', 'key', 'galf', 'synt'
    ]
...
def main():
    ...
    if os.path.isfile(file_name):
        ...
        color_print_result(result_list)
    elif os.path.isdir(file_name):
        for root, dirs, files in os.walk(file_name):
            for f in files:
                f_name = os.path.join(root, f)
                logger.info('\n---------------------')
                logger.info('file: %s' % f_name)
            ...
            logger.info('\n#####################')
            result_list = clean_find_ctf_flag_result(list(result_dict.keys()))
            # 只显示所有结果的前20条
            color_print_result(result_list[:20])
