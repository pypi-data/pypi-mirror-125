"""
PYTHON WRAPPER FOR VITEX.NET API v2 (DECENTRALIZED EXCHANGE https://x.vite.net/)
API DOCUMENTATION: https://vite.wiki/dex/api/dex-apis.html#overview

---------------------------------------
  vitex-py version 1.0 by `blacktyger
---------------------------------------
"""
...
class PublicAPI:
    ...
    @staticmethod
    def _response_parser(response: dict) -> Union[dict, list, int, float, str]:
        """Parse API responses"""
        if isinstance(response, dict):
            if not response['code'] and response['data']:
    ...
    def get_token_detail(self, **kwargs) -> dict:
        """
...
class TradingAPI(PublicAPI):
    ...
    which you can apply for at API (opens new window)on ViteX platform.
    ...
    def _prepare_decimals(self, params: dict) -> dict:
        ...
        try:
            params['amount'] = Decimal(params['amount']).quantize(amount_decimals)
            params['price'] = Decimal(params['price']).quantize(price_decimals)
        except Exception as e:
            print(e)

        return params

    def place_order(self, symbol=None, amount=None, price=None, side=None) -> dict:
        """
        :param symbol: Trading pair name. For example EPIC-002_BTC-000
