...
class StaticScraper:
    ...
    """
    def _attribute_format(element_type, search_by = None, identifier = None, formatting = None, find_all = False):
        return { 'et' : element_type, 'fi' : find_all, 'sb' : search_by, 'id' : identifier, 'fm' : formatting }
    def print_help():
        print("[1] Use StaticScraper._attribute_format(element_type, search_by = None, identifier = None, formatting = None, find_all = None)")
        print("\tVar = StaticScraper._attribute_format('div', 'class_', 'row-2-col-3', formatting = 'text'")
        print("\tVar = StaticScraper._attribute_format('div', 'class_', 'row-2-col-3', find_all = True", end = '\n\n')
    ...
    def scrape_website(self, start = 1, pages = 1):
        for page in range(start, start + pages, 1):
            ...
            itemList = StaticScraper.custom_find(BeautifulSoup(requests.get(self.website + self.pageformat.format(page)).text, 'lxml'), 
                                                 ...
                                                 self.itemformat['fi'], self.itemformat['fm'])
            #print(self.itemformat)
            #print(itemList)
            ...
            for item in itemList:
                ...
                for index, attribute in enumerate(self.attribute_list):
                    ...
                    """
                    temp = StaticScraper.custom_find(item, attribute['et'], attribute['sb'], attribute['id'], attribute['fi'], attribute['fm'])
                    itemData[index] = temp
                self.pageInfo.append(itemData)
            self.save_to_file()
            time.sleep(self.sleeptime)
    def custom_find(source, element_type, search_by = None, identifier = None, find_all = False, formatting = None):
        ...
        return (result if formatting == None else result.text if formatting == 'text' else result[formatting])
