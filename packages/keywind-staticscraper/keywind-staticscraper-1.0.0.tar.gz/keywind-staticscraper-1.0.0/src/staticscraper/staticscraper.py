...
class StaticScraper:
    ...
    def save_record(self, filename = None, start_empty = False):
        
        filename = (filename if (filename != None) else self.filename)
        
        if (start_empty):
        
            self.erase_file(filename)
    ...
    def scrape_webpage(self, webpage):
        ...
        for item in itemList:
            ...
            self.scrapeList.append( [ self.custom_find(item, attr) for attr in self.webinfo.f_attr ] )
    ...
    def prompt_overwrite(self, filename):
    ...
    def scrape_webpages(self, start = 1, pages = 1):
        ...
        if (self.prompt_overwrite(self.filename) == None):
        ...
        for page in range(start, start + pages):
            ...
            self.scrape_webpage(self.webinfo.f_url + self.webinfo.f_page.format(page))
            
            print("\tScraped page {} of {} pages.".format(page, start + pages - 1), end = '\n\n')
            
            time.sleep(self.timesleep)
    
    def custom_find(self, source, format_info):
    ...
    def filter_by_threshold(self, thresh_index, thresh_function, inputname = None, outputname = None):
        ...
        if (self.prompt_overwrite(outputname) == None):
        ...
        if (os.path.isfile(file_name)):
            ...
            try:
                ...
                for data in DATA:
                    ...
                    if ((outputname != None) and (self.buffer_exceeded())):
                        
                        self.save_record(filename = outputname)
                        
                        self.scrapeList = []
                
                if (len(self.scrapeList)):
                    
                    self.save_record(filename = outputname, start_empty = (outputname == None))
    ...
    def _extract(self, source, column_indices):
        ...
        if (os.path.isfile(file_name)):
            ...
            try:
                ...
                for data in DATA:
                    ...
                    self.scrapeList.append(self._extract([ x for index, x in enumerate(eval(data)) if index in column_indices ], column_indices))
