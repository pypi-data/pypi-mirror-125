...
dynamodb_state_table_name = settings.DYNAMODB_STATE_TABLE_NAME
...
def _get_metadata_from_dynamodb(consumer, latest_timestamp):
    """
    ...
    table = _get_or_create_metadata_table(consumer) 
...
def _get_one_batch_after_timestamp(consumer, latest_timestamp):
    """
    get from S3
    """

    metadata = _get_metadata_from_dynamodb(consumer, latest_timestamp)
    if metadata is None:
        return None, None

    batch = s3.Object(metadata["bucket_name"], metadata["file_name"]).get()['Body'].read()
    return batch, metadata["timestamp"]
    

def start_processor(process, consumer): 
    latest_timestamp = _get_latest_processed_timestamp(consumer)
    while True:
        print('latest_timestamp before : ', latest_timestamp)
        batch, latest_timestamp = _get_one_batch_after_timestamp(consumer, latest_timestamp)
        print('latest_timestamp after : ', latest_timestamp)
        if batch is None:
            print("batch is None")
            return
        process(batch)
        _update_latest_processed_timestamp(consumer, latest_timestamp)
