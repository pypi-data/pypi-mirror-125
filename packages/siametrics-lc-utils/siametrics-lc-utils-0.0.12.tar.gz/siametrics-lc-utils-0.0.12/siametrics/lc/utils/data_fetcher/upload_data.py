...
def _upload_metadata(consumer_services, data_type, source_company, item):
    """
    ...
    for consumer_service in consumer_services:
        table_name = f'{consumer_service}-{data_type}-{source_company}'
...
def upload_to_datalake(object_in, consumer_services, data_type, source_company, file_name):
    # upload object to S3
    now = datetime.now()
    date = now.date()
    # uid = hashing(now)
    ...
    _upload_metadata(consumer_services, data_type, source_company, item)
