...
class TextEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if seq_len > self.block_size:
            raise ValueError(f"passed sequence of length {seq_len} is too long, only {self.block_size} is allowed")
...
class BERTEmbedding(torch.nn.Module):
    def __init__(self, block_size=128, n_features=512, vocab_size=26, drop_p=0.0):
        ...
        self.sentence_number_embedding = torch.nn.Parameter(torch.zeros(1, 2, n_features))
...
class Attention(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_heads, drop_attn_p=0.0, drop_out_p=0.0, block_size=128, causal=False):
        super().__init__()
        self.n_channels = n_channels
        ...
        if causal:
            self.register_buffer("mask", 
                                 torch.tril(torch.ones(1, 1, block_size, block_size)).log()
                                 )
        else:
            self.mask = None
        if drop_attn_p > 0.0:
    ...
    def forward(self, x):
        ...
        if self.mask is not None:
            logits += self.mask  # this relies on logits being less than inf, but I guess that is OK
...
class Block(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_heads, n_hidden, drop_attn_p=0.0, drop_attn_out_p=0.0, drop_mlp_p=0.0, block_size=None, causal=False, ln_eps=1e-6):
        super().__init__()
        self.norm1 = torch.nn.LayerNorm(n_channels, eps=ln_eps)
        self.attn = Attention(n_channels, n_heads, drop_attn_p=drop_attn_p, drop_out_p=drop_attn_out_p, causal=causal, block_size=block_size)
...
class DeiTTiny(torch.nn.Module):
    def __init__(self, n_classes=1000):
        ...
        self.blocks = torch.nn.Sequential(
            *[Block(n_channels, n_heads, n_hidden, ln_eps=1e-6) for i in range(n_blocks)]
        )
...
class GPT(torch.nn.Module):
    def __init__(self, *, vocab_size, block_size, n_hidden, n_heads, n_blocks, n_mlp_hidden=None):
        ...
        self.embedding = TextEmbedding(block_size=block_size, n_features=n_hidden, vocab_size=vocab_size,
                              drop_p=0.1)
        self.blocks = torch.nn.Sequential(
            *[Block(n_hidden, n_heads, n_mlp_hidden, drop_attn_p=0.1, drop_attn_out_p=0.1, drop_mlp_p=0.1, block_size=block_size, causal=True, ln_eps=1e-5) for i in range(n_blocks)]
        )
...
class BERT(torch.nn.Module):
    def __init__(self, *, vocab_size, block_size, n_hidden, n_heads, n_blocks, n_mlp_hidden=None):
        ...
        if n_mlp_hidden is None:
            n_mlp_hidden = 4 * n_hidden
        self.embedding = BERTEmbedding(block_size=block_size, n_features=n_hidden, vocab_size=vocab_size,
                              drop_p=0.1)
        self.blocks = torch.nn.Sequential(
            *[BERTBlock(n_hidden, n_heads, n_mlp_hidden, drop_attn_p=0.1, drop_attn_out_p=0.1, drop_mlp_p=0.1,
                    block_size=block_size, causal=False, ln_eps=1e-12) for i in range(n_blocks)]
        )
