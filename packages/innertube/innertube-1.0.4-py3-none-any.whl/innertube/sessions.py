...
class JSONSession(BaseUrlSession):
    def send(self, request, **kwargs):
        ...
        content_type = mimit.parse(response.headers.get(str(enums.Header.CONTENT_TYPE)))

        if content_type.subtype != mimit.MediaSubtype.JSON:
            if not response.ok:
            ...
            (
                ...
                (
                    expected_type = mimit.MimeType \
                    (
                        type    = mimit.MediaType.APPLICATION,
                        subtype = mimit.MediaSubtype.JSON,
                    ).string(),
