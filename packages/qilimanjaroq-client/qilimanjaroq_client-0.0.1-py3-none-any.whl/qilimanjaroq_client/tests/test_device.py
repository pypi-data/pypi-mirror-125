...
from qilimanjaroq_client.devices.simulator_device_characteristics import SimulatorDeviceCharacteristics
from qilimanjaroq_client.typings.device import DeviceInput, SimulatorDeviceInput, SimulatorDeviceCharacteristicsInput
from .data import (device_inputs, simulator_device_inputs, simulator_device_characteristics_inputs)
...
def test_simulator_device_dict_representation(simulator_device_input: SimulatorDeviceInput):
    ...
    if device._characteristics:
        expected_dict |= {
            "characteristics": device._characteristics.__dict__()
        }
    assert device.__dict__() == expected_dict
...
def test_simulator_device_characteristics_constructor(simulator_device_characteristics_input: SimulatorDeviceCharacteristicsInput):
...
@pytest.mark.parametrize("simulator_device_characteristics_input", simulator_device_characteristics_inputs)
def test_simulator_device_characteristics_json_representation(simulator_device_characteristics_input: SimulatorDeviceCharacteristicsInput):
    characteristics = SimulatorDeviceCharacteristics(characteristics_input=simulator_device_characteristics_input)
    expected_dict = {
