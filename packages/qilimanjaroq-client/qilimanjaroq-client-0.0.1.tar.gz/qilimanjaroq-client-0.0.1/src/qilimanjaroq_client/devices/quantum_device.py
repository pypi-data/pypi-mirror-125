...
from qilimanjaroq_client.typings.device import QuantumDeviceInput
...
class QuantumDevice(Device):
    ...
    def __init__(self, device_input: QuantumDeviceInput):
        ...
        if 'characteristics' in device_input:
            self._characteristics = QuantumDeviceCharacteristics(device_input['characteristics'])
            device_input.pop('characteristics')
        if 'calibration_details' in device_input:
            self._calibration_details = CalibrationDetails(device_input['calibration_details'])
    ...
    def __dict__(self) -> dict:
        ...
        if self._characteristics:
            device_dict |= {
                "characteristics": self._characteristics.__dict__()
            }
        if self._calibration_details:
            device_dict |= {
                "calibration_details": self._calibration_details.__dict__()
            }
        return device_dict
