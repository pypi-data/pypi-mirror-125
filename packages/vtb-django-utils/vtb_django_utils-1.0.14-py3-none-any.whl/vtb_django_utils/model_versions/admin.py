...
from .utils.consts import START_VERSION, RE_VERSION
...
class VersionMixin(admin.ModelAdmin):
    ...
    def is_version_changed(obj):
        requests_name = ['json'] if obj.is_version_changed else []
        if hasattr(obj, 'is_raw_version_changed') and obj.is_raw_version_changed:
            requests_name.append('raw_json')
        return format_html(
            '''
            <div style="color: crimson">
            Текущая версия не сохранена (и не используется при запросе <span style="font-weight: bold">%s</span>)
            </div>
            ''' % ', '.join(requests_name)
        ) if requests_name else ''
