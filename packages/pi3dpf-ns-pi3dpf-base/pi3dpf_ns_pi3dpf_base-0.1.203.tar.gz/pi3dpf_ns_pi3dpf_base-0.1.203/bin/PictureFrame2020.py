...
import pi3dpf.PictureFrame2020config as config
import pi3dpf.pf
...
config_file    = [os.path.join(os.path.dirname(pfc.__file__), 'cfg/pf.config')] # os.path.join(os.path.dirname(this_dir), 'etc','pf.config')
cfg            = configparser.ConfigParser(inline_comment_prefixes=';', empty_lines_in_values=False, converters={'list': lambda x: [i.strip() for i in x.split(',')]})
...
PIC_LIBREOFFICE_BITMAP_OUTmdate= pi3dpf.pf.getMdate(PIC_LIBREOFFICE_BITMAP_OUT)
...
txt_cp   = {}
txt_cp   = pi3dpf.pf.mk_codepoints(PI3D_FT_CODEPOINT_RANGES, pi3d_log)
pf_opts  = pi3dpf.pf.folderOpts(config.PIC_DIR, cfg, pi3d_log, txt_cp, config.SHOW_NAMES_TM)
...
def tex_load(fname, orientation, size=None):
  ...
  pi3d_log.debug("tex_load: {:02d}min {:03.1f}sec, fname: '{}' ({:,d} bytes)".format(int(delta[1]), float(delta[2]), fname, os.stat(fname).st_size))
  return tex
  ...
  for root, _, _ in os.walk(config.PIC_DIR):
      ...
      if mod_tm > last_file_change:
        ...
        for file in os.listdir(root):
           if os.path.isfile(os.path.join(root, file)) and os.path.splitext(file)[1].lower() in PIC_EXTENSIONS and os.stat(os.path.join(root, file)).st_mtime > last_file_change:
...
def getf_cache_or_fsys(dt_from=None, dt_to=None):
  if dt_from != None or dt_to != None:
    ...
    iFiles_cached = pi3dpf.pf.get_iFiles_from_cache(config.PIC_DIR, PIC_EXTENSIONS, PIC_IFILES_CACHE_DIR)
    if iFiles_cached == None:
      iFiles, nFi = get_files(dt_from, dt_to)
      pi3dpf.pf.put_iFiles_to_cache(config.PIC_DIR, iFiles, PIC_IFILES_CACHE_DIR)
      return iFiles, nFi
    else:
      if shuffle:
...
def get_files(dt_from=None, dt_to=None):
  ...
  file_list = []
  f_cnt = {'no_exif': 0, 'exif_orient_missing': 0, 'exif_date_missing': 0, 'f_not_open': 0, 'nomedia': 0, 'total_files': 0, 'out_of_d_range': 0}
      ...
      if len(_dirnames) > 0:
        for d in _dirnames:
          if os.path.exists(os.path.join(root, d, '.nomedia')):
            pi3d_log.info("skiping pictures from directory '{}'. (dir contains .nomedia file)".format(os.path.join(root, d)))
            _dirnames.remove(d)
            f_cnt['nomedia'] += 1
          ...
          if ext in extensions and not '.AppleDouble' in root and not filename.startswith('.'):
              ...
              if EXIF_DATID is not None and EXIF_ORIENTATION is not None:
                try:
                  ...
                  pi3d_log.info("Image.open for '{}' failed with error '{}'. Skipping file.".format(file_path_name, e))
                  f_cnt['f_not_open'] += 1
                  continue
                ...
                try:
                  ...
                  f_cnt['no_exif'] += 1
                try:
                  dt = time.mktime(time.strptime(exif_data[EXIF_DATID][:19], '%Y:%m:%d %H:%M:%S')) # [:19]: at times, there is a \x00 appended so only use first 19 chars, e.g. 2019:06:12 15:07:05
                except Exception as e:
                  dt = os.path.getmtime(file_path_name)
...
font = pi3d.Font(PI3D_FT_FONT, codepoints=txt_cp['codepoints'], grid_size=txt_cp['grid_size'], shadow_radius=4.0, shadow=(0,0,0,128))

# pic_fname
pfnm_txtb = pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'pic_fname:', {'DISPLAY': globals()['DISPLAY']}, font, CAMERA, cfg, pi3d_log)
# pic_title
titl_txtb = pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'pic_title:', {'DISPLAY': globals()['DISPLAY']}, font, CAMERA, cfg, pi3d_log)

PIC_CLOCK_MAXCHAR = 40
clock_txtb = pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'clock:', {'DISPLAY': globals()['DISPLAY']}, font, CAMERA, cfg, pi3d_log)

if OWM_RETRIEVE_LOCAL_WEATHER:
  owm_info = owm_pf.updateWeatherInfo()

if now_playing_enabled:
  nplay_txtb = pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'now_playing:', {'DISPLAY': globals()['DISPLAY']}, font, CAMERA, cfg, pi3d_log)
...
# plane  = pi3d.Plane(w=libre.ix, h=libre.iy, z=0.1) #NB x,y both default zero puts it in the middle of the screen #@@#
  lo_templ = pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'libreoffice_to_png:', {'DISPLAY': globals()['DISPLAY']}, font, CAMERA, cfg, pi3d_log)
  lo_templ.update()
...
if OWM_FORMATSTRING_USE:
  ...
  for i in range(0, len(owm_info)):
    wthr_scis.append(pi3dpf.pf.pi3dObj(PIC_TEXT_POSITION_FILE, None, 'owm_formatstring.{}:'.format(i), {'DISPLAY': globals()['DISPLAY']}, None, None, cfg, pi3d_log))

tex_n = pi3dpf.pf.pi3dTexThreaded(iFiles, DISPLAY, config, next_pic_num)
if nFi <= 0:
  # no images found
...
#     if config.SHOW_NAMES_TM > 0.0:
      ...
      if pf_opts.getOpt('show_dname', config.PIC_DIR):
        ...
        titl_txtb.update(text_format="{}".format(pf_opts.getOpt('title_for_dname', iFiles[pic_num][0])), pos_dname_on_fname=pf_opts.getOpt('pos_dname_on_fname', config.PIC_DIR))
