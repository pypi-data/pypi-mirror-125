...
import pi3dpf.pf_mqtt as mqh
import pi3dpf.pf_common as mqc

this_host      = os.uname()[1]
this_dir       = os.path.dirname(__file__)
this_file      = os.path.basename(__file__)

config_file    = [os.path.join(os.path.dirname(mqh.__file__), 'cfg/pf.config')] # os.path.join(os.path.dirname(this_dir), 'etc','pf.config')
...
pic_frame_dir_nicknames        = mqc.get_config_param(config, 'PIC_FRAME_FAVORITE_DIRS')
PIC_FRAME_FAVORITE_DIRS        = dict(map(lambda x: x.split(':'), pic_frame_dir_nicknames)) # make dict with nicknames as key. E.g. nick:/var/tmp will be x[nick] contains /var/tmp
...
def on_connect(client, userdata, flags, rc):
  ...
  log.info("Successfully connected with MQTT broker {}:{} as user {}".format(MQTT_SERVER_NAME, MQTT_SERVER_PORT, MQTT_CLIENT_USERNAME))
...
def on_message(client, userdata, msg):
  ...
  if msg.topic in [MQTT_TOPLEVEL_TOPIC+'/pictureframe/start',
    ...
    if not os.path.isdir(pic_dir):
      log.warning("cannot access directory '{}' given in mqtt paylod. Using PIC_FRAME_DEFAULT_PIC_LOCATION='{}'".format(pic_dir, PIC_FRAME_DEFAULT_PIC_LOCATION))
      pic_dir = PIC_FRAME_DEFAULT_PIC_LOCATION
    log.info("mqh.pictureframe(prog_path='{}', operation='{}', pic_dir='{}', use_cec='{}')".format(PIC_FRAME_CTL_PROG, os.path.basename(msg.topic), pic_dir, CEC_CONTROL_TV))
    if os.path.basename(msg.topic) in ['dirlist']:
      lopu.show_siblings(pic_dir)
    else:
      status = mqh.pictureframe(PIC_FRAME_CTL_PROG, os.path.basename(msg.topic), pic_dir, CEC_CONTROL_TV, PIC_FRAME_TRACE_FOPEN)
      lopu.info(status)
...
while True:
  if isOpen(MQTT_SERVER_NAME, MQTT_SERVER_PORT):
    ...
    log.info("Could not reach {}:{}. Sleeping 5 seconds before retrying ({} retries left).".format(MQTT_SERVER_NAME, MQTT_SERVER_PORT, connect_retry_count))
    time.sleep(5)
  connect_retry_count -= 1
  if connect_retry_count <= 0:
    log.error("failed to establish network connection to mqtt server at {}:{}".format(MQTT_SERVER_NAME, MQTT_SERVER_PORT))
    exit(1)
