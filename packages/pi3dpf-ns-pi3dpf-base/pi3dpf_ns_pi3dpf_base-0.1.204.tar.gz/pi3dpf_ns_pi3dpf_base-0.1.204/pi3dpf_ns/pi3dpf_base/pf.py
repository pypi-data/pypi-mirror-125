import os, re, datetime, regex, pi3d, distutils, logging, threading, json, traceback, pdb
from . import pf_common as pfc
import numpy as np
    ...
    with open(self.positionfile, 'r') as f:
      for line in f:
        if not re.match('^'+self.entry+':?', line):
          continue
        self.log.debug("getPositionFromFile - reading properties for entry {}".format(self.entry))
        val_change  = 0
        entry_found = True
