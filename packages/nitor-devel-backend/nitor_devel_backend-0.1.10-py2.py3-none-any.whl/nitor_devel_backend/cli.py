...
def patch_service(service, port):
    if service["type"] == "proxy" and (".internal" in service["host"] or "targetGroup" in service):
...
def run_backend(args, instance):
    ...
    try:
        ...
        if docker := which("docker"):
            ...
            mounts = [
                "--mount", f"type=bind,src={fname},dst=/app/config.json,ro"
            ]
            if args.directory:
                mounts += [
                    "--mount", f"type=bind,src={path.abspath(args.directory)},dst=/app/static,ro"
                ]
            end = [
            ...
            # Login to ecr
            repo = ecr(region="eu-west-1").describe_repositories(repositoryNames=["devel-backend/dev-proxy"])["repositories"][0]
            token_resp = ecr(region="eu-west-1").get_authorization_token(registryIds=[repo["registryId"]])
            if "authorizationData" in token_resp:
                ...
                user = full_token[0]
                token = full_token[1]
                login_proc = Popen([docker, "login", "-u", user, "-p", token, auth_data["proxyEndpoint"]])
...
def cli_run_backend():
    parser = ArgumentParser(description="Run a nitor backend locally to enable serving nitor data backends and your development server resources from the same port." +\
    ...
    parser.add_argument("environment", help="Run agains dev or prod data resources", choices=["dev", "prod"])
    source_args = parser.add_mutually_exclusive_group(required=True)
    source_args.add_argument("--port", "-p", help="The port your local dev server is listening to")
    source_args.add_argument("--directory", "-d", help="The directory holding your created site static artifacts")
    source_args.add_argument("--service", "-s", help="Give dev service backend configuration as json on the command line in case you want to for example mix a local dynamic server "+\
    ...
    source_args.add_argument("--file", "-f", help="Give dev service backend configuration as json in a file. See above.")
    parser.add_argument("--impersonate", "-i", help="Impersonate a virtualhost service in the existing configuration by forwarding any proxy subservices with the target in " +\
    ...
    if "AWS_PROFILE" not in environ:
        parser.error("AWS_PROFILE environment variable needs to be defined to give access to the container image and port forwardings. Preferably via 'eval \"$(ndt enable-profile infra)\"'")
    instance = choice(list(ec2(region="eu-west-1").instances.filter(Filters=[{"Name": "tag:Name", "Values": [f"ecs-infra-{args.environment}"]}])))
