...
class BusSynchronizer:
    ...
    def __update_buffers_and_check_for_completeness(self, message):
        ...
        if self.previous_collection and self.processor.contains_sync(self.data.ids) and self.processor.contains_sync([message.msg_id]):
    ...
    def _state_waiting_data(self, message: CanMessage) -> ProcessedCanDataBlock:
        ...
        if full_collection:
            ...
            self.collection_monitor.add_event(full_collection.first.timestamp)
