...
class RateLimitDecorator:
    """
    Decorator to rate limit requests in the aiohttp.web framework with memcached
    """

    def __init__(self, db: aiomcache.Client, path_id: str, keyfunc: Callable, ratelimit: str, exempt_ips: Optional[set] = None, error_handler: Optional[Union[Callable, Awaitable]] = None) -> None:
    ...
    def __call__(self, func: Callable) -> Awaitable:
        ...
        async def wrapper(request: Request) -> Response:
            ...
            db_key = bytes(f"{key}:{self.path_id or request.path}", "utf-8")
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                if nc is not None:
                    ...
                    await db.set(db_key, b"0", exptime=self.period)
                    nc = 0
                if nc >= self.calls:
...
class MemcachedLimiter:
    """
    ```
    limiter = MemcachedLimiter(host="localhost", port=8000)
    @routes.get("/")
    @limiter.limit("5/1")
    def foo():
    ...
    def __init__(self, keyfunc: Callable, exempt_ips: Optional[set] = None, error_handler: Optional[Union[Callable, Awaitable]] = None, **memcached_args) -> None:
        self.exempt_ips = exempt_ips or set()
        self.keyfunc = keyfunc
        self.db = aiomcache.Client(**memcached_args)
        self.error_handler = error_handler
