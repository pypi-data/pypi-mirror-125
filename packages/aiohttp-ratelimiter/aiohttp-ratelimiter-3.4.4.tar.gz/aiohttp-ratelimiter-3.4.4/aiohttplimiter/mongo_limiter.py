...
class RateLimitDecorator:
    ...
    def __init__(self, db: AsyncIOMotorDatabase, ttl: int, path_id: str, keyfunc: Callable, ratelimit: str, exempt_ips: Optional[set] = None, error_handler: Optional[Union[Callable, Awaitable]] = None) -> None:
    ...
    def __call__(self, func: Callable) -> Awaitable:
        ...
        async def wrapper(request: Request) -> Response:
            ...
            # try:
            #     self.db["rate_limiting"]
            # except KeyError:
            #     await self.db.create_collection("rate_limiting", capped=False)
            # await self.db["rate_limiting"].create_index("Clear Old Data", expireAfterSeconds=self.ttl)
...
class MongoLimiter:
    ...
    def __init__(self, keyfunc: Callable, db: AsyncIOMotorDatabase, exempt_ips: Optional[set] = None, error_handler: Optional[Union[Callable, Awaitable]] = None, ttl: int = 60) -> None:
    ...
    def limit(self, ratelimit: str, keyfunc: Callable = None, exempt_ips: Optional[set] = None, error_handler: Optional[Union[Callable, Awaitable]] = None, path_id: str = None) -> Callable:
        def wrapper(func: Callable) -> Awaitable:
            ...
            return RateLimitDecorator(db=self.db, keyfunc=_keyfunc, ttl=self.ttl, ratelimit=ratelimit, exempt_ips=_exempt_ips, error_handler=_error_handler, path_id=path_id)(func)
        return wrapper
