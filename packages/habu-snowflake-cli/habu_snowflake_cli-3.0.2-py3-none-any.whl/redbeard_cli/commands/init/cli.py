...
def full_framework(config_file: str, organization_id: str, account_id: str):
    habu_provider(config_file, organization_id, account_id)
    habu_requester(config_file, organization_id, account_id)
...
@click.option('-o', '--organization_id', required=True, help='Habu Organization ID')
@click.option('-c', '--config-file', default="./habu_snowflake_config.yaml", help='Snowflake account configuration file')
def habu_provider(config_file: str, organization_id: str):
    account_id, sf_connection = snowflake_utils.new_connection_from_yaml_file(config_file)
    habu_setup_command.init_provider(sf_connection, organization_id, account_id)
...
@click.option('-o', '--organization_id', required=True, help='Habu Organization ID')
@click.option('-c', '--config-file', default="./habu_snowflake_config.yaml", help='Snowflake account configuration file')
def habu_requester(config_file: str, organization_id: str):
    account_id, sf_connection = snowflake_utils.new_connection_from_yaml_file(config_file)
    habu_setup_command.init_requester(sf_connection, organization_id, account_id)
