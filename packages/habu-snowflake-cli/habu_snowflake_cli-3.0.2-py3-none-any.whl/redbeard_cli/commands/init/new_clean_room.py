...
def install_handle_new_clean_rooms_procedure(sf_connection):
    ...
    $$
        try {
            var crRequestSql = "SELECT id AS request_id, request_data:clean_room_id AS clean_room_id, request_data:account_id AS account_id, request_data:habu_account_id AS habu_account_id FROM HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CLEAN_ROOM_REQUESTS WHERE request_type = :1 AND request_status = :2";
...
def install_new_clean_room_stored_procedure(sf_connection):
    sp_sql = """
    ...
    $$        
        try {
            ...
            snowflake.execute({
                sqlText: "CREATE OR REPLACE SCHEMA HABU_CLEAN_ROOM_" + sf_clean_room_id + ".CLEAN_ROOM COMMENT = 'HABU_" + CUSTOMER_ACCOUNT_ID + "'"
            });
            ...
            snowflake.execute({
                sqlText: "ALTER SHARE HABU_CR_PROVIDER_" + sf_clean_room_id + "_HABU_SHARE ADD ACCOUNTS = :1",
