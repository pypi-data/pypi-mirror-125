...
from redbeard_cli.commands.init.clean_room_partners import (
    install_handle_new_clean_room_partners_procedure,
    install_new_clean_room_partner_stored_procedure,
    install_handle_accept_clean_room_invitations_procedure,
    install_accept_clean_room_invitation_stored_procedure
)
...
from redbeard_cli.commands.init.new_dataset import (
    ...
    install_handle_new_requester_datasets_procedure,
    install_create_new_requester_dataset_procedure
)
from redbeard_cli.commands.init.question_runs import (
    ...
    install_handle_new_question_runs_requester_procedure,
    install_run_question_requester_procedure
)
from redbeard_cli.commands.init.questions import (
    install_handle_new_questions_procedure,
    install_create_new_question_procedure
)


def install_clean_room_objects(sf_connection, organization_id: str, customer_account_id: str, setup_type: str):
    install_habu_error_handler_procedure(sf_connection)
    if setup_type == 'provider':
        install_habu_controller_provider_procedure(sf_connection)
        install_handle_new_clean_rooms_procedure(sf_connection)
        install_new_clean_room_stored_procedure(sf_connection)
        install_new_clean_room_partner_stored_procedure(sf_connection)
        install_handle_new_clean_room_partners_procedure(sf_connection)
    ...
    elif setup_type == 'requester':
        install_habu_controller_requester_procedure(sf_connection)
        install_accept_clean_room_invitation_stored_procedure(sf_connection)
        install_handle_accept_clean_room_invitations_procedure(sf_connection)
        install_create_new_requester_dataset_procedure(sf_connection)
        install_handle_new_requester_datasets_procedure(sf_connection)
        install_create_new_question_procedure(sf_connection)
        install_handle_new_questions_procedure(sf_connection)
        install_run_question_requester_procedure(sf_connection)
        install_handle_new_question_runs_requester_procedure(sf_connection)
    ...
    WAREHOUSE = HABU_CLEAN_ROOM_COMMON_XSMALL_WH
    SCHEDULE = '1 MINUTE'
...
def install_habu_controller_provider_procedure(sf_connection):
    """
    ...
    $$
        try {
            ...
            var stmt = snowflake.createStatement({sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.HANDLE_NEW_CLEAN_ROOM_PARTNERS();'})
            stmt.execute();
...
def install_habu_controller_requester_procedure(sf_connection):
    """
    Install stored procedure that will be called when a new clean room request is generated
    via the Habu application. The Controller procedure calls individual stored procedures
    to handle various types of clean room requests.  For example, HANDLE_NEW_CLEAN_ROOMS is
    ...
    $$
        try {
            // copy all new requests from stream into a local table to reset the stream
            var sqlCommand = "INSERT INTO HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CLEAN_ROOM_REQUESTS (ID, REQUEST_TYPE, REQUEST_DATA, CREATED_AT, REQUEST_STATUS) (SELECT ID, REQUEST_TYPE, REQUEST_DATA, CREATED_AT, REQUEST_STATUS FROM HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CLEAN_ROOM_REQUESTS_STREAM)";
            snowflake.execute({sqlText: sqlCommand});
            
            var stmt = snowflake.createStatement({sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.HANDLE_ACCEPT_CLEAN_ROOM_INVITATIONS();'})
            stmt.execute();
            
            var stmt = snowflake.createStatement({sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.HANDLE_NEW_REQUESTER_DATASETS();'})
            stmt.execute();
            
            var stmt = snowflake.createStatement({sqlText: 'CALL HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.HANDLE_NEW_QUESTIONS();'})
            stmt.execute();
