...
def install_handle_new_clean_room_partners_procedure(sf_connection):
    ...
    $$
        try {
            var crRequestSql = "SELECT id AS request_id, request_data:clean_room_id AS clean_room_id, request_data:partner_account_id AS partner_account_id FROM HABU_CLEAN_ROOM_COMMON.CLEAN_ROOM.CLEAN_ROOM_REQUESTS WHERE request_type = :1 AND request_status = :2";
...
def install_accept_clean_room_invitation_stored_procedure(sf_connection):
    sp_sql = """
    ...
    $$
        try {
            ...
            snowflake.execute({
                sqlText: "CREATE OR REPLACE SCHEMA HABU_CLEAN_ROOM_" + sf_clean_room_id + ".CLEAN_ROOM COMMENT = 'HABU_" + REQUESTER_ACCOUNT_ID + "'"
            });
        
            snowflake.execute({
                sqlText: "CREATE OR REPLACE SCHEMA HABU_CLEAN_ROOM_" + sf_clean_room_id + ".CLEAN_ROOM_RUN_RESULTS COMMENT = 'HABU_" + REQUESTER_ACCOUNT_ID + "'"
            });
    
            snowflake.execute({
                sqlText: "CREATE TABLE IF NOT EXISTS HABU_CLEAN_ROOM_" + sf_clean_room_id + ".CLEAN_ROOM.ORG_USER_IDENTITIES (CLEAN_ROOM_ID VARCHAR, ID_TYPE VARCHAR(20) NOT NULL, ID_VALUE VARCHAR(255) NOT NULL, HABU_USER_ID VARCHAR(64) NOT NULL)"
            });
            ...
            snowflake.execute({
                sqlText: "ALTER SHARE HABU_CR_REQUESTER_" + sf_clean_room_id + "_HABU_SHARE ADD ACCOUNTS = :1",
