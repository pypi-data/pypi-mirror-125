...
PartialSummary.__new__.__defaults__ = ("", "", "")
...
def set_label(value, label):
    data = summarize(value)
    data = data._replace(label=label)
    return data
...
class Input(InputSignal, _Signal):
    ...
    def __init__(self, name, type, id=None, doc=None, replaces=None, *,
                 ...
                 auto_summary=None, closing_sentinel=None, label=None):
        flags = self.get_flags(multiple, default, explicit, False)
        super().__init__(name, type, "", flags, id, doc, replaces or [], label=label)
...
class Output(OutputSignal, _Signal):
    ...
    def send(self, value, *args, **kwargs):
        ...
        if self.auto_summary:
            data = set_label(value, self.label)
            self.widget.set_partial_output_summary(
                self.name, data, id=id)
...
class WidgetSignalsMixin:
    ...
    def _update_summary(self, summaries):
        ...
        def join_multiples(partials):
            ...
            label = " ".join(map(format_label, partials.values()))
            return shorts, details, label
        ...
        if not any(summaries.values()):
            ...
            summary, details, label = zip(*map(join_multiples, summaries.values()))
            summary = " | ".join(summary)
            detail = "<hr/><table>" \
                     + "".join(f"<tr><th><nobr>{name}</nobr>: "
                               f"</th><td>{detail}</td></tr>"
                               for name, detail in zip(label, details)) \
                     + "</table>"
