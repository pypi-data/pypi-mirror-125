...
from defektor_api.apis import DefaultApi
from defektor_api import ApiClient
...
def get_config():
    ...
    if CONFIG_DATA['address']:
        config.host = CONFIG_DATA['address']

    return config


def write_config(address: str):
    objectBuilder = ObjectBuilder(config_schema)
    ns = objectBuilder.build_classes()
    Config = ns.Config

    dfkConfig = Config(address=address)
...
def require_login(func):
    ...
    api_client = ApiClient(get_config())
    DfkCli.api_instance = DefaultApi(api_client)
